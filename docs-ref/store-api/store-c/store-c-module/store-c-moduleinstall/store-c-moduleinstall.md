...
| 名称 | 描述 |
| --- | --- |
| [module\_install.h](/ref/store-api/store-c/store-c-h/store-c-module_install/store-c-module_install) | 声明按需分发能力提供的API。 |
...
| FAILURE = -1 | 失败。 |
| SUCCESS = 0 | 成功。 |
| DOWNLOAD\_WAIT\_WIFI = 1 | 当前使用的是流量，开发者需要调用  [HMS\_ModuleInstall\_ShowCellularDataConfirmation](#hms_moduleinstall_showcellulardataconfirmation)接口，提醒用户确认是否使用流量下载。 |
...
返回模块类型, 取值参考：[bundleManager.ModuleType](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#moduletype)。当[ModuleInstall\_InstallStatus](#moduleinstall_installstatus)=1时，默认返回0。
