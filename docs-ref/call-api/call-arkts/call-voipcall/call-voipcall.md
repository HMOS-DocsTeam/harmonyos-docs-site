...
| userProfile | image.[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 否 | 否 | 用户头像图片，需导入图像处理模块，详情见[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)。  支持传入的最大图片大小为221x221像素，推荐传入的图片大小为112x112像素。通过 [PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)的[getPixelBytesNumber](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap#getpixelbytesnumber7)接口获取到的图片大小要小于196608bit。 |
...
| callback | Callback&lt;[VoipCallUiEventInfo](#voipcalluieventinfo)&gt; | 是 | 回调函数，返回通话事件详细信息对象。 |

****错误码：****

以下错误码的详细介绍请参见[ArkTS API错误码](/ref/call-api/call-arkts/call-error-code/call-error-code)。
...
| --- | --- | --- | --- |
| type | string | 是 | 通话事件类型，参数固定为“voipCallUiEvent”。 |
| callback | Callback&lt;[VoipCallUiEventInfo](#voipcalluieventinfo)&gt; | 否 | 需要取消监听的回调函数，返回通话事件详细信息对象。若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | [VoipCallAttribute](#voipcallattribute) | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见[VoipCallAttribute](#voipcallattribute)。（应用上报去电时，VoipCallAttribute中的voipCallState属性必须为VOIP\_CALL\_STATE\_DIALING。） |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| Promise&lt;[ErrorReason](#errorreason)&gt; | Promise对象，返回错误码类型。 |
