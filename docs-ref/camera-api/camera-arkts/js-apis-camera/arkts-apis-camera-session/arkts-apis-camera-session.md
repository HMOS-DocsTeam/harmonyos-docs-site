...
sidebar_position: 27
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-session
kit: 媒体
...
会话类，保存一次相机运行所需要的所有资源[CameraInput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput)、[CameraOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameraoutput/arkts-apis-camera-cameraoutput)，并向相机设备申请完成相机功能（录像，拍照）。

![](../../../../images/4cbebbb3/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;void&gt; | 是 | 回调函数。当提交配置信息成功，err为undefined，否则为错误对象。错误码类型[CameraErrorCode](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#cameraerrorcode)，比如预览流与录像输出流的分辨率的宽高比不一致，会返回7400201。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| cameraInput | [CameraInput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput) | 是 | 需要添加的CameraInput实例。传参异常（如超出范围、传入null、未定义等），实际接口不会生效。 |
...
移除[CameraInput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput)。当前函数需要在[beginConfig](#beginconfig11)和[commitConfig](#commitconfig11-1)之间生效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| cameraOutput | [CameraOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameraoutput/arkts-apis-camera-cameraoutput) | 是 | 需要添加的CameraOutput实例。传参异常（如超出范围、传入null、未定义等），实际接口不会生效。 |
