...
![](../../../images/bdec88fe/note_3.0-zh-cn.png) 
...
调整接口调用顺序,可参考自定义界面扫码的[业务流程](/scan-kit-guide/scan-customscan#业务流程)。例如：先调用customScan.init接口，再调用customScan.start。
...
使用[scanBarcode.startScanForResult](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanbarcodestartscanforresult)会返回此错误码，请根据使用场景处理用户取消默认界面扫码后的业务流程。
