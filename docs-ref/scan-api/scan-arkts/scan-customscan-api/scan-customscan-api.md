...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-customscan-api
kit: 媒体
...
| ****名称**** | ****类型**** | 只读 | ****可选**** | ****说明**** |
| --- | --- | --- | --- | --- |
| width | number | 否 | 否 | [XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)组件的宽，默认使用单位为vp，支持px、lpx和vp。 |
| height | number | 否 | 否 | XComponent组件的高，默认使用单位为vp，支持px、lpx和vp。 |
| surfaceId | string | 否 | 否 | XComponent持有surface的ID。 |

![](../../../images/247aea90/note_3.0-zh-cn.png) 

1. ViewControl的width和height需和XComponent的保持一致，start接口根据设置宽高值会匹配最接近的相机分辨率，如果宽高比例与相机的分辨率比例相差过大会影响预览流体验。XComponent组件为预览流提供的Surface，而XComponent的能力由UI提供，相关介绍可参见[XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)。
...
| scanCodeRects | Array&lt;scanBarcode.[ScanCodeRect](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scancoderect)&gt; | 否 | 是 | 相机预览流的码图检测位置信息。  ****设备行为差异：**** 该属性在带有Kirin NPU（Neural-network Processing Unit，神经网络处理器）的设备可正常返回，在不带有Kirin NPU的设备上返回undefined。 |
...
![](../../../images/c77109dd/note_3.0-zh-cn.png) 
...
****设备行为差异：**** 对于6.0.2(22)及之前版本，该接口在Phone、Tablet中可正常调用。对于6.1.0(23)及之后版本，该接口在Phone、Tablet、带后置相机的Wearable中可正常调用，申请相机权限成功后，在不带后置相机的Wearable中返回错误码1000500001。可以通过[cameraManager.getSupportedCameras](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedcameras)接口查询是否带后置相机。
...
| ****参数名**** | ****类型**** | ****必填**** | ****说明**** |
| --- | --- | --- | --- |
| options | scanBarcode.[ScanOptions](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanoptions) | 否 | 自定义界面扫码参数。 |

****错误码：****

以下错误码的详细介绍请参见[ArkTS API错误码](/ref/scan-api/scan-arkts/scan-error-code/scan-error-code)。
...
![](../../../images/9e7b3869/note_3.0-zh-cn.png) 
...
| ****类型**** | ****说明**** |
| --- | --- |
| Promise&lt;Array&lt;scanBarcode.[ScanResult](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanresult)&gt;&gt; | Promise对象，返回启动相机流扫码结果对象。 |
...
![](../../../images/ac6572a9/note_3.0-zh-cn.png) 
...
| --- | --- | --- | --- |
| viewControl | [ViewControl](#viewcontrol) | 是 | 相机控制参数。 |
| callback | AsyncCallback&lt;Array&lt;scanBarcode.[ScanResult](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanresult)&gt;&gt; | 是 | 回调函数，当启动相机流扫码成功，err为undefined，data为获取到的Array&lt;scanBarcode.[ScanResult](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanresult)&gt;；否则为错误对象。 |
| frameCallback | AsyncCallback&lt;[ScanFrame](#scanframe)&gt; | 否 | 回调函数，当启动相机流成功，err为undefined，data为获取到的相机预览流（YUV）[ScanFrame](#scanframe)；否则为错误对象。  ****起始版本：**** 5.0.0(12) |
...
struct CustomScanPage {
  ...
  // 回调获取ScanFrame
  private frameCallback: AsyncCallback<customScan.ScanFrame> =
    (err: BusinessError, scanFrame: customScan.ScanFrame) => {
      if (err) {
        hilog.error(0x0001, '[Scan Sample]',
          `Failed to get ScanFrame by callback. Code: ${err.code}, message: ${err.message}`);
        return;
...
![](../../../images/c186e198/note_3.0-zh-cn.png) 

本接口必须在启动相机流start接口后使用，相机流初始化、停止和释放阶段使用都会抛出内部错误的异常。

****系统能力：**** SystemCapability.Multimedia.Scan.ScanBarcode

****设备行为差异：**** 对于6.0.2(22)及之前版本，该接口在Phone、Tablet中可正常调用。对于6.1.0(23)及之后版本，该接口在Phone、Tablet、带后置相机的Wearable中可正常调用，在不带后置相机的Wearable中返回错误码1000500001。可以通过[cameraManager.getSupportedCameras](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedcameras)接口查询是否带后置相机。
...
![](../../../images/a27d883f/note_3.0-zh-cn.png) 
...
![](../../../images/985e4ceb/note_3.0-zh-cn.png) 
...
![](../../../images/6f307d23/note_3.0-zh-cn.png) 
...
![](../../../images/814637e4/note_3.0-zh-cn.png) 
...
![](../../../images/31339743/note_3.0-zh-cn.png) 
...
| ****参数名**** | ****类型**** | ****必填**** | ****说明**** |
| --- | --- | --- | --- |
| point | scanBarcode.[Point](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#point) | 是 | 焦点。x、y设置范围应在[0，1]之内，超过范围，如果小于0设置0，大于1设置1。 |
...
![](../../../images/0d175acb/note_3.0-zh-cn.png) 
...
![](../../../images/6304d8b7/note_3.0-zh-cn.png) 
...
![](../../../images/64c5b54c/note_3.0-zh-cn.png) 
...
![](../../../images/67f922aa/note_3.0-zh-cn.png) 
...
![](../../../images/ca684897/note_3.0-zh-cn.png) 
...
![](../../../images/04021d27/note_3.0-zh-cn.png) 
...
![](../../../images/ad556a98/note_3.0-zh-cn.png) 
...
![](../../../images/9ed514f7/note_3.0-zh-cn.png) 
...
![](../../../images/55022eea/note_3.0-zh-cn.png) 
