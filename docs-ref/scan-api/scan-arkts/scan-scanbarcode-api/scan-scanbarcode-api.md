...
sidebar_position: 4
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-scanbarcode-api
kit: 媒体
...
| ****名称**** | ****类型**** | 只读 | ****可选**** | ****说明**** |
| --- | --- | --- | --- | --- |
| scanType | scanCore.[ScanType](/ref/scan-api/scan-arkts/scan-scancore/scan-scancore#scantype) | 否 | 否 | 码类型。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| originalValue | string | 否 | 否 | 码识别内容结果。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| scanCodeRect | [ScanCodeRect](#scancoderect) | 否 | 是 | 码识别位置信息。  ****起始版本：**** 4.1.0(11) |
| cornerPoints | Array&lt;[Point](#point)&gt; | 否 | 是 | 码识别角点位置信息，返回QR Code四个角点。此参数仅图像识码接口[detectBarcode.decodeImage](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectbarcodedecodeimage)返回。  ****起始版本：**** 5.0.0(12) |
...
| source | scanCore.[ScanSource](/ref/scan-api/scan-arkts/scan-scancore/scan-scancore#scansource) | 否 | 是 | 扫码结果来源。此参数仅默认界面扫码接口返回。  ****模型约束：**** 此接口仅可在Stage模型下使用。  ****元服务API：**** 从版本6.0.2(22)开始，该接口支持在元服务中使用。  ****起始版本：**** 6.0.2(22) |
...
![](../../../images/be4f5109/note_3.0-zh-cn.png) 

自定义界面扫码返回的坐标单位与传入参数[ViewControl](/ref/scan-api/scan-arkts/scan-customscan-api/scan-customscan-api#viewcontrol)中width、height单位一致。
...
****设备行为差异：**** 对于6.0.2(22)及之前版本，该接口在Phone、Tablet中可正常调用。对于6.1.0(23)及之后版本，该接口在Phone、Tablet、带后置相机的Wearable中可正常调用，在不带后置相机的Wearable中无响应。可以通过[cameraManager.getSupportedCameras](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedcameras)接口查询是否带后置相机。
...
| ****参数名**** | ****类型**** | 必填 | ****说明**** |
| --- | --- | --- | --- |
| context | [common.Context](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-common/js-apis-app-ability-common#context) | 是 | 应用上下文。 |
