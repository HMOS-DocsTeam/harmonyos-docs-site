...
![](../../../../../images/9801543a/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [FunctionKey](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#functionkey10) | 是 | 热键的单个字符（可以通过键盘输入的字符）或[FunctionKey](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#functionkey10)。  空字符串意为取消快捷键绑定。 |
| keys | Array&lt;[ModifierKey](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#modifierkey10)&gt; | 是 | 热键组合。  仅当value为[FunctionKey](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#functionkey10)的情况下keys的值可以为空。 |
...
快捷键是对系统按键的响应，优先于普通的按键事件OnKeyEvent，按键事件触发的逻辑详见[按键事件数据流](/arkui/arkts-ui-development/arkts-interaction-development-guide-overview/rkts-interaction-development-guide-raw-input-event/arkts-interaction-development-guide-keyboard#按键事件数据流)。

| 场景 | 快捷键处理逻辑 | 例子 |
| --- | --- | --- |
| 所有支持onClick事件的组件 | 支持自定义组合键 | 无 |
| 自定义组合键要求 | 控制键Ctrl、Shift、Alt及它们的组合加上热键的单个字符（可以通过键盘输入的字符）或[FunctionKey](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#functionkey10) | Button('button1').keyboardShortcut('a',[ModifierKey.CTRL]) |
...
| 隐藏组件 | 响应快捷键 | 无 |
| 不可交互（[enabled](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/interaction-property/ts-universal-attributes-enable/ts-universal-attributes-enable#enabled)设置为false）状态的组件 | 不响应快捷键 | 无 |
...
表中的按键事件与自定义按键事件的触发有优先级关系，高优先级的事件会拦截低优先级事件，焦点事件响应优先级详见[按键事件数据流](/arkui/arkts-ui-development/arkts-interaction-development-guide-overview/rkts-interaction-development-guide-raw-input-event/arkts-interaction-development-guide-keyboard#按键事件数据流)。
...
![](../../../../../images/d72569b8/zh-cn_image_0000002552799822.gif)
...
![](../../../../../images/0ab8e6b5/zh-cn_image_0000002583439517.gif)
