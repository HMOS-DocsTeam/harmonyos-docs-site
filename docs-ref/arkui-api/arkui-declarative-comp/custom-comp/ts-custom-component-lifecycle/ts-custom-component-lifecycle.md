...
![](../../../../images/d361d539/note_3.0-zh-cn.png) 
...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变[状态变量](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-glossary#状态变量state-variables)，更改将在后续执行build()函数中生效。实现[自定义布局](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-layout/ts-custom-component-layout)的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见[自定义组件生命周期指南](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-page-custom-components-lifecycle)。

![](../../../../images/048a06f2/note_3.0-zh-cn.png) 
...
onDidBuild函数在自定义组件的build()函数执行后调用，开发者可以在这个阶段实现埋点数据上报等不影响实际UI的功能。具体使用说明，详见[自定义组件生命周期指南](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-page-custom-components-lifecycle)。
...
aboutToDisappear函数在自定义组件析构销毁时执行。不允许在aboutToDisappear函数中改变状态变量，特别是@Link变量的修改可能会导致应用程序行为不稳定。具体使用说明，详见[自定义组件生命周期指南](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-page-custom-components-lifecycle)。不建议在aboutToDisappear函数调用后再触发例如[自定义弹窗的创建](/ref/arkui-api/arkui-declarative-comp/dialog-boxes/ts-methods-custom-dialog-box/ts-methods-custom-dialog-box#open)等逻辑，这可能会因为组件树信息丢失导致应用行为异常，例如[@Consume](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-provide-and-consume)找不到对应的[@Provide](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-provide-and-consume)、弹窗内白屏不显示组件等。

![](../../../../images/d174b5c4/note_3.0-zh-cn.png) 
...
router路由页面（即[@Entry](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-create-custom-components#entry)装饰的自定义组件）每次显示时触发一次，包括路由跳转、应用进入前台等场景。
...
router路由页面（即[@Entry](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-create-custom-components#entry)装饰的自定义组件）每次隐藏时触发一次，包括路由跳转、应用进入后台等场景。

![](../../../../images/8d34b4f2/note_3.0-zh-cn.png) 
...
在router路由页面（即[@Entry](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-create-custom-components#entry)装饰的自定义组件）生效，当用户点击返回按钮时触发。返回true表示页面自己处理返回逻辑，不进行页面路由；返回false表示使用默认的路由返回逻辑，不设置返回值按照false处理。
...
![](../../../../images/50f320cc/zh-cn_image_0000002583440135.gif)

## onNewParam19+

onNewParam?(param: ESObject): void

该回调仅生效于由[@Entry](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-create-custom-components#entry)装饰的、作为[router](/ref/arkui-api/arkui-arkts/ui/js-apis-router/js-apis-router)路由页面存在的自定义组件。当之前存在于路由栈中的页面，通过单实例模式[RouterMode](/ref/arkui-api/arkui-arkts/ui/js-apis-router/js-apis-router#routermode9)移动到栈顶时触发该回调。
...
![](../../../../images/d9abc326/note_3.0-zh-cn.png) 
...
详细内容请参考[@ReusableV2](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-component-reusable/arkts-new-reusablev2)。
...
![](../../../../images/4e0cd746/note_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [Theme](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-theme/js-apis-arkui-theme#theme) | 自定义组件当前生效的Theme对象。 |
...
![](../../../../images/e13289d8/zh-cn_image_0000002552960090.png)
...
![](../../../../images/e41c2112/zh-cn_image_0000002583480091.png)
