...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arengine-api-arengine
kit: 图形
last_updated: "2026-04-22"
slug: arengine-api-arengine
---
...
| ****名称**** | ****值**** | ****说明**** |
| --- | --- | --- |
| NORMAL | 0 | 正常模式。  当添加的图片数量超过默认的最多图片数量（50）时，AR Engine会报告错误，错误信息见[1009200011](/ref/ar-engine-api/arengine-error-code/arengine-error-code#section1009200011-添加的图片数量超过最大数量)。 |
...
| GRAVITY\_AND\_HEADING | 1 | 由重力和指南针确定。  世界坐标系的Y轴与重力方向一致，X轴指向指南针北向，原点为设备所在的原始位置。参见[坐标系说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview#坐标系说明)。  ****说明：**** 该选项当前仅支持省电模式，请将[ARPowerMode](#arpowermode)置为POWER\_SAVING以启动该选项。 |
...
****设备行为差异：**** 该接口在部分Phone、部分Tablet中可正常调用，在不支持的设备中无法正常调用。可使用[arViewController.isARTypeSupported](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arviewcontroller/arengine-api-arviewcontroller#arviewcontrollerisartypesupported)接口查询能力是否支持。
...
![](../../../images/d6b8efba/note_3.0-zh-cn.png) 
...
以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[AR Engine错误码](/ref/ar-engine-api/arengine-error-code/arengine-error-code)。
...
以下错误码的详细介绍请参见[AR Engine错误码](/ref/ar-engine-api/arengine-error-code/arengine-error-code)。
...
| pixelMap | [image.PixelMap](/ref/image-api/image-arkts/js-apis-sendableimage/js-apis-sendableimage#pixelmap) | 是 | 图像的信息。 |
...
| planeFindingMode | [ARPlaneFindingMode](#arplanefindingmode) | 否 | 是 | 平面检测模式。  默认为HORIZONTAL\_AND\_VERTICAL。 |
...
| multiFaceMode | [ARMultiFaceMode](#armultifacemode) | 否 | 是 | 多人脸模式配置项。  默认为MULTIFACE\_DISABLE。  ****起始版本：**** 6.1.0(23) |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| id | ArrayBuffer | 是 | 否 | 稠密点云数据所有点的id数组。  ****说明：**** 该类型在使用时，如需转换则为int32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
| position | ArrayBuffer | 是 | 否 | 稠密点云数据所有点的坐标，以及它们的置信度数组，格式为[x0, y0, z0, c0, x1, y1, z1, c1, x2, ...]。  ****说明：**** 该类型在使用时，如需转换则为float32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
| color | ArrayBuffer | 是 | 否 | 稠密点云数据所有点的颜色，以 rgba 格式表示，格式为 [r0, g0, b0, a0, r1, g1, b1, a1, r2,...]。  ****说明：**** 该类型在使用时，如需转换则为int32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
...
![](../../../images/9b30d0d1/note_3.0-zh-cn.png) 
...
| buffer | ArrayBuffer | 是 | 否 | 当前帧的平面数据缓冲区对象。  ****说明：**** 该类型在使用时，如需转换则为int32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
...
![](../../../images/983bedb3/note_3.0-zh-cn.png) 
...
| viewMatrix | [Matrix4](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-graphics/js-apis-arkui-graphics#matrix4) | 是 | 否 | 摄像机在最新帧中的视图矩阵。 |
...
| ****类型**** | ****说明**** |
| --- | --- |
| [Matrix4](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-graphics/js-apis-arkui-graphics#matrix4) | 返回一个Matrix4对象。  Matrix4是一个包含16个浮点数的数组，表示OpenGL中的列主旋转变换矩阵。 |
...
如果[ARPointOrientationMode](#arpointorientationmode)是ESTIMATED\_SURFACE\_NORMAL，则 X+ 是射线的垂直向量，也就是平面的平行向量，Y+ 是平面的法向量，Z+ 是平面的平行向量，大致指向相机。

如果[ARPointOrientationMode](#arpointorientationmode)是INITIALIZED\_TO\_IDENTITY，则坐标的旋转不会随着平面的角度变化而变化。X+ 是射线的垂直向量，指向右侧（从设备的视角），Y+ 指向上方，Z+ 大致指向相机。
...
| ****类型**** | ****说明**** |
| --- | --- |
| ArrayBuffer | 返回一个ArrayBuffer缓冲区对象，这个对象包含顶点坐标的数组。表示平面局部坐标系X-Z平面的坐标。  ****说明：**** 该类型在使用时，如需转换则为float32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| rotation | [Quaternion](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-types/js-apis-inner-scene-types#quaternion) | 是 | 否 | 来自位姿对象的姿势数据，包括旋转分量。 |
| translation | [Vec3](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-types/js-apis-inner-scene-types#vec3) | 是 | 否 | 来自位姿对象的姿势数据，包括平移组件。 |
...
| ****类型**** | ****说明**** |
| --- | --- |
| [Matrix4](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-graphics/js-apis-arkui-graphics#matrix4) | 返回一个包含16个浮点数的数组，按照列主序（column-major order）存储。 |
...
| ****类型**** | ****说明**** |
| --- | --- |
| ArrayBuffer | 返回一个ArrayBuffer缓冲区对象。这个缓冲区包含了顶点在平面局部坐标系X-Z平面的坐标。  ****说明：**** 该类型在使用时，如需转换则为float32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
...
| ****类型**** | ****说明**** |
| --- | --- |
| ArrayBuffer | 返回一个ArrayBuffer缓冲区对象。这个缓冲区包含了顶点在平面局部坐标系X-Z平面的坐标。  ****说明：**** 该类型在使用时，如需转换则为int32类型，转换方法参考[数据类型转换说明](/ar-engine-kit-guide/arengine-guide/arengine-arraybuffer-info)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| rotation | [Quaternion](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-types/js-apis-inner-scene-types#quaternion) | 是 | 表示旋转组件的数据。这个参数是Quaternion类型，通常是一个包含四个元素的数组，为四元数，用于表示和操作3D旋转。 |
| translation | [Vec3](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-types/js-apis-inner-scene-types#vec3) | 是 | 表示平移组件的数据。这个参数是Vec3类型，通常是一个包含三个元素的数组，表示3D向量的x、y和z分量，用于表示物体的平移位置。 |
