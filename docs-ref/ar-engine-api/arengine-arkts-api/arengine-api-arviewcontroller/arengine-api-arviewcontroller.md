...
****设备行为差异：**** 该接口在部分Phone、部分Tablet、TV中可正常调用，在不支持的设备中返回801错误码，可使用[arViewController.isARTypeSupported](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arviewcontroller/arengine-api-arviewcontroller#arviewcontrollerisartypesupported)接口查询能力是否支持。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scene | [Scene](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene/js-apis-inner-scene#scene-1) | 是 | AR呈现场景，用于管理空间节点。 |
...
| 类型 | 说明 |
| --- | --- |
| [Scene](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene/js-apis-inner-scene#scene-1) | AR呈现场景，用于管理空间节点。 |
...
| 类型 | 说明 |
| --- | --- |
| [arEngine.ARSession](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#arsession) | undefined | 获得的AR会话内容，AR会话正常设置之后返回为arEngine.ARSession，异常设置或异常操作导致ARViewContext未初始化成功，返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| conf | [arEngine.ARConfig](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#arconfig) | 是 | [ARSession](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#arsession)的功能配置参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| resourcePath | [ResourceStr](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-types/ts-types#resourcestr) | 是 | 存储模型的路径。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| node | [Node](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-nodes/js-apis-inner-scene-nodes#node) | 是 | 新建锚点对应的AR场景节点。  ****说明：**** Node属于AGP（Ark Graphics Platform）渲染引擎能力，使用了AGP齐世界坐标系，与AR Engine所使用的重力对齐世界坐标系不一致，与anchor参数一同使用时需注意区分。具体参见[坐标系说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview#坐标系说明)。 |
| type | [arEngine.ARBlendShapeType](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#arblendshapetype) | 是 | 微表情类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| position | [Vec3](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-types/js-apis-inner-scene-types#vec3) | 是 | 位姿坐标信息 |
| rotation | [Quaternion](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-types/js-apis-inner-scene-types#quaternion) | 是 | 位姿朝向信息。 |

****返回值：****

| ****类型**** | ****说明**** |
| --- | --- |
| arEngine.ARPose | null | 转换后的位姿信息，若传入参数无效或创建[ARPose](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#arpose)对象失败则返回null。 |
...
| node | [Node](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-nodes/js-apis-inner-scene-nodes#node) | 是 | 新建锚点对应的AR场景节点。  ****说明：**** Node属于AGP（Ark Graphics Platform）渲染引擎能力，使用了AGP齐世界坐标系，与AR Engine所使用的重力对齐世界坐标系不一致，与anchor参数一同使用时需注意区分。具体参见[坐标系说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview#坐标系说明)。 |
| anchor | [arEngine.ARAnchor](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#aranchor) | 是 | 新平面对应的新创建的锚点。  ****说明：**** [ARAnchor](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#aranchor)的位姿使用了AR Engine重力对齐世界坐标系。具体参见[坐标系说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview#坐标系说明)。 |
...
| ctx | [ARViewContext](#arviewcontext) | 是 | 当前ARView的上下文信息（ARViewContext）。 |
| node | [Node](/ref/arkgraphics-3d-api/arkgraphics-3d-arkts/graphics3d/js-apis-inner-scene-nodes/js-apis-inner-scene-nodes#node) | 是 | 新建锚点对应的AR场景节点。  ****说明：**** Node属于AGP（Ark Graphics Platform）渲染引擎能力，使用了AGP齐世界坐标系，与AR Engine所使用的重力对齐世界坐标系不一致，与anchor参数一同使用时需注意区分。具体参见[坐标系说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview#坐标系说明)。 |
| anchor | [arEngine.ARAnchor](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#aranchor) | 是 | 更新锚点。  ****说明：**** [ARAnchor](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#aranchor)的位姿使用了AR Engine重力对齐世界坐标系。具体参见[坐标系说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-overview#坐标系说明)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | [arEngine.ARFeatureType](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arengine/arengine-api-arengine#arfeaturetype) | 是 | 表示想要检查能否正常运行的AR特性。 |
