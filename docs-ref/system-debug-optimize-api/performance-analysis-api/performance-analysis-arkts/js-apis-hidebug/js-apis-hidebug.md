...
sidebar_position: 2
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-hidebug
kit: 系统
...
![](../../../../images/dc9d8c2a/note_3.0-zh-cn.png) 
...
![](../../../../images/9f5fea34/caution_3.0-zh-cn.png) 

由于/proc/\{pid\}/smaps\_rollup的读取耗时较长，建议不要在主线程中使用该接口，可通过[@ohos.taskpool](/ref/arkts-api/arkts-arkts/js-apis-taskpool/js-apis-taskpool)或[@ohos.worker](/ref/arkts-api/arkts-arkts/js-apis-worker/js-apis-worker)开启异步线程以避免应用出现卡顿。
...
![](../../../../images/68f30122/caution_3.0-zh-cn.png) 
...
![](../../../../images/cee358ea/caution_3.0-zh-cn.png) 
...
![](../../../../images/084ec02d/caution_3.0-zh-cn.png) 
...
以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)与[HiDebug错误码](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-errcode/errorcode-hiviewdfx-hidebug/errorcode-hiviewdfx-hidebug)。
...
![](../../../../images/cc5eb2f7/caution_3.0-zh-cn.png) 
...
![](../../../../images/749baa05/note_3.0-zh-cn.png) 
...
![](../../../../images/2314119e/note_3.0-zh-cn.png) 
...
![](../../../../images/6d101864/note_3.0-zh-cn.png) 
...
![](../../../../images/ddc57333/caution_3.0-zh-cn.png) 
...
该接口补充了[hitrace](/system-debug-optimize/debugging-commands/hitrace)功能，开发者可通过该接口完成指定范围的trace自动化采集。由于该接口中trace采集过程中消耗的性能与需要采集的范围成正相关，建议开发者在使用该接口前，通过hitrace命令抓取应用的trace日志，从中筛选出所需trace采集的关键范围，以提高该接口性能。
...
| 类型 | 说明 |
| --- | --- |
| string | 返回trace文件名路径（接口返回真实物理路径，若应用内需要访问，请参考[应用沙箱路径和真实物理路径的对应关系](/core-file-kit/app-file/app-sandbox-directory#应用沙箱路径和真实物理路径的对应关系)进行路径转换）。 |

****错误码****：

以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)与[HiDebug Trace错误码](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-errcode/errorcode-hiviewdfx-hidebug-trace/errorcode-hiviewdfx-hidebug-trace)。
...
以下错误码的详细介绍请参见[HiDebug Trace错误码](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-errcode/errorcode-hiviewdfx-hidebug-trace/errorcode-hiviewdfx-hidebug-trace)。
...
![](../../../../images/8e51901b/caution_3.0-zh-cn.png) 
...
以下错误码的详细介绍请参见[HiDebug CpuUsage错误码](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-errcode/errorcode-hiviewdfx-hidebug-cpuusage/errorcode-hiviewdfx-hidebug-cpuusage)。
...
主要应用场景在于构造内存泄漏故障，参见[订阅资源泄漏事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/resource-leak-events/hiappevent-watcher-resourceleak-events-arkts)、[订阅资源泄漏事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/resource-leak-events/hiappevent-watcher-resourceleak-events-ndk)。

![](../../../../images/bdb8a80b/caution_3.0-zh-cn.png) 
...
![](../../../../images/b53641bb/caution_3.0-zh-cn.png) 
...
![](../../../../images/0ef93dc0/caution_3.0-zh-cn.png) 

由于读取 /proc/\{pid\}/smaps\_rollup 比较耗时，建议不在主线程中使用该接口。可以通过[@ohos.taskpool](/ref/arkts-api/arkts-arkts/js-apis-taskpool/js-apis-taskpool)或[@ohos.worker](/ref/arkts-api/arkts-arkts/js-apis-worker/js-apis-worker)开启异步线程，以避免应用卡顿。
...
获取系统[GC](/arkts/arkts-runtime/gc-introduction)统计信息。
...
根据参数获取指定的系统[GC](/arkts/arkts-runtime/gc-introduction)统计信息。
...
支持trace使用场景的标签，用户可通过[hitrace](/system-debug-optimize/debugging-commands/hitrace)抓取指定标签的trace内容。

![](../../../../images/25b50356/caution_3.0-zh-cn.png) 
...
以下错误码的详细介绍请参见[HiDebug错误码](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-errcode/errorcode-hiviewdfx-hidebug/errorcode-hiviewdfx-hidebug)。
...
![](../../../../images/6cb93b63/caution_3.0-zh-cn.png) 
...
为当前线程转储虚拟机的原始堆快照，并生成的rawheap文件，该文件可通过[rawheap-translator工具](/system-debug-optimize/debugging-commands/command-line-utilities/rawheap-translator)将所生成文件转化为heapsnapshot文件进行解析。生成的文件路径使用Promise进行异步回调。

![](../../../../images/f4a14fd7/caution_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;string&gt; | Promise对象，返回生成的快照文件路径（[应用沙箱内路径](/core-file-kit/app-file/app-sandbox-directory#应用沙箱路径和真实物理路径的对应关系)）。 |
...
![](../../../../images/e1624cab/note_3.0-zh-cn.png) 
...
![](../../../../images/434608fc/caution_3.0-zh-cn.png) 

默认裁剪级别是TRIM\_LEVEL\_1。如果设置了TRIM\_LEVEL\_2裁剪，需使用API version 20之后的[rawheap-translator](/system-debug-optimize/debugging-commands/command-line-utilities/rawheap-translator)工具才能将.rawheap文件转换为.heapsnapshot文件，否则可能导致转换失败。
