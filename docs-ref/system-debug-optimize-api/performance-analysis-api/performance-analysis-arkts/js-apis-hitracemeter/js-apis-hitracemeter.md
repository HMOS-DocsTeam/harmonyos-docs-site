...
详细开发流程请参考：[使用HiTraceMeter跟踪性能（ArkTS）](/system-debug-optimize/performance-analysis-kit/hitracemeter/hitracemeter-guidelines-arkts)。

![](../../../../images/cd18caf1/note_3.0-zh-cn.png) 
...
[用户态trace格式](/system-debug-optimize/performance-analysis-kit/hitracemeter/hitracemeter-view#用户态trace格式说明)使用竖线 | 作为分隔符，所以通过性能打点接口传递的字符串类型参数应避免包含该字符，防止trace解析异常。

[用户态trace](/system-debug-optimize/performance-analysis-kit/hitracemeter/hitracemeter-view#用户态trace格式说明)总长度限制512字符，超过的部分将会被截断。
...
| 类型 | 说明 |
| --- | --- |
| boolean | 使用[hitrace](/system-debug-optimize/debugging-commands/hitrace)命令行工具等方式开启采集时返回true。未开启采集或停止采集后返回false，此时调用HiTraceMeter性能跟踪打点接口无效。 |
...
![](../../../../images/98d27181/note_3.0-zh-cn.png) 
