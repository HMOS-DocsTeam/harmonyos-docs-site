...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [HiAppEvent\_AppEventInfo](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-appeventinfo/capi-hiappevent-hiappevent-appeventinfo) | HiAppEvent\_AppEventInfo | 单个事件信息，包含事件领域、事件名称、事件类型和事件携带的用json格式字符串表示的自定义参数列表。 |
| [HiAppEvent\_AppEventGroup](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-appeventgroup/capi-hiappevent-hiappevent-appeventgroup) | HiAppEvent\_AppEventGroup | 一组事件信息，包含事件组的名称，按名称分组的单个事件信息数组，事件数组的长度。 |
| [ParamListNode\*](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-paramlistnode8h/capi-hiappevent-paramlistnode8h) | ParamList | 事件参数列表节点。 |
| [HiAppEvent\_Watcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-watcher/capi-hiappevent-hiappevent-watcher) | HiAppEvent\_Watcher | 用于接收app事件的监听器。 |
| [HiAppEvent\_Processor](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-processor/capi-hiappevent-hiappevent-processor) | HiAppEvent\_Processor | 应用事件上报的处理者。 |
| [HiAppEvent\_Config](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-config/capi-hiappevent-hiappevent-config) | HiAppEvent\_Config | 用于设置系统事件触发条件的配置对象。 |
...
| [typedef void (\*OH\_HiAppEvent\_OnTrigger)(int row, int size)](#oh_hiappevent_ontrigger) | OH\_HiAppEvent\_OnTrigger | 监听器收到事件后，若监听器中未设置OH\_HiAppEvent\_OnReceive回调，将保存该事件。  当保存的事件满足通过OH\_HiAppEvent\_SetTriggerCondition设定的条件后，将触发该回调。回调结束后，当新保存的事件消息再次满足设定的条件后，将再次进行回调。 |
...
| [ParamList OH\_HiAppEvent\_CreateParamList(void)](#oh_hiappevent_createparamlist) | - | 创建一个指向参数列表对象的指针。  ****注意****：创建的指向参数列表对象的指针不再使用后，必须通过[OH\_HiAppEvent\_DestroyParamList](#oh_hiappevent_destroyparamlist)接口进行销毁。 |
...
| [HiAppEvent\_Watcher\* OH\_HiAppEvent\_CreateWatcher(const char\* name)](#oh_hiappevent_createwatcher) | - | 创建一个用于监听app事件的监听器。  ****注意****：创建的监听器不再使用后，必须通过[OH\_HiAppEvent\_DestroyWatcher](#oh_hiappevent_destroywatcher)接口进行销毁。 |
...
| [int OH\_HiAppEvent\_SetTriggerCondition(HiAppEvent\_Watcher\* watcher, int row, int size, int timeOut)](#oh_hiappevent_settriggercondition) | - | 用于设置监听器OH\_HiAppEvent\_OnTrigger回调的触发条件。  分别可以从监视器新接收事件数量、新接收事件大小、onTrigger触发超时时间，设置触发条件。调用方应至少保证从一个方面设置触发条件。 |
...
| [int OH\_HiAppEvent\_SetWatcherOnTrigger(HiAppEvent\_Watcher\* watcher, OH\_HiAppEvent\_OnTrigger onTrigger)](#oh_hiappevent_setwatcherontrigger) | - | 用于设置监听器onTrigger回调的接口。  如果未设置OnReceive回调或已将其设置为nullptr，则将保存观察者接收到的应用事件。当保存的应用事件满足onTrigger回调的触发条件时，将调用onTrigger回调。 |
...
| [int OH\_HiAppEvent\_AddWatcher(HiAppEvent\_Watcher\* watcher)](#oh_hiappevent_addwatcher) | - | 添加监听器的接口，监听器开始监听系统消息。  ****注意****：OH\_HiAppEvent\_AddWatcher接口涉及I/O操作。在对性能敏感的业务场景中，开发者应根据实际需要确定该接口是在主线程还是在子线程中调用。  订阅接口OH\_HiAppEvent\_AddWatcher传入的名称name是唯一的，相同的name，后一次调用会覆盖前一次的订阅。 |
...
| [HiAppEvent\_Processor\* OH\_HiAppEvent\_CreateProcessor(const char\* name)](#oh_hiappevent_createprocessor) | - | 创建一个用于处理app事件上报的处理者。  ****注意****：创建的处理者不再使用后，必须通过[OH\_HiAppEvent\_DestroyProcessor](#oh_hiappevent_destroyprocessor)接口进行销毁。 |
...
| [HiAppEvent\_Config\* OH\_HiAppEvent\_CreateConfig(void)](#oh_hiappevent_createconfig) | - | 创建一个指向设置系统事件触发条件的配置对象的指针。  ****注意****：创建的指向设置系统事件触发条件的配置对象的指针不再使用后，必须通过[OH\_HiAppEvent\_DestroyConfig](#oh_hiappevent_destroyconfig)接口进行销毁。 |
...
| [int OH\_HiAppEvent\_SetEventConfig(const char\* name, HiAppEvent\_Config\* config)](#oh_hiappevent_seteventconfig) | - | 事件相关的配置参数设置方法。  不同的事件有不同的配置项，目前仅支持以下事件：  MAIN\_THREAD\_JANK（参数配置详见[主线程超时事件检测](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events#oh_hiappevent_seteventconfig接口参数设置说明)）  MAIN\_THREAD\_JANK\_V2（参数配置详见[主线程超时事件检测](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events#oh_hiappevent_seteventconfig接口参数设置说明)） |
...
| [const struct HiAppEvent\_AppEventGroup](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-appeventgroup/capi-hiappevent-hiappevent-appeventgroup)\* appEventGroups | 按照不同事件名称分组的事件组数组。 |
...
![](../../../../../images/47347e31/caution_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [ParamList](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-paramlistnode8h/capi-hiappevent-paramlistnode8h) | 指向参数列表对象的指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ParamList](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-paramlistnode8h/capi-hiappevent-paramlistnode8h) list | 参数列表对象指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ParamList](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-paramlistnode8h/capi-hiappevent-paramlistnode8h) list | 需要添加参数的参数列表指针。 |
...
| 类型 | 说明 |
| --- | --- |
| [ParamList](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-paramlistnode8h/capi-hiappevent-paramlistnode8h) | 添加参数后的参数列表指针。 |
...
| enum EventType type | 事件类型，在[EventType](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#eventtype)中定义。 |
| [const ParamList](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-paramlistnode8h/capi-hiappevent-paramlistnode8h) list | 事件参数列表，每个参数由参数名和参数值组成，其规格定义如下：  1、参数名为字符串类型。  首字符必须为字母字符或$字符，中间字符必须为数字字符、字母字符或下划线字符，结尾字符必须为数字字符或字母字符，长度非空且不超过32个字符。  2、参数值支持字符串、数值、布尔、数组类型。字符串类型参数长度需在8\*1024个字符以内，超出会做丢弃处理；  数组类型参数中的元素类型只能为字符串、数值、布尔中的一种，且元素个数需在100以内，超出会做丢弃处理。  3、参数个数需在32个以内，超出的参数会做丢弃处理。 |
...
| 参数项 | 描述 |
| --- | --- |
| const char\* name | 配置项名称。名称可填[DISABLE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-cfg-h/capi-hiappevent-cfg-h#disable)和[MAX\_STORAGE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-cfg-h/capi-hiappevent-cfg-h#max_storage)。 |
| const char\* value | 配置项值。如果配置项名称是[DISABLE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-cfg-h/capi-hiappevent-cfg-h#disable)，值可以填“true”或者“false”；  如果配置项名称是[MAX\_STORAGE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-cfg-h/capi-hiappevent-cfg-h#max_storage)，配额值字符串只由数字字符和大小单位字符（单位字符支持[b|k|kb|m|mb|g|gb|t|tb]，不区分大小写）构成。  配额值字符串必须以数字开头，后面可以选择不传单位字符（默认使用byte作为单位），或者以单位字符结尾。 |
...
![](../../../../../images/1f484e4a/caution_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent\_Watcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-watcher/capi-hiappevent-hiappevent-watcher)\* | 接口调用成功时返回指向的新建监听器的指针，name参数异常时返回nullptr。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent\_Watcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-watcher/capi-hiappevent-hiappevent-watcher)\* watcher | 指向监听器的指针（即OH\_HiAppEvent\_CreateWatcher接口返回的指针）。 |
...
| --- | --- |
| [HiAppEvent\_Watcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-watcher/capi-hiappevent-hiappevent-watcher)\* watcher | 指向监听器的指针（即OH\_HiAppEvent\_CreateWatcher接口返回的指针）。 |
| [OH\_HiAppEvent\_OnTrigger](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#oh_hiappevent_ontrigger) onTrigger | 需要设置的回调。 |
...
| --- | --- |
| [HiAppEvent\_Watcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-watcher/capi-hiappevent-hiappevent-watcher)\* watcher | 指向监听器的指针（即OH\_HiAppEvent\_CreateWatcher接口返回的指针）。 |
| [OH\_HiAppEvent\_OnReceive](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#oh_hiappevent_onreceive) onReceive | 回调函数的函数指针。 |
...
| [OH\_HiAppEvent\_OnTake](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#oh_hiappevent_ontake) onTake | 回调函数指针，事件通过调用该函数返回事件信息。 |
...
![](../../../../../images/d7532251/caution_3.0-zh-cn.png) 
...
![](../../../../../images/43228cea/caution_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent\_Processor](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-processor/capi-hiappevent-hiappevent-processor)\* | 接口调用成功时返回指向的新建处理者的指针，name参数异常时返回nullptr。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent\_Processor](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-processor/capi-hiappevent-hiappevent-processor)\* processor | 指向处理者的指针（即OH\_HiAppEvent\_CreateProcessor接口返回的指针）。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT\_SUCCESS](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功；  [HIAPPEVENT\_PROCESSOR\_IS\_NULL](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：processor入参为空；  [HIAPPEVENT\_INVALID\_PARAM\_VALUE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：参数值无效；  [HIAPPEVENT\_INVALID\_UID](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效；  [HIAPPEVENT\_INVALID\_PARAM\_VALUE\_LENGTH](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：参数值长度无效。  具体可参考[HiAppEvent\_ErrorCode](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT\_SUCCESS](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功；  [HIAPPEVENT\_PROCESSOR\_IS\_NULL](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：processor入参为空；  [HIAPPEVENT\_INVALID\_PARAM\_VALUE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：参数值无效；  [HIAPPEVENT\_INVALID\_UID](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效。  具体可参考[HiAppEvent\_ErrorCode](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)。 |
...
| 类型 | 说明 |
| --- | --- |
| int64\_t | 调用成功时返回处理者唯一ID，大于0；  [HIAPPEVENT\_PROCESSOR\_IS\_NULL](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：processor入参为空；  [HIAPPEVENT\_INVALID\_PARAM\_VALUE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：参数值无效；  [HIAPPEVENT\_OPERATE\_FAILED](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：数据处理者名称未找到或注册失败；  [HIAPPEVENT\_INVALID\_UID](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效。  具体可参考[HiAppEvent\_ErrorCode](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT\_SUCCESS](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功；  [HIAPPEVENT\_PROCESSOR\_NOT\_FOUND](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：事件处理者不存在；  [HIAPPEVENT\_OPERATE\_FAILED](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：操作失败；  [HIAPPEVENT\_INVALID\_UID](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效。  具体可参考[HiAppEvent\_ErrorCode](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)。 |
...
![](../../../../../images/c5529d12/caution_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent\_Config](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-config/capi-hiappevent-hiappevent-config)\* | 指向设置系统事件触发条件的配置对象的指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent\_Config](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-struct/capi-hiappevent-hiappevent-config/capi-hiappevent-hiappevent-config)\* config | 指向配置对象的指针（即OH\_HiAppEvent\_CreateConfig接口返回的指针）。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT\_SUCCESS](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功；  [HIAPPEVENT\_EVENT\_CONFIG\_IS\_NULL](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：传入的指向配置对象的指针为空；  [HIAPPEVENT\_INVALID\_PARAM\_VALUE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：设定的配置项无效。  具体可参考[HiAppEvent\_ErrorCode](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)。 |
...
MAIN\_THREAD\_JANK（参数配置详见[主线程超时事件检测](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events#oh_hiappevent_seteventconfig接口参数设置说明)）

MAIN\_THREAD\_JANK\_V2（参数配置详见[主线程超时事件检测](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events#oh_hiappevent_seteventconfig接口参数设置说明)）
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT\_SUCCESS](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功；  [HIAPPEVENT\_INVALID\_PARAM\_VALUE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)：设置的参数无效。  具体可参考[HiAppEvent\_ErrorCode](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-hiappevent-h/capi-hiappevent-h#hiappevent_errorcode)。 |
