...
更新企业公钥证书流程，需要先调[getAuthChallenge](#getauthchallenge)接口获取挑战值并[签名](/system-security/data-guard-kit-guide/dataguard-enterprise-recoverykey/recoverykey-signature)。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| signature | Uint8Array | 是 | 挑战值的[签名](/system-security/data-guard-kit-guide/dataguard-enterprise-recoverykey/recoverykey-signature)。 |
...
删除企业恢复密钥相关数据，需要先调[getAuthChallenge](#getauthchallenge)接口获取挑战值并[签名](/system-security/data-guard-kit-guide/dataguard-enterprise-recoverykey/recoverykey-signature)。使用Promise异步回调。
...
| userType | number | 是 | [用户类型](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/account-management-arkts/js-apis-osaccount/js-apis-osaccount#osaccounttype) 。可以通过调用[getOsAccountType](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/account-management-arkts/js-apis-osaccount/js-apis-osaccount#getosaccounttype9)获取。 |
...
导出用于重置锁屏密码的企业恢复密钥。先需要调用[verifyUserIdentityEnterprise](#verifyuseridentityenterprise)接口验证身份，并在30秒内调用此接口。若超时后调用，系统会返回异常代码[1014400001](/ref/system-security-api/data-guard-api/data-guard-arkts/dataguard-error-code/dataguard-error-code#section1014400001-系统内部错误)。使用Promise异步回调。
