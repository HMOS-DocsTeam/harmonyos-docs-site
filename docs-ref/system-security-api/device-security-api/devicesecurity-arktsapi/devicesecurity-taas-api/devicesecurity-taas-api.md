...
以下错误码的详细介绍请参见[ArkTS API错误码](/ref/system-security-api/device-security-api/devicesecurity-arktsapi/devicesecurity-arktsapi-errcode/devicesecurity-arktsapi-errcode-taas/devicesecurity-arktsapi-errcode-taas) ****。****
...
| value | boolean|number|bigint|Uint8Array | 否 | 否 | 开发者应用传入的标签对应的值，用于生成证明密钥的配置信息。  ****boolean：****  预留参数，暂未使用。  ****number：****  1）tag为ATTEST\_TAG\_ALGORITHM，其值为[AttestKeyAlg](#attestkeyalg)类型。  2）tag为ATTEST\_TAG\_KEY\_SIZE，其值为[AttestKeySize](#attestkeysize)类型。  3）tag为ATTEST\_TAG\_DEVICE\_TYPE，其值为[AttestType](#attesttype)类型。  ****bigint：****  tag为ATTEST\_TAG\_DEVICE\_ID，其值为设备ID，取值范围int64类型的随机值。  ****Uint8Array：****  预留参数，暂未使用。 |
...
| value | number | [CropRegion](#cropregion) | 否 | 否 | 开发者应用传入的标签对应的值，用于安全图像压缩、裁剪处理的配置信息。  ****number：****  1）tag为SECIMAGE\_TAG\_SRC\_IMAGE\_FORMAT或者  SECIMAGE\_TAG\_DEST\_IMAGE\_FORMAT  ，其值为[SecImageProcParamsArray](#secimageprocparamsarray)类型；  2）tag为SECIMAGE\_TAG\_PROC\_OPERATION，其值为[SecImageProcOperation](#secimageprocoperation)类型；  3）tag为SECIMAGE\_TAG\_COMPRESSION\_QUALITY，其值为1到100之间；  ****CropRegion：****  tag为SECIMAGE\_TAG\_CROP\_REGION，其值为[CropRegion](#cropregion)类型。 |
...
![](../../../../images/f1ac7ca7/zh-cn_image_0000002583480533.jpg)
