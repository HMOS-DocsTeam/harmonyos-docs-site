...
title: "SOTER"
sidebar_position: 3
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/onlineauthentication-soter-api
kit: 系统
...
以下错误码的详细介绍请参见[SOTER免密认证错误码](/ref/system-security-api/online-authentication-api/onlineauthentication_arkts/onlineauthentication-error-code/onlineauthentication-error-code-soter/onlineauthentication-error-code-soter)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keyType | [KeyType](#keytype) | 是 | 密钥选型，当前只支持 soter.KeyType.ECC\_P256。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keyType | [KeyType](#keytype) | 是 | 密钥选型，目前只支持 soter.KeyType.ECC\_P256。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keyType | [KeyType](#keytype) | 是 | 密钥选型，目前只支持 soter.KeyType.ECC\_P256，其余枚举值均报错。 |
...
appSecureKeyPromise.then(result => {
  console.info('Succeeded in doing generateAppSecureKey.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
...
appSecureKeyPromise.then(result => {
  console.info('Succeeded in doing getAppSecureKey.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
...
hasAuthKeyPromise.then(result => {
  console.info('Succeeded in doing hasAuthKey.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
...
authKeyPromise.then(result => {
  console.info('Succeeded in doing generateAuthKey.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
...
authKeyPromise.then(result => {
  console.info('Succeeded in doing getAuthKey.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
...
challengePromise.then(result => {
  console.info('Succeeded in doing generateChallenge.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
...
authResultPromise.then(result => {
  console.info('Succeeded in doing signWithAuthKey.');
  // 开发者处理结果
}).catch((error: BusinessError) => {
