...
sidebar_position: 28
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-worker
kit: 应用框架
...
请查看[Worker注意事项](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction)，了解Worker使用过程中的相关注意点。

![](../../../images/f5bd5c32/note_3.0-zh-cn.png) 
...
Worker线程的优先级枚举，各优先级对应关系请参考[QoS等级定义](/coding/thread-scheduling/qos-guidelines#qos等级定义)。
...
| MEDIUM | 1 | 任务完成需要几秒钟。是[ThreadWorkerPriority](#threadworkerpriority18)的默认值。对应QOS\_DEFAULT。  ****元服务API****：从API version 18开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scriptURL | string | 是 | Worker线程文件的路径。  路径规则详细参考[文件路径注意事项](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction#文件路径注意事项)。 |
| options | [WorkerOptions](#workeroptions) | 否 | Worker构造的选项。此参数不填时，对应各属性取其默认值。 |

****错误码：****

以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[语言基础类库错误码](/ref/arkts-api/arkts-arkts-errcode/errorcode-utils/errorcode-utils)。
...
以下示例展示了在Stage模型的entry模块Index.ets文件中加载Worker文件的方法，使用Library加载Worker线程文件的场景参考[文件路径注意事项](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction#文件路径注意事项)。
...
宿主线程可以通过转移对象所有权或拷贝数据的方式向Worker线程发送消息。在传递[Sendable对象](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable)时，使用拷贝数据的方式。
...
宿主线程向Worker线程发送消息，消息中的[Sendable对象](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable)通过引用传递，非Sendable对象通过拷贝数据的方式传递。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | Object | 是 | 发送至Worker的数据，该数据对象必须是可序列化对象，序列化支持类型见[序列化类型说明](#序列化支持类型)。如果需要共享数据，支持类型见[Sendable支持的数据类型](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable#sendable支持的数据类型)。 |
...
以下错误码的详细介绍请参见[语言基础类库错误码](/ref/arkts-api/arkts-arkts-errcode/errorcode-utils/errorcode-utils)。
...
Worker线程通过转移对象所有权或拷贝数据的方式向宿主线程发送消息。在传递[Sendable对象](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable)时，使用拷贝数据的方式进行传递。
...
Worker线程向宿主线程发送消息，消息中的[Sendable对象](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable)通过引用传递，非Sendable对象通过拷贝数据的方式传递。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | Object | 是 | 发送至宿主线程的数据，该数据对象必须是可序列化或可共享，序列化支持类型见[序列化类型说明](#序列化支持类型)，共享支持类型见[Sendable支持的数据类型](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable#sendable支持的数据类型)。 |
...
![](../../../images/0842c0a6/note_3.0-zh-cn.png) 
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onexit(deprecated) | (code: number) => void | 否 | 是 | 回调函数。表示Worker销毁时被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中code类型为number，异常退出为1，正常退出为0。默认值为undefined。  ****说明****：从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker.onexit](#属性-1)替代。 |
| onerror(deprecated) | (err: [ErrorEvent](#errorevent)) => void | 否 | 是 | 回调函数。表示Worker在执行过程中发生异常被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中err类型为[ErrorEvent](#errorevent)，表示收到的异常数据。默认值为undefined。  ****说明****：从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker.onerror](#属性-1)替代。 |
| onmessage(deprecated) | (event: [MessageEvent](#messageeventt)) => void | 否 | 是 | 回调函数。表示宿主线程接收到来自其创建的Worker通过workerPort.postMessage接口发送的消息时被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中event类型为[MessageEvent](#messageeventt)，表示收到的Worker消息数据。默认值为undefined。  ****说明****：从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker.onmessage](#属性-1)替代。 |
| onmessageerror(deprecated) | (event: [MessageEvent](#messageeventt)) => void | 否 | 是 | 回调函数。表示当Worker对象接收到一条无法被序列化的消息时被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中event类型为[MessageEvent](#messageeventt)，表示收到的Worker消息数据。默认值为undefined。  ****说明****：从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker.onmessageerror](#属性-1)替代。 |
...
![](../../../images/667fba78/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scriptURL | string | 是 | Worker线程文件的路径，详细参考[文件路径注意事项](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction#文件路径注意事项)。 |
| options | [WorkerOptions](#workeroptions) | 否 | Worker构造的选项。 |

****示例：****

此处以在Stage模型的entry模块Index.ets文件中加载Worker文件为例，使用Library加载Worker线程文件的场景参考[文件路径注意事项](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction#文件路径注意事项)。
...
![](../../../images/24931353/note_3.0-zh-cn.png) 
...
![](../../../images/829fceff/note_3.0-zh-cn.png) 
...
![](../../../images/56b972c8/note_3.0-zh-cn.png) 
...
![](../../../images/207a0101/note_3.0-zh-cn.png) 
...
![](../../../images/8d9ab5c5/note_3.0-zh-cn.png) 
...
![](../../../images/08daa00a/note_3.0-zh-cn.png) 
...
![](../../../images/90504154/note_3.0-zh-cn.png) 
...
![](../../../images/8b27128b/note_3.0-zh-cn.png) 
...
![](../../../images/0cca0f0f/note_3.0-zh-cn.png) 
...
![](../../../images/f76d5ecb/note_3.0-zh-cn.png) 
...
![](../../../images/22659de6/note_3.0-zh-cn.png) 
...
![](../../../images/7b71bc13/note_3.0-zh-cn.png) 
...
![](../../../images/27b5b40e/note_3.0-zh-cn.png) 
...
![](../../../images/347dbdc6/note_3.0-zh-cn.png) 
...
![](../../../images/ddcc34c9/note_3.0-zh-cn.png) 
...
![](../../../images/c7f7c754/note_3.0-zh-cn.png) 
...
![](../../../images/2b8c68c3/note_3.0-zh-cn.png) 
...
![](../../../images/5f4c82a5/note_3.0-zh-cn.png) 
...
![](../../../images/5a3d2776/note_3.0-zh-cn.png) 
...
![](../../../images/d18f7459/note_3.0-zh-cn.png) 
...
![](../../../images/2067bd30/note_3.0-zh-cn.png) 
