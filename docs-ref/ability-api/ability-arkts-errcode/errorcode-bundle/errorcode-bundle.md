...
title: "包管理子系统通用错误码"
sidebar_position: 3
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-bundle
kit: 应用框架
...
![](../../../images/b1b3f287/note_3.0-zh-cn.png) 
...
2. 可以使用[查询应用信息命令（dump）](/system-debug-optimize/debugging-commands/bm-tool#查询应用信息命令dump)查看应用是否安装。查看输出的打印信息，应用未安装时，该命令执行会报错。
...
2. 可以使用[查询应用信息命令（dump）](/system-debug-optimize/debugging-commands/bm-tool#查询应用信息命令dump)查看对应的模块是否安装。查看输出的打印信息中hapModuleNames字段对应的列表是否存在该moduleName，不存在则说明应用未安装该模块。
...
3. 调用[bundleManager.getProfileByAbility](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetprofilebyability)、[bundleManager.getProfileByExtensionAbility](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetprofilebyextensionability) 等通过abilityName、moduleName组合查询的接口时，对应的应用没有安装moduleName对应的模块，对应模块下的ability也不存在。

****处理步骤****

1. 检查abilityName拼写是否正确。
2. 可以使用[查询应用信息命令（dump）](/system-debug-optimize/debugging-commands/bm-tool#查询应用信息命令dump)查看对应的应用是否存在这个abilityName。查看输出的打印信息中hapModuleInfos字段对应的abilityInfos下是否包含name等于该abilityName，不包含则说明该abilityName不存在。
3. 可以使用[查询应用信息命令（dump）](/system-debug-optimize/debugging-commands/bm-tool#查询应用信息命令dump)查看输出的打印信息中hapModuleNames字段对应的列表是否存在对应的moduleName，不存在则说明应用未安装该模块，对应模块下的ability也不存在。
...
调用bundleManager模块中的[getBundleNameByUid接口](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetbundlenamebyuid14)时，指定的uid无效。
...
检查系统中是否存在对应的应用uid值。可以使用[查询应用信息命令（dump）](/system-debug-optimize/debugging-commands/bm-tool#查询应用信息命令dump)查看已安装应用的uid。执行命令行后会输出对应已安装应用的uid，如果输出多个uid，选择大于0的。如果该命令无打印值输出，表示应用未安装。
...
确认指定的ability是否被禁用，可以使用[bm工具](/system-debug-optimize/debugging-commands/bm-tool)查询对应的应用信息。
...
调用bundleManager模块中的[canOpenLink接口](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagercanopenlink12)时，指定的link无效。
...
调用bundleManager模块中的[canOpenLink接口](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagercanopenlink12)时，指定link的scheme未在querySchemes字段下配置。
...
检查是否在querySchemes字段下配置了相应的URL scheme，可以参考[使用canOpenLink判断应用是否可访问](/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/canopenlink)。
...
调用[bundleManager.getLaunchWant](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetlaunchwant13)接口时，应用的启动组件Want信息不存在。
