...
WantAgent模块封装了[Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want)对象，允许应用程序在未来的某个时间点触发WantAgent实例执行指定操作（如启动Ability、发送公共事件等）。

该模块提供了创建WantAgent实例、获取WantAgent实例所属应用的包名、获取WantAgent实例所属应用的UID、主动触发WantAgent实例、判断两个WantAgent实例是否相等等功能。WantAgent的一个典型应用场景是通知处理。例如，当用户点击通知时，会触发WantAgent的[trigger](#wantagenttrigger)接口，并拉起目标应用。具体使用请参考[通知模块](/notification-kit/publish-notification/notification-with-wantagent)。

![](../../../../images/5dfd48e5/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [WantAgentInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/wantagent/js-apis-inner-wantagent-wantagentinfo/js-apis-inner-wantagent-wantagentinfo) | 是 | 表示创建WantAgent所需的配置信息，包括目标UIAbility、操作类型、请求码等。三方应用在WantAgentInfo中只能设置本应用的UIAbility。 |
...
这里所执行的操作类型，是在创建WantAgent实例时通过[WantAgentInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/wantagent/js-apis-inner-wantagent-wantagentinfo/js-apis-inner-wantagent-wantagentinfo)参数中的actionType属性指定的。
...
| --- | --- | --- | --- |
| agent | WantAgent | 是 | WantAgent对象。 |
| triggerInfo | [TriggerInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/wantagent/js-apis-inner-wantagent-triggerinfo/js-apis-inner-wantagent-triggerinfo) | 是 | 表示触发WantAgent实例时携带的信息，如自定义的extraInfos。 |
...
当两个WantAgent实例由当前用户下的同一应用使用相同的[WantAgentInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/wantagent/js-apis-inner-wantagent-wantagentinfo/js-apis-inner-wantagent-wantagentinfo)信息创建，并且实例未被[cancel](#wantagentcancel)取消，这两个实例相等。在通知（携带WantAgent实例）场景，通知更新时会比较2个通知中的WantAgent实例，不相等时会把旧通知的WantAgent实例删除。
...
| want | [Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want) | 否 | 否 | 触发wantAgent时实际使用的want信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [\_TriggerInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/wantagent/js-apis-inner-wantagent-triggerinfo/js-apis-inner-wantagent-triggerinfo) | TriggerInfo对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [\_WantAgentInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/wantagent/js-apis-inner-wantagent-wantagentinfo/js-apis-inner-wantagent-wantagentinfo) | WantAgentInfo对象。 |
