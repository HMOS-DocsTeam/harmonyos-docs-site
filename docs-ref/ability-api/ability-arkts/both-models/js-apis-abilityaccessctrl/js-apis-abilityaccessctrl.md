...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-abilityaccessctrl
kit: 应用框架
...
![](../../../../images/efdee94b/note_3.0-zh-cn.png) 
...
AtManager接口调用依赖于tokenID，应用可通过[bundleManager.getBundleInfoForSelf](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetbundleinfoforself)获取tokenID。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| tokenID | number | 是 | 要校验的目标应用的身份标识，可通过应用的[ApplicationInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/bundlemanager/js-apis-bundlemanager-applicationinfo/js-apis-bundlemanager-applicationinfo)的accessTokenId字段获得。 |
...
以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[访问控制错误码](/ref/ability-api/ability-arkts-errcode/errorcode-access-token/errorcode-access-token)。
...
用于UIAbility/UIExtensionAbility拉起弹框请求[用户授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。使用callback异步回调。

如果用户拒绝授权，将无法再次拉起弹框，需要用户在系统应用“设置”的界面中，手动授予权限，或是调用[requestPermissionOnSetting](#requestpermissiononsetting12)，拉起权限设置弹框，引导用户授权。

![](../../../../images/6d0e019e/zh-cn_image_0000002552799746.png)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 是 | 请求权限的UIAbility/UIExtensionAbility的Context。 |
...
| requestCallback | AsyncCallback&lt;[PermissionRequestResult](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-security/js-apis-permissionrequestresult/js-apis-permissionrequestresult)&gt; | 是 | 回调函数。当拉起权限请求弹框成功，err为undefined，data为获取到的PermissionRequestResult；否则err为错误对象。 |
...
下述示例中context的获取方式请参见[获取UIAbility的上下文信息](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-usage#获取uiability的上下文信息)。

关于向用户申请授权的完整流程及示例，请参见[向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。
...
用于UIAbility/UIExtensionAbility拉起弹框请求[用户授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。使用Promise异步回调。
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[PermissionRequestResult](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-security/js-apis-permissionrequestresult/js-apis-permissionrequestresult)&gt; | Promise对象，返回接口的结果。 |
...
用于[UIAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability#uiability)/[UIExtensionAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiextensionability/js-apis-app-ability-uiextensionability#uiextensionability)二次拉起权限设置弹框。使用Promise异步回调。

在调用此接口前，应用需要先调用[requestPermissionsFromUser](#requestpermissionsfromuser9)，如果用户在首次弹窗授权时已授权，调用当前接口将无法拉起弹窗。

![](../../../../images/04d10f13/zh-cn_image_0000002583439441.png)
...
| --- | --- | --- | --- |
| context | [Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 是 | 请求权限的UIAbility/UIExtensionAbility的Context。 |
| permissionList | Array&lt;[Permissions](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-permissions)&gt; | 是 | 权限名列表，合法的权限名取值可在[应用权限组列表](/system-security/access-control/app-permission-mgmt/app-permission-group-list)中查询。 |
...
![](../../../../images/a0d0e228/zh-cn_image_0000002552959396.png)
...
以下错误码的详细介绍请参见[访问控制错误码](/ref/ability-api/ability-arkts-errcode/errorcode-access-token/errorcode-access-token)。
...
| --- | --- | --- | --- |
| context | [Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 是 | 请求权限的UIAbility/UIExtensionAbility的Context。 |
| permission | [Permissions](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-permissions) | 是 | 权限名，只支持授权方式为[manual\_settings](/system-security/access-control/app-permission-mgmt/app-permission-mgmt-overview#manual_settings手动设置授权)类型的权限。 |
...
![](../../../../images/e067fe05/note_3.0-zh-cn.png) 
...
![](../../../../images/979b7425/note_3.0-zh-cn.png) 
...
| tokenID | number | 否 | 否 | 被订阅的应用身份标识，可通过应用的[ApplicationInfo](/ref/ability-api/ability-arkts/ability-api-interface-depend/bundlemanager/js-apis-bundlemanager-applicationinfo/js-apis-bundlemanager-applicationinfo)的accessTokenId字段获得。 |
...
| 类型 | 说明 |
| --- | --- |
| [\_PermissionRequestResult](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-security/js-apis-permissionrequestresult/js-apis-permissionrequestresult) | 权限请求结果对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [\_Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 提供了ability或application的上下文的能力，包括访问特定应用程序的资源等。 |
...
| DENIED | -1 | 表示用户未授权。 |
| GRANTED | 0 | 表示已授权。 |
| NOT\_DETERMINED | 1 | 表示未操作。应用声明[用户授权权限](/system-security/access-control/app-permission-mgmt/app-permissions/permissions-for-all-user)，暂未调用[requestPermissionsFromUser](#requestpermissionsfromuser9)接口请求用户授权时，或用户在设置中将权限状态修改为每次询问时，查询权限状态将返回此值。 |
| INVALID | 2 | 表示无效。应用未[声明权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions)或当前无法处理。例如：当模糊位置权限的状态为NOT\_DETERMINED时，查询精确位置权限状态，返回此值。 |
