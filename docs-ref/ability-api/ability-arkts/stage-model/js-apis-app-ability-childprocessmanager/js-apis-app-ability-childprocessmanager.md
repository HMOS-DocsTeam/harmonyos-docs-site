...
sidebar_position: 12
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-childprocessmanager
kit: 应用框架
...
![](../../../../images/f1d66ea4/note_3.0-zh-cn.png) 
...
- 通过本模块中接口创建的子进程有如下限制:
  ...
  - 创建的子进程内不支持再次创建子进程。
- 通过本模块中定义的创建子进程的接口和[native\_child\_process.h](/ref/ability-api/ability-c/ability-headerfile/capi-native-child-process-h/capi-native-child-process-h)中定义的创建子进程的接口启动的子进程总数最大为512个（系统资源充足情况下），其中[startChildProcess](#childprocessmanagerstartchildprocess)接口在SELF\_FORK模式下启动的子进程不计入总数内。
...
启动[ArkTS子进程](/ability-kit/ability-terminology#arkts子进程)。使用Promise异步回调。

![](../../../../images/58de7df0/note_3.0-zh-cn.png) 

调用该接口创建子进程成功会返回子进程pid，然后执行子进程的[ChildProcess.onStart](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-childprocess/js-apis-app-ability-childprocess#childprocessonstart)函数，[ChildProcess.onStart](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-childprocess/js-apis-app-ability-childprocess#childprocessonstart)函数执行完后子进程会自动销毁。
...
启动[ArkTS子进程](/ability-kit/ability-terminology#arkts子进程)。使用callback异步回调。

![](../../../../images/4a099c78/note_3.0-zh-cn.png) 
...
![](../../../../images/63ba28fd/note_3.0-zh-cn.png) 

调用该接口创建的子进程不会继承父进程资源，子进程创建成功会返回子进程pid，然后执行子进程的[ChildProcess.onStart](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-childprocess/js-apis-app-ability-childprocess#childprocessonstart)函数。[ChildProcess.onStart](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-childprocess/js-apis-app-ability-childprocess#childprocessonstart)函数执行完后子进程不会自动销毁，需要子进程调用[process.abort](/ref/arkts-api/arkts-arkts/js-apis-process/js-apis-process#processabort)销毁。调用该接口的进程销毁后，所创建的子进程也会一并销毁。
...
| args | [ChildProcessArgs](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-childprocessargs/js-apis-app-ability-childprocessargs) | 是 | 传递到子进程的参数。 |
| options | [ChildProcessOptions](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-childprocessoptions/js-apis-app-ability-childprocessoptions) | 否 | 子进程的启动配置选项。 |
...
主进程部分，示例中的context的获取方式请参见[获取UIAbility的上下文信息](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-usage#获取uiability的上下文信息)：
...
启动[Native子进程](/ability-kit/ability-terminology#native子进程)。使用Promise异步回调。

![](../../../../images/8271018e/note_3.0-zh-cn.png) 
...
子进程部分，详见[Native子进程开发指导（C/C++）- 创建支持参数传递的Native子进程](/ability-kit/native-childprocess-development/capi-nativechildprocess-development-guideline#创建支持参数传递的native子进程)：
