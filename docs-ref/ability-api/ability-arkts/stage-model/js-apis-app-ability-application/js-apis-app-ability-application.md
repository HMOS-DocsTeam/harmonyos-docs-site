...
sidebar_position: 6
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-application
kit: 应用框架
...
开发者可以通过该模块管理和获取应用的上下文[Context](/ability-kit/stage-model-development/stage-model-application-components/application-context-stage)，以及控制应用进程的状态。

![](../../../../images/970a7323/note_3.0-zh-cn.png) 
...
| TYPE\_CREATE\_ABILITY\_STAGE | 2 | 进程最终预加载到[AbilityStage](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage)创建完成阶段。 |
| TYPE\_CREATE\_WINDOW\_STAGE | 3 | 进程最终预加载到[WindowStage](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-windowstage/arkts-apis-window-windowstage)创建完成阶段。 |
| TYPE\_CREATE\_BACKGROUND\_ABILITY 23+ | 4 | 进程最终预加载到[onBackground](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability#onbackground)执行完成阶段。 |

## application.createModuleContext

createModuleContext(context: Context, moduleName: string): Promise&lt;Context&gt;

创建指定模块的上下文。创建出的模块上下文中[resourceManager.Configuration](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#configuration)资源继承自入参上下文，便于开发者获取[跨HAP/HSP包应用资源](/resource-categories-and-access#跨haphsp包应用资源)。使用Promise异步回调。

![](../../../../images/89c7ebb0/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 是 | 表示应用上下文。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context)&gt; | Promise对象。返回创建的Context。 |

****错误码：****

以下错误码详细介绍请参考[通用错误码说明文档](/ref/errorcode-universal/errorcode-universal)。
...
开发者可以调用该接口将当前进程放入[备选主控进程](/ability-kit/ability-terminology#candidatemasterprocess备选主控进程)链表。使用Promise异步回调。

当[主控进程](/ability-kit/ability-terminology#masterprocess主控进程)销毁后，再次启动配置了isolationProcess为true的UIAbility/UIExtensionAbility组件时，系统会根据是否存在备选主控进程执行相应操作。

- 如果存在备选主控进程，系统会将备选主控进程链表首节点的进程设置为主控进程，触发[onNewProcessRequest](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#onnewprocessrequest11)回调。
...
![](../../../../images/7c40e470/note_3.0-zh-cn.png) 

如果当前进程已经是[主控进程](/ability-kit/ability-terminology#masterprocess主控进程)，调用该接口无效并且不会抛出错误码。
...
以下错误码详细介绍请参考[通用错误码说明文档](/ref/errorcode-universal/errorcode-universal)和[元能力子系统错误码](/ref/ability-api/ability-arkts-errcode/errorcode-ability/errorcode-ability)。
...
放弃当前进程的[主控进程](/ability-kit/ability-terminology#masterprocess主控进程)身份。使用Promise异步回调。
...
![](../../../../images/2ea25226/note_3.0-zh-cn.png) 

- 只有在进程首次执行[AbilityStage.onCreate](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#oncreate)完成之前调用该接口，才可以返回真实的预加载类型。
