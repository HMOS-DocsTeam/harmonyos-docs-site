...
UIAbility是包含UI界面的应用组件，继承自[Ability](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-ability/js-apis-app-ability-ability)，提供UIAbility组件创建、销毁、前后台切换等[生命周期](#uiability生命周期状态)回调，同时也具备[后台通信能力](#后台通信能力)。

![](../../../../images/6335e69d/note_3.0-zh-cn.png) 

本模块首批接口从API version 9 开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。

本模块接口仅可在Stage模型下使用。

各类Ability的继承关系详见[继承关系说明](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-ability/js-apis-app-ability-ability#ability的继承关系说明)。

## UIAbility生命周期状态

****图1**** UIAbility生命周期状态

![](../../../../images/5740a176/zh-cn_image_0000002583479395.png)
...
![](../../../../images/e8125afd/zh-cn_image_0000002583437537.png)

- Caller UIAbility调用[startAbilityByCall()](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#startabilitybycall)接口获取[Caller](#caller)对象，并使用Caller对象的[call](#call)方法向Callee UIAbility发送数据。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | [UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 否 | 否 | UIAbility的上下文。  ****元服务API****：从API version 11开始，该接口支持在元服务中使用。 |
| launchWant | [Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want) | 否 | 否 | UIAbility[冷启动](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction#目标uiability冷启动)时接收到的Want参数，取值为[onCreate](#oncreate)接收到的Want参数。  ****元服务API****：从API version 11开始，该接口支持在元服务中使用。 |
| lastRequestWant | [Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want) | 否 | 否 | 最近一次拉起UIAbility请求的Want参数。  - 首次拉起UIAbility时，取值为[onCreate](#oncreate)接收到的Want参数。  - 重复拉起UIAbility时，取值为[onNewWant](#onnewwant)最近一次接收到的Want参数。  ****元服务API****：从API version 11开始，该接口支持在元服务中使用。 |
| callee | [Callee](#callee) | 否 | 否 | 系统为UIAbility创建的后台通信对象，Callee UIAbility（被调用方）可以通过Callee对象接收Caller对象发送的数据。 |
| specifiedId23+ | string | 否 | 是 | 仅当UIAbility启动模式为[specified](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-launch-type#specified启动模式)时存在，取值为开发者自定义的UIAbility标识。 |

### onCreate

onCreate(want: Want, launchParam: AbilityConstant.LaunchParam): void

当UIAbility实例创建完成时，系统会触发该回调，开发者可在该回调中执行初始化逻辑（如定义变量、加载资源等）。该回调仅会在UIAbility[冷启动](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction#目标uiability冷启动)时触发。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want) | 是 | 调用方拉起该UIAbility时传递的数据。 |
| launchParam | [AbilityConstant.LaunchParam](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#launchparam) | 是 | 应用启动参数，包含应用启动原因、应用上次退出原因等。 |
...
当[WindowStage](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-windowstage/arkts-apis-window-windowstage)实例创建完成后，系统会触发该回调。开发者可以在该回调中通过WindowStage加载页面。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| windowStage | [window.WindowStage](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-windowstage/arkts-apis-window-windowstage) | 是 | WindowStage实例对象。 |
...
![](../../../../images/58c4f728/note_3.0-zh-cn.png) 

在应用迁移启动时，无论是[冷启动](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction#目标uiability冷启动)还是[热启动](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction#目标uiability热启动)，都会在执行完[onCreate()](#oncreate)/[onNewWant()](#onnewwant)后，触发onWindowStageRestore()生命周期函数，不执行onWindowStageCreate()生命周期函数。
...
当UIAbility被销毁（例如使用[terminateSelf](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#terminateself)接口停止UIAbility）时，系统触发该回调。开发者可以在该生命周期中执行资源清理、数据保存等相关操作。

使用同步回调或Promise异步回调。

![](../../../../images/1ddb38e9/note_3.0-zh-cn.png) 
...
![](../../../../images/37ce27d7/note_3.0-zh-cn.png) 
...
当已经启动的UIAbility实例再次被拉起时，系统会触发该回调。若在特定场景下（参见[Scenarios](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-contextconstant/js-apis-app-ability-contextconstant#scenarios20)），不需要触发该生命周期回调，可以使用[setOnNewWantSkipScenarios](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#setonnewwantskipscenarios20)接口设置。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want) | 是 | 调用方再次拉起该UIAbility时传递的数据。 |
| launchParam | [AbilityConstant.LaunchParam](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#launchparam) | 是 | UIAbility启动参数，包含启动原因等。 |
...
该接口需要与[appRecovery](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-apprecovery/js-apis-app-ability-apprecovery)配合使用。如果应用已使能故障恢复功能（即[enableAppRecovery](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-apprecovery/js-apis-app-ability-apprecovery#apprecoveryenableapprecovery)接口中的saveOccasion参数设置为SAVE\_WHEN\_ERROR），当应用出现故障时，系统将触发该回调来保存UIAbility的数据。

![](../../../../images/5bd95192/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| reason | [AbilityConstant.StateType](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#statetype) | 是 | 触发应用保存状态的原因，当前仅支持APP\_RECOVERY（即应用故障恢复场景）。 |
...
| 类型 | 说明 |
| --- | --- |
| [AbilityConstant.OnSaveResult](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#onsaveresult) | 返回一个数据保存策略的对象（如全部拒绝、全部允许、只允许故障恢复场景等）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stateType | [AbilityConstant.StateType](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#statetype) | 是 | 触发应用保存状态的原因，当前仅支持APP\_RECOVERY（即应用故障恢复场景）。 |
| wantParam | Record&lt;string, Object&gt; | 是 | 用户自定义的应用状态数据，应用再启动时被保存在[onCreate](#oncreate)的Want.parameters中。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[AbilityConstant.OnSaveResult](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#onsaveresult)&gt; | Promise对象。返回一个数据保存策略的对象（如全部拒绝、全部允许、只允许故障恢复场景等）。 |
...
在UIAbility即将关闭前（例如用户通过点击应用窗口右上角的关闭按钮、或者通过Dock栏/托盘右键退出应用时），系统会触发该回调，用于在UIAbility正式关闭前执行其他操作。开发者可以在该回调中返回true阻拦此次关闭，然后在合适时机主动调用[terminateSelf](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#terminateself)接口关闭。例如，询问用户是否确认关闭UIAbility，再主动销毁UIAbility。

![](../../../../images/aef10d9f/note_3.0-zh-cn.png) 

- 从API version 15开始，当[UIAbility.onPrepareToTerminateAsync](#onpreparetoterminateasync15)实现时，本回调函数将不执行。当[AbilityStage.onPrepareTerminationAsync](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#onprepareterminationasync15)或[AbilityStage.onPrepareTermination](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#onpreparetermination15)实现时，在dock栏或系统托盘处右键点击关闭，本回调函数将不执行。
- 如果应用本身或者所使用的三方框架注册了[window.WindowStage.on('windowStageClose')](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-windowstage/arkts-apis-window-windowstage#onwindowstageclose14)监听，本回调函数将不执行。
...
开发者可以在该回调中返回true阻拦此次关闭，然后在合适时机主动调用[terminateSelf](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#terminateself)接口关闭。例如，询问用户是否确认关闭UIAbility，再主动销毁UIAbility。

![](../../../../images/f7dfee15/note_3.0-zh-cn.png) 

- 当[AbilityStage.onPrepareTerminationAsync](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#onprepareterminationasync15)或[AbilityStage.onPrepareTermination](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#onpreparetermination15)实现时，在dock栏或系统托盘处右键点击关闭，本回调函数将不执行。
- 如果应用本身或者所使用的三方框架注册了[window.WindowStage.on('windowStageClose')](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-windowstage/arkts-apis-window-windowstage#onwindowstageclose14)监听，本回调函数将不执行。
...
![](../../../../images/82d23cd3/note_3.0-zh-cn.png) 

- 该生命周期回调不支持[specified启动模式](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-launch-type#specified启动模式)。
- 通过[startAbility](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#startability)等方法拉起协同方应用时，需要在Want对象中设置协同标记[Flags](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-wantconstant/js-apis-app-ability-wantconstant#flags)为FLAG\_ABILITY\_ON\_COLLABORATE。
- [冷启动](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction#目标uiability冷启动)时，该回调在[onForeground](#onforeground)前或[onBackground](#onbackground)后调用；[热启动](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction#目标uiability热启动)时，该回调在[onNewWant](#onnewwant)前调用。
...
| 类型 | 说明 |
| --- | --- |
| [AbilityConstant.CollaborateResult](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#collaborateresult18) | 协同方是否接受协同的结果。 |
...
调用方Caller UIAbility通过[startAbilityByCall](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#startabilitybycall)接口拉起目标Callee UIAbility，目标UIAbility启动成功后，返回一个Caller对象给调用方进行通信。
...
| data | [rpc.Parcelable](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#parcelable9) | 是 | 由Caller向Callee发送的消息内容，消息内容是序列化的数据。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[rpc.MessageSequence](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#messagesequence9)&gt; | Promise对象，返回Callee UIAbility的应答数据。 |
...
| --- | --- | --- | --- |
| method | string | 是 | 由Caller和Callee双方约定好的方法名，Callee方通过该字段区分消息类型。 |
| callback | [CalleeCallback](#calleecallback) | 是 | 一个[rpc.MessageSequence](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#messagesequence9)类型入参的js通知同步回调函数, 回调函数至少要返回一个空的[rpc.Parcelable](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#parcelable9)数据对象, 其他视为函数执行错误。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| indata | [rpc.MessageSequence](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#messagesequence9) | 是 | 发送需传递的数据。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [rpc.Parcelable](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#parcelable9) | 返回的数据对象。 |
