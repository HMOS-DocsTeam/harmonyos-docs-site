...
title: "EventHub"
sidebar_position: 15
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-eventhub
kit: 应用框架
...
主要用于[UIAbility组件与UI的数据通信](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-data-sync-with-ui)。

不同的Context对象拥有不同的EventHub对象，不同EventHub对象之间无法直接通信。事件的订阅、取消订阅、触发都作用在某一个具体的EventHub对象上。

由于Worker、Taskpool通过Actor模型实现[多线程并发](/arkts/arkts-concurrency/multithread-concurrency/multi-thread-concurrency-overview#多线程并发模型)，不同虚拟机实例之间拥有独占的内存，因此EventHub对象不能用于线程间的数据通信。

![](../../../../../images/9a762c70/note_3.0-zh-cn.png) 
...
- 不支持在Worker、TaskPool线程间通过EventHub对象进行数据通信。如需进行跨线程通信，参考[使用Emitter进行线程间通信](/system-basicfun/basic-services-kit/app-events/itc-with-emitter)。
- 不支持同一线程内不同Context对象的EventHub对象间进行数据通信。
- 通过[sendableContextManager](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-sendablecontextmanager/js-apis-app-ability-sendablecontextmanager)转换后的Context对象与原先的Context对象属于不同Context对象，不支持其EventHub对象间的数据通信。
...
![](../../../../../images/f7ebf010/note_3.0-zh-cn.png) 
