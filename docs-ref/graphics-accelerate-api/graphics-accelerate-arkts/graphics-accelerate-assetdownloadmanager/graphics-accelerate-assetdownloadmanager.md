...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/graphics-accelerate-assetdownloadmanager
kit: 图形
...
| url | string | 否 | 否 | 当前下载任务URL，要求以http或者https开头，最大长度512个字节。  URL的域名需要符合域名白名单要求：  - 三方CDN：请前往AppGallery Connect查看已配置的CDN域名白名单。具体操作步骤请参见[创建下载任务](/graphics-accelerate-kit-guide/graphics-accelerate-assetdownload/graphics-accelerate-assetdownload-service/graphics-accelerate-assetdownload-release#创建下载任务)。  - 华为CDN：域名白名单将基于资源包下载任务自动配置。 |
...
错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[ArkTS API错误码](/ref/graphics-accelerate-api/graphics-accelerate-arkts/graphics-accelerate-arkts-errorcode/graphics-accelerate-arkts-errorcode)。
...
| callback | Callback&lt;[DownloadProgressInfo[]](#downloadprogressinfo)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数一致。若不填，则取消当前监听该事件的所有回调函数。 |
...
| callback | Callback&lt;[AssetDownloadTask](#assetdownloadtask)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数一致。若不填，则取消当前监听该事件的所有回调函数。 |
...
| callback | Callback&lt;[DownloadCompletedInfo](#downloadcompletedinfo)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数一致。若不填，则取消当前监听该事件的所有回调函数。 |
...
| callback | Callback&lt;[DownloadFailedInfo](#downloadfailedinfo)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数一致。若不填，则取消当前监听该事件的所有回调函数。 |
...
以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[ArkTS API错误码](/ref/graphics-accelerate-api/graphics-accelerate-arkts/graphics-accelerate-arkts-errorcode/graphics-accelerate-arkts-errorcode)。
...
| ****参数名**** | ****类型**** | 必填 | ****说明**** |
| --- | --- | --- | --- |
| context | [common.BaseContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-basecontext/js-apis-inner-application-basecontext) | 是 | 应用程序上下文。 |
...
以下错误码的详细介绍请参见[ArkTS API错误码](/ref/graphics-accelerate-api/graphics-accelerate-arkts/graphics-accelerate-arkts-errorcode/graphics-accelerate-arkts-errorcode)。
...
错误码的详细介绍请参见[ArkTS API错误码](/ref/graphics-accelerate-api/graphics-accelerate-arkts/graphics-accelerate-arkts-errorcode/graphics-accelerate-arkts-errorcode)。
