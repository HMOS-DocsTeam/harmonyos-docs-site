...
![](../../../images/d2f883a6/note_3.0-zh-cn.png) 
...
1.根据当前系统的[访问控制机制](/system-security/access-control/access-token-overview)，应用无法使用分享给其他应用的URI。

2.根据[系统Picker](/ability-kit/stage-model-development/inter-app-redirection/system-app-startup#拉起系统应用的方式)的运行机制，通过Picker获取到的URI仅有临时权限，无法持久化保存使用。
...
3.确认文件的路径是否为应用内的沙箱路径（[应用沙箱目录与应用沙箱路径](/core-file-kit/app-file/app-sandbox-directory#应用沙箱目录与应用沙箱路径)），文件管理系统禁止操作应用沙箱以外的文档。
