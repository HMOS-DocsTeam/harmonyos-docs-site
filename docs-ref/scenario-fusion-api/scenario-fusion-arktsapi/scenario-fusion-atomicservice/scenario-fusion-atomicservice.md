...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scenario-fusion-atomicservice
kit: 应用服务
...
| screenSafeArea | [window.AvoidArea](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-i/arkts-apis-window-i#avoidarea7) | 否 | 是 | 竖屏正方向下的安全区域。 |
| language | string | 否 | 是 | 系统语言。 |
| osFullName | string | 否 | 是 | 系统版本。 |
| fontSizeSetting | number | 否 | 是 | 显示设备逻辑像素的密度，代表物理像素与逻辑像素的缩放系数，计算方式为：  该参数为浮点数，受[densityDPI](/ref/arkui-api/arkui-arkts/display-manager-api/js-apis-display/js-apis-display#display)范围限制，取值范围在[0.5，4.0]。一般取值1.0、3.0等，实际取值取决于不同设备提供的[densityDPI](/ref/arkui-api/arkui-arkts/display-manager-api/js-apis-display/js-apis-display#display)。 |
| sdkApiVersion | number | 否 | 是 | 系统软件API版本。 |
| bluetoothEnabled | boolean | 否 | 是 | 蓝牙系统开关。“true”表示开关已打开，“false”表示开关已关闭。 |
| locationEnabled | boolean | 否 | 是 | 地理位置的系统开关。“true”表示开关已打开，“false”表示开关已关闭。 |
| wifiEnabled | boolean | 否 | 是 | Wi-Fi 的系统开关。“true”表示开关已打开，“false”表示开关已关闭。 |
| deviceOrientation | string | 否 | 是 | 设备当前显示的方向。“portrait”表示竖屏，“landscape”表示横屏。 |
| theme | [ColorMode](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-state-management-environment-variables/ts-state-management-environment-variables#colormode) | 否 | 是 | 深浅色模式，可选值为：  - ColorMode.LIGHT：浅色模式；  - ColorMode.DARK：深色模式。 |
...
| offset | [Position](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-types/ts-types#position) | [Edges](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-types/ts-types#edges12) | [LocalizedEdges](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-types/ts-types#localizededges12) | 否 | 是 | 组件放置位置，基于屏幕中心点的位置偏移。  默认值：{ x: 0, y: 300 } |
...
| 'language' | 系统语言。 |
| 'osFullName' | 系统版本。 |
| 'fontSizeSetting' | 显示设备逻辑像素的密度，代表物理像素与逻辑像素的缩放系数，计算方式为：  该参数为浮点数，受[densityDPI](/ref/arkui-api/arkui-arkts/display-manager-api/js-apis-display/js-apis-display#display)范围限制，取值范围在[0.5，4.0]。一般取值1.0、3.0等，实际取值取决于不同设备提供的[densityDPI](/ref/arkui-api/arkui-arkts/display-manager-api/js-apis-display/js-apis-display#display)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| properties | Array&lt;[SystemInfoType](#systeminfotype)&gt; | 否 | 获取设备、网络状态、屏幕、语言、主题等系统信息的请求对象。  ****说明：****  - 使用5.0.1(13)及以上版本时不需要配置Wi-Fi权限、蓝牙权限。  - 获取Wi-Fi信息需要配置权限ohos.permission.GET\_WIFI\_INFO。  - 获取蓝牙开启状态需要配置权限ohos.permission.ACCESS\_BLUETOOTH。  - 获取用户程序访问控制权限可参考[程序访问控制管理](/ref/ability-api/ability-arkts/both-models/js-apis-abilityaccessctrl/js-apis-abilityaccessctrl#requestpermissionsfromuser9)。 |

![](../../../images/09aec37b/note_3.0-zh-cn.png) 
...
![](../../../images/f4e95fd2/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| properties | Array&lt;[SystemSettingType](#systemsettingtype)&gt; | 否 | 获取蓝牙、定位、Wi-Fi开关，以及设备方向等系统信息的请求对象。  ****说明：****  - 使用5.0.1(13)及以上版本时不需要配置Wi-Fi权限、蓝牙权限。  - 获取Wi-Fi信息需要配置权限ohos.permission.GET\_WIFI\_INFO。  - 获取蓝牙开启状态需要配置权限ohos.permission.ACCESS\_BLUETOOTH。  - 获取用户程序访问控制权限可参考[程序访问控制管理](/ref/ability-api/ability-arkts/both-models/js-apis-abilityaccessctrl/js-apis-abilityaccessctrl#requestpermissionsfromuser9)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| ctx | [UIContext](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-api/ts-custom-component-api#uicontext) | 是 | UIContext实例对象。 |
...
| 错误码ID | 错误信息 |
| --- | --- |
| [801](/ref/errorcode-universal/errorcode-universal#section801-该设备不支持此api) | Capability not supported. |
| [1009601001](/ref/scenario-fusion-api/scenario-fusion-arktsapi/scenario-fusion-arkts-api-error-code/scenario-fusion-arkts-api-error-code#section1009601001-非法的服务号id) | Invalid service account id. |
| [1009601002](/ref/scenario-fusion-api/scenario-fusion-arktsapi/scenario-fusion-arkts-api-error-code/scenario-fusion-arkts-api-error-code#section1009601002-用户未登录华为账号) | The user has not logged in with HUAWEI ID. |
| [1009601003](/ref/scenario-fusion-api/scenario-fusion-arktsapi/scenario-fusion-arkts-api-error-code/scenario-fusion-arkts-api-error-code#section1009601003-请求服务号云失败) | Request server failed. |
| [1009601004](/ref/scenario-fusion-api/scenario-fusion-arktsapi/scenario-fusion-arkts-api-error-code/scenario-fusion-arkts-api-error-code#section1009601004-网络连接失败) | Network connection error. |
| [1009601005](/ref/scenario-fusion-api/scenario-fusion-arktsapi/scenario-fusion-arkts-api-error-code/scenario-fusion-arkts-api-error-code#section1009601005-其他异常) | Other error. |
