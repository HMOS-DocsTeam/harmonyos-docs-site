...
title: "Functions"
sidebar_position: 2
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-f
kit: 媒体
...
![](../../../../images/6bcec7d0/note_3.0-zh-cn.png) 
...
由于图片占用内存较大，所以当Picture对象使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-picture/arkts-apis-image-picture#release13)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mainPixelmap | [PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 是 | 主图的pixelmap。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [Picture](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-picture/arkts-apis-image-picture) | 返回Picture对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sequence | [rpc.MessageSequence](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#messagesequence9) | 是 | 保存有Picture信息的MessageSequence。 |
...
以下错误码的详细介绍请参见[通用错误码说明文档](/ref/errorcode-universal/errorcode-universal)和[Image错误码](/ref/image-api/image-arkts-errcode/errorcode-image/errorcode-image)。
...
由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap#release7)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colors | ArrayBuffer | 是 | 图像像素数据的缓冲区，用于初始化PixelMap的像素。初始化前，缓冲区中的像素格式需要由[InitializationOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#initializationoptions8).srcPixelFormat指定，否则默认缓冲区的像素格式为BGRA\_8888。  ****说明：**** 图像像素数据的缓冲区长度：length = width \* height \* 单位像素字节数。 |
| options | [InitializationOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#initializationoptions8) | 是 | 创建像素的属性，包括透明度，尺寸，缩略值，像素格式和是否可编辑。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| Promise&lt;[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)&gt; | Promise对象，返回PixelMap。  当创建的pixelMap大小超过原图大小时，返回原图pixelMap大小。 |
...
| colors | ArrayBuffer | 是 | 图像像素数据的缓冲区，用于初始化PixelMap的像素。初始化前，缓冲区中的像素格式需要由[InitializationOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#initializationoptions8).srcPixelFormat指定，否则默认缓冲区的像素格式为BGRA\_8888。  ****说明：**** 图像像素数据的缓冲区长度：length = width \* height \* 单位像素字节数。 |
| options | [InitializationOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#initializationoptions8) | 是 | 创建像素的属性，包括透明度、尺寸、缩略值、像素格式和是否可编辑。 |
| callback | AsyncCallback&lt;[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)&gt; | 是 | 回调函数，当创建PixelMap成功，err为undefined，data为获取到的PixelMap对象；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colors | ArrayBuffer | 是 | 图像像素数据的缓冲区，用于初始化PixelMap的像素。初始化前，必须通过[InitializationOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#initializationoptions8).srcPixelFormat指定缓冲区中的像素格式。  ****说明：**** 图像像素数据的缓冲区长度：length = width \* height \* 单位像素字节数。 |
| param | [InitializationOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#initializationoptions8) | 是 | 创建像素的属性，包括透明度、尺寸、缩略值、像素格式和是否可编辑。 |
| allocatorType | [AllocatorType](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#allocatortype15) | 否 | 指定创建pixelmap的内存类型，默认内存类型是AllocatorType.AUTO。  1. image.AllocatorType.AUTO：不支持该内存类型的格式有UNKNOWN、YCBCR\_P010、YCRCB\_P010和ASTC\_4x4。RGBA\_1010102默认申请DMA内存。其他格式（RGB\_565、RGBA\_8888、BGRA\_8888和RGBAF\_16）尺寸大于512\*512默认申请DMA内存，否则申请共享内存。  2. image.AllocatorType.DMA：RGBA\_1010102、RGB\_565、RGBA\_8888、BGRA\_8888和RGBAF\_16支持DMA内存类型，其余格式不支持。  3. image.AllocatorType.SHARED：UNKNOWN、RGBA\_1010102、YCBCR\_P010、YCRCB\_P010和ASTC\_4x4不支持共享内存，其余格式支持。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| Promise&lt;[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)&gt; | Promise对象，返回PixelMap。 |

****错误码：****

以下错误码的详细介绍请参见[Image错误码](/ref/image-api/image-arkts-errcode/errorcode-image/errorcode-image)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sequence | [rpc.MessageSequence](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#messagesequence9) | 是 | 保存有PixelMap信息的MessageSequence。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 成功同步返回PixelMap对象，失败抛出异常。 |
...
根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#region8).size指定。使用Promise异步回调。

由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap#release7)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。

![](../../../../images/9c67ebf8/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| surfaceId | string | 是 | 对应Surface的ID，可通过预览组件获取，如[XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)组件。 |
| region | [Region](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#region8) | 是 | 截取的画面区域。仅支持从画面左上角开始截取部分或整个画面，即Region中的x和y必须为0，Region.size中width和height的取值范围分别为[1, 预览流宽度]和[1, 预览流高度]。如需截取任意区域，可先使用[image.createPixelMapFromSurface](#imagecreatepixelmapfromsurface15)获取整个画面，再使用[crop](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap#crop9)截取所需区域。 |
...
以同步方式，根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#region8).size指定。

由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap#release7)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。

![](../../../../images/9d2f1ec5/note_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 成功会同步返回PixelMap对象，失败则抛出异常。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | [PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 是 | 源PixelMap对象。 |
| dst | [PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 是 | 目标PixelMap对象。 |
...
由于图片占用内存较大，所以当ImageSource实例使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#release)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uri | string | 是 | 图片路径，当前仅支持应用沙箱路径。  当前支持格式有：.jpg .png .gif .bmp .webp .dng .heic12+ .wbmp23+ .heifs23+ .tiff23+ [.svg10+](#svg标签说明) .ico11+。部分格式的解码能力依赖于具体的设备硬件，建议在调用前使用[image.getImageSourceSupportedFormats20+](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#imagegetimagesourcesupportedformats20)接口，动态查询当前设备上的解码能力。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [ImageSource](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource) | 返回ImageSource类实例，失败时返回undefined。 |
...
| --- | --- | --- | --- |
| uri | string | 是 | 图片路径，当前仅支持应用沙箱路径。  当前支持格式有：.jpg .png .gif .bmp .webp .dng .heic12+ .wbmp23+ .heifs23+ .tiff23+ [.svg10+](#svg标签说明) .ico11+。部分格式的解码能力依赖于具体的设备硬件，建议在调用前使用[image.getImageSourceSupportedFormats20+](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#imagegetimagesourcesupportedformats20)接口，动态查询当前设备上的解码能力。 |
| options | [SourceOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#sourceoptions9) | 是 | 图片属性，包括图片像素密度、像素格式和图片尺寸。 |
...
通过缓冲区创建ImageSource实例。buf数据是未解码的数据，不可以传入类似于RBGA，YUV的像素buffer数据，如果想通过像素buffer数据创建pixelMap，可以调用[image.createPixelMapSync](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#imagecreatepixelmapsync12)这一类接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| rawfile | [resourceManager.RawFileDescriptor](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#rawfiledescriptor9) | 是 | 图像资源文件的RawFileDescriptor。 |
| options | [SourceOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#sourceoptions9) | 否 | 图片属性，包括图片像素密度、像素格式和图片尺寸。 |
...
- 获取图片信息：指定序号-[getImageInfo](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#getimageinfo)、直接获取-[getImageInfo](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#getimageinfo-1)
- 获取图片中给定索引处图像的指定属性键的值：[getImageProperty](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#getimageproperty11)
- 批量获取图片中的指定属性键的值：[getImageProperties](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#getimageproperties12)
- 更新增量数据：[updateData](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#updatedata9)
- 创建PixelMap对象：通过图片解码参数创建-[createPixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmap7)、通过默认参数创建-[createPixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmap7-1) 、通过图片解码参数-[createPixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmap7-2)
- 释放ImageSource实例：[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#release)
...
| 类型 | 说明 |
| --- | --- |
| [ImageSource](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource) | 返回ImageSource，失败时返回undefined。 |
...
由于图片占用内存较大，所以当ImagePacker实例使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagepacker/arkts-apis-image-imagepacker#release)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 类型 | 说明 |
| --- | --- |
| [ImagePacker](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagepacker/arkts-apis-image-imagepacker) | 返回ImagePacker实例。 |
...
由于图片占用内存较大，所以当AuxiliaryPicture实例使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-auxiliarypicture/arkts-apis-image-auxiliarypicture#release13)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| size | [Size](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#size) | 是 | 辅助图的尺寸。单位：像素。 |
| type | [AuxiliaryPictureType](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#auxiliarypicturetype13) | 是 | 辅助图类型。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [AuxiliaryPicture](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-auxiliarypicture/arkts-apis-image-auxiliarypicture) | 如果操作成功，则返回AuxiliaryPicture实例。 |
...
通过图片大小、图片格式、容量创建ImageReceiver实例。ImageReceiver做为图片的接收方、消费者，它的参数属性实际上不会对接收到的图片产生影响。图片属性的配置应在发送方、生产者进行，如相机预览流[createPreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createpreviewoutput)。

由于图片占用内存较大，所以当ImageReceiver实例使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver#release9)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [Size](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#size) | 是 | 图像的默认大小。该参数不会影响接收到的图片大小，实际返回大小由生产者决定，如相机。 |
| format | [ImageFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#imageformat9) | 是 | 图像格式，取值为[ImageFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#imageformat9)常量（目前仅支持 ImageFormat:JPEG，实际返回格式由生产者决定，如相机）。 |
...
| 类型 | 说明 |
| --- | --- |
| [ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver) | 如果操作成功，则返回ImageReceiver实例。 |
...
通过ImageReceiverOptions创建ImageReceiver实例。ImageReceiver做为图片的接收方、消费者，其参数属性实际上不会对接收到的图片产生影响。图片属性的配置应在发送方、生产者进行，如相机预览流[createPreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createpreviewoutput)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [ImageReceiverOptions](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#imagereceiveroptions23) | 否 | 创建ImageReceiver的属性，包括图片的默认大小和同时访问的最大图片数。  未传入options时，默认的size为1920\*1080，单位为像素（px），表示期望接收宽为1920px，高为1080px的图片。  未传入options时，默认的capacity为3，表示期望同时最多有3张图片等待读取。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver) | undefined | 操作成功时返回ImageReceiver实例，否则返回undefined。 |
...
由于图片占用内存较大，所以当ImageCreator实例使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagecreator/arkts-apis-image-imagecreator#release9)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [Size](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#size) | 是 | 图像的默认大小。 |
| format | [ImageFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#imageformat9) | 是 | 图像格式，如YCBCR\_422\_SP，JPEG。 |
| capacity | number | 是 | 同时访问的最大图像数。该参数仅作为期望值，实际capacity由设备硬件决定。 |
...
| 类型 | 说明 |
| --- | --- |
| [ImageCreator](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagecreator/arkts-apis-image-imagecreator) | 如果操作成功，则返回ImageCreator实例。 |

****错误码：****

以下错误码的详细介绍请参见[通用错误码说明文档](/ref/errorcode-universal/errorcode-universal)和。
...
通过宽、高、图片格式、容量创建ImageReceiver实例。ImageReceiver做为图片的接收方、消费者，它的参数属性实际上不会对接收到的图片产生影响。图片属性的配置应在发送方、生产者进行，如相机预览流[createPreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createpreviewoutput)。

由于图片占用内存较大，所以当ImageReceiver实例使用完成后，应主动调用[release](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver#release9)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。

![](../../../../images/8045684e/note_3.0-zh-cn.png) 
...
| format | number | 是 | 图像格式，取值为[ImageFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#imageformat9)常量（目前仅支持 ImageFormat:JPEG，实际返回格式由生产者决定，如相机）。 |
| capacity | number | 是 | 同时访问的最大图像数。该参数仅作为期望值，实际capacity由设备硬件决定。 |
...
![](../../../../images/1f5cf613/note_3.0-zh-cn.png) 
