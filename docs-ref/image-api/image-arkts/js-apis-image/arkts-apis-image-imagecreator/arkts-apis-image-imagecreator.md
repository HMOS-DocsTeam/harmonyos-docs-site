...
在调用以下方法前需要先通过[image.createImageCreator](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#imagecreateimagecreator11)创建ImageCreator实例，ImageCreator不支持多线程。

由于图片占用内存较大，所以当ImageCreator实例使用完成后，应主动调用[release](#release9)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。

![](../../../../images/cebfc50e/note_3.0-zh-cn.png) 
...
| format9+ | [ImageFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#imageformat9) | 是 | 否 | 图像格式。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;[Image](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-image/arkts-apis-image-image)&gt; | 是 | 回调函数，当获取最新图片成功，err为undefined，data为获取到的最新图片；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| image | [Image](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-image/arkts-apis-image-image) | 是 | 绘制好的buffer图像。 |
