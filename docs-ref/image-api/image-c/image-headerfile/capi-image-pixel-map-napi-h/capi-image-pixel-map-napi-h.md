...
sidebar_position: 12
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-image-pixel-map-napi-h
kit: 媒体
last_updated: "2026-04-22"
slug: capi-image-pixel-map-napi-h
---
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [OhosPixelMapInfo](/ref/image-api/image-c/image-struct/capi-image-ohospixelmapinfo/capi-image-ohospixelmapinfo) | - | 用于定义PixelMap的相关信息。 |
...
| 名称 | 描述 |
| --- | --- |
| [int32\_t OH\_GetImageInfo(napi\_env env, napi\_value value, OhosPixelMapInfo \*info)](#oh_getimageinfo) | 获取PixelMap的信息，并记录信息到[OhosPixelMapInfo](/ref/image-api/image-c/image-struct/capi-image-ohospixelmapinfo/capi-image-ohospixelmapinfo)结构体中。 |
| [int32\_t OH\_AccessPixels(napi\_env env, napi\_value value, void\*\* addrPtr)](#oh_accesspixels) | 获取PixelMap对象数据的内存地址，并锁定该内存。  函数执行成功后，\*addrPtr就是获取的待访问的内存地址。访问操作完成后，必须要使用[OH\_UnAccessPixels](#oh_unaccesspixels)来释放锁，否则的话资源无法被释放。待解锁后，内存地址就不可以再被访问和操作。 |
...
获取PixelMap的信息，并记录信息到[OhosPixelMapInfo](/ref/image-api/image-c/image-struct/capi-image-ohospixelmapinfo/capi-image-ohospixelmapinfo)结构体中。
...
| [OhosPixelMapInfo](/ref/image-api/image-c/image-struct/capi-image-ohospixelmapinfo/capi-image-ohospixelmapinfo) \*info | 用于保存信息的指针对象。 |

****返回：****

| 类型 | 说明 |
| --- | --- |
| int32\_t | [错误码](#方法返回的错误码)：  OHOS\_IMAGE\_RESULT\_SUCCESS：操作成功。  OHOS\_IMAGE\_RESULT\_BAD\_PARAMETER：操作失败。 |
