...
sidebar_position: 9
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-hid
kit: 系统
last_updated: "2026-04-22"
slug: js-apis-bluetooth-hid
---

# @ohos.bluetooth.hid (蓝牙hid模块)

本模块提供基于人机接口协议（Human Interface Device Profile，[HID](/system-network/connectivity-kit/terminology#hid)）技术的蓝牙人机交互能力，支持获取连接状态等方法。

当本端设备被注册为HID设备的角色时，可以使用[HidDeviceProfile](#hiddeviceprofile23)相关接口，且仅支持与传统蓝牙类型设备连接和交互。

![](../../../../images/ecba4ef1/note_3.0-zh-cn.png) 
...
| 类型 | 说明 |
| --- | --- |
| [baseProfile.BaseProfile](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-bluetooth-baseprofile/js-apis-bluetooth-baseprofile#baseprofile) | 基础Profile接口定义。 |
...
创建蓝牙[HID Host](/system-network/connectivity-kit/terminology#hid-host)实例。通过该实例可使用本端作为HID Host的接口，如：获取和其他设备间的蓝牙HID连接状态。
...
| 类型 | 说明 |
| --- | --- |
| HidHostProfile | 返回HID Host实例。  - 该类继承于[BaseProfile](#baseprofile)，因此可以使用其父类中的方法。  - 和该实例角色相对应的是[HID Device](/system-network/connectivity-kit/terminology#hid-device)角色。 |
...
创建蓝牙[HID Device](/system-network/connectivity-kit/terminology#hid-device)实例。通过该实例可使用本端作为HID Device的接口，如：获取和其他设备间的蓝牙HID连接状态。
...
| 类型 | 说明 |
| --- | --- |
| [HidDeviceProfile](#hiddeviceprofile23) | 返回HID Device实例。  - 该类继承于[BaseProfile](#baseprofile)，因此可以使用其父类中的方法。  - 和该实例角色相对应的是[HID Host](/system-network/connectivity-kit/terminology#hid-host)角色。 |
...
该实例表示蓝牙HID通信中的[HID Device](/system-network/connectivity-kit/terminology#hid-device)角色。
...
- 和该实例角色相对应的是[HID Host](/system-network/connectivity-kit/terminology#hid-host)。
...
- 可通过订阅[on('connectionStateChange')](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-bluetooth-baseprofile/js-apis-bluetooth-baseprofile#baseprofileonconnectionstatechange)事件来感知连接是否成功。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceId | [BluetoothAddress](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-bluetooth-common/js-apis-bluetooth-common#bluetoothaddress) | 是 | 需要连接的对端蓝牙设备地址信息，HID设备中不涉及rawAddressType，无需给定该参数。 |
...
订阅HID主机向HID设备发送的[GET\_REPORT](/system-network/connectivity-kit/terminology#hid)传输请求事件，使用callback异步回调。收到回调后可以通过调用接口[replyReport](#replyreport23)进行回复。当收到的数据不符合预期时，可以通过调用接口[reportError](#reporterror23)进行回复。
...
取消订阅主机向HID设备发出的[GET\_REPORT](/system-network/connectivity-kit/terminology#hid)传输请求事件的回调。使用callback异步回调。
...
订阅HID主机向HID设备发送的[SET\_REPORT](/system-network/connectivity-kit/terminology#hid)传输请求事件，使用callback异步回调。当收到的数据不符合预期时，可以通过调用接口[reportError](#reporterror23)进行回复。
...
取消订阅主机向HID设备发出的[SET\_REPORT](/system-network/connectivity-kit/terminology#hid)传输请求事件的回调。使用callback异步回调。
...
订阅HID主机向HID设备发送的[SET\_PROTOCOL](/system-network/connectivity-kit/terminology#hid)请求事件，使用callback异步回调。
...
取消订阅主机向HID设备发送的[SET\_PROTOCOL](/system-network/connectivity-kit/terminology#hid)请求事件的回调。使用callback异步回调。
...
描述HID设备在服务发现协议([SDP](/system-network/connectivity-kit/terminology#sdp))中的服务注册配置。该结构定义了HID设备的身份标识、能力描述和协议特征，是HID主机发现、识别和连接HID设备的关键参数。
...
描述HID主机向HID设备发送的[GET\_REPORT](/system-network/connectivity-kit/terminology#hid)传输请求事件的信息。
...
描述HID主机向HID设备发送的[SET\_REPORT](/system-network/connectivity-kit/terminology#hid)传输请求事件的信息。
