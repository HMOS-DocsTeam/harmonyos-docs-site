...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-gameplayer
kit: 应用服务
...
| idCompatibleType | number | 否 | 否 | ID兼容类型。  0：gamePlayerId与openId、playerId不兼容，即调用[getLocalPlayer](#gameplayergetlocalplayer)接口时，玩家首次登录游戏生成的玩家标识；teamPlayerId与unionId不兼容，即调用[unionLogin](#gameplayerunionlogin)接口时，玩家首次登录游戏未选择转移APK游戏数据生成的玩家标识。  1：gamePlayerId兼容playerId，即玩家首次登录游戏时选择转移APK游戏数据，且APK游戏使用了playerId作为玩家标识，Game Service Kit将playerId作为新的gamePlayerId。  2：gamePlayerId兼容openId，即玩家首次登录游戏时选择转移APK游戏数据，且APK游戏使用了openId作为玩家标识，Game Service Kit将openId作为新的gamePlayerId。 |
...
![](../../../images/7737e61e/caution_3.0-zh-cn.png) 
...
![](../../../images/638be7cd/note_3.0-zh-cn.png) 
...
| ****名称**** | ****类型**** | 只读 | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| thirdAccountInfos | Array&lt;[ThirdAccountInfo](#thirdaccountinfo)&gt; | 否 | 否 | 游戏官方账号信息，用于在联合登录面板中展示。不同账号的账号名称不能相同。  ****说明****：若只需实现华为账号快速登录，此参数传空数组。 |
...
| ****名称**** | ****类型**** | 只读 | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| accountIcon | [Resource](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-types/ts-types#resource) | 否 | 否 | 游戏官方账号图标资源信息。总和最大支持35KB。  当前仅支持****media****目录下的图片。 |
...
错误码的详细介绍请参见[ArkTS API错误码](/ref/game-service-api/gameservice-arkts/gameservice-error-code/gameservice-error-code)。
...
| ****参数名**** | ****类型**** | ****必填**** | ****说明**** |
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | 上下文信息。  ****说明****：严格要求继承UIAbility，并且获取上下文的时机是onWindowStageCreate生命周期中页面加载成功后。 |
...
| ****参数名**** | ****类型**** | ****必填**** | ****说明**** |
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | 上下文信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[UnionLoginResult](#unionloginresult)&gt; | Promise对象。返回联合登录结果对象。 |

****错误码****：

错误码的详细介绍请参见[ArkTS API错误码](/ref/game-service-api/gameservice-arkts/gameservice-error-code/gameservice-error-code)和[通用错误码](/ref/errorcode-universal/errorcode-universal)。
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[GSKLocalPlayer](#gsklocalplayer)&gt; | Promise对象。返回玩家信息。 |
...
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | 上下文信息。 |
| callback | AsyncCallback&lt;[GSKLocalPlayer](#gsklocalplayer)&gt; | 是 | 回调函数。当获取玩家信息成功，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;[PlayerChangedResult](#playerchangedresult)&gt; | 是 | 回调函数，返回玩家变化结果。 |

****错误码****：

错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)。
...
| callback | Callback&lt;[PlayerChangedResult](#playerchangedresult)&gt; | 否 | 回调函数，返回玩家变化结果。  如果该参数不为空，则取消当前callback订阅。如果该参数为空，则取消playerChanged事件的所有callback订阅。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[MiniGamePlayer](#minigameplayer)&gt; | Promise对象。返回玩家信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[CreatePurchaseResult](#createpurchaseresult)&gt; | Promise对象。返回创建商品结果信息。 |
