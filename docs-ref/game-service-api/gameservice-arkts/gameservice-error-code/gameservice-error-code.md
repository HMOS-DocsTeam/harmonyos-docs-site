...
![](../../../images/1c6f9c91/note_3.0-zh-cn.png) 

以下仅介绍Game Service Kit特有错误码，通用错误码请参考[通用错误码](/ref/errorcode-universal/errorcode-universal)。
...
2. 请检查[init](/ref/game-service-api/gameservice-arkts/gameservice-gameperformance/gameservice-gameperformance#gameperformanceinit)接口传参是否正确。
...
请先调用[init](/ref/game-service-api/gameservice-arkts/gameservice-gameperformance/gameservice-gameperformance#gameperformanceinit)接口，并确保调用成功。
...
- [unionLogin](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunionlogin)登录接口的入参accountIcon总大小超过35KB。
- 接口中的入参context不符合要求。
...
1. 请检查配置项是否配置正确，详情请参见[开发准备](/game-service-kit-guide/gameservice-gameplayer-dev/gameservice-gameplayer-gamelogin/gameservice-gameplayer-network/gameservice-gameplayer-huawei#开发准备)。
2. 请检查网络连接是否正常。
...
调用[unionLogin](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunionlogin)接口再次引导用户进行实名认证。
...
调用[bindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerbindplayer)或[unbindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunbindplayer)接口时，teamPlayerId传错。

****处理步骤****

请检查teamPlayerId是否正确，并重新调用[bindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerbindplayer)或[unbindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunbindplayer)接口传递正确teamPlayerId。
...
重新调用[init](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerinit)接口。
...
调用[bindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerbindplayer)接口时，游戏官方账号与华为teamPlayerId已绑定。
...
调用[unbindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunbindplayer)接口时，游戏官方账号与华为teamPlayerId未绑定。
...
1. 请确认当前游戏调用[unionLogin](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunionlogin)接口是否报错。
...
请重新调用[unionLogin](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunionlogin)接口，获取玩家信息。
...
如需继续登录，请重新调用[unionLogin](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunionlogin)接口。
...
检查[配置签名证书指纹](/game-service-kit-guide/gameservice-gameplayer-dev/gameservice-gameplayer-gamelogin/gameservice-gameplayer-network/gameservice-gameplayer-huawei#配置签名证书指纹)、[配置APP ID和Client ID](/game-service-kit-guide/gameservice-gameplayer-dev/gameservice-gameplayer-gamelogin/gameservice-gameplayer-network/gameservice-gameplayer-huawei#配置app-id和client-id)等信息是否已配置或正确，并根据情况补充配置或进行修改。
...
请登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，在“我的应用 > 运营 > 商品管理 > 商品列表” 查看对应商品是否存在或必填信息是否完整及已经提交审核并审核通过。如未审核通过，可使用沙盒账号来测试（参见[沙盒测试](/iap-kit-guide/iap-sandbox)）。
...
可通过[queryPurchases](/ref/iap-api/iap-arkts/iap-iap/iap-iap#iapquerypurchases)接口确认用户是否购买了该商品。

- 若商品为消耗型商品或非续期订阅商品，检查商品是否发货，确认发货成功之后调用[finishPurchase](/ref/iap-api/iap-arkts/iap-iap/iap-iap#iapfinishpurchase)接口完成购买，下次可正常购买。
...
可通过[queryPurchases](/ref/iap-api/iap-arkts/iap-iap/iap-iap#iapquerypurchases)接口查询是否有该商品的确认发货记录。
...
1. 首次使用设备进行登录时，请确认网络连接正常。
2. 请检查[create](/ref/game-service-api/gameservice-arkts/gameservice-nearbytransfer/gameservice-nearbytransfer#gamenearbytransfercreate)接口传参是否正确。
3. 未配置白名单，请参见[开发准备](/game-service-kit-guide/gameservice-nearbytransfer-dev/gameservice-nearbytransfer-config-agc)联系运营申请开通。
...
请先调用[create](/ref/game-service-api/gameservice-arkts/gameservice-nearbytransfer/gameservice-nearbytransfer#gamenearbytransfercreate)接口初始化，并确保调用成功。
