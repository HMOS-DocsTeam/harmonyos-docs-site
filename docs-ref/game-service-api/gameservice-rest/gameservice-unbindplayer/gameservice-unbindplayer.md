...
title: "解绑账号"
sidebar_position: 4
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-unbindplayer
kit: 应用服务
...
玩家选择华为账号登录游戏时，游戏需要将游戏官方账号与华为teamPlayerId绑定，并将绑定结果上报给华为游戏服务器。若需将游戏官方账号与华为teamPlayerId解绑，需将解绑结果上报给华为，此时客户端侧可通过Game Service Kit的[unbindPlayer](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunbindplayer)接口实现，服务器侧则可通过本接口实现。
...
| teamPlayerId | 是 | String | HarmonyOS 5.0及以上游戏的玩家标识，通过调用[unionLogin](/ref/game-service-api/gameservice-arkts/gameservice-gameplayer/gameservice-gameplayer#gameplayerunionlogin)接口返回，  最大长度256个字符。 |
...
1. 将请求参数Body中除****sign****和****method****以外的参数，按参数名首字母的ASCII码将参数升序排序。若遇到相同首字母，则比较第二个字母，以此类推。
   ...
   ![](../../../images/6f2c8012/caution_3.0-zh-cn.png) 
...
4. 将待签名字符串使用游戏私钥按照RSA算法（SHA256WithRSA/PSS）进行签名，将签名后的字符串经过Base64编码，得到的字符串即为签名字符串。
   ...
   ![](../../../images/b11a1407/note_3.0-zh-cn.png) 

   - 游戏私钥获取请参见[获取游戏密钥](/game-service-kit-guide/gameservice-appendix/gameservice-key)。
