...
sidebar_position: 5
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-scenemap
kit: 应用服务
...
![](../../../images/ce99c65c/note_3.0-zh-cn.png) 
...
| ****参数名**** | ****类型**** | 必填 | ****说明**** |
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | [UIAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability)或[UIExtensionAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiextensionability/js-apis-app-ability-uiextensionability)所对应的context。 |
...
![](../../../images/7b01aa1f/note_3.0-zh-cn.png) 
...
![](../../../images/21a61112/note_3.0-zh-cn.png) 
...
| ****参数名**** | ****类型**** | 必填 | ****说明**** |
| --- | --- | --- | --- |
| context | [common.Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 是 | [UIAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability)或[UIExtensionAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiextensionability/js-apis-app-ability-uiextensionability)所对应的context。 |
...
| ****名称**** | ****类型**** | 只读 | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| siteId | string | 否 | 是 | 地点详情页的地点ID，异常值返回401错误码。  siteId可通过[MapEventManager](/ref/map-api/map-arkts/map-map/map-map-mapeventmanager/map-map-mapeventmanager)中[on](/ref/map-api/map-arkts/map-map/map-map-mapeventmanager/map-map-mapeventmanager#onpoiclick)(type: 'poiClick', callback: Callback&lt;[mapCommon.Poi](/ref/map-api/map-arkts/map-common/map-common#poi)&gt;)方法获取。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| language | string | 否 | 是 | 语言，请参见[地图Picker支持语言](/map-kit-guide/map-appendix/map-language#地图picker支持语言)，如果未设置，默认使用系统语言。异常值按默认值处理。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| location | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 是 | 地图中心点坐标。如果没有siteId，使用location查询地点详情。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
| themeColor | [CustomColors](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-theme/js-apis-arkui-theme#customcolors) | 否 | 是 | 自定义主题颜色对象，默认为brand（品牌色）。  ****起始版本：**** 5.0.3(15)  ****元服务API：**** 从版本5.0.3(15)开始，该接口支持在元服务中使用。 |
| cancelCallback | Callback&lt;void&gt; | 否 | 是 | 回调函数。无返回结果的回调函数。地点详情控件关闭事件回调。  ****起始版本：**** 5.0.3(15)  ****元服务API：**** 从版本5.0.3(15)开始，该接口支持在元服务中使用。 |
| transitionDuration | number | 否 | 是 | 转场动效时间，默认值：-1，单位：ms，取值范围[0, Number.MAX\_VALUE)，异常值将按照默认值-1处理，保持[interpolatingSpring](/ref/arkui-api/arkui-arkts/ui/js-apis-curve/js-apis-curve#curvesinterpolatingspring10)(0.5, 1, 328, 36)（初始速度为0.5，质量为1，刚度为1，阻尼为1）。2in1设备当前不显示转场动效，该配置不生效。  ****起始版本：**** 6.0.2(22)  ****元服务API：**** 从版本6.0.2(22)开始，该接口支持在元服务中使用。 |

![](../../../images/e336f8ad/note_3.0-zh-cn.png) 
...
| ****名称**** | ****类型**** | 只读 | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| location | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 是 | 地图中心点坐标。  如果参数未传，使用设备当前位置作为中心点；如果未获取到设备当前位置，默认以故宫博物院为中心点。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| language | string | 否 | 是 | 语言，请参见[地图Picker支持语言](/map-kit-guide/map-appendix/map-language#地图picker支持语言)，如果未设置，默认使用系统语言。异常值按默认值处理。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| poiTypes | Array&lt;string&gt; | 否 | 是 | 指定需要展示的POI类别。取值范围参见[HwLocationType](/ref/map-api/map-arkts/map-appendix-api/map-poi/map-poi#hwlocationtype)。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
| location | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 否 | 选点的坐标点。  ****元服务API：**** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
| addressComponent | [site.AddressComponent](/ref/map-api/map-arkts/map-site/map-site#addresscomponent) | 否 | 是 | 选点地址的详细信息。  ****起始版本：**** 5.0.0(12)  ****元服务API：**** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| zoom | number | 否 | 否 | 选点地址的缩放层级，取值范围：[2, 20]，超出按边界值处理。  ****起始版本：**** 5.0.0(12)  ****元服务API：**** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| snapshot | [image.PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 否 | 是 | 地图快照。  ****起始版本：**** 5.0.0(12)  ****元服务API：**** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
...
| language | string | 否 | 是 | 设置页面语言，请参见[地图Picker支持语言](/map-kit-guide/map-appendix/map-language#地图picker支持语言)，如果未设置，默认使用系统语言。异常值按默认值处理。  ****元服务API：**** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
...
| transitionDuration | number | 否 | 是 | 转场动效时间，默认值：-1，单位：ms，取值范围[0, Number.MAX\_VALUE)，异常值将按照默认值-1处理，保持[interpolatingSpring](/ref/arkui-api/arkui-arkts/ui/js-apis-curve/js-apis-curve#curvesinterpolatingspring10)(0.5, 1, 328, 36)（初始速度为0.5，质量为1，刚度为1，阻尼为1）。当subWindowEnabled为true时该配置不生效。  ****起始版本：**** 6.0.2(22)  ****元服务API：**** 从版本6.0.2(22)开始，该接口支持在元服务中使用。 |
...
| location | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 是 | 区划的坐标点 |
...
| adminCode | string | 否 | 是 | [行政区划码](/ref/map-api/map-arkts/map-appendix-api/map-citycode/map-citycode)。  ****说明：****  接口返回的行政区划码覆盖中国大陆及港澳地区，包括省份、城市、区/县、乡镇/街道等层级，文档附录中仅提供部分城市行政区划码示例。 |
| cityCode | string | 否 | 是 | [城市码](/ref/map-api/map-arkts/map-appendix-api/map-citycode/map-citycode)。 |
