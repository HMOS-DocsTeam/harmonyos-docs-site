...
sidebar_position: 4
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-petal-maps
kit: 应用服务
...
| ****名称**** | ****类型**** | 只读 | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| destinationPosition | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 否 | 终点的坐标。  取值范围：经度[-180, 180)，纬度[-85.2, 85.2]。对异常值进行处理，返回错误码401。 |
...
| coordinateType | [mapCommon.CoordinateType](/ref/map-api/map-arkts/map-common/map-common#coordinatetype) | 否 | 是 | 地图坐标系类型。默认值[mapCommon.CoordinateType](/ref/map-api/map-arkts/map-common/map-common#coordinatetype).GCJ02，异常值按照默认值处理。  ****起始版本：**** 6.0.1(21) |
...
| ****名称**** | ****类型**** | 只读 | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| originPosition | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 是 | 起点的坐标。默认值为用户当前坐标。  取值范围：经度[-180, 180)，纬度[-85.2, 85.2]。对异常值进行处理，返回错误码401。 |
...
| destinationPoiId | string | 否 | 是 | 终点的POI ID。  POI ID和经纬度都入参时，POI ID具有更高优先级。 |
| vehicleType | [VehicleType](#vehicletype) | 否 | 是 | 出行方式。  默认值为[VehicleType](#vehicletype).DRIVING，异常值按默认值处理。 |
| originPosition | [mapCommon.LatLng](/ref/map-api/map-arkts/map-common/map-common#latlng) | 否 | 是 | 起点的坐标。默认值为用户当前坐标。  取值范围：经度[-180, 180)，纬度[-85.2, 85.2]。对异常值进行处理，返回错误码401。  ****起始版本：**** 6.0.1(21) |
...
| originName | string | 否 | 是 | 起点的名称，超长名称超出部分用省略号“...”表示。 |
| originPoiId | string | 否 | 是 | 起点的POI ID。  POI ID和经纬度都入参时，POI ID具有更高优先级。 |
| coordinateType | [mapCommon.CoordinateType](/ref/map-api/map-arkts/map-common/map-common#coordinatetype) | 否 | 是 | 地图坐标系类型。默认值[mapCommon.CoordinateType](/ref/map-api/map-arkts/map-common/map-common#coordinatetype).GCJ02，异常值按照默认值处理。 |
