...
sidebar_position: 5
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pen-stylusinteraction
kit: 系统
...
| receiver | Callback&lt;[SqueezeEvent](#squeezeevent)&gt; | 是 | 回调函数，返回手写笔笔身轻捏事件的详细信息。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 固定填写"squeeze"字符串，表示手写笔笔身轻捏事件。 |
| receiver | Callback&lt;[SqueezeEvent](#squeezeevent)&gt; | 否 | 需要取消注册的回调函数，需与订阅时传入的回调函数是同一个。若无此参数，则取消注册所有的轻捏事件。 |
...
| receiver | Callback&lt;[DoubleTapEvent](#doubletapevent)&gt; | 是 | 回调函数，返回手写笔笔身双击事件的详细信息。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 固定填写"doubleTap"字符串，表示手写笔笔身双击事件。 |
| receiver | Callback&lt;[DoubleTapEvent](#doubletapevent)&gt; | 否 | 需要取消注册的回调函数，需与订阅时传入的回调函数是同一个。若无此参数，则取消注册所有的双击事件。 |
