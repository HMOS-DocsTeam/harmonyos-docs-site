...
通用错误码的详细介绍请参见和[模块错误码](/ref/preview-api/preview-api-reference/preview-arkts-errcode/preview-arkts-errcode)。
...
![](../../../images/0dd1f7d2/note_3.0-zh-cn.png) 

1. 需要先调用[openFileBoost.on('filePreloadStateChanged')](#openfileboostonfilepreloadstatechanged)接口后再调用该接口添加文件预加载状态监听。
2. 当前一个应用最多添加50个文件监听。

当前支持加速的文件类型见[文件打开加速支持的文件类型](/preview-kit-guide/preview-introduction#文件打开加速支持的文件类型)，不支持的文件类型默认为未预加载状态，不需要调用该接口监听文件预加载状态变更。
