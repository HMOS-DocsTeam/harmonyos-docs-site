...
![](../../../../../images/3c71a3ba/note_3.0-zh-cn.png) 

1. 为保证回调请求的可靠性，系统具备重试机制，可能会出现重发的通知。
2. 为保证信息合法性，开发者需要对返回的支付信息进行[SM2验签](/ref/payment-api/payment-rest/payment-rest-overview/payment-rest-overview#验签规则)，验签注意事项：

   - 需直接使用通知的完整内容进行验签。
   - 验签前需要对返回数据进行排序拼接，sign字段是签名值，排序拼接后的待验签内容需要排除sign字段。
   - 验签公钥使用[华为支付证书](/payment-kit-guide/payment-preparations/payment-certificates-config#华为支付证书)。
3. 退款状态需根据refundOrderStatus字段判断。

## 场景描述

一笔订单退款成功后，华为支付服务器会调用开发者的应用/元服务在[申请退款](/ref/payment-api/payment-rest/payment-merc/payment-pay/payment-service--refund/payment-service--refund)接口中传递的回调接口（callbackUrl）传递给开发者退款订单具体信息。
