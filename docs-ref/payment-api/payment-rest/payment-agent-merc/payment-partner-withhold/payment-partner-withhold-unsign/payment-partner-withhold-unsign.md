...
| callbackUrl | 否 | String | 回调通知地址，通知URL必须为外网环境可直接访问的URL，要求为https地址。具体要求参考[通知回调接口说明](/ref/payment-api/payment-rest/payment-rest-overview/payment-rest-overview#通知回调接口说明)。最大长度为512。 |
...
| planId | 否 | String | 协议模板ID。该模板ID是商户在向华为支付[提交代扣权限申请](/payment-kit-guide/payment-password-free-pay/payment-password-free-pay-overview)时由华为支付生成。 |
