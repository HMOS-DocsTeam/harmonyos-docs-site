...
| sceneInfos | 是 | List&lt;[RequestSceneInfo](/ref/payment-api/payment-model/payment-model#requestsceneinfo)&gt; | 匹配券的场景条件列表，要求列表内的sceneId唯一。如果场景信息匹配上券，对应的sceneId会在响应参数matchedCoupon返回。 |
...
| coupons | 否 | List&lt;[ServCouponInfo](/ref/payment-api/payment-model/payment-model#servcouponinfo)&gt; | 场景条件匹配上的平台券信息列表。 |
| matchedCoupon | 否 | List&lt;[SceneMatchedCouponNos](/ref/payment-api/payment-model/payment-model#scenematchedcouponnos)&gt; | 各场景条件匹配到的券号信息。 |
