...
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-paymentservice
kit: 应用服务
...
| [extraInfo](/ref/payment-api/payment-model/payment-model#extrainfo) | string | 否 | 是 | 保留字段。json string格式。 |
| [payload](/ref/payment-api/payment-model/payment-model#payload) | string | 否 | 是 | 预留信息，在请求接口时，入参如果传递，接口响应中则会原样返回。  ****说明：**** 拉起H5支付场景下需要固定传递“AP”。 |
...
| [extraInfo](/ref/payment-api/payment-model/payment-model#extrainfo) | string | 否 | 是 | 保留字段。json string格式。若未填写，默认为空。  ****说明：**** 商户可以通过保留字段指定支付方式。指定收银台支付方式列表传递内容示例为{"selectPayType":"wechat\_pay|xxx"}。 |
...
| ****参数名**** | ****类型**** | 必填 | ****说明**** |
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | UIAbility上下文，不传会报401参数错误。 |
| [orderStr](/ref/payment-api/payment-model/payment-model#orderstr) | string | 是 | 拉起收银台传入的订单信息，[orderStr](/ref/payment-api/payment-model/payment-model#orderstr)是json字符串的格式。不传会报401参数错误。 |
...
以下错误码的详细介绍请参见[ArkTS API错误码](/ref/payment-api/payment-arkts/payment-error-code/payment-error-code)。
...
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | UIAbility上下文，不传会报401参数错误。 |
| [contractStr](/ref/payment-api/payment-model/payment-model#contractstr) | string | 是 | 拉起签约收银台入参，[contractStr](/ref/payment-api/payment-model/payment-model#contractstr)是json字符串的格式。不传会报401参数错误。 |
...
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | 是 | UIAbility上下文，不传会报401参数错误。 |
| [orderStr](/ref/payment-api/payment-model/payment-model#orderstr) | string | 是 | 拉起收银台或跳转三方支付传入的订单信息。  orderStr是json字符串的格式，不传会报401参数错误。 |
| [payload](/ref/payment-api/payment-model/payment-model#payload) | string | 是 | 预留信息，在请求接口时，入参如果传递，接口响应中则会原样返回。  ****说明：**** 拉起华为支付收银台，需传空或空字符。H5支付场景下跳转三方支付收银台需要固定传递“AP”。 |

****返回值****：

| 类型 | 说明 |
| --- | --- |
| Promise&lt;[PayResult](#payresult)&gt; | Promise对象。带[PayResult](#payresult)返回结果的Promise对象。  ****说明：**** 华为支付场景下，[PayResult](#payresult)可能返回为空，支付结果以回调通知或查询结果为准。 |
...
| ****参数名**** | ****类型**** | 必填 | ****说明**** |
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext) | common.[UIExtensionContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiextensioncontext/js-apis-inner-application-uiextensioncontext) | 是 | UIAbility上下文，不传会报401参数错误。 |
