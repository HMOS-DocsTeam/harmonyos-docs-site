...
| 参数名 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| controller | pdfViewManager.[PdfController](/ref/pdf-api/pdf-arkts/pdf-arkts-pdfviewmanage/pdf-arkts-pdfviewmanage#pdfcontroller) | 否 | 否 | PdfView组件控制器。 |
| pageLayout | pdfService.[PageLayout](/ref/pdf-api/pdf-arkts/pdf-arkts-pdfservice/pdf-arkts-pdfservice#pagelayout) | 否 | 否 | 页面布局显示模式。 |
| isContinuous | boolean | 否 | 否 | 是否连续预览，true：是，false：否。 |
| showScroll | boolean | 否 | 否 | 是否显示滚动条，true：显示，false：隐藏。 |
| pageFit | pdfService.[PageFit](/ref/pdf-api/pdf-arkts/pdf-arkts-pdfservice/pdf-arkts-pdfservice#pagefit) | 否 | 否 | 页面适配模式。 |
