...
title: "ArkTS错误码"
sidebar_position: 3
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-error-code
kit: 应用服务
last_updated: "2026-04-22"
slug: account-api-error-code
---

# ArkTS错误码

![](../../images/e0f96eb5/note_3.0-zh-cn.png) 
...
请参见Account Kit常见问题[1001500001 应用指纹证书校验失败的可能原因和解决办法](/account-kit-guide/account-faq/account-faq-1)。
...
4. 元服务开发过程中，调[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)接口时传了profile这个scope，且supportAtomicService参数未设置为true。
5. 元服务开发过程中，调[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)接口时传了phone这个scope。
...
4. 元服务开发过程中，调[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)接口时传了profile这个scope，并将supportAtomicService参数设置为true。
5. 元服务不能直接调用该接口获取手机号，可参考场景化控件[快速验证手机号Button](/scenario-fusion-guide/scenario-fusion-button/scenario-fusion-button-getphonenumber)获取。
...
获取手机号一致性接口[getMobileNumberConsistency](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#getmobilenumberconsistency)在24h内被调用超过50次。
...
可调用[LoginWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#loginwithhuaweiidrequest)，将forceLogin参数设置为true拉起登录页面引导用户登录，或引导用户在系统设置中登录华为账号。
...
1. 调用[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)且forceAuthorization参数设置为false的场景，用户未对应用授权。
2. idValue未按照[getMobileNumberConsistency](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#getmobilenumberconsistency)接口中的[ConsistencyRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#consistencyrequest)正确传入或用户已对应用取消授权。
3. idValue未按照[getHuaweiIDState](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#gethuaweiidstate)接口中的[StateRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#staterequest)正确传入或用户已对应用取消授权。

****处理步骤****

1. 如需引导用户授权，可调用[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)，通过将forceAuthorization参数设置为true（不设置默认为true）拉起用户授权页面引导用户授权。
2. 参考[ConsistencyRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#consistencyrequest)中的idValue正确方式获取或重新授权。
3. 参考[StateRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#staterequest)中的idValue正确方式获取或重新授权。
...
2. Profile文件无效。
3. 一键登录场景，传入的匿名手机号不正确，或是未调用授权API（[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)）获取匿名手机号。
4. 接口传参异常，如调用授权API（[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)）时scope和permission都未传。
...
1. 在 AppGallery Connect（简称AGC）的[开发与服务](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html#/myProject)中，选择对应的项目和对应的应用，在“常规 > 应用 ”下，找到****应用****的Client ID和APP ID。
   ...
   ![](../../images/02105468/zh-cn_image_0000002552799096.png)

   - 若Client ID和APP ID不同：请检查module type为entry的模块下module.json5中的client\_id是否配置或配置的值是否正确，参考[配置Client ID](/account-kit-guide/account-preparations/account-client-id)。
...
3. 需要通过授权API（[AuthorizationWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#authorizationwithhuaweiidrequest)）获取到匿名手机号，将其作为参数调用一键登录接口。
4. 检查[authentication](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication)相关接口参数。
...
请参见Account Kit常见问题[1001502014 应用未申请scopes或permissions权限的可能原因和解决方法](/account-kit-guide/account-faq/account-faq-2)。
...
如需引导用户登录，可调用[LoginWithHuaweiIDRequest](/ref/account-api/account-api-arkts/account-api-authentication/account-api-authentication#loginwithhuaweiidrequest)，将forceLogin参数设置为true拉起登录页面引导用户登录，或引导用户在系统设置中登录华为账号。
...
1. 检查module type为entry的模块下的module.json5配置文件中的Client ID是否正确，请参考[配置Client ID](/account-kit-guide/account-preparations/account-client-id)。
...
1. 检查module type为entry的模块下的module.json5配置文件中的Client ID是否正确，详情请见[配置Client ID](/account-kit-guide/account-preparations/account-client-id)。
2. 检查AGC上应用的指纹证书，详情请见[添加公钥指纹](https://developer.huawei.com/consumer/cn/doc/app/agc-help-cert-fingerprint-0000002278002933#section7398154810570)。
...
确认对应接口permissions是否已申请，申请步骤参见[开发前提](/account-kit-guide/account-get-user-info/account-choose-address-dev#开发前提)。
...
[开启未成年人模式](/account-kit-guide/account-minorsprotection/account-follow-minorsprotection/account-app-turn-on-minorsprotection)。
...
调用[getMinorsProtectionInfoSync](/ref/account-api/account-api-arkts/account-api-minorsprotection/account-api-minorsprotection#getminorsprotectioninfosync)()或[getMinorsProtectionInfo](/ref/account-api/account-api-arkts/account-api-minorsprotection/account-api-minorsprotection#getminorsprotectioninfo)()判断当前未成年人模式的状态，如果未成年人模式是关闭状态，再调用接口。
...
调用[getMinorsProtectionInfoSync](/ref/account-api/account-api-arkts/account-api-minorsprotection/account-api-minorsprotection#getminorsprotectioninfosync)()或[getMinorsProtectionInfo](/ref/account-api/account-api-arkts/account-api-minorsprotection/account-api-minorsprotection#getminorsprotectioninfo)()判断当前未成年人模式的状态，如果未成年人模式是开启状态，再调用接口，否则无需重复调用。
...
1. 重启设备后重试。
2. 请检查当前设备是否支持调用该API，建议先使用[canIUse](/ref/syscap/syscap#判断-api-是否可以使用)判断当前设备是否支持该API，再进行调用。
