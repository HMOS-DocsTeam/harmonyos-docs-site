...
![](../../../../images/c930e7b1/caution_3.0-zh-cn.png) 
...
[获取头像昵称](/account-kit-guide/account-get-user-info/account-get-avatar-nickname)场景，应用服务端向华为账号服务器调用该接口获取头像昵称及其相关信息和UnionID/OpenID。
...
- 需确保调用端网络正常。
- 应用获取华为账号头像昵称之前需要在获取Authorization Code时携带profile（昵称和头像）scope，详见[业务流程](/account-kit-guide/account-get-user-info/account-get-avatar-nickname#业务流程)。

![](../../../../images/fb4d3662/note_3.0-zh-cn.png) 
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access\_token | 是 | String | 通过[获取用户级凭证](/ref/account-api/account-api-rest/account-api-obtain-token/account-api-obtain-user-token/account-api-obtain-user-token)获取的Access Token。获取凭证时Authorization Code需包含profile scope的授权。 |
...
| displayName | 否 | String | 用户昵称，该字段返回场景详见[获取头像昵称](/account-kit-guide/account-get-user-info/account-get-avatar-nickname)。  - 请求参数“getNickName”为0或不传时，返回匿名化账号。  - 请求参数“getNickName”为1时，返回昵称，没有昵称时返回匿名化账号。  ****说明：****  调用成功后响应中若不包含用户昵称displayName，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](/account-kit-guide/account-get-user-info/account-get-avatar-nickname#客户端开发) |
| displayNameFlag | 否 | int | 返回的昵称类型 。  0：昵称。  1：匿名账号。  ****说明：****  调用成功后响应中若不包含昵称类型displayNameFlag，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](/account-kit-guide/account-get-user-info/account-get-avatar-nickname#客户端开发) |
| headPictureURL | 否 | String | 用户头像，该字段返回场景详见[获取头像昵称](/account-kit-guide/account-get-user-info/account-get-avatar-nickname)。用户未设置头像时不返回。 |
...
![](../../../../images/b803e666/note_3.0-zh-cn.png) 

Response Header中的NSP\_STATUS字段，在处理成功时不会返回。

| NSP\_STATUS | 描述 | 解决方法 |
| --- | --- | --- |
| 6 | 会话失效，session timeout。  可能原因:  - access\_token无效或已过期  - access\_token格式不正确  - 其他内部原因 | - 请检查传参是否正确，如无问题请尝试重新获取。  - 未对access\_token进行URLEncode处理，可参考[示例代码](#示例代码)组装参数。  - 根据返回的错误描述进行处理，若仍无法解决，请通过[在线提单](https://developer.huawei.com/consumer/cn/support/feedback/#/)提交问题。 |
