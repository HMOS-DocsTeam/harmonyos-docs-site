...
![](../../../../images/db2718e8/caution_3.0-zh-cn.png) 
...
针对用户登录需要获取GroupUnionID场景时，可以在[华为账号一键登录（获取手机号和UnionID/OpenID）](/account-kit-guide/account-quick-login/account-phone-unionid-login)、[华为账号登录（获取UnionID/OpenID）](/account-kit-guide/account-quick-login/account-unionid-login/account-unionid-login-button)、[静默登录](/account-kit-guide/account-quick-login/account-silent-login)等场景获取到Authorization Code后，调用该接口获取GroupUnionID、Access Token、Refresh Token、ID Token等信息。
...
| code | 是 | String | Authorization Code可通过[华为账号一键登录（获取手机号和UnionID/OpenID）](/account-kit-guide/account-quick-login/account-phone-unionid-login)、[华为账号登录（获取UnionID/OpenID）](/account-kit-guide/account-quick-login/account-unionid-login/account-unionid-login-button)、[静默登录](/account-kit-guide/account-quick-login/account-silent-login)等场景获取。 |
| supportAlg | 否 | String | 生成ID Token的算法，当前支持的算法如下：  - PS256（推荐使用）  - RS256  如果未指定该参数或指定的算法不在支持的范围内，则默认使用RS256。 |
| need\_group\_union\_id | 否 | Boolean | 是否需要获取GroupUnionID，传值如下：  - true  - false  如果未指定该参数，则响应结果中不会返回group\_union\_id字段。  GroupUnionID使用场景详见[不同开发者的应用之间如何实现用户数据互通](/account-kit-guide/account-faq/account-faq-19)。 |
...
| access\_token | 是 | String | Access Token，访问被权限管控资源的凭证。Access Token长度详见[Access Token和Refresh Token长度限制要求](/account-kit-guide/account-faq/account-faq-11)。 |
| scope | 是 | String | Access Token中的scope，以空格分隔，最大不会超过150个。 |
| expires\_in | 是 | Integer | Access Token的过期时间，以秒为单位。有效期为3600秒。 |
| refresh\_token | 是 | String | Refresh Token，用于刷新Access Token。Refresh Token有效期为180天，长度详见[Access Token和Refresh Token长度限制要求](/account-kit-guide/account-faq/account-faq-11)。 |
| id\_token | 是 | String | ID Token（JWT格式），详细信息请参见[验证ID Token有效性](/ref/account-api/account-api-rest/account-api-extend-function/account-api-verify-id-token/account-api-verify-id-token)中ID Token描述。 |
| group\_union\_id | 否 | String | GroupUnionID是用户在关联主体账号组内的统一身份标识，使用场景详见[不同开发者的应用之间如何实现用户数据互通](/account-kit-guide/account-faq/account-faq-19)。当请求参数need\_group\_union\_id不传或者为false时，该字段不返回。 |
...
| 1101 | 20154 | code中的client\_id和入参不一致。 | 检查入参client\_id是否与[配置Client ID](/account-kit-guide/account-preparations/account-client-id)中的值一致。 |
