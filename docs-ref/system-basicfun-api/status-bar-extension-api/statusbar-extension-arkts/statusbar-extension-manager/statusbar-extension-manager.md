...
| statusBarGroupMenu | [StatusBarGroupMenu](#statusbarmanagerstatusbargroupmenu)[] | 否 | 是 | 状态栏图标的右键分组菜单相关信息。  默认值：undefined。  ****说明：**** 当前所有分组下一级菜单项的总和不可超过20个。 |
| hoverTips | string | 否 | 是 | 状态栏图标hover时的显示信息。  取值范围：1~128。  ****说明：**** 如不配置该参数，则hover时显示内容默认取值为[statusBarManager.QuickOperation](#statusbarmanagerquickoperation)中配置模块的[label](/ref/ability-api/ability-arkts/ability-api-interface-depend/bundlemanager/js-apis-bundlemanager-hapmoduleinfo/js-apis-bundlemanager-hapmoduleinfo#hapmoduleinfo-1)。  ****起始版本：**** 6.0.2(22) |
...
| ****名称**** | ****类型**** | ****只读**** | 可选 | ****说明**** |
| --- | --- | --- | --- | --- |
| white | [image.PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 否 | 否 | 深色壁纸下展示的图标，建议采用纯白色图标，支持JPEG、PNG、GIF、WebP、BMP、SVG、ICO、DNG等图片类型。  ****说明：**** 建议使用24[vp](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-pixel-units/ts-pixel-units) \* 24vp的图片。 |
| black | [image.PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap) | 否 | 否 | 浅色壁纸下展示的图标，建议采用纯黑色图标，支持JPEG、PNG、GIF、WebP、BMP、SVG、ICO、DNG等图片类型。  ****说明：**** 建议使用24vp \* 24vp的图片。 |
...
| abilityName | string | 否 | 否 | 状态栏图标左键业务弹窗的应用定制区域对应的自定义[StatusBarViewExtensionAbility](/ref/system-basicfun-api/status-bar-extension-api/statusbar-extension-arkts/statusbar-extension-ability/statusbar-extension-ability)的名称。  ****说明：**** 当传空字符串时，支持通过监听[statusBarIconClick](#statusbarmanageronstatusbariconclick)事件处理点击业务。 |
| moduleName | string | 否 | 是 | 状态栏图标左键业务弹窗的应用定制区域对应的自定义[StatusBarViewExtensionAbility](/ref/system-basicfun-api/status-bar-extension-api/statusbar-extension-arkts/statusbar-extension-ability/statusbar-extension-ability)所在的模块名称。  默认值：''。 |
...
| menuAction | [StatusBarMenuAction](#statusbarmanagerstatusbarmenuaction) | 否 | 是 | 菜单项行为信息。  默认值：undefined。  ****说明：**** 当前菜单行为仅支持打开应用内定义的UIAbility，且menuAction和subMenu两个参数不可同时缺省。 |
| subMenu | [StatusBarSubMenuItem](#statusbarmanagerstatusbarsubmenuitem)[] | 否 | 是 | 一级菜单项的子菜单项。  默认值：undefined。  ****说明：**** 当前单个一级菜单项最多支持20个子菜单项。 |
...
| menuAction | [StatusBarMenuAction](#statusbarmanagerstatusbarmenuaction) | 否 | 否 | 菜单项行为信息。  ****说明：**** 当前菜单行为仅支持打开应用内定义的UIAbility。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [common.Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context#context) | 是 | 上下文信息。  ****说明：**** 当前context仅支持传入Context的子类UIAbilityContext、ServiceExtensionContext、FormExtensionContext。 |
| statusBarItem | [StatusBarItem](#statusbarmanagerstatusbaritem) | 是 | 应用自定义接入状态栏的图标的详细信息。 |

****错误码：****

以下错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[ArkTS API错误码](/ref/system-basicfun-api/status-bar-extension-api/statusbar-extension-arkts/statusbar-extension-error-code/statusbar-extension-error-code)。
...
| --- | --- | --- | --- |
| context | [common.Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context#context) | 是 | 上下文信息。  ****说明：**** 当前context仅支持传入Context的子类UIAbilityContext、ServiceExtensionContext、FormExtensionContext。 |
| statusBarGroupMenus | [StatusBarGroupMenu](#statusbarmanagerstatusbargroupmenu)[] | 是 | 更新后的应用右键菜单栏相关信息。  ****说明：**** 当前所有分组下一级菜单项的总和不可超过20个。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'statusBarIconClick'，当点击应用图标时，触发该事件。  ****说明：**** 当调用[addToStatusBar](#statusbarmanageraddtostatusbar)，入参[quickOperation](#statusbarmanagerquickoperation)未指定abilityName时，支持通过监听statusBarIconClick事件处理点击业务。 |
| callback | Callback&lt;[emitter.EventData](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/process-thread-communi-arkts/js-apis-emitter/js-apis-emitter#eventdata)&gt; | 是 | 需要注册的回调函数，返回信息为图标点击相关信息。  当前仅支持返回iconClickType（点击事件类型）中的leftClick（左键）。 |
...
| callback | Callback&lt;[emitter.EventData](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/process-thread-communi-arkts/js-apis-emitter/js-apis-emitter#eventdata)&gt; | 否 | 取消该事件的回调处理函数。  默认值：undefined。  ****说明：****  若不传入callback，会取消该事件的所有回调函数；  若传入callback，需要传入对应on方法相同的对象，否则会导致回调函数未成功注销，导致内存泄露。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'rightMenuClick'，当点击应用图标右键菜单时，触发该事件。  ****说明：**** 当调用[updateStatusBarMenu](#statusbarmanagerupdatestatusbarmenu)，添加菜单项时，指定[menuAction](#statusbarmanagerstatusbarmenuaction)的notifyOnly使能和菜单项menuCode时生效。 |
| callback | Callback&lt;[emitter.EventData](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/process-thread-communi-arkts/js-apis-emitter/js-apis-emitter#eventdata)&gt; | 是 | 需要注册的回调函数，返回信息为图标右键菜单点击相关信息。  当前返回信息menuCode（点击菜单项唯一标识）。 |
