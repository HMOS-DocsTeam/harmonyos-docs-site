...
title: "FAST"
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/fast-kit-fast
kit: 系统
...
| 名称 | 描述 |
| --- | --- |
| [fast\_ads\_segment\_map.h](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-files/fast-kit-fast-ads-segment-map-8h/fast-kit-fast-ads-segment-map-8h) | 线段表相关数据结构及函数定义。 |
| [fast\_common\_def.h](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-files/fast-kit-fast-common-def-8h/fast-kit-fast-common-def-8h) | FAST Kit错误码等类型的公共定义。 |
| [fast\_solver\_rect\_partition.h](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-files/fast-kit-fast-solver-rect-partition-8h/fast-kit-fast-solver-rect-partition-8h) | 矩形划分求解器相关数据结构及函数定义。 |

### 结构体

| 名称 | 描述 |
| --- | --- |
| struct [FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect) | 定义矩形的数据结构。 |
...
| typedef struct [FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect) [FAST\_Rect](#fast_rect) | 定义矩形的数据结构。 |
...
| [FAST\_ErrorCode](#fast_errorcode-1) {  FAST\_ERROR\_CODE\_SUCCESS = 1023100000, FAST\_ERROR\_CODE\_FAIL = 1023100001, FAST\_ERROR\_CODE\_ILLEGAL\_INPUT = 1023100002, FAST\_ERROR\_CODE\_INVALID\_PTR = 1023100003,  FAST\_ERROR\_CODE\_OOM = 1023199001  } | FAST Kit的错误码。 |
...
| FAST\_EXPORT [FAST\_ErrorCode](#fast_errorcode-1) [HMS\_FAST\_RectPartition\_SetAlgo](#hms_fast_rectpartition_setalgo) ([FAST\_RectPartitionConfig](#fast_rectpartitionconfig) \*config, const char \*name) | 设置矩形划分求解器使用的算法。目前仅支持扫描线算法“SweepLineAlgo”，输出数量尽可能少（不保证最优性）的不相交矩形集合，复杂度为。 |
| FAST\_EXPORT [FAST\_ErrorCode](#fast_errorcode-1) [HMS\_FAST\_RectPartition\_Solve](#hms_fast_rectpartition_solve) ([FAST\_RectPartitionConfig](#fast_rectpartitionconfig) \*config, size\_t size, const [FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect) \*origin, [FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect) \*result, size\_t \*resultSize) | 在指定不透明配置下解决矩形划分问题。函数接收若干个彼此不相交的矩形作为输入，计算出覆盖相同区域的矩形划分方案，并使输出的矩形数量尽可能少。  ****说明****：  1. 输入须保证矩形两两不相交（即任意两个矩形满足： 或 或或 ），否则函数返回FAST\_ERROR\_CODE\_ILLEGAL\_INPUT。  2. 函数能保证输出矩形的数量小于等于输入矩形的数量。 |
...
| size | 待划分的矩形[FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect)数量。 |
| origin | 待划分的矩形[FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect)源数组。 |
| result | 由矩形划分求解器得到的[FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect)结果，在调用函数之前需要初始化该结果数组，大小需要和源数组相等，否则可能导致溢出。 |
| resultSize | 划分之后的[FAST\_Rect](/ref/system-basicfun-api/fast-api/fast-c/fast-kit-total/fast-kit-struct/fast-kit--fast-rect/fast-kit--fast-rect)数量。 |
