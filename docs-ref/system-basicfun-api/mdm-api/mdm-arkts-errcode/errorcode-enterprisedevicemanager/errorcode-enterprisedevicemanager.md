...
title: "企业设备管理错误码"
sidebar_position: 1
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-enterprisedevicemanager
kit: 系统
last_updated: "2026-04-22"
slug: errorcode-enterprisedevicemanager
---

# 企业设备管理错误码

![](../../../../images/f1bdef6e/note_3.0-zh-cn.png) 
...
1. 当设置了禁止运行应用名单（例如调用[addDisallowedRunningBundlesSync](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-applicationmanager/js-apis-enterprise-applicationmanager#applicationmanageradddisallowedrunningbundlessync)接口），且应用在禁止运行应用名单中时，启动组件会失败。
2. 当设置了允许运行应用名单（例如调用[addAllowedRunningBundles](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-applicationmanager/js-apis-enterprise-applicationmanager#applicationmanageraddallowedrunningbundles21)接口），且应用不在允许运行应用名单中时，启动组件会失败。

****处理步骤****

1. 将应用从禁止运行名单中移除，例如调用[removeDisallowedRunningBundlesSync](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-applicationmanager/js-apis-enterprise-applicationmanager#applicationmanagerremovedisallowedrunningbundlessync)接口。
2. 将应用添加到允许运行名单，例如调用[addAllowedRunningBundles](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-applicationmanager/js-apis-enterprise-applicationmanager#applicationmanageraddallowedrunningbundles21)接口。
...
当已有一个日志收集任务正在执行时，调用了[systemManager.startCollectLog](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-systemmanager/js-apis-enterprise-systemmanager#systemmanagerstartcollectlog23)接口创建日志收集任务。

****处理步骤****

等前一个日志收集任务完成（即收到[EnterpriseAdminExtensionAbility.onLogCollected](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterpriseadminextensionability/js-apis-enterpriseadminextensionability#onlogcollected23)回调）后，再调用[systemManager.startCollectLog](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-systemmanager/js-apis-enterprise-systemmanager#systemmanagerstartcollectlog23)接口创建日志收集任务。
