...
title: "Enums"
sidebar_position: 14
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e
kit: 媒体
last_updated: "2026-04-22"
slug: arkts-apis-audio-e
---

# Enums

![](../../../../images/8e7bb6f2/note_3.0-zh-cn.png) 
...
当用户监听到音频中断（即收到[InterruptEvent](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#interruptevent9)事件）时，获取此信息。

此类型表示音频打断是否已由系统强制执行，具体操作信息（如音频暂停、停止等）可通过[InterruptHint](#interrupthint)获取。关于音频打断策略的详细说明可参考文档[音频焦点介绍](/audio-kit/audio-session/audio-playback-concurrency)。
...
当用户监听到音频中断事件（即收到[InterruptEvent](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#interruptevent9)事件）时，获取此信息。

此类型表示根据焦点策略，对音频流执行的具体操作（如暂停、调整音量等）。

可以结合InterruptEvent中的[InterruptForceType](#interruptforcetype9)信息，判断该操作是否已由系统强制执行。详情请参阅文档[音频焦点介绍](/audio-kit/audio-session/audio-playback-concurrency)。
...
| INTERRUPT\_HINT\_RESUME | 1 | 提示音频恢复，应用可主动触发开始渲染或开始采集的相关操作。  此操作无法由系统强制执行，其对应的[InterruptForceType](#interruptforcetype9)一定为INTERRUPT\_SHARE类型。  ****元服务API：**** 从API version 12开始，该接口支持在元服务中使用。 |
...
| PRIVACY\_TYPE\_SHARED21+ | 2 | 表示音频流可以被其他应用录制或屏幕投射，包含隐私类型的流。  例如，在PRIVACY\_TYPE\_PUBLIC策略下，[STREAM\_USAGE\_VOICE\_COMMUNICATION](#streamusage)类型音频流不会被其他应用录制或屏幕投射。  然而，在PRIVACY\_TYPE\_SHARED策略下，这些音频流将会允许被其他应用录制或屏幕投射。 |
...
| SOURCE\_TYPE\_PLAYBACK\_CAPTURE(deprecated) | 2 | 播放音频流（内录）录制音频源。  ****系统能力：**** SystemCapability.Multimedia.Audio.PlaybackCapture  从API version 10开始支持，从API version 12开始废弃，建议使用[录屏接口AVScreenCapture](/ref/media-api/media-c/media-module/capi-avscreencapture/capi-avscreencapture)替代。 |
| SOURCE\_TYPE\_VOICE\_COMMUNICATION | 7 | 语音通话场景的音频源（单独启动录制不会开启3A算法，需同时使用[STREAM\_USAGE\_VOICE\_COMMUNICATION](#streamusage)或[STREAM\_USAGE\_VIDEO\_COMMUNICATION](#streamusage)类型的AudioRender起播才会触发开启3A算法）。  ****系统能力：**** SystemCapability.Multimedia.Audio.Core |
...
当用户监听到音频会话状态变化事件（即收到[AudioSessionStateChangedEvent](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#audiosessionstatechangedevent20)事件）时，获取相关信息。

此类型表示根据焦点策略对音频会话执行的操作，包括暂停、调整音量等。

详情请参阅文档[音频会话管理](/audio-kit/audio-session/audio-session-management)。
...
| AUDIO\_SESSION\_STATE\_CHANGE\_HINT\_DUCK | 4 | 提示音频会话躲避开始，降低音量播放。  如果已启用[enableMuteSuggestionWhenMixWithOthers](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audiosessionmanager/arkts-apis-audio-audiosessionmanager#enablemutesuggestionwhenmixwithothers23)，此时可以选择执行静音操作。 |
| AUDIO\_SESSION\_STATE\_CHANGE\_HINT\_UNDUCK | 5 | 提示音频会话躲避结束，恢复音量播放。  如果已启用[enableMuteSuggestionWhenMixWithOthers](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audiosessionmanager/arkts-apis-audio-audiosessionmanager#enablemutesuggestionwhenmixwithothers23)，此时可取消静音。 |
...
![](../../../../images/4b99028f/note_3.0-zh-cn.png) 
...
![](../../../../images/c8227ef7/note_3.0-zh-cn.png) 
...
![](../../../../images/72bb98b9/note_3.0-zh-cn.png) 
