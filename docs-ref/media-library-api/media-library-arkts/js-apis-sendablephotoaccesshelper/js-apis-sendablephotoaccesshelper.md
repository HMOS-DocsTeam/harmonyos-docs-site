...
该模块基于[Sendable](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable)对象，提供相册管理功能，包括创建相册和访问、修改相册中的媒体数据。

![](../../../images/b335ea78/note_3.0-zh-cn.png) 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context) | 是 | 传入Ability实例的Context。 |
...
对于未申请'ohos.permission.READ\_IMAGEVIDEO'权限的应用，可以通过picker的方式调用该接口来查询指定URI对应的图片或视频资源，详情请参考[指定URI获取图片或视频资源](/medialibrary-kit/photoaccesshelper-photoviewpicker#指定uri获取图片或视频资源)。

****参数：****

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [photoAccessHelper.FetchOptions](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-i/arkts-apis-photoaccesshelper-i#fetchoptions) | 是 | 图片和视频检索选项。 |
...
| options | [photoAccessHelper.FetchOptions](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-i/arkts-apis-photoaccesshelper-i#fetchoptions) | 是 | 连拍照片检索选项。 |
...
此接口在未申请相册管理模块权限'ohos.permission.WRITE\_IMAGEVIDEO'时，可以使用安全控件创建媒体资源，详情请参考[保存媒体库资源](/medialibrary-kit/photoaccesshelper-savebutton)。
...
| options | [photoAccessHelper.CreateOptions](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-i/arkts-apis-photoaccesshelper-i#createoptions) | 否 | 创建选项，例如{title: 'testPhoto'}。  文件名中不允许出现非法英文字符。  API18开始，非法字符包括： \ / : \* ? " < > |  API10-17，非法字符包括：. .. \ / : \* ? " ' ` < > | { } [ ] |
...
| options | [photoAccessHelper.FetchOptions](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-i/arkts-apis-photoaccesshelper-i#fetchoptions) | 否 | 检索选项，不填时默认根据相册类型检索。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [photoAccessHelper.FetchOptions](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-i/arkts-apis-photoaccesshelper-i#fetchoptions) | 是 | 检索选项。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| uri12+ | string | 是 | 否 | 媒体文件资源URI（如：file://media/Photo/1/IMG\_datetime\_0001/displayName.jpg），详情参见用户文件URI介绍中的[媒体文件URI](/core-file-kit/user-files/user-file-uri-intro#媒体文件uri)。  ****元服务API：**** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| photoAccessHelper.PhotoAsset | 返回非Sendable类型的[PhotoAsset](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-photoasset/arkts-apis-photoaccesshelper-photoasset)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| member | string | 是 | 成员参数名称，在get时，除了'uri'、'media\_type'、'subtype'和'display\_name'四个属性之外，其他的属性都需要在fetchColumns中填入需要get的[PhotoKeys](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-e/arkts-apis-photoaccesshelper-e#photokeys)，例如：get title属性fetchColumns: ['title']。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| [photoAccessHelper.MemberType](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-t/arkts-apis-photoaccesshelper-t#membertype) | 获取PhotoAsset成员参数的值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| member | string | 是 | 成员参数名称例如：[PhotoKeys](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-e/arkts-apis-photoaccesshelper-e#photokeys).TITLE。字符串长度的取值范围为[1, 255]。 |
| value | string | 是 | 设置成员参数名称，只能修改[PhotoKeys](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-e/arkts-apis-photoaccesshelper-e#photokeys).TITLE的值。title的参数规格为：  - 不应包含扩展名。  - 文件名字符串长度的取值范围为[1, 255]（资产文件名为标题+扩展名）。  - 不允许出现的非法英文字符，包括：. \ / : \* ? " ' ` < > | { } [ ] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [image.Size](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#size) | 否 | 缩略图尺寸。 |

****返回值：****

| 类型 | 说明 |
| --- | --- |
| Promise&lt;[image.PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)&gt; | Promise对象，返回缩略图的PixelMap。 |
...
详细错误码请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[文件管理错误码](/ref/core-file-api/core-file-arkts-errcode/errorcode-filemanagement/errorcode-filemanagement)。
...
| 类型 | 说明 |
| --- | --- |
| [photoAccessHelper.Album](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-album/arkts-apis-photoaccesshelper-album) | 返回非Sendable类型的Album。 |
