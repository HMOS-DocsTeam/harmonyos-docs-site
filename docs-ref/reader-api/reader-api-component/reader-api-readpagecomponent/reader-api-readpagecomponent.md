...
![](../../../images/0a26a7d9/note_3.0-zh-cn.png) 

- 支持根据阅读排版设置[ReaderSetting](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#readersetting)对书籍内容进行按页进行排版、渲染。
- 支持适配不同的设备屏幕尺寸（Phone、PC/2in1、Tablet，包括横竖屏适配），并在此基础上支持通过点击、滑动的方式进行阅读交互，支持仿真、横滑翻页方式（包括翻页过程动效）。
- 支持排版结果通知能力，打开书籍或者触发翻页后按页提供当前页的排版结果信息[PageDataInfo](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#pagedatainfo)。
...
| 参数名 | 类型 | 装饰器类型 | 说明 |
| --- | --- | --- | --- |
| controller | [ReaderComponentController](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#readercomponentcontroller) | \_ | ReadPageComponent控制器。 |
| readerCallback | AsyncCallback&lt;readerCore.[ReaderComponentController](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#readercomponentcontroller)&gt; | \_ | 回调函数。 |
