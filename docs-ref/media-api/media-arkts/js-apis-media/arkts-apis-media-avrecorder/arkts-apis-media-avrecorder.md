...
sidebar_position: 6
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder
kit: 媒体
...
音视频录制管理类，用于音视频媒体录制。在调用AVRecorder的方法前，需要先调用[createAVRecorder](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-f/arkts-apis-media-f#mediacreateavrecorder9)接口构建一个AVRecorder实例。

音视频录制demo可参考：[音频录制开发指导](/media-kit/media-kit-dev--arkts/media-recording-arkts/using-avrecorder-for-recording)、[视频录制开发指导](/media-kit/media-kit-dev--arkts/media-recording-arkts/video-recording)。

![](../../../../images/6db12a60/note_3.0-zh-cn.png) 

- 本模块首批接口从API version 6开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。
- 本Interface首批接口从API version 9开始支持。
- 相机视频录制功能需配合相机模块使用，相机模块接口的使用详情请参考[相机管理](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera/arkts-apis-camera)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| state9+ | [AVRecorderState](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-t/arkts-apis-media-t#avrecorderstate9) | 是 | 否 | 音视频录制的状态。  ****元服务API：**** 从API version 12 开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [AVRecorderConfig](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#avrecorderconfig9) | 是 | 配置音视频录制的相关参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;[audio.AudioCapturerChangeInfo](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#audiocapturerchangeinfo9)&gt; | 是 | 回调函数。当获取音频采集参数成功时，err为undefined，data为获取到的audio.AudioCapturerChangeInfo，否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[audio.AudioCapturerChangeInfo](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#audiocapturerchangeinfo9)&gt; | Promise对象，返回获取的当前音频采集参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;Array&lt;[EncoderInfo](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#encoderinfo11)&gt;&gt; | 是 | 回调函数。获取可用的编码器参数成功时，err为undefined，data为获取到的编码器参数，否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;Array&lt;[EncoderInfo](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#encoderinfo11)&gt;&gt; | Promise对象，返回获取的可用的编码器参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;[AVRecorderConfig](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#avrecorderconfig9)&gt; | 是 | 回调函数。获取实时配置的参数成功时，err为undefined，data为获取到的配置参数，否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[AVRecorderConfig](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#avrecorderconfig9)&gt; | Promise对象。返回实时配置参数。 |
...
| callback | [OnAVRecorderStateChangeHandler](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-t/arkts-apis-media-t#onavrecorderstatechangehandler12) | 是 | 回调函数，返回录制状态机切换事件。 |
...
取消订阅录制状态机[AVRecorderState](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-t/arkts-apis-media-t#avrecorderstate9)切换的事件。使用callback异步回调。
...
| --- | --- | --- | --- |
| type | string | 是 | 录制状态机切换事件回调类型，支持的事件：'stateChange'，用户操作和系统都会触发此事件。 |
| callback12+ | [OnAVRecorderStateChangeHandler](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-t/arkts-apis-media-t#onavrecorderstatechangehandler12) | 否 | 回调函数，返回录制状态机切换事件。如果指定参数则取消对应callback（callback对象不能是匿名函数），否则取消所有callback。  从API version 12开始支持此参数。 |
...
订阅AVRecorder的错误事件，该事件仅用于错误提示，不需要用户停止播控动作。如果此时[AVRecorderState](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-t/arkts-apis-media-t#avrecorderstate9)也切换至error状态，用户需要通过[reset](#reset9)或者[release](#release9)接口退出录制操作。使用callback异步回调。
...
| callback | [ErrorCallback](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/basic-services-others/js-apis-base/js-apis-base#errorcallback) | 是 | 回调函数，返回录制错误事件。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 录制错误事件回调类型'error'。  - 'error'：录制过程中发生错误，触发该事件。 |
| callback12+ | [ErrorCallback](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/basic-services-others/js-apis-base/js-apis-base#errorcallback) | 否 | 回调函数，返回录制错误事件。如果指定参数则取消对应callback（callback对象不能是匿名函数），否则取消所有callback。  从API version 12开始支持此参数。 |
...
| callback | Callback&lt;[audio.AudioCapturerChangeInfo](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#audiocapturerchangeinfo9)&gt; | 是 | 回调函数，返回变化后的录音配置全量信息。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 录音配置变化的回调类型，支持的事件：'audioCapturerChange'。 |
| callback12+ | Callback&lt;[audio.AudioCapturerChangeInfo](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-i/arkts-apis-audio-i#audiocapturerchangeinfo9)&gt; | 否 | 回调函数，返回变化后的录音配置全量信息。如果指定参数则取消对应callback（callback对象不能是匿名函数），否则取消所有callback。  从API version 12开始支持此参数。 |
...
订阅媒体资源回调事件，当[FileGenerationMode](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-e/arkts-apis-media-e#filegenerationmode12)枚举设置为系统创建媒体文件时，会在[stop](#stop9)操作结束后把[PhotoAsset](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-photoasset/arkts-apis-photoaccesshelper-photoasset)对象回调给应用。使用callback异步回调。
...
| callback | Callback&lt;[photoAccessHelper.PhotoAsset](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-photoasset/arkts-apis-photoaccesshelper-photoasset)&gt; | 是 | 回调函数，返回系统创建的资源文件对应的PhotoAsset对象。 |
...
| callback | Callback&lt;[photoAccessHelper.PhotoAsset](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-photoasset/arkts-apis-photoaccesshelper-photoasset)&gt; | 否 | 回调函数，返回系统创建的资源文件对应的PhotoAsset对象。如果指定参数则取消对应callback（callback对象不能是匿名函数），否则取消所有callback。 |
