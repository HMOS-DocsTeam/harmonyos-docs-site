...
如果接入了IAP Kit订单/订阅功能，建议在[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站[配置通知接收地址](/iap-kit-guide/iap-preparations/iap-basic-preparation/iap-set-necessary-parameters#配置订单订阅通知接收地址)，用于接收IAP服务器发送的关键事件通知。IAP关键事件通知版本只支持v3。

![](../../../images/b7ff159b/note_3.0-zh-cn.png) 

建议所有提供消耗型/非消耗型/自动续期订阅/非续期订阅商品的App均配置该通知接收地址，以便接收关键事件通知，为用户提供更好、更及时的服务。另外，通知接收地址必须基于HTTPS并且配置有商业域名机构颁发的证书。

## 功能介绍

用户[购买消耗型/非消耗型](/iap-kit-guide/iap-purchases/iap-purchase/iap-integrate-purchase)/[自动续期订阅](/iap-kit-guide/iap-purchases/iap-subscription/iap-subscription-functions)/[非续期订阅](/iap-kit-guide/iap-purchases/iap-nonrenewable/iap-integrate-nonrenewable)商品后，IAP服务器调用此接口向开发者服务器发送关键事件通知。

## 场景描述

如果在[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)申请支付服务时提前[配置通知接收地址](/iap-kit-guide/iap-preparations/iap-basic-preparation/iap-set-necessary-parameters#配置订单订阅通知接收地址)，并且通知版本设置为v3，用户购买消耗型/非消耗型/自动续期订阅/非续期订阅商品后，IAP服务器会在某些关键事件发生时调用此接口通知已配置的服务器。
...
![](../../../images/a00b66c3/note_3.0-zh-cn.png) 
...
![](../../../images/6223e158/zh-cn_image_0000002552801250.png)

1. IAP服务器发送订单/订阅关键事件通知。
2. 应用服务器收到通知请求后，从通知中获取购买Token。
3. 调用IAP服务器提供的[订单状态查询](/ref/iap-api/iap-rest/iap-query-order-status/iap-query-order-status)/[订阅状态查询](/ref/iap-api/iap-rest/iap-query-subscription-status/iap-query-subscription-status)接口，查询购买数据及其签名数据。
4. IAP服务器返回购买数据及其签名数据。
5. 校验订单/订阅状态提供商品服务。消耗型/非消耗型/非续期订阅商品请根据[订单状态查询](/ref/iap-api/iap-rest/iap-query-order-status/iap-query-order-status)接口获取消耗型/非消耗型/非续期订阅的订单最新状态，再根据订单状态调用[订单确认发货](/ref/iap-api/iap-rest/iap-confirm-purchase-for-order/iap-confirm-purchase-for-order)接口执行发货操作。自动续期订阅商品请根据[订阅状态查询](/ref/iap-api/iap-rest/iap-query-subscription-status/iap-query-subscription-status)接口响应经JWS解码之后SubGroupStatusPayload中的status字段决定是否发货。若status为1，则调用[订阅确认发货](/ref/iap-api/iap-rest/iap-confirm-purchase-for-sub/iap-confirm-purchase-for-sub)接口执行发货操作。
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| jwsNotification | 是 | String | 通知内容信息，对通知内容NotificationPayload签名后的字符串。具体请参见表[NotificationPayload](#notificationpayload)。  需要参见[对返回结果验签](/ref/iap-api/iap-rest/iap-verifying-signature/iap-verifying-signature)使用JWS签名方式对NotificationPayload消息体进行验签。 |
...
| EXPIRE | 订阅已过期。 |
| TEST | 测试服务端通知，仅开发者调用[测试服务端通知](/ref/iap-api/iap-rest/iap-server-notifications-test/iap-server-notifications-test)接口才会发送此类型通知。  此场景下无notificationSubtype。 |
...
| subGroupGenerationId | 否 | String | 订阅组的代ID。  - 用户切换订阅商品时，此ID不会改变。  - 订阅失效且超出[保留期](/iap-kit-guide/iap-purchases/iap-subscription/iap-subscription-functions#保留期)后，用户重新购买商品时，此ID会改变。 |
| subscriptionId | 否 | String | 商品的订阅ID。以下场景，此ID会发生改变：  - 用户切换订阅商品时。  - 订阅失效且超出[保留期](/iap-kit-guide/iap-purchases/iap-subscription/iap-subscription-functions#保留期)后，用户重新购买商品时。 |
