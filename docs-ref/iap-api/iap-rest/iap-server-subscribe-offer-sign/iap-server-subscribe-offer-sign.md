...
开发者在促销优惠场景下需要传入JWT格式的jwsRepresentation参数，该参数包含购买订单涉及的优惠及商品信息。JSON Web Token（JWT）是一个开放标准（RFC 7519），定义了一种安全传输信息的方法，具体请参见[jwt.io](https://jwt.io/)。开发者可以使用从[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)下载的私钥签名生成JWT。密钥生成和下载请参见[配置密钥](/iap-kit-guide/iap-preparations/iap-basic-preparation/iap-set-necessary-parameters#配置密钥)。创建JWT格式的签名购买参数需要以下几步：
...
| kid | 是 | String | 密钥ID，获取方式请参见[配置密钥](/iap-kit-guide/iap-preparations/iap-basic-preparation/iap-set-necessary-parameters#配置密钥)。如果有多个密钥，请使用对JWT进行签名的同一私钥的密钥ID。 |
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| iss | 是 | String | 标识密钥颁发者ID（Issuer ID），获取方式请参见[配置密钥](/iap-kit-guide/iap-preparations/iap-basic-preparation/iap-set-necessary-parameters#配置密钥)的说明。 |
...
| aid | 是 | String | APP ID，获取方式参见[配置应用身份信息](/iap-kit-guide/iap-preparations/iap-config-app-identity-info)。 |
| data | 是 | String | 优惠信息扩展信息，为[PurchaseReservedInfo](/ref/iap-api/iap-rest/iap-server-data-model/iap-server-data-model#purchasereservedinfo)结构的Json字符串。 |
