...
1. 在菜单栏单击****View > Tool Windows > Device File Browser****，打开Device File Browser。
   ...
   ![](../../images/91423a07/zh-cn_image_0000002530913150.png)
2. 从下拉列表中选择设备（设备需已连接）。
3. 选择设备后，显示文件/文件夹列表，可进行以下操作：
   ...
   2. 右键单击****Save As****将选定的文件或目录下载到PC上，右键单击****Upload****将PC上的文件上传到设备指定目录。
      ...
      ![](../../images/b68bdaab/note_3.0-zh-cn.png) 
...
Device File Browser可访问的文件夹有五种类型：[应用沙箱目录](/core-file-kit/app-file/app-sandbox-directory)、一般暂存区目录、日志目录、设备公共目录、媒体库目录。

![](../../images/4e185046/zh-cn_image_0000002530913138.png)
...
- ****普通文件视图****
  ...
  ![](../../images/cba41b96/zh-cn_image_0000002561833091.png)
...
- ****应用沙箱视图****
  ...
  从DevEco Studio 6.1.0 Beta1版本开始，点击![](../../images/e114d856/zh-cn_image_0000002561833111.png)即可过滤出当前工程对应的沙箱目录。

  ![](../../images/b68492ec/zh-cn_image_0000002561833087.png)

  API 15以下的版本，当需要以沙箱视图查看应用的文件结构时，需在module.json5文件内配置ohos.permission.INTERNET开启网络权限，卸载并重新安装应用。

  ![](../../images/d3bab5db/zh-cn_image_0000002561833115.png)
...
![](../../images/c5dee3e1/zh-cn_image_0000002530913140.png)

### 日志目录

应用运行时的日志通过HiLog工具实时输出，此工具的输出缓存区一般为256K，超出大小的历史日志将会以压缩包的形式保存在/data/log/hilog/路径下。

当需要查看历史日志时，需要将此目录下的压缩包文件和数据字典压缩包文件hilog\_dict.XXX.zip下载到本地计算机，然后使用[hilogtool工具](/system-debug-optimize/debugging-commands/hilog-tool)解析出对应的日志原文件。

![](../../images/415e113d/zh-cn_image_0000002561833097.png)

### 公共目录

用户的桌面、文档、下载等公共目录位于/storage/media/100/local/files/Docs路径下，支持删除、上传、下载操作。

![](../../images/db02bd42/zh-cn_image_0000002530753196.png)
...
![](../../images/8c402921/note_3.0-zh-cn.png) 

Wearable设备不支持媒体库目录。

![](../../images/d0495e2f/zh-cn_image_0000002530753182.png)
...
1. 执行命令，查询此路径下的崩溃日志文件列表。
   - 设备版本低于5.1.0.54时，执行以下命令。
     ...
     ![](../../images/6e60345f/zh-cn_image_0000002530913152.png)
   - 设备版本为5.1.0.54及以上时，崩溃日志文件名时间戳新增了毫秒级信息，执行以下命令。
     ...
     ![](../../images/d67ded35/zh-cn_image_0000002530913178.png)
2. 执行命令，查看指定的崩溃日志文件的内容。
   ...
   ![](../../images/39b9bf57/zh-cn_image_0000002530913164.png)
3. 执行命令，将文件保存到本地计算机指定的路径下。
   ...
   ![](../../images/aa0b884a/zh-cn_image_0000002530913142.png)
...
1. 在图库中查看文件名及后缀，例如a.mp4。
   ...
   ![](../../images/bce9de3d/zh-cn_image_0000002561753101.png)
2. 查询文件路径，记录为\{FilePath\}。
   ...
   - 如果查询的结果中包含uri字段，则返回值第三行对应的文件路径不允许直接下载。
     ...
     ![](../../images/856ba4e3/zh-cn_image_0000002530913182.png)
     ...
     ![](../../images/4492c921/zh-cn_image_0000002561833071.png)
   - 如果查询结果不包含uri字段，则返回值第二行即为文件路径\{FilePath\}。

     ![](../../images/f772c6b4/zh-cn_image_0000002561753103.png)
...
可以通过命令行的方式访问debug应用的[沙箱目录](/core-file-kit/app-file/app-sandbox-directory#应用沙箱路径和真实物理路径的对应关系)。

- 从API 15开始，支持通过hdc工具访问debug应用的沙箱目录，推荐开发者使用此种方式。更多关于命令行工具hdc的说明请参见[hdc工具使用指导](/system-debug-optimize/debugging-commands/hdc)。
  1. 在设备侧启动应用。
...
![](../../images/7400ffcf/zh-cn_image_0000002530913154.png)
