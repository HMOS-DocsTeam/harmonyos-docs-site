...
![](../../../images/0e92c704/note_3.0-zh-cn.png) 
...
1. 在工具栏中，选择调试的设备，并单击****Run****![](../../../images/4614febe/zh-cn_image_0000002561833247.png)或****Debug**** ![](../../../images/bb5c8333/zh-cn_image_0000002530753334.png)启动工程。
2. 在修改完代码后，点击****Apply Changes****![](../../../images/c60f3cf9/zh-cn_image_0000002530913316.png)推送增量包安装至设备。

   点击Apply Changes按钮后，DevEco Studio启动构建的增量构建任务，构建出增量包hqf。增量包构建完成后，将推送安装至设备。

   ![](../../../images/d8570cd0/zh-cn_image_0000002561753255.png)

   ![](../../../images/1871125e/note_3.0-zh-cn.png) 
...
1. 在工具栏中，选择调试的设备，并单击****Run****![](../../../images/f02bab90/zh-cn_image_0000002561833249.png)或****Debug**** ![](../../../images/b33d78a5/zh-cn_image_0000002561833255.png)启动工程。
2. 在工程的资源resources文件目录下的resfile或rawfile目录下，新增或者修改资源文件。

   ![](../../../images/d46d142c/zh-cn_image_0000002530913330.png)

   ![](../../../images/c4f41c7e/note_3.0-zh-cn.png) 

   当前对rawfile/resfile资源的增量调试，仅支持代码中直接调用的资源文件。
3. 点击****Apply Changes****![](../../../images/e9c73b9b/zh-cn_image_0000002530753324.png)推送增量包安装至设备。
...
1. 检查待运行模块和依赖模块下是否存在build/config/buildConfig.json文件，如果不存在，先通过DevEco Studio全量运行工程，生成该文件。
   ...
   ![](../../../images/c2806646/note_3.0-zh-cn.png) 
...
2. 根据运行所需的模块，及模块的product、target，编写命令行执行HAP/HSP编译任务，如entry模块依赖HSP模块library：
   ...
   关于命令行的使用指导请参考[hvigorw](/ide-hvigor-commandline)。
3. 执行hdc命令安装HAP、HSP，关于hdc工具的使用指导请参考[hdc](/system-debug-optimize/debugging-commands/hdc)。
...
![](../../../images/a2ac15cd/zh-cn_image_0000002530913312.png "点击放大")
...
1. 全量编译应用并安装到设备。
   ...
   ```
2. 开发者通过独立的构建流程，识别出希望构建增量hqf包的so，根据ABI编译环境（可查看build-profile.json5的[abiFilters](/ide-hvigor-configuration/ide-hvigor-cpp#section0721057575)字段），汇总到某一目录下，例如汇总在change\_test目录下，编译环境是arm64-v8a，示例如下。

   ![](../../../images/613da7a7/zh-cn_image_0000002530753326.png)
...
5. 准备patch.json文件，示例如下。
   ...
   ```
6. 在hqf[打包工具](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool#hqf打包指令)目录下（默认在DevEco Studio安装目录\sdk\default\openharmony\toolchains\lib下），执行命令打包，示例如下。
