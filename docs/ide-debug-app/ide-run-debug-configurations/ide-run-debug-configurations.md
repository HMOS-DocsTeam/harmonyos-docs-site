...
2. 如果没有配置debug字段，则根据build-profile.json5的debuggable字段判断应用是否支持调试。
   - true：应用支持调试。当[编译模式](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section192461528194916)不是release时，debuggable的缺省值是true，即支持调试。
...
![](../../images/bcd3a9d0/zh-cn_image_0000002530913390.png)
...
![](../../images/babb4a16/zh-cn_image_0000002530913378.png)

### 配置自定义调试参数

如果未进行自定义，将按默认配置安装和运行应用。如果开发者需要对应用安装、运行等流程增加参数配置，可在“Installation Options”和“Launch Options”下进行配置。

![](../../images/0120f7e1/zh-cn_image_0000002530913374.png)

- Installation Options
  - DebugLine Support：勾选Enable DebugLine表示在build产物中系统组件增加debugline属性，用于开启[ArkUI Inspector源码跳转](/ide-debug-app/ide-arkui-inspector#section1226015494335)功能。
  - Install Flags：输入bm install命令相关的选项，请参见[bm install 参数](/system-debug-optimize/debugging-commands/bm-tool#安装命令install)。如可以设置“-w 360”，表示将超时等待时间设置为360秒。
- Launch Options
  - Launch：指定在安装应用后启动的Ability。

    ![](../../images/4ac9887a/zh-cn_image_0000002530753382.png)
    ...
    - Default Ability：默认的EntryAbility。
      - Stage模型：module.json5文件中配置了“skills”属性的第一个ability；若无配置“skills”属性的ability，则取“mainElement”指定的ability（该ability需存在于“abilities”数组内）；若“mainElement”未指定，则取“abilities”数组内的第一个ability。
        ...
        ![](../../images/d4d80e9e/zh-cn_image_0000002530913382.png "点击放大")
    ...
    - Specified Ability：工程中的UIAbility或ExtensionAbility。
      ...
      ![](../../images/5f761968/zh-cn_image_0000002530913376.png)
  - Launch Flags：输入aa start命令相关的选项，请参见[aa start 参数](/system-debug-optimize/debugging-commands/aa-tool)。

### 配置环境变量

如果开发者需要配置和管理应用开发环境，以及控制应用程序的行为，可在****Environment Variables****下配置环境变量。

![](../../images/ac95f6a0/zh-cn_image_0000002561833303.png)

点击![](../../images/35fa0f79/zh-cn_image_0000002561833309.png)按钮，新增一行配置项。当前支持以下配置项：
...
![](../../images/58071b42/note_3.0-zh-cn.png) 

当配置Environment Variables后，“Keep Application Data”覆盖安装不生效。

环境变量配置完成后，需确保环境变量已勾选，勾选后点击****Apply****才可生效。

![](../../images/2cb40988/zh-cn_image_0000002561753325.png)

## 自动映射WebView调试链接

当应用中含有需要调试的WebView组件页面时，可以通过浏览器的DevTools工具进行页面调试，具体可参考[使用DevTools工具调试前端页面](/arkweb/web-debugging/web-debugging-with-devtools)。调试WebView组件需要执行转发端口等繁琐的命令行操作，因此可以在DevEco Studio中勾选****Auto WebView Debug****，该操作会在应用启动后两分钟内自动监听可调试的WebView进程并完成端口转发。
...
![](../../images/f016f771/zh-cn_image_0000002561753313.png)

开启后，当检测到设备上有可调试的WebView组件进程时，会在Run面板中打印转发成功的端口，通过浏览器的DevTools工具连接该端口即可进行WebView调试。

![](../../images/063683bf/zh-cn_image_0000002561753337.png)
...
![](../../images/270c97c7/zh-cn_image_0000002561833295.png)
...
![](../../images/7dc66ea7/zh-cn_image_0000002561753333.png)

在Before launch窗格中，您可以点击![](../../images/d10600a5/zh-cn_image_0000002530913388.png)添加应用启动前的任务。

![](../../images/5cfc41d7/zh-cn_image_0000002561753327.png)

也可以点击![](../../images/b1a3de04/zh-cn_image_0000002561753331.png)移除任务。

![](../../images/43656a39/zh-cn_image_0000002530753378.png)
...
1. 在设备选择框中，点击****Select Multiple Devices****，弹出多设备选择框。
   ...
   ![](../../images/353cec79/zh-cn_image_0000002530753380.png)
2. 选择需要推包运行的设备。

   ![](../../images/545f0df4/zh-cn_image_0000002530753394.png)
3. 设备栏会出现Multiple Devices(N)，表示选中N个设备，点击运行按钮即可同时在选中设备上运行应用。

   ![](../../images/cd9a129b/zh-cn_image_0000002561833307.png)
