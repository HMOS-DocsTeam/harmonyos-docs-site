...
![](../images/aacbc8c7/note_3.0-zh-cn.png) 
...
4. 执行如下命令，检查JDK安装结果。
   ...
   ![](../images/8ff41695/zh-cn_image_0000002561752785.png)

### 获取命令行工具

1. [命令行工具获取](/ide-commandline-get#section21298572437)。其他系统(Windows/macOS)请根据实际情况下载对应版本。
2. 执行如下命令，解压命令行工具commandline-tools-linux-xxx.zip，工具名称请根据实际情况进行修改。
   ...
   ![](../images/f9ca7769/zh-cn_image_0000002530752854.png)
...
1. 配置Node.js环境变量。
   ...
   ![](../images/637669d4/note_3.0-zh-cn.png) 
...
2. 执行如下命令，查询Node.js版本信息，确认配置成功。
   ...
   ![](../images/7fe2f7d4/zh-cn_image_0000002561832775.png)

![](../images/c97b0422/note_3.0-zh-cn.png) 
...
在linux系统的构建场景下，使用[纹理压缩](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section2095319147103)功能需要安装libGL1库。例如Ubuntu/Debian上安装libgl1-mesa-dev，CentOS/RHEL上安装mesa-libGL-devel。
...
2. 定义变量 ****PROJECT\_PATH****，表示工程目录路径，示例如下。
   ...
   ![](../images/893bf115/caution_3.0-zh-cn.png) 
...
- 如果在非daemon模式下，需要修改node内存配置，可在command-line-tools的hvigor/bin/hvigorw文件中取消第15行的注释，并配置对应的数值。如将node内存配置为10240，示例如下：
  ...
  ```
- 如果是在daemon模式下，请参考[设置守护进程内存](/ide-build-optimized/ide-hvigor-daemon#section327617383145)。

编译构建常见的任务和扩展参数如下，更多关于Hvigor命令行参数详见：[常用命令](/ide-hvigor-commandline#section16300629103)。

****表1**** HarmonyOS应用构建常用扩展参数

| 选项 | 说明 |
| --- | --- |
| -p buildMode={debug | release} | 采用debug/release模式进行编译构建。  缺省时：构建Hap/Hsp/Har时为debug模式，构建App时为release模式。  关于构建模式的详细说明，请参考[指定构建模式](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section192461528194916)。针对HAR构建，请参考[构建HAR](/ide-hvigor-configuration/ide-hvigor-build-har)。 |
...
生成证书请求文件后，在AppGallery Connect中申请、下载调试数字证书和Profile文件，具体请参考[申请调试证书](/ide-signing#section081822416419)和[申请Profile文件和添加权限信息](/ide-signing#section89479413571)。
...
2. 在签名工具目录下，使用如下命令进行签名。详细的签名工具指导请参考[Hap包签名工具](https://gitcode.com/openharmony/developtools_hapsigner)。
   ...
   ![](../images/f7d8f7b2/note_3.0-zh-cn.png) 

   如果要对APP进行签名，只需将****inFile****和****outFile****参数修改为APP包即可。

### 运行应用

通过[hdc工具](/system-debug-optimize/debugging-commands/hdc)将HAP推送到真机设备上进行安装，需要注意的是，推送的HAP必须是携带签名信息的，否则会导致HAP安装失败。
...
![](../images/16ccee18/note_3.0-zh-cn.png) 
...
1. 请在可访问网络的电脑上创建一个空文件夹，在文件夹中创建一个package.json文件，在文件中填写如下内容：
   ...
   ```
2. 先配置[环境变量](/ide-appendix/ide-environment-config#zh-cn_topic_0000001056725590_li358362302311)，再打开[命令行工具](/ide-commandline-get)，在文件夹下执行 npm install 命令，会生成node\_modules文件夹。

   ![](../images/5ec61e26/zh-cn_image_0000002530752848.png)
...
1. 请在可访问网络的电脑上创建一个空文件夹，在文件夹中创建一个package.json文件，配置npm依赖，示例如下：
   ...
   ```
2. 打开[命令行工具](/ide-commandline-get)，在文件夹下执行 npm install 命令，会生成node\_modules文件夹。

   ![](../images/365ccdab/zh-cn_image_0000002530752850.png)
3. 将node\_modules文件夹拷贝到无网络电脑的工程根目录下。

### 安装ohpm依赖插件

请参考[安装三方库](/ide-no-network#section208435578175)。

### 安装libGL1库

在linux系统的构建场景下，使用[纹理压缩](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section2095319147103)功能需要安装libGL1库。
