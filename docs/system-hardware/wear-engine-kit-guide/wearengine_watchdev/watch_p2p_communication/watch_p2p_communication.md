...
1. 参见[已连接对端设备查询](/system-hardware/wear-engine-kit-guide/wearengine_watchdev/watch_query_connected_devices)章节，从已连接设备列表中选定需要通信的对端设备。
2. 调用[getP2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#wearenginegetp2pclient)方法，获取[P2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#p2pclient)对象。
3. 调用[isRemoteAppInstalled](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#isremoteappinstalled)方法，查看对端设备是否安装指定应用。
...
1. 为了使用工具类构造消息体，请先导入所需模块。
   ...
   ```
2. 参见[已连接对端设备查询](/system-hardware/wear-engine-kit-guide/wearengine_watchdev/watch_query_connected_devices)章节，从已连接设备列表中选定需要通信的对端设备。
3. 构造对端应用参数[P2pAppParam](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#p2pappparam)。
...
5. 调用[getP2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#wearenginegetp2pclient)方法，获取[P2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#p2pclient)对象。
6. 调用[sendMessage](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#sendmessage)方法，从穿戴侧应用发送简短消息到对端应用。对端应用已注册监听消息接收后，即可收到穿戴侧应用发送的消息。
...
5. 调用[getP2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#wearenginegetp2pclient)方法，获取[P2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#p2pclient)对象。
6. 调用[transferFile](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#transferfile)方法，从穿戴侧应用发送文件到对端应用。
...
2. 调用[getP2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#wearenginegetp2pclient)方法，获取[P2pClient](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#p2pclient)对象。
3. 构造对端应用参数[P2pAppParam](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#p2pappparam)。
4. 构造接收到对端传来消息后的回调函数[Callback](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/basic-services-others/js-apis-base/js-apis-base#callback)。
5. 调用[registerMessageReceiver](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#registermessagereceiver)方法，订阅监听消息接收事件。
   ...
   ```
6. 调用[unregisterMessageReceiver](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#unregistermessagereceiver)方法，穿戴侧应用取消接收对端应用发过来的消息，需要传入订阅监听时的同一个回调函数对象。
...
4. 构造接收到设备侧传来文件后的回调函数[Callback](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/basic-services-others/js-apis-base/js-apis-base#callback)。
5. 调用[registerFileReceiver](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#registerfilereceiver)方法，订阅监听文件接收事件。
   ...
   ```
6. 调用[unregisterFileReceiver](/ref/system-hardware-api/wear-engine-api/wearengine_arkts_api/wearengine_api/wearengine_api#unregisterfilereceiver)方法，穿戴侧应用取消接收对端应用发过来的文件，需要传入订阅监听时的同一个回调函数对象。
...
1. 创建名为HiWearMainAbility.ets的文件，需继承UIAbility，重写onWindowStageCreate函数，配置要跳转的界面。
   ...
   ```
2. 在module.json5中的abilities中配置HiWearMainAbility。更多配置详情参考[abilities标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#abilities标签)。
