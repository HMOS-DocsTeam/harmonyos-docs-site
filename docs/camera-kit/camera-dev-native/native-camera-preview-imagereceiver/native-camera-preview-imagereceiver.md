...
2. 在CMake脚本中链接相关动态库。
   ...
   ```
3. 初始化图片接收器[ImageReceiver](/image-kit/image-native/image-receiving-c/image-receiver-c)实例，获取SurfaceId。
   ...
   ```
4. 通过上一步获取到的SurfaceId创建预览流（在创建预览流之前需要将SurfaceId类型转成char \*），参考[预览(C/C++)](/camera-kit/camera-dev-native/native-camera-preview)步骤4。
5. 创建会话，使能会话，参考[会话管理(C/C++)](/camera-kit/camera-dev-native-mandatory/native-camera-session-management)。
