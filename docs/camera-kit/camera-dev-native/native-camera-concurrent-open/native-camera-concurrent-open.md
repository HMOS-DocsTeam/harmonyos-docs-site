...
![](../../../images/f005d23d/note_3.0-zh-cn.png) 
...
详细的API说明请参考[Camera](/ref/camera-api/camera-c/camera-module/capi-oh-camera/capi-oh-camera)。
...
2. 在CMake脚本中链接相关动态库。
   ...
   target_link_libraries(entry PUBLIC
      ...
      librawfile.z.so)
   ```
3. 通过[OH\_CameraManager\_GetCameraDevice](/ref/camera-api/camera-c/camera-headerfile/capi-camera-manager-h/capi-camera-manager-h#oh_cameramanager_getcameradevice)方法获取对应的前置相机和后置相机。如果接口返回值为CAMERA\_SERVICE\_FATAL\_ERROR，基于示例中的配置信息，表示当前设备不支持指定位置（前置/后置）的默认类型相机，无法实现多摄同开功能。
   ...
   ```
4. 获取对应的并发能力集。通过[OH\_CameraManager\_GetCameraConcurrentInfos](/ref/camera-api/camera-c/camera-headerfile/capi-camera-manager-h/capi-camera-manager-h#oh_cameramanager_getcameraconcurrentinfos)方法，获取[Camera\_ConcurrentInfo](/ref/camera-api/camera-c/camera-struct/capi-oh-camera-camera-concurrentinfo/capi-oh-camera-camera-concurrentinfo)相机并发能力集对象数组，数组包含了前置和后置同时开启的相机在指定多摄同开模式下支持的相机模式和相机输出能力，****在多摄同开场景下设置的模式和输出能力必须在并发能力集的范围之内****。
...
5. 对应并发能力集创建PreviewOutput对象。
   ...
   ```
6. 打开相机。通过[OH\_CameraInput\_OpenConcurrentCameras](/ref/camera-api/camera-c/camera-headerfile/capi-camera-input-h/capi-camera-input-h#oh_camerainput_openconcurrentcameras)以并发状态打开指定相机，在使用该接口前，请先查询接口是否支持并发能力集，并优先调用[OH\_CameraManager\_GetCameraConcurrentInfos](/ref/camera-api/camera-c/camera-headerfile/capi-camera-manager-h/capi-camera-manager-h#oh_cameramanager_getcameraconcurrentinfos)方法，获取多摄同开状态下的相机并发能力集。请勿在未查询并发能力集的情况下使用[OH\_CameraInput\_OpenConcurrentCameras](/ref/camera-api/camera-c/camera-headerfile/capi-camera-input-h/capi-camera-input-h#oh_camerainput_openconcurrentcameras)，否则会导致打开相机失败。
   ...
   ```
7. 会话流程。配置对应的相机输入流[Camera\_Input](/ref/camera-api/camera-c/camera-struct/capi-oh-camera-camera-input/capi-oh-camera-camera-input)和需要的相机输出流，此处以预览输出流[Camera\_PreviewOutput](/ref/camera-api/camera-c/camera-struct/capi-oh-camera-camera-previewoutput/capi-oh-camera-camera-previewoutput)为例 。
