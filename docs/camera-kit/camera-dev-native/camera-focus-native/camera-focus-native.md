...
详细的API说明请参考[Camera API参考](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h)。
...
2. 在CMake脚本中链接相关动态库。
   ...
   ```
3. 调用[OH\_CaptureSession\_SetFocusMode](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_setfocusmode)设置对焦模式。

   ![](../../../images/497c0d3c/note_3.0-zh-cn.png) 

   - 需要先调用[OH\_CaptureSession\_IsFocusModeSupported](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_isfocusmodesupported)检查设备是否支持指定的焦距模式。
   ...
   ```
4. 如果通过[OH\_CaptureSession\_SetFocusMode](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_setfocusmode)设置对焦模式为自动对焦模式，支持调用[OH\_CaptureSession\_SetFocusPoint](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_setfocuspoint)设置对焦点，根据对焦点执行一次自动对焦。

   ![](../../../images/aca7fcf2/note_3.0-zh-cn.png) 
...
通过注册[OnFocusStateChange](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_onfocusstatechange)的回调函数获取监听结果，仅当自动对焦模式时，且相机对焦状态发生改变时触发该事件。
