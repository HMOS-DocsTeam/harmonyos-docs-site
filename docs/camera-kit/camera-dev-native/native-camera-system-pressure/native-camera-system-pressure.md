...
可以通过注册[OH\_CaptureSession\_OnSystemPressureLevelChange](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_onsystempressurelevelchange)的回调函数获取系统压力的监听结果。

当系统压力发生变化时，callback返回Camera\_SystemPressureLevel参数。

参数的具体内容可参考相机管理器回调接口实例[Camera\_SystemPressureLevel](/ref/camera-api/camera-c/camera-headerfile/capi-camera-h/capi-camera-h#camera_systempressurelevel)。
