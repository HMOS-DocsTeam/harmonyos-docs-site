...
![](../../../images/65f36232/zh-cn_image_0000002583438629.png)

与普通的[预览](/camera-kit/camera-dev-native/native-camera-preview)流程相比，动态调整预览帧率的注意点如图上标识：

1. 调用[OH\_CameraManager\_CreateCaptureSession](/ref/camera-api/camera-c/camera-headerfile/capi-camera-manager-h/capi-camera-manager-h#oh_cameramanager_createcapturesession)创建会话（Session）时，需要指定模式为NORMAL\_PHOTO或NORMAL\_VIDEO。
...
如何配置会话（Session）、释放资源，请参考[会话管理](/camera-kit/camera-dev-native-mandatory/native-camera-session-management) > [预览](/camera-kit/camera-dev-native/native-camera-preview)。
...
相机使用预览等功能前，均需创建相机会话，调用[OH\_CameraManager\_CreateCaptureSession](/ref/camera-api/camera-c/camera-headerfile/capi-camera-manager-h/capi-camera-manager-h#oh_cameramanager_createcapturesession)创建一个会话。

创建会话时调用[OH\_CaptureSession\_SetSessionMode](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_setsessionmode)指定[Camera\_SceneMode](/ref/camera-api/camera-c/camera-headerfile/capi-camera-h/capi-camera-h#camera_scenemode)为NORMAL\_PHOTO或NORMAL\_VIDEO，创建出的Session处于拍照或录像模式。
...
1. 调用[OH\_PreviewOutput\_GetSupportedFrameRates](/ref/camera-api/camera-c/camera-headerfile/capi-preview-output-h/capi-preview-output-h#oh_previewoutput_getsupportedframerates)，查询当前previewOutput支持的帧率范围。

   ![](../../../images/cc1cd778/note_3.0-zh-cn.png) 

   ****调用时机****：

   需要在Session调用[OH\_CaptureSession\_CommitConfig](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_commitconfig)完成配流之后调用。
   ...
   ****OH\_PreviewOutput\_GetSupportedFrameRates调用限制：****
   ...
   ```
2. 根据实际开发需求，调用[OH\_PreviewOutput\_SetFrameRate](/ref/camera-api/camera-c/camera-headerfile/capi-preview-output-h/capi-preview-output-h#oh_previewoutput_setframerate)接口对帧率进行动态调整。

   ![](../../../images/0d5c9032/note_3.0-zh-cn.png) 

   - 需要在Session调用[OH\_CaptureSession\_CommitConfig](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_commitconfig)完成配流之后调用。
   - 可在Session调用[OH\_PreviewOutput\_Start](/ref/camera-api/camera-c/camera-headerfile/capi-preview-output-h/capi-preview-output-h#oh_previewoutput_start)启动预览前后任意时刻调用。
   ...
   ```
3. （可选）通过[OH\_PreviewOutput\_GetActiveFrameRate](/ref/camera-api/camera-c/camera-headerfile/capi-preview-output-h/capi-preview-output-h#oh_previewoutput_getactiveframerate)接口查询已设置过并生效的帧率。

   仅通过[OH\_PreviewOutput\_SetFrameRate](/ref/camera-api/camera-c/camera-headerfile/capi-preview-output-h/capi-preview-output-h#oh_previewoutput_setframerate)接口显式设置过帧率才可查询当前生效帧率信息。
