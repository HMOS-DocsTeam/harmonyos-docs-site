...
2. 在CMake脚本中链接相关动态库。
   ...
   ```
3. 相机初始化及拍照触发参考[拍照(C/C++)](/camera-kit/camera-dev-native/native-camera-shooting)。
4. 注册分段式（PhotoAssetAvailable）拍照回调，对比单段式拍照，仅注册的拍照回调接口不同。

   ![](../../../images/7400a167/caution_3.0-zh-cn.png) 
   ...
   ****分段式拍照开发流程（PhotoAssetAvailableCallback）****：
   ...
   - 通过mediaAsset直接落盘图片或者通过mediaAsset配置策略模式请求图像资源，业务处理后通过buffer保存图片，或显示图片(参考[拍照(C/C++)](/camera-kit/camera-dev-native/native-camera-shooting)步骤5)。
   - 使用完后解注册分段式拍照回调函数。
