...
- 添加闪光灯、调整焦距等配置。具体支持的配置及接口说明请参考[OH\_Camera](/ref/camera-api/camera-c/camera-module/capi-oh-camera/capi-oh-camera)。
...
2. 在CMake脚本中链接相关动态库。
   ...
   ```
3. 调用[OH\_CameraManager\_CreateCaptureSession()](/ref/camera-api/camera-c/camera-headerfile/capi-camera-manager-h/capi-camera-manager-h#oh_cameramanager_createcapturesession)方法创建一个会话。
   ...
   ```
4. 调用[OH\_CaptureSession\_SetSessionMode()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_setsessionmode)方法配置会话模式。
   ...
   ```
5. 调用[OH\_CaptureSession\_BeginConfig()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_beginconfig)方法配置会话。
   ...
   ```
6. 使能。向会话中添加相机的输入流和输出流，调用[OH\_CaptureSession\_AddInput()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_addinput)添加相机的输入流；调用[OH\_CaptureSession\_AddPreviewOutput()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_addpreviewoutput)和[OH\_CaptureSession\_AddPhotoOutput()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_addphotooutput)添加相机的输出流。以下示例代码以添加预览流previewOutput和拍照流photoOutput为例，即当前模式支持拍照和预览。

   调用[OH\_CaptureSession\_CommitConfig()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_commitconfig)和[OH\_CaptureSession\_Start()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_start)方法提交相关配置，并启动会话。
   ...
   ```
7. 会话控制。调用[OH\_CaptureSession\_Stop()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_stop)方法可以停止当前会话。调用[OH\_CaptureSession\_RemovePhotoOutput()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_removephotooutput)和[OH\_CaptureSession\_AddVideoOutput()](/ref/camera-api/camera-c/camera-headerfile/capi-capture-session-h/capi-capture-session-h#oh_capturesession_addvideooutput)方法可以完成会话切换控制。以下示例代码以移除拍照流photoOutput，添加视频流videoOutput为例，完成了拍照到录像的切换。
