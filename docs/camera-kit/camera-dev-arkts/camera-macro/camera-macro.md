...
1. 导入camera接口，接口中提供了相机相关的属性和方法，导入方法如下。
   ...
   ```
2. 通过[isMacroSupported](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-macroquery/arkts-apis-camera-macroquery#ismacrosupported19)接口，查询当前设备是否支持微距能力。

   ```
   let isSupported: boolean = photoSession.isMacroSupported();
   ```
3. 通过[enableMacro](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-macro/arkts-apis-camera-macro#enablemacro19)接口，开启或关闭微距能力。
...
注册macroStatusChanged事件监听微距能力变化，事件监听可参考[on('macroStatusChanged')](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession#onmacrostatuschanged20)。
