...
开发者在开发相机应用时，在获取到[CameraDevice](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#cameradevice)之后，如果遵循通用流程开发，步骤较为繁琐。需要先调用[CameraManager](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager)的[getSupportedOutputCapability](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedoutputcapability11)来查询当前相机在指定模式下所支持的各类输出的配置信息，拿到[CameraOutputCapability](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#cameraoutputcapability)之后，应用开发者还需要对里面的各类数据进行解析，筛选，找到自己需要的配置数据[Profile](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#profile)以及[VideoProfile](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#videoprofile)。最后使用对应的Profile以及VideoProfile创建对应的[PreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput)、[PhotoOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput)以及[VideoOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-videooutput/arkts-apis-camera-videooutput)。

为了解决上述问题，优化应用开发流程，系统针对拍照（[PhotoSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession)）、录像（[VideoSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-videosession/arkts-apis-camera-videosession)）两类场景，提供了[preconfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession#preconfig12)接口帮助开发者快速完成相机参数配置。推荐仅需要自定义拍照界面的无需开发专业相机应用的开发者，使用相机预配置功能快速开发应用。

在开发相机应用时，需要先[申请相关权限](/camera-kit/camera-preparation)。以拍照（PhotoSession）为例，相机预配置（Preconfig）开发流程与[通用流程](/camera-kit/camera-dev-arkts/camera-shooting-case)开发，存在以下差异：

![](../../../images/8ac0a96c/zh-cn_image_0000002552958582.png)

****其他相关能力：****

- [CameraPicker](/camera-kit/camera-dev-arkts/camera-picker)：无需开发相机功能，拉起系统相机获取照片或视频。
- [调用全量相机接口开发](/camera-kit/camera-overview)：可开发自定义界面、分辨率、图像效果的专业相机应用。

## 规格说明

系统提供了4种预配置类型（[PreconfigType](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#preconfigtype12)），分别为PRECONFIG\_720P、PRECONFIG\_1080P、PRECONFIG\_4K、PRECONFIG\_HIGH\_QUALITY。以及3种画幅比例规格（[PreconfigRatio](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#preconfigratio12)），1:1画幅（PRECONFIG\_RATIO\_1\_1）、4:3画幅（PRECONFIG\_RATIO\_4\_3）、16:9画幅（PRECONFIG\_RATIO\_16\_9）。

![](../../../images/fdc32c3e/caution_3.0-zh-cn.png) 

由于不同的设备所支持的能力不同。使用相机预配置（Preconfig）功能时，需要先调用[canPreconfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession#canpreconfig12)检查对应的PreconfigType和PreconfigRatio的组合在当前设备上是否支持。
...
详细的API说明请参考[Camera API参考](/ref/camera-api/camera-arkts/js-apis-camera/js-apis-camera)。
...
2. 创建输出流Output。
   ...
   创建预览输出流时，涉及参数surfaceId。XComponent组件为预览流提供Surface（获取surfaceId请参考[getXcomponentSurfaceId](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent#getxcomponentsurfaceid9)方法），而XComponent的能力由UI提供，相关介绍可参考[XComponent组件参考](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)。

   创建cameraManager对象的方法可参考[getCameraManager](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-f/arkts-apis-camera-f#cameragetcameramanager)。
   ...
   ```
3. 调用[CameraManager](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager)中的[createCameraInput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createcamerainput)方法，创建输入流Input。
   ...
   ```
4. 调用[createSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createsession11)创建会话（Session）。

   ![](../../../images/15495d95/note_3.0-zh-cn.png) 

   SceneMode需要指定为NORMAL\_PHOTO或NORMAL\_VIDEO，对应拍照场景[PhotoSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession)和录像场景[VideoSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-videosession/arkts-apis-camera-videosession)。
   ...
   ```
5. 调用[canPreconfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession#canpreconfig12)检查对应的PreconfigType和PreconfigRatio的组合在当前设备上是否支持。确认支持后，调用[preconfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photosession/arkts-apis-camera-photosession#preconfig12)启用Preconfig配置。
   ...
   ![](../../../images/c484a633/note_3.0-zh-cn.png) 

   若当前模式为普通录像模式，且[PreconfigType](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#preconfigtype12)使用PRECONFIG\_HIGH\_QUALITY类型时，为避免录制失败，请务必将[AVRecorderProfile](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#avrecorderprofile9)配置文件中videoCodec参数必须配置为[VIDEO\_HEVC](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-e/arkts-apis-media-e#codecmimetype8)，isHdr参数配置为true。
6. Session添加Input和Output。

   ![](../../../images/43f662d5/note_3.0-zh-cn.png) 
