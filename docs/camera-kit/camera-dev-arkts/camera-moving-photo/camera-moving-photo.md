...
- 应用开发动态照片前，请参考[申请相机开发的权限](/camera-kit/camera-preparation)、[相机管理](/camera-kit/camera-dev-arkts-mandatory/camera-device-management)、[设备输入](/camera-kit/camera-dev-arkts-mandatory/camera-device-input)、[会话管理](/camera-kit/camera-dev-arkts-mandatory/camera-session-management)等流程完成相机应用开发必选能力配置。
- 查询当前设备的当前模式是否支持拍摄动态照片。
- 如果支持动态照片，可以调用相机框架提供的使能接口****使能****动态照片能力。
- 监听照片回调，将照片存入媒体库。可参考[MediaLibrary Kit-访问和管理动态照片资源](/medialibrary-kit/movingphoto/photoaccesshelper-movingphoto)。

## 开发步骤

详细的API说明请参考[@ohos.multimedia.camera (相机管理)](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera/arkts-apis-camera)。

![](../../../images/fec7dace/note_3.0-zh-cn.png) 

- 拍摄动态照片需要麦克风权限ohos.permission.MICROPHONE，权限申请和校验的方式请参考[开发准备](/camera-kit/camera-preparation)。否则拍摄的照片没有声音。
...
3. 查询当前设备当前模式是否支持动态照片能力。
   ...
   ![](../../../images/fcbe625e/note_3.0-zh-cn.png) 

   查询是否支持动态照片前需要先完成相机会话配置、提交和启动会话，详细开发步骤请参考[会话管理](/camera-kit/camera-dev-arkts-mandatory/camera-session-management)。
...
4. 使能动态照片拍照能力。
   ...
   ![](../../../images/06ddd101/note_3.0-zh-cn.png) 

   使能动态照片前需要使能[分段式拍照](/camera-kit/camera-dev-arkts/camera-deferred-capture)能力。
   ...
   ```
5. 触发拍照，与普通拍照方式相同，请参考[拍照](/camera-kit/camera-dev-arkts/camera-shooting)。
...
- 在配置预览输出流时，需要先通过接口[getSupportedFullOutputCapability](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedfulloutputcapability23)查询当前镜头和模式支持的完整能力，选择的预览输出格式为P010（CAMERA\_FORMAT\_YCRCB\_P010/CAMERA\_FORMAT\_YCBCR\_P010）。
- 在配置色彩空间时，需要先通过接口[getSupportedColorSpaces](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-colormanagementquery/arkts-apis-camera-colormanagementquery#getsupportedcolorspaces12)获取当前设备所支持的色彩空间，再通过接口[setColorSpace](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-colormanagement/arkts-apis-camera-colormanagement#setcolorspace12)设置色彩空间为BT2020\_HLG。具体请参考[setColorSpace](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-colormanagement/arkts-apis-camera-colormanagement#setcolorspace12)说明。
