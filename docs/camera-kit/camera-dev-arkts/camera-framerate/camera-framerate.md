...
![](../../../images/5d1652fe/zh-cn_image_0000002583438627.png)

与普通的[预览](/camera-kit/camera-dev-arkts/camera-preview)流程相比，动态调整预览帧率的注意点如图上标识：

1. 调用[createSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createsession11)创建会话（Session）时，需要指定模式为NORMAL\_PHOTO或NORMAL\_VIDEO。
...
如何配置会话（Session）、释放资源，请参考[会话管理](/camera-kit/camera-dev-arkts-mandatory/camera-session-management) > [预览](/camera-kit/camera-dev-arkts/camera-preview)，或是[完整流程](#完整流程)示例。
...
相机使用预览等功能前，均需完成[相机管理](/camera-kit/camera-dev-arkts-mandatory/camera-device-management)和创建相机会话，调用[CameraManager](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager)的[createSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createsession11)创建一个会话。

创建会话时需指定[SceneMode](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#scenemode11)为NORMAL\_PHOTO或NORMAL\_VIDEO，创建出的Session处于拍照或录像模式。
...
1. 调用[PreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput)的[getSupportedFrameRates](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput#getsupportedframerates12)，查询当前支持的帧率范围。

   ![](../../../images/2225d712/note_3.0-zh-cn.png) 

   需要在Session调用[commitConfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#commitconfig11)完成配流之后调用。
   ...
   ```
2. 根据实际开发需求，调用[PreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput)类提供的[setFrameRate](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput#setframerate12)接口对帧率进行动态调整。

   ![](../../../images/396b0ada/note_3.0-zh-cn.png) 

   ****调用时机：****

   - 需要在Session调用[commitConfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#commitconfig11)完成配流之后调用。
   - 可在Session调用[start](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#start11)启动预览前后任意时刻调用。
   ...
   ****setFrameRate调用限制：****
   ...
   ```
3. （可选）通过[PreviewOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput)提供的[getActiveFrameRate](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput#getactiveframerate12)接口查询已设置过并生效的帧率。

   仅通过[setFrameRate](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-previewoutput/arkts-apis-camera-previewoutput#setframerate12)接口显式设置过帧率才可查询当前生效帧率信息。
