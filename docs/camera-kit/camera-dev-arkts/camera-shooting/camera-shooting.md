...
目前相机开发有两种相机拍照方案，分别是相机[分段式拍照](/camera-kit/camera-dev-arkts/camera-deferred-capture)和相机单段式拍照（****本文将以单段式拍照为基础进行说明****）。
...
详细的API说明请参考[@ohos.multimedia.camera (相机管理)](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera/arkts-apis-camera)。

1. 导入[Image](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-image/arkts-apis-image-image)接口。获取拍照输出的数据需要用到系统提供的Image接口能力，导入Image接口的方法如下。
...
2. 创建拍照输出流。
   ...
   通过[CameraOutputCapability](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#cameraoutputcapability)中的photoProfiles属性，可获取当前设备支持的拍照输出流。通过[createPhotoOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#createphotooutput11)方法传入支持的某一个输出流[Profile](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#profile)创建拍照输出流。
   ...
   ```
3. 设置拍照[on('photoAvailable')](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#onphotoavailable11)的回调，并将拍照的buffer保存为图片。

   Context获取方式请参考：[获取UIAbility的上下文信息](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-usage#获取uiability的上下文信息)。

   如需要在图库中看到所保存的图片、视频资源，需要将其保存到媒体库，保存方式请参考：[保存媒体库资源](/medialibrary-kit/photoaccesshelper-savebutton)。

   需要在[photoOutput.on('photoAvailable')](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#onphotoavailable11)接口获取到buffer时，将buffer在安全控件中保存到媒体库。
...
5. 触发拍照。
   ...
   通过photoOutput的[capture](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#capture-2)方法，执行拍照任务。该方法有两个参数，第一个参数为拍照设置参数的setting，setting中可以设置照片的质量和旋转角度，第二参数为回调函数。

   获取拍照旋转角度的方法为，通过[PhotoOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput)中的[getPhotoRotation](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#getphotorotation12)方法获取rotation实际的值。

   ![](../../../images/6eb4da09/note_3.0-zh-cn.png) 

   图片地理位置信息[Location](/ref/location-api/location-arkts/js-apis-geolocationmanager/js-apis-geolocationmanager#geolocationmanagergetcurrentlocation)，使用方法可参考[capture](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#capture-3)示例。
...
![](../../../images/a345aa73/caution_3.0-zh-cn.png) 

仅单段式拍照支持设置画质优先策略。若在分段式拍照中设置画质优先策略，该设置将无效。

### 画质优先策略

在使用单段式拍照时，支持设置速度优先和画质优先两种画质优先策略类型，并且分别对应着不同的[PhotoQualityPrioritization](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#photoqualityprioritization21)枚举类型。

- [SPEED](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#photoqualityprioritization21)对应着速度优先，表示降低画质来提升拍照的速度。如果开发者在进行单段式拍照时没有设置明确的画质优先策略，****单段式拍照就默认为速度优先状态****。
- [HIGH\_QUALITY](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#photoqualityprioritization21)对应着画质优先，表示通过较长的耗时来得到画质更高的图片。

### 如何正确设置画质优先策略

为了正确的在单段式拍照中设置画质优先策略，高性能拍照功能提供了如下两个接口：

- [isPhotoQualityPrioritizationSupported](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#isphotoqualityprioritizationsupported21)：查询当前设备是否支持指定的画质优先策略。返回true表示支持，返回false表示不支持。在进行设置画质优先策略之前，必须先查询将要设置的画质优先策略在当前设备上是否可用。
- [setPhotoQualityPrioritization](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#setphotoqualityprioritization21)：画质优先策略设置接口，通过该接口设置对应的画质优先策略，实现高性能拍照。

### 开发步骤

高性能拍照相关接口需要在[会话管理(ArkTS)](/camera-kit/camera-dev-arkts-mandatory/camera-session-management)流程的使能步骤中进行调用。

具体调用时机如下：

- 在[会话管理(ArkTS)](/camera-kit/camera-dev-arkts-mandatory/camera-session-management)流程的使能步骤中的[commitConfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#commitconfig11)结束之后进行调用。
  ...
  ```
- 在[会话管理(ArkTS)](/camera-kit/camera-dev-arkts-mandatory/camera-session-management)流程的使能步骤中的[commitConfig](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#commitconfig11)之前调用。
...
- 通过注册固定的captureStart回调函数获取监听拍照开始结果，photoOutput创建成功时即可监听，相机设备已经准备开始这次拍照时触发，该事件返回此次拍照的captureId。
  ...
  ```
- 通过注册固定的captureEnd回调函数获取监听拍照结束结果，photoOutput创建成功时即可监听，该事件返回结果为拍照完全结束后的相关信息[CaptureEndInfo](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#captureendinfo)。
...
- 通过注册固定的captureReady回调函数获取监听可拍下一张结果，photoOutput创建成功时即可监听，当下一张可拍时触发，该事件返回结果为下一张可拍的相关信息。
  ...
  ```
- 通过注册固定的error回调函数获取监听拍照输出流的错误结果。回调返回拍照输出接口使用错误时的对应错误码，错误码类型参见[CameraErrorCode](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#cameraerrorcode)。
