...
![](../../../images/d1505ab3/note_3.0-zh-cn.png) 
...
1. 需要导入相机框架、媒体库、图片等相关领域依赖。
   ...
   ```
2. 通过[getCameraDevice](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getcameradevice18)获取对应的前置和后置相机。如果接口返回undefined，基于示例中的配置信息，表示当前设备不支持指定位置（前置/后置）的默认类型相机，无法实现多摄同开功能。
   ...
   ```
3. 获取对应的并发能力集。通过[getCameraConcurrentInfos](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getcameraconcurrentinfos18)获取相机的输出并发能力信息数组[CameraConcurrentInfo](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#cameraconcurrentinfo18)，数组内部包含相机在对应并发模式下支持的模式和输出能力，****在多摄同开场景下设置的模式和输出能力必须在并发能力集的范围之内****。若[getCameraConcurrentInfos](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getcameraconcurrentinfos18)接口返回空数组，则表明当前设备不支持并发功能。
...
6. 确定录像输出流。
   ...
   ```
7. 打开相机。通过[open](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput#open18)以多摄同开状态打开指定相机。在使用[open](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput#open18)接口前，请先查询接口是否支持并发能力集，并优先调用[getCameraConcurrentInfos](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getcameraconcurrentinfos18)方法，获取多摄同开状态下的相机并发能力集。请勿在未查询并发能力集的情况下使用[open](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput#open18)，否则会导致打开相机失败。
