...
- 在第一阶段，系统快速上报轻量处理的图片，轻量处理的图片比全质量图低，出图速度快。应用通过回调会收到一个[PhotoAsset](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-photoasset/arkts-apis-photoaccesshelper-photoasset)对象，通过该对象可调用媒体库接口，读取图片或落盘图片。
...
- 通过PhotoOutput，监听photoAssetAvailable回调，获取[photoAccessHelper](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper/arkts-apis-photoaccesshelper)的PhotoAsset对象。
- 通过PhotoAsset对象，调用媒体库相关接口，读取或落盘图片。

![](../../../images/4cc9a01a/note_3.0-zh-cn.png) 
...
3. 设置拍照photoAssetAvailable的回调。
   ...
   ![](../../../images/16e79559/caution_3.0-zh-cn.png) 
   ...
   不建议开发者同时注册[photoAvailable](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#onphotoavailable11)和[photoAssetAvailable](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-photooutput/arkts-apis-camera-photooutput#onphotoassetavailable12)。
   ...
   落盘图片参考媒体库接口：[saveCameraPhoto](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/kts-apis-photoaccesshelper-mediaassetchangerequest/kts-apis-photoaccesshelper-mediaassetchangerequest#savecameraphoto12)

   请求图片参考媒体库接口：[requestImageData](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-mediaassetmanager/arkts-apis-photoaccesshelper-mediaassetmanager#requestimagedata11) 和 [onDataPrepared](/ref/media-library-api/media-library-arkts/js-apis-photoaccesshelper/arkts-apis-photoaccesshelper-mediaassetdatahandler/arkts-apis-photoaccesshelper-mediaassetdatahandler#ondataprepared11)
4. 拍照时的会话配置及触发拍照的方式，与普通拍照相同，请参考[拍照](/camera-kit/camera-dev-arkts/camera-shooting)的步骤4-5。
...
- 通过注册固定的captureReady回调函数获取监听可拍下一张结果，photoOutput创建成功时即可监听，当下一张可拍时触发，该事件返回结果为下一张可拍的相关信息。
  ...
  ```
- 通过注册固定的error回调函数获取监听拍照输出流的错误结果。callback返回拍照输出接口使用错误时的对应错误码，错误码类型参见[CameraErrorCode](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#cameraerrorcode)。
