...
- 通过[Device Security Kit](/system-security/device-security-kit-guide/devicesecurity-introduction)来创建证明密钥（安全摄像头序列号会作为入参）、初始化证明会话。Device Security Kit初始化证明会话完成后会返回给应用****匿名证书链****。
- 通过Camera Kit配置安全相机输入输出流，重点是配置****安全数据流****，注册安全数据流每帧安全图像回调监听。
- 解析安全数据流每帧安全图像，在服务器侧完成安全图像的签名验证。

![](../../../images/aab2a4b4/note_3.0-zh-cn.png) 

当前文档主要说明通过Camera Kit完成的步骤，证明会话相关步骤需通过Device Security Kit完成，具体可参考[可信应用服务-安全摄像头](/system-security/device-security-kit-guide/devicesecurity-taas-dev/devicesecurity-taas-securecamera)。

![](../../../images/bab04654/zh-cn_image_0000002552798932.png)
...
2. 选择支持安全相机的设备。
   ...
   通过[CameraManager](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager)中的[getSupportedSceneModes](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedscenemodes11)方法，可以获取当前设备支持的所有模式，如果当前设备支持安全相机模式，即可使用该设备做后续安全相机操作。
...
4. 创建设备输入输出。
   ...
   - 一路是普通的预览流，用于界面显示，普通预览流的创建流程请参考[预览开发指导](/camera-kit/camera-dev-arkts/camera-preview)。
   - 一路是安全数据流，用于安全服务校验，安全数据流需要通过[image.createImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#imagecreateimagereceiver11)创建图像接收类[ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver)，再通过其[getReceivingSurfaceId](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver#getreceivingsurfaceid9)方法获取surfaceId。

     ![](../../../images/74d9d456/note_3.0-zh-cn.png) 
...
5. 打开安全设备。
   ...
   [CameraInput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput)提供了[open(isSecureEnabled)](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-camerainput/arkts-apis-camera-camerainput#open12)方法用于打开安全相机并返回安全摄像头序列号，该序列号是安全模块创建证明会话的必需参数。
   ...
   ```
6. 使用Device Security Kit的能力，创建证明密钥、打开证明会话。请参考Device Security Kit（设备安全服务）的开发指导：[可信应用服务-安全摄像头](/system-security/device-security-kit-guide/devicesecurity-taas-dev/devicesecurity-taas-securecamera)。
7. 创建安全相机会话，配流启流。

   创建安全相机模式的会话，将输入流、输出流加入会话，需要将安全数据流通过[SecureSession](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-securesession/arkts-apis-camera-securesession)的[addSecureOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-securesession/arkts-apis-camera-securesession#addsecureoutput12)方法标记成安全输出。
   ...
   ```
8. 安全模式会话正常启动后，预览流和安全数据流数据逐帧上报，安全数据流每帧数据可以通过[ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver)注册[imageArrival](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver#on9)回调获取。
   ...
   如果有在端侧验证图像数据或地理位置数据签名的需求，可参考[验证签名](/system-security/device-security-kit-guide/devicesecurity-taas-dev/devicesecurity-taas-verify/devicesecurity-taas-verifysignature)中与安全图像相关的部分。
9. 释放安全相机，使用[Session](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session)的[release](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#release11-1)方法。
