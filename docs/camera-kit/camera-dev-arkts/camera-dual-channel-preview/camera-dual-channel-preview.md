...
在开发相机应用时，需要先[申请相关权限](/camera-kit/camera-preparation)。

双路预览，即应用可同时使用两路预览流，一路用于在屏幕上显示，一路用于图像处理等其他操作，提升处理效率。

相机应用通过控制相机，实现图像显示（预览）、照片保存（拍照）、视频录制（录像）等基础操作。相机开发模型为Surface模型，即应用通过Surface进行数据传递，通过[ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver)的Surface获取拍照流的数据、通过XComponent的Surface获取预览流的数据。

如果要实现双路预览，可以先参考[拍照](/camera-kit/camera-dev-arkts/camera-shooting)，在双路预览中将拍照流改为另一路预览流，通过[ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver)的Surface创建另一个previewOutput，其余流程与拍照一致。

详细的API说明请参考[@ohos.multimedia.camera (相机管理)](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera/arkts-apis-camera)。

## 约束与限制

- 暂不支持动态添加流，即不能在没有调用[session.stop](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#stop11)的情况下，调用[addOutput](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-session/arkts-apis-camera-session#addoutput11)添加流。
...
![](../../../images/09be81ce/zh-cn_image_0000002583438611.png)
...
2. 获取第一路预览流SurfaceId：创建ImageReceiver对象，通过ImageReceiver对象可获取其SurfaceId。
   ...
   ```
3. ImageReceiver接收预览流图像数据获取图像格式请参考[Image](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-image/arkts-apis-image-image)中的format参数，[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)格式请参考[PixelMapFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#pixelmapformat7)。
   ...
   ```
4. 注册监听处理预览流每帧图像数据：通过ImageReceiver组件中imageArrival事件监听获取底层返回的图像数据，详细的API说明请参考[ImageReceiver](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagereceiver/arkts-apis-image-imagereceiver)。

   ![](../../../images/5216d641/note_3.0-zh-cn.png) 

   - 在通过[createPixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#imagecreatepixelmap8)接口创建[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)实例时，设置的Size、srcPixelFormat等属性必须和相机预览输出流previewProfile中配置的Size、Format属性保持一致，ImageReceiver图片像素格式请参考[PixelMapFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#pixelmapformat7)，相机预览输出流previewProfile输出格式请参考[CameraFormat](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#cameraformat)。
   - 由于不同设备产品差异性，应用开发者在创建相机预览输出流前，必须先通过[getSupportedOutputCapability](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-cameramanager/arkts-apis-camera-cameramanager#getsupportedoutputcapability11)方法获取当前设备支持的预览输出流previewProfile，再根据实际业务需求选择[CameraFormat](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-e/arkts-apis-camera-e#cameraformat)和[Size](/ref/camera-api/camera-arkts/js-apis-camera/arkts-apis-camera-i/arkts-apis-camera-i#size)适合的预览输出流previewProfile。
   - ImageReceiver接收预览流图像数据实际format格式由应用开发者在创建预览输出流相机预览输出流时，根据实际业务需求选择的previewProfile中format格式参数影响，详细步骤请参考[创建预览流获取数据](/camera-kit/camera-dev-arkts/camera-dual-channel-preview#创建预览流获取数据)。
   ...
   通过 [image.Component](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#component9) 解析图片buffer数据参考：

   ![](../../../images/7bbe575d/caution_3.0-zh-cn.png) 
