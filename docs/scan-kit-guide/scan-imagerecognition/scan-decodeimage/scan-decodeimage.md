...
![](../../../images/88c5f54b/zh-cn_image_0000002552958614.png)

1. 用户向应用发起识码请求。
2. 应用通过调用[Camera Kit](/camera-kit/camera-overview)启动相机，获取预览流数据。
...
识别图像数据中的码图，以Promise形式返回识别结果。具体API说明详见[接口文档](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode)。

| 接口名 | 描述 |
| --- | --- |
| [decodeImage](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectbarcodedecodeimage)(image: [ByteImage](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#byteimage), options?: scanBarcode.[ScanOptions](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanoptions)): Promise&lt;[DetectResult](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectresult)&gt; | 启动图像识码，通过传入ByteImage类型的图像数据信息，使用Promise异步回调返回识码结果。 |
...
1. 导入图像识码接口和相关接口模块，该模块提供了图像识码参数和方法，导入方法如下。
   ...
   ```
2. 使用Camera Kit启动相机能力，实现双路预览功能，具体实现详见[双路预览](/camera-kit/camera-dev-arkts/camera-dual-channel-preview)。
3. 通过ImageReceiver实时获取预览图像数据，详见[双路预览](/camera-kit/camera-dev-arkts/camera-dual-channel-preview)，调用detectBarcode.decodeImage接口解析图像数据。请在识别完成后再释放图像数据。
   ...
   ```
4. detectBarcode.[DetectResult](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectresult)中返回的cornerPoints可参考以下说明使用。
