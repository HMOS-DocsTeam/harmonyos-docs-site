...
![](../../../images/93eb17de/zh-cn_image_0000002583438659.png)
...
接口返回值有两种返回形式：Callback和Promise回调。下表中为启动图片识码Callback和Promise形式接口，Callback和Promise只是返回值方式不一样，功能相同。具体API说明详见[接口文档](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode)。

| 接口名 | 描述 |
| --- | --- |
| [decode](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectbarcodedecode)(inputImage: [InputImage](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#inputimage), options?: scanBarcode.[ScanOptions](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanoptions)): Promise&lt;Array&lt;scanBarcode.[ScanResult](/ref/scan-api/scan-arkts/scan-scanbarcode-api/scan-scanbarcode-api#scanresult)&gt;&gt; | 启动图片识码，通过InputImage传入图片信息，通过ScanOptions进行识码参数设置（options为可选参数），使用Promise异步回调返回识码结果。 |
| [decode](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectbarcodedecode-1)(inputImage: InputImage, options: scanBarcode.ScanOptions, callback: AsyncCallback&lt;Array&lt;scanBarcode.ScanResult&gt;&gt;): void | 启动图片识码，通过InputImage传入图片信息，通过ScanOptions进行识码参数设置，使用Callback异步回调返回识码结果。 |
| [decode](/ref/scan-api/scan-arkts/scan-imagedecode/scan-imagedecode#detectbarcodedecode-2)(inputImage: InputImage, callback: AsyncCallback&lt;Array&lt;scanBarcode.ScanResult&gt;&gt;): void | 启动图片识码，通过InputImage传入图片信息，使用Callback异步回调返回识码结果。 |
...
2. 调用detectBarcode.decode接口解析码图。
   ...
   - 通过Promise回调函数得到扫码结果，InputImage对象中uri参数推荐通过[picker](/medialibrary-kit/photoaccesshelper-photoviewpicker)方式获取。
     ...
     ```
   - 通过Callback回调函数得到扫码结果，InputImage对象中uri参数推荐通过[picker](/medialibrary-kit/photoaccesshelper-photoviewpicker)方式获取。
