...
![](../../../images/32ab8d4d/zh-cn_image_0000002552798966.png)
...
通过字节数组生成码图，以Promise形式生成码图。具体API说明详见[接口文档](/ref/scan-api/scan-arkts/scan-generatebarcode/scan-generatebarcode)。

| 接口名 | 接口描述 |
| --- | --- |
| [createBarcode](/ref/scan-api/scan-arkts/scan-generatebarcode/scan-generatebarcode#generatebarcodecreatebarcode-2)(content: ArrayBuffer, options: [CreateOptions](/ref/scan-api/scan-arkts/scan-generatebarcode/scan-generatebarcode#createoptions)): Promise&lt;image.[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)&gt; | 码图生成接口，返回生成的码图，类型为image.PixelMap，可以使用Image组件渲染成图片。使用Promise异步回调。 |
