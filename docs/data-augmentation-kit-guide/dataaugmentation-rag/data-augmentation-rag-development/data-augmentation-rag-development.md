...
1. 知识问答需要先对数据源库进行[知识加工](/data-augmentation-kit-guide/data-augmentation-knowledge-processing)生成知识库，否则无法问答。
...
![](../../../images/9b0df874/note_3.0-zh-cn.png) 

接口需在页面或自定义组件生命周期内调用。

RAG关键接口如下表所示，具体API说明详见[API参考](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#导入模块)。除接口外，还可以通过配置文件进行深度定制RAG，详见[RAG配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-augmentation-config-prompt-template)。

| 接口名 | 描述 |
| --- | --- |
| abstract [streamChat](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#streamchat)(query: string, callback: Callback&lt;[LLMStreamAnswer](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#llmstreamanswer)&gt;): Promise&lt;[LLMRequestInfo](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#llmrequestinfo)&gt; | 继承ChatLLM类实现大模型客户端时需要实现的函数。RAG在检索前的问题预处理、检索后的回答生成时，会调用这个函数与大语言模型交互。 |
| [createRagSession](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#createragsession)(context: [common.Context](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context), config: [Config](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#config)): Promise&lt;[RagSession](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#ragsession)&gt; | 获得一个会话用于进行知识问答。不支持多线程调用。 |
| [streamRun](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#streamrun)(question: string, config: [RunConfig](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#runconfig), callback: AsyncCallback&lt;[Stream](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#stream)&gt;): Promise&lt;number&gt; | 知识问答接口，传入问题以及问答配置项。当RAG生成问题结果时，触发callback回调函数来流式传递数据。支持的长度为1000个字符内（UTF-8下一个汉字占3个字符）。不支持多线程调用。 |

## 开发准备

1. 申请网络权限。[streamChat](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#streamchat)中需要开发者实现与LLM交互的功能，因此需要为应用申请网络权限。
   ...
   ```
2. 完成知识加工配置。请参考[知识加工](/data-augmentation-kit-guide/data-augmentation-knowledge-processing)。

## 开发步骤

下面仅对关键步骤关键代码进行片段式说明，省略了很多非核心代码，如果需要查看完整功能示例代码，请参考[示例代码](/data-augmentation-kit-guide/dataaugmentation-rag/data-augmentation-rag-demo)。应用的一次流式问答过程，和RagSession、ChatLLM、知识库的交互流程，可参考[流式问答调用流程图](#流式问答调用流程图)。
...
1. 导入@kit.DataAugmentationKit模块，其余依赖需要开发者按需添加。
   ...
   ```
2. 创建http工具类，用以和大模型交互，用户也可选择webSocket（可参考[Network Kit](/ref/system-network-api/network-api/network-api-arkts/network-api-arkts)）或者其他方式与大模型交互。本示例选用了[ModelArts平台](https://console.huaweicloud.com/modelarts)的qwen3-235b-a22b模型作为示例，开发者使用时需根据实际情况选择合适大模型。示例代码包括如下三个环节：
   ...
   ```
3. 继承实现[ChatLLM](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#chatllm)类，在此函数中与大模型进行交互，并将大模型返回结果通过callback函数返回给RagSession。
   ...
   ```
4. 创建[Config](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#config)配置中的属性。下面简要介绍几个主要属性，有关全量配置字段的详细含义，请参见[智慧化数据检索](/data-augmentation-kit-guide/dataaugmentation-retrieval)中的说明。开发者可以根据自身需求进行选择性配置。

   - [RetrievalConfig](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-retrieval-api/dataaugmentation-retrieval-api#retrievalconfig)主要配置知识库的数据库配置。[知识加工](/data-augmentation-kit-guide/data-augmentation-knowledge-processing)将会生成向量及倒排两种知识库表。
     ...
     ```
   - [RetrievalCondition](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-retrieval-api/dataaugmentation-retrieval-api#retrievalcondition)主要配置检索条件及多路召回之后的排序配置。其中fromClause为查询目标索引名，可按照如下示例代码配置为业务数据库表及知识加工产生的数据库表联合形成的虚拟表；responseColumns为召回的字段集合，范围为fromClause配置的数据库表中的列。关于知识库的数据库表结构可参见：[知识加工](/data-augmentation-kit-guide/data-augmentation-knowledge-processing)。
     ...
     ```
   - 完成Config数据的构造。[ChatLLM](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#chatllm)参数则使用步骤3继承实现的ChatLLM的自定义的类的实例。
...
6. 使用步骤5创建的RagSession的streamRun()函数进行问答。
   ...
   - answerTypes属性用来指定流式输出的数据类型（[StreamType](/ref/data-augmentation-api/dataaugmentation-arkts/dataaugmentation-rag-api/dataaugmentation-rag-api#streamtype)），当前示例代码配置了三种数据类型，所以最终streamRun()函数的callback回调函数将会输出这三种类型的数据。
...
![](../../../images/3349af62/zh-cn_image_0000002583438327.png)
