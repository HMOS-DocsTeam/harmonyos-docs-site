...
![](../../../../images/bd9309bd/zh-cn_image_0000002552798730.png)
...
1. 导入[证书算法库框架模块](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert)。
   ...
   ```
2. [cert.createCertChainValidator](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#certcreatecertchainvalidator)创建证书链校验器对象。
3. 基于已有的证书数据，创建证书链数据对象[CertChainData](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#certchaindata)。

   证书算法库框架提供了证书链校验器对象可用于校验证书链，以验证信任链根源，但待校验的证书链数据对象应符合证书链数据对象的数据结构定义[CertChainData](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#certchaindata)。
4. 调用[CertChainValidator.validate](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#validate)校验证书链数据。
