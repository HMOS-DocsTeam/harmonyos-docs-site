...
1. 导入[证书算法库框架模块](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert)。
   ...
   ```
2. 调用[证书CMS封装](/system-security/device-certificate-kit/certificate-framework/create-cms-enveloped-object)进行CMS数据封装。
3. 调用[cert.createCmsParser](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#certcreatecmsparser22)创建CmsParser对象。
4. 调用[cmsParser.setRawData](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#setrawdata22)设置CMS数据。
5. 调用[cmsParser.decryptEnvelopedData](/ref/system-security-api/device-certificate-api/device-certificate-arkts/js-apis-cert/js-apis-cert#decryptenvelopeddata22)解密封装数据。
