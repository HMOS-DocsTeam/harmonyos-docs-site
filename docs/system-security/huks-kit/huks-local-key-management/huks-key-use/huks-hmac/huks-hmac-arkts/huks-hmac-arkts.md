...
2. 初始化密钥属性集。
3. 调用[generateKeyItem](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksgeneratekeyitem9)生成密钥，HMAC支持的规格请参考[密钥生成支持的算法](/system-security/huks-kit/huks-local-key-management/huks-key-generation-import/huks-key-generation/huks-key-generation-overview#支持的算法)。
...
1. 获取密钥别名。
2. 获取待运算的数据。
3. 调用[initSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksinitsession9)初始化密钥会话，并获取会话的句柄handle。
4. 调用[finishSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksfinishsession9)结束密钥会话，获取哈希后的数据。
