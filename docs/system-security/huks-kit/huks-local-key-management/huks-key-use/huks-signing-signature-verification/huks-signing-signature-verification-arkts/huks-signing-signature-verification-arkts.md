...
具体的场景介绍及支持的算法规格，请参考[签名/验签支持的算法](/system-security/huks-kit/huks-local-key-management/huks-key-use/huks-signing-signature-verification/huks-signing-signature-verification-overview#支持的算法)。
...
2. 初始化密钥属性集。
3. 调用[generateKeyItem](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksgeneratekeyitem9)生成密钥，具体请参考[密钥生成](/system-security/huks-kit/huks-local-key-management/huks-key-generation-import/huks-key-generation/huks-key-generation-overview)。

除此之外，开发者也可以参考[密钥导入](/system-security/huks-kit/huks-local-key-management/huks-key-generation-import/huks-key-import/huks-key-import-overview)，导入已有的密钥。

****签名****

1. 获取密钥别名。
2. 指定待签名的明文数据。
3. 获取属性参数[HuksOptions](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksoptions)，包括两个字段properties和inData。inData传入明文数据，properties使用[HuksParam](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksparam)设置算法参数配置。
4. 调用[initSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksinitsession9)初始化密钥会话，并获取会话的句柄handle。
5. 调用[finishSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksfinishsession9)结束密钥会话，获取签名signature。

****验签****

1. 获取密钥别名。
2. 获取待验证的签名signature。
3. 获取属性参数[HuksOptions](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksoptions)，包括两个字段properties和inData。inData传入签名signature，properties使用[HuksParam](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksparam)设置算法参数配置。
4. 调用[initSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksinitsession9)初始化密钥会话，并获取会话的句柄handle。
5. 调用[updateSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksupdatesession9)更新密钥会话。
6. 调用[finishSession](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksfinishsession9)结束密钥会话，验证签名。

****删除密钥****

当密钥废弃不用时，需要调用[deleteKeyItem](/ref/system-security-api/universal-keystore-api/universal-keystore-arkts/js-apis-huks/js-apis-huks#huksdeletekeyitem9)删除密钥，具体请参考[密钥删除](/system-security/huks-kit/huks-local-key-management/huks-delete-key/huks-delete-key-arkts)。
