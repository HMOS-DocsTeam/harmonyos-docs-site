...
- ****基于群组的访问控制：****
  ...
  - 关键资产所属的群组信息，由开发者ID和群组ID组成。其中，开发者ID无需配置，群组ID在应用开发期配置，参考[配置文件标签说明](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)。应用发布时使用开发者私钥签名保证群组ID的完整性。
...
- 基于群组的访问控制
  ...
  - [IS\_PERSISTENT](/ref/system-security-api/asset-store-api/asset-store-arkts/js-apis-asset/js-apis-asset#tag)属性设置为True的关键资产，不允许设置为群组共享。
- 关键资产删除时机

  - 业务主动调用remove删除关键资产时，删除符合条件的数据。详见删除关键资产[ArkTS](/system-security/asset-store-kit/asset-arkts/asset-js-remove)、[C/C++](/system-security/asset-store-kit/asset-native/asset-native-remove)开发指导。
  - 应用卸载时，会清除对应的数据。[IS\_PERSISTENT](/ref/system-security-api/asset-store-api/asset-store-arkts/js-apis-asset/js-apis-asset#tag)属性设置为true的数据将保留。
...
关键资产存储服务仅提供短敏感数据（不超过1KB）的安全存储能力，如业务希望对超长的数据提供保护时，请考虑使用[通用密钥库系统](/system-security/huks-kit/huks-overview)或[加解密算法库框架](/system-security/crypto-architecture-kit/crypto-architecture-kit-intro)保护您的数据。
