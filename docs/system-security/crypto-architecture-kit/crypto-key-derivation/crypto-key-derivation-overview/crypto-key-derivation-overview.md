...
PBKDF2是将伪随机函数PRF（Pseudo-Random Function，例如基于散列的[HMAC](/system-security/crypto-architecture-kit/crypto-compute-mac/crypto-compute-hmac)），输入密码明文和盐值，重复多次运算来进行密钥派生。
...
HKDF（HMAC-based Extract-and-Expand Key Derivation Function，是基于[HMAC](/system-security/crypto-architecture-kit/crypto-compute-mac/crypto-compute-hmac)）消息身份验证代码的简单密钥派生算法。
