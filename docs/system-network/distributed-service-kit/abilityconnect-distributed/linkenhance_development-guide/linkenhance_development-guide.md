...
两个设备的交互实现如下，在使用[linkEnhance](/ref/system-network-api/distributed-service-api/distributed-service-arkts/js-apis-link-enhance/js-apis-link-enhance)能力后，当两端同时发起连接时，会自动识别合并底层多余物理链路，减少实际物理链路的个数，减少蓝牙链路资源的消耗，增加可用连接数量。

![](../../../../images/382985c8/zh-cn_image_0000002583478423.png)

### 约束与限制

- 设备互联时需要开启蓝牙功能。
- 通过蓝牙广播/扫描接口获取对端设备BLE MAC。蓝牙BLE接口参见[查找设备](/system-network/connectivity-kit/bluetooth/bluetooth-ble/ble-development-guide)。
...
常用接口说明如下表。更多接口的详细介绍参考@ohos.distributedsched.linkEnhance[增强连接](/ref/system-network-api/distributed-service-api/distributed-service-arkts/js-apis-link-enhance/js-apis-link-enhance)。
...
- 服务端开启蓝牙后，创建Server对象，并调用[start()接口](/ref/system-network-api/distributed-service-api/distributed-service-arkts/js-apis-link-enhance/js-apis-link-enhance#start)开启服务，让服务端处于可连接状态，通过注册的事件监听，监听事件的变化通知。
- 客户端开启蓝牙后，创建Connection对象，并调用[connect()接口](/ref/system-network-api/distributed-service-api/distributed-service-arkts/js-apis-link-enhance/js-apis-link-enhance#connect)发起连接，通过注册的事件监听，监听事件的变化通知。
- 连接成功后，可以使用[sendData接口](/ref/system-network-api/distributed-service-api/distributed-service-arkts/js-apis-link-enhance/js-apis-link-enhance#senddata)发送数据。
