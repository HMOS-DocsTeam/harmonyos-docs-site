...
![](../../../../images/d9cc6a5c/note_3.0-zh-cn.png) 
...
![](../../../../images/20efebef/note_3.0-zh-cn.png) 
...
2. 创建一个双向认证TLSSocket连接，返回一个TLSSocket对象。
   ...
   ```
3. 绑定本地IP地址和端口，确保bind成功后，再订阅TLS Socket相关的订阅事件。上传客户端CA证书及数字证书，调用[connect](/ref/system-network-api/network-api/network-api-arkts/js-apis-socket/js-apis-socket#connect9)接口建立连接。连接成功后，可调用[send](/ref/system-network-api/network-api/network-api-arkts/js-apis-socket/js-apis-socket#send9)接口发送数据。
