...
- 发现本地服务，通过[DiscoveryService](/ref/system-network-api/network-api/network-api-arkts/js-apis-net-mdns/js-apis-net-mdns#discoveryservice)对象，对指定类型的本地服务状态变化进行监听。

![](../../../../images/5c3ff5fb/note_3.0-zh-cn.png) 

为了保证应用的运行效率，大部分API调用都是异步的，对于异步调用的API均提供了callback和Promise两种方式，以下示例均采用promise函数，更多方式可以查阅[@ohos.net.mdns (MDNS管理)](/ref/system-network-api/network-api/network-api-arkts/js-apis-net-mdns/js-apis-net-mdns)。

以下分别介绍具体开发方式。

![](../../../../images/3a97a27f/note_3.0-zh-cn.png) 
...
1. 设备连接WiFi。
2. 从@kit.NetworkKit里导入mdns、错误码、以及[common命名空间](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-common/js-apis-app-ability-common)。
   ...
   ```
3. 调用[addLocalService](/ref/system-network-api/network-api/network-api-arkts/js-apis-net-mdns/js-apis-net-mdns#mdnsaddlocalservice-1)方法，添加本地服务。
   ...
   // ...
     ...
     // ...
   ```
4. 通过[resolveLocalService](/ref/system-network-api/network-api/network-api-arkts/js-apis-net-mdns/js-apis-net-mdns#mdnsresolvelocalservice-1)方法，解析本地网络的IP地址（非必要，根据需求使用）。
   ...
   ```
5. 通过[removeLocalService](/ref/system-network-api/network-api/network-api-arkts/js-apis-net-mdns/js-apis-net-mdns#mdnsremovelocalservice-1)方法，移除本地服务。
...
2. 从@kit.NetworkKit里导入mdns的命名空间。
   ...
   import { hilog } from '@kit.PerformanceAnalysisKit';
   ```
3. 创建[DiscoveryService](/ref/system-network-api/network-api/network-api-arkts/js-apis-net-mdns/js-apis-net-mdns#mdnscreatediscoveryservice)对象，用于发现指定服务类型的MDNS服务。
