...
NFC卡模拟完整的API说明以及实例代码请参考：[NFC卡模拟接口](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-cardemulation/js-apis-cardemulation)。
...
1. 在配置文件module.json5中，不需要静态声明NFC读卡器选择的应用ID（AID，参考ISO/IEC 7816-4规范），而是通过[start](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-cardemulation/js-apis-cardemulation#start9)来动态注册。
2. HCE应用的刷卡页面退出时，需要显式调用[stop](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-cardemulation/js-apis-cardemulation#stop9)来释放动态注册的AID刷卡配置项。
...
5. HCE应用后台刷卡建议使用单独专用的HceAbility实现，以减少[onCreate](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability#oncreate)函数耗时。
6. HceAbility的OnCreate函数中除了[hceService.on](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-cardemulation/js-apis-cardemulation#on8)外，尽量减少其它操作。
7. HceAbility的OnCreate函数中hceService.on要同步执行，不能异步执行，以免影响HCE通信时序。

![](../../../../images/ca59f72b/caution_3.0-zh-cn.png) 

- 从API version 9之后的应用开发新增支持[Stage模型](/ability-kit/ability-terminology#stage模型)，作为目前主推并长期演进的模型。
- HCE示例代码的提供，全部按照[Stage模型](/ability-kit/ability-terminology#stage模型)来说明。
...
![](../../../../images/75f61408/note_3.0-zh-cn.png) 
