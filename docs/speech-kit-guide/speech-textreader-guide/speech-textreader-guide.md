...
![](../../images/fe5f2db8/zh-cn_image_0000002552799702.png)

## 接口说明

以下仅列出demo中调用的部分主要接口，具体API说明详见[API参考](/ref/speech-api/speech-arkts/speech-textreader-api/speech-textreader-api)。

| 接口名 | 描述 |
| --- | --- |
| [init](/ref/speech-api/speech-arkts/speech-textreader-api/speech-textreader-api#init)(context: [common.BaseContext](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-basecontext/js-apis-inner-application-basecontext), readParams: [ReaderParam](/ref/speech-api/speech-arkts/speech-textreader-api/speech-textreader-api#readerparam)): Promise&lt;void&gt; | 初始化TextReader。 |
| [start](/ref/speech-api/speech-arkts/speech-textreader-api/speech-textreader-api#start)(readInfoList: [ReadInfo](/ref/speech-api/speech-arkts/speech-textreader-api/speech-textreader-api#readinfo)[], articleId?: string): Promise&lt;void&gt; | 启动TextReader。 |
| on(type: string, callback: function): void | 注册所有事件回调，具体事件类型详见[API参考](/ref/speech-api/speech-arkts/speech-textreader-api/speech-textreader-api)。 |
...
8. 启动朗读控件。
   ...
   ```
9. （可选）若要配置长时任务，需要在[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中添加ohos.permission.KEEP\_BACKGROUND\_RUNNING权限，并且加入backgroundModes选项，然后在readerParam中将keepBackgroundRunning配置为true，确保朗读控件后台播报正常。
   ...
   ```
10. （可选）若要在控件使用功能时切换音色，需要在[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中添加ohos.permission.INTERNET和ohos.permission.GET\_NETWORK\_INFO权限，确保朗读控件可以正常切换音色。
