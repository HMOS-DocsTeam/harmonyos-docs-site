...
![](../../images/6c538a0e/note_3.0-zh-cn.png) 
...
支持商户模型：[运营机构或受理服务机构入网的商户](/payment-kit-guide/payment-preparations/payment-digital-cny-pay-preparations)

数字人民币收银台展示：

![](../../images/df688afe/zh-cn_image_0000002552799440.png)
...
| 步骤 | 说明 |
| --- | --- |
| 开发准备 | 请先完成开发准备后再进行下面的开发接入。  - [数字人民币接入准备](/payment-kit-guide/payment-preparations/payment-digital-cny-pay-preparations) |
...
![](../../images/ad37dc78/zh-cn_image_0000002583439135.png)

1. 商户客户端请求商户服务器创建商品订单。
2. 商户服务器按照商户模式（运营机构商户或受理服务机构商户）调用运营机构或受理服务机构提供的下单接口到数字人民币服务端下单，接口详情请参照商户合作的[运营机构或受理服务机构提供的开发指引](/payment-kit-guide/payment-faq/payment-faq-27)。
3. 数字人民币服务端返回订单加密信息。
4. 商户服务端组建订单信息参数[orderInfo](/ref/payment-api/payment-arkts/payment-digitalcnyservice/payment-digitalcnyservice#ecnyorderinfo)返回给商户客户端。
5. 商户客户端调用[requestEcnyPayment](/ref/payment-api/payment-arkts/payment-digitalcnyservice/payment-digitalcnyservice#ecnypaymentservicerequestecnypayment)接口拉起Payment Kit客户端数字人民币收银台。
...
接口返回值返回形式为Promise。具体API说明详见[接口文档](/ref/payment-api/payment-arkts/payment-digitalcnyservice/payment-digitalcnyservice)。
...
商户客户端使用[orderInfo](/ref/payment-api/payment-arkts/payment-digitalcnyservice/payment-digitalcnyservice#ecnyorderinfo)作为参数调用[requestEcnyPayment](/ref/payment-api/payment-arkts/payment-digitalcnyservice/payment-digitalcnyservice#ecnypaymentservicerequestecnypayment)接口拉起数字人民币收银台。

当接口通过.then()方法返回时，则表示当前订单支付成功，通过.catch()方法返回表示订单支付失败。

当此次请求有异常时，可通过error.code获取错误码，错误码相关信息请参见[错误码](/ref/payment-api/payment-arkts/payment-error-code/payment-error-code)。
...
![](../../images/6534a2bc/note_3.0-zh-cn.png) 
