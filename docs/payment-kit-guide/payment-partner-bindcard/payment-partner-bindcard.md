...
![](../../images/2590ff5b/zh-cn_image_0000002583479093.png)

## 提供绑卡跳转应用信息

开发者需与华为支付业务侧沟通（合作咨询可[点击此处](/payment-kit-guide/payment-service-support)）后提供以下应用信息：
...
![](../../images/6288c3c8/zh-cn_image_0000002552799444.png)

****场景1：用户取消绑卡****

1. 商户客户端调用[requestBindCard](/ref/payment-api/payment-arkts/payment-paymentservice/payment-paymentservice#paymentservicerequestbindcard)接口拉起用户绑卡页面。
...
拉起用户绑卡页面接口。具体API说明详见[接口文档](/ref/payment-api/payment-arkts/payment-paymentservice/payment-paymentservice#paymentservicerequestbindcard)。
...
商户客户端调用[requestBindCard](/ref/payment-api/payment-arkts/payment-paymentservice/payment-paymentservice#paymentservicerequestbindcard)接口拉起用户绑卡页面。

当接口通过.then()方法返回结果，则表示接口请求成功，通过.catch()方法返回异常表示请求失败。当此次请求有异常时，可通过****error.code****获取错误码，错误码相关信息请参见[错误码](/ref/payment-api/payment-arkts/payment-error-code/payment-error-code)。
