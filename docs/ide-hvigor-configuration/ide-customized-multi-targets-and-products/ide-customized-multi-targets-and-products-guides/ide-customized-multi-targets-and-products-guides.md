...
每一个target均可以指定支持的设备类型deviceType，也可以不定义。如果不定义，则该target默认支持module.json5/[config.json](/development-fundamentals/application-configuration-file-fa/application-configuration-file-overview-fa)中定义的设备类型。
...
在未定义target的分发规则distributionFilter时，以module配置[distributionFilter](/development-fundamentals/application-configuration-file-stage/module-configuration-file#distributionfilter标签)/[distroFilter](/development-fundamentals/application-configuration-file-fa/module-structure#distrofilter对象的内部结构)分发规则为准。
...
2. 支持hap、hsp、har（请注意：开启[文件/文件夹名称混淆](/arkts/arkts-compilation-tool-chain/arkts-arkguard/source-obfuscation#section-enable-filename-obfuscation)的har模块需要使用-keep-file-name指定sourceRoot，sourcePath，sourceFileName对应的文件/文件夹名称不被混淆）。
...
![](../../../images/2338e667/note_3.0-zh-cn.png) 
...
![](../../../images/afb981a9/zh-cn_image_0000002561753291.png)
...
![](../../../images/6d957a3d/note_3.0-zh-cn.png) 

products中的icon和label字段在编译时会替换[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)中对应的字段，app.json5和module.json5均可以配置这两个字段，如果都配置，优先级顺序请参考[配置优先级](/development-fundamentals/application-typical-scenarios/layered-image#配置优先级和生成策略)。
...
单击右上角的![](../../../images/84cf62a1/zh-cn_image_0000002530913336.png)图标，指定需要打包的****Product****及****Target****，然后单击****Apply****保存。例如选择"ProductA"中，entry模块对应的"free" Target。

- ****Product****：选择需要构建的APP包。
- ****Build Mode****：选择[编译模式](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section192461528194916)。
...
![](../../../images/de36f780/zh-cn_image_0000002530913342.png)
...
![](../../../images/508fc6db/zh-cn_image_0000002530913340.png)

## 调试和运行指定的Target

使用DevEco Studio调试或运行应用/元服务时，每个模块只能选择其中的一个target运行，可以通过单击右上角的![](../../../images/c20bf980/zh-cn_image_0000002561833263.png)图标，指定需要调试或运行的****Product****下对应的****Module Target****，然后单击****Apply****保存。

![](../../../images/dada35b7/note_3.0-zh-cn.png) 

在选择需要调试或运行的target时，需要注意选择该target所属的Product，否则将找不到可调试和运行的target。

![](../../../images/2c37ca32/zh-cn_image_0000002530753352.png)
...
![](../../../images/a65e3d9c/note_3.0-zh-cn.png) 

- align target和fallback target配置方式命令行优先级高于配置文件。
- 使用配置文件配置align target和fallback target，仅支持DevEco Studio界面****Build****菜单栏功能，不支持****Run****菜单栏功能，可通过[hdc命令行工具](/system-debug-optimize/debugging-commands/hdc)进行推包运行、调试。

多个target的优先级顺序为：align target > 命令行指定模块target > 父级模块target > fallback target > default。

![](../../../images/b14f2011/zh-cn_image_0000002561833267.png)
...
![](../../../images/4322ac6a/note_3.0-zh-cn.png) 
