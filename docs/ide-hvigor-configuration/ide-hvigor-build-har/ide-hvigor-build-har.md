...
HAR自身的构建不建议引用本地模块，可能导致其他模块依赖该HAR包时安装失败，如果安装失败，需要在工程级oh-package.json5中配置[overrides](/ide-ohpm-cli/ide-oh-package-json5#zh-cn_topic_0000001792256137_overrides)。

## 创建模块

1. 新建工程时选择API 10及以上的Stage模型，工程创建完成后，新建“Static Library”模块。模块创建方法可参考[在工程中添加Module](/ide-project/ide-module-management/ide-add-new-module)。

   ![](../../images/a96e0504/zh-cn_image_0000002561753161.png)
2. 编写代码。
...
![](../../images/e325efa2/note_3.0-zh-cn.png) 
...
- 字节码HAR使用的依赖需要配置在本模块的oh-package.json5的dependencies或dynamicDependencies中，如果不配置，后续字节码HAR被集成时可能会出现运行时异常。如果出现异常，部分场景可通过在hvigor-config.json5中配置ohos.byteCodeHar.integratedOptimization后重新编译，具体请参考[编译行为差异说明](/ide-hvigor-configuration/ide-hvigor-dependencies#section957371853712)。
- 字节码HAR的oh-package.json5中配置的依赖名和依赖包的包名（即包内oh-package.json5中的name）需要保持一致。
- 依赖字节码HAR包时，该工程的build-profile.json5中的[useNormalizedOHMUrl](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section13181758123312)必须设置为true。
...
1. 将工程级build-profile.json5的useNormalizedOHMUrl设置为true。
   ...
   ![](../../images/bd628965/note_3.0-zh-cn.png) 
...
2. 在HAR模块的build-profile.json5中，将byteCodeHar设置为true。
   ...
   ```
3. 点击DevEco Studio右上角图标![](../../images/384da858/zh-cn_image_0000002561833159.png)，选择****Build Mode，****默认为****&lt;Default&gt;****模式：在编译App时使用release模式，编译HAP/HSP/HAR时使用debug模式。

   ![](../../images/5adfeb31/zh-cn_image_0000002530753236.png)
4. （可选）在编译模式为release时，为保护代码资产，建议开启混淆，在模块级build-profile.json5文件的release的buildOptionSet配置中，将obfuscation/ruleOptions下的enable字段设置为true。混淆相关能力和具体规则请参考[代码混淆](/ide-build-obfuscation)。
...
5. （可选）如果开发者希望自定义打包到HAR产物中的文件，可在HAR模块的build-profile.json5文件中，配置include或exclude字段，支持glob语法。
   ...
   ![](../../images/970bbeee/note_3.0-zh-cn.png) 
...
6. 选中HAR模块的根目录，点击****Build > Make Module '&lt;module-name&gt;'****启动构建。
   ...
   ![](../../images/fbff458b/note_3.0-zh-cn.png) 

   若修改了HAR模块级oh-package.json5文件的version字段，请先执行****Build > Clean Project****操作，再重新进行Build全量构建。

   ![](../../images/d7292bd6/zh-cn_image_0000002561833149.png)

   构建完成后，build目录下生成HAR包产物。

   ![](../../images/762d15fa/zh-cn_image_0000002561753149.png)

   HAR包产物解压后，结构如下：

   ![](../../images/6b8990d6/zh-cn_image_0000002561833135.png)
...
![](../../images/f0afe7cc/note_3.0-zh-cn.png) 

- 源码HAR包中包含源代码，请谨慎分发，避免造成源代码泄露。
- 如果是native工程，以debug模式构建的native产物中不包含调试信息和符号表，如需调试，请参考[三方源码调试](/ide-debug-app/ide-code-debugging/ide-source-code-debugging)。
...
1. 在HAR模块的build-profile.json5中，将byteCodeHar设置为false。
   ...
   ![](../../images/25d253b7/note_3.0-zh-cn.png) 

   使用DevEco Studio NEXT Beta1（5.0.3.800）之前的版本，模块级build-profile.json5的byteCodeHar字段的缺省默认值为false，无需执行本步骤。
2. 点击DevEco Studio右上角图标![](../../images/0017ef07/zh-cn_image_0000002530753214.png)，****Build Mode****选择****debug。****默认为****&lt;Default&gt;****模式：在编译App时使用release模式，编译HAP/HSP/HAR时使用debug模式。

   ![](../../images/8921d1e2/zh-cn_image_0000002561753157.png)
3. （可选）若部分工程源文件无需构建到HAR包中，可在模块目录下新建.ohpmignore文件，或者在模块目录下的.gitignore文件中，配置打包时要忽略的文件，.ohpmignore文件中支持正则表达式写法，.gitignore文件中支持glob语法。DevEco Studio构建时将过滤掉.ohpmignore或.gitignore文件中所包含的文件/文件夹。
   ...
   ![](../../images/ea3dbc5b/zh-cn_image_0000002530753222.png)
4. （可选）如果开发者希望自定义打包到HAR产物中的文件，可在HAR模块的build-profile.json5文件中，配置include或exclude字段，支持glob语法。配置include或exclude字段后，.gitignore和.ohpmignore文件将不再生效。
   ...
   ![](../../images/9b884e74/note_3.0-zh-cn.png) 

   - 配置include字段时，以下目录不生效，即不会被打包到产物中：node\_modules、oh\_modules、.preview、build、.cxx、.test。
   - 配置exclude字段时，以下文件不生效，默认会打包：oh-package.json5。
5. 选中HAR模块的根目录，点击****Build > Make Module '&lt;module-name&gt;'****启动构建。

   ![](../../images/c852c529/note_3.0-zh-cn.png) 

   若修改了HAR模块级oh-package.json5文件的version字段，请先执行****Build > Clean Project****操作，再重新进行Build全量构建。

   ![](../../images/1d4faf32/zh-cn_image_0000002561833137.png)

   构建完成后，build目录下生成HAR包产物。

   ![](../../images/bfde24de/zh-cn_image_0000002530753238.png)

   HAR包产物解压后，结构如下：

   ![](../../images/83449161/zh-cn_image_0000002561833145.png)
...
1. 在HAR模块的build-profile.json5中，将byteCodeHar设置为false。
   ...
   ![](../../images/d2c5579f/note_3.0-zh-cn.png) 

   使用DevEco Studio NEXT Beta1（5.0.3.800）之前的版本，模块级build-profile.json5的byteCodeHar字段的缺省默认值为false，无需执行本步骤。
2. 点击DevEco Studio右上角图标![](../../images/645b20c3/zh-cn_image_0000002561753187.png)，****Build Mode****中选择****release。****默认为****&lt;Default&gt;****模式：在编译App时使用release模式，编译HAP/HSP/HAR时使用debug模式。

   ![](../../images/b78b3047/zh-cn_image_0000002530913234.png)
3. 在[编译模式](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section192461528194916)为release时，为保护代码资产，建议开启混淆，在模块级build-profile.json5文件的release的buildOptionSet配置中，将obfuscation/ruleOptions下的enable字段设置为true。混淆相关能力和具体规则请参考[代码混淆](/ide-build-obfuscation)。
...
4. （可选）如果开发者希望自定义打包到HAR产物中的文件，可在HAR模块的build-profile.json5文件中，配置include或exclude字段，支持glob语法。
   ...
   ![](../../images/aef11468/note_3.0-zh-cn.png) 

   - 配置include字段时，以下目录不生效，即不会被打包到产物中：node\_modules、oh\_modules、.preview、build、.cxx、.test。
   - 配置exclude字段时，以下文件不生效，默认会打包：oh-package.json5。
5. 选中HAR模块的根目录，点击****Build > Make Module '&lt;module-name&gt;'****启动构建。

   ![](../../images/4b2f0b14/note_3.0-zh-cn.png) 

   若修改了HAR模块级oh-package.json5文件的version字段，请先执行****Build > Clean Project****操作，再重新进行Build全量构建。

   ![](../../images/3e0dc04b/zh-cn_image_0000002561833151.png)

   构建完成后，build目录下生成HAR包产物。

   ![](../../images/d3299b7b/zh-cn_image_0000002530753226.png)

   HAR包产物解压后，结构如下：

   ![](../../images/c2f0172e/zh-cn_image_0000002530753244.png)
...
![](../../images/b287463a/note_3.0-zh-cn.png) 
...
1. 在hvigor-config.json5中，开启构建签名HAR开关：
   ...
   ```
2. 配置工程签名信息，配置流程请参考[配置签名信息](/ide-publish-app#section793484619307)。
3. 选中HAR模块的根目录，点击****Build > Make Module '&lt;module-name&gt;'****启动构建。

   ![](../../images/4f5a33a8/zh-cn_image_0000002530913240.png)

   构建完成后，build目录下生成签名HAR包产物。

   ![](../../images/b1b35662/zh-cn_image_0000002530913244.png)
