...
| 接口名 | 描述 |
| --- | --- |
| [onDestroy(): void](/ref/notification-api/notification-arkts/js-apis-notificationsubscriberextensionability/js-apis-notificationsubscriberextensionability#ondestroy) | 通知订阅扩展被销毁时的回调。 |
| [onReceiveMessage(notificationInfo: NotificationInfo): void](/ref/notification-api/notification-arkts/js-apis-notificationsubscriberextensionability/js-apis-notificationsubscriberextensionability#onreceivemessage) | 收到通知时的回调。 |
| [onCancelMessages(hashCodes: Array&lt;string&gt;): void](/ref/notification-api/notification-arkts/js-apis-notificationsubscriberextensionability/js-apis-notificationsubscriberextensionability#oncancelmessages) | 取消通知时的回调。 |

## 前提条件

申请[ohos.permission.SUBSCRIBE\_NOTIFICATION](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions#ohospermissionsubscribe_notification)权限。

## 开发步骤

开发者在实现[NotificationSubscriberExtensionAbility](/ref/notification-api/notification-arkts/js-apis-notificationsubscriberextensionability/js-apis-notificationsubscriberextensionability)提供方时，需在[DevEco Studio](/ide-tools-overview)工程中新建一个NotificationSubscriberExtensionAbility。具体步骤如下。
...
2. 在entry/src/main/ets/extensionability目录下创建NotificationSubscriberExtAbility.ets，其内容如下。
   ...
   ```
3. 使用[蓝牙模块](/system-network/connectivity-kit/connectivity-kit-intro#蓝牙简介)接口与穿戴设备配对（蓝牙处于配对状态）并获取地址，然后通过[subscribe](/ref/notification-api/notification-arkts/js-apis-notificationextensionsubscription/js-apis-notificationextensionsubscription#notificationextensionsubscriptionsubscribe)/[unsubscribe](/ref/notification-api/notification-arkts/js-apis-notificationextensionsubscription/js-apis-notificationextensionsubscription#notificationextensionsubscriptionunsubscribe)接口订阅或取消订阅通知。
4. 实现[NotificationSubscriberExtensionAbility](/ref/notification-api/notification-arkts/js-apis-notificationsubscriberextensionability/js-apis-notificationsubscriberextensionability)后，还需要在合适的时机调用[openSubscriptionSettings](/ref/notification-api/notification-arkts/js-apis-notificationextensionsubscription/js-apis-notificationextensionsubscription#notificationextensionsubscriptionopensubscriptionsettings)接口，打开通知扩展订阅设置页面，引导用户授予获取本机通知的权限，该页面以半模态弹窗显示。建议在设备管理页面提供一个通知授权的按钮，用户点击按钮则调用[openSubscriptionSettings](/ref/notification-api/notification-arkts/js-apis-notificationextensionsubscription/js-apis-notificationextensionsubscription#notificationextensionsubscriptionopensubscriptionsettings)接口。
...
5. 需要申请权限ohos.permission.ACCESS\_BLUETOOTH。如何配置和申请权限，具体操作请参考[声明权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions)和[向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。
