...
![](../../../../images/b97c90cf/zh-cn_image_0000002583438645.png)
...
![](../../../../images/1bc0da96/note_3.0-zh-cn.png) 
...
![](../../../../images/a60322c6/note_3.0-zh-cn.png) 

AVRecorder只负责视频数据的处理，需要与视频数据采集模块配合才能完成视频录制。视频数据采集模块需要通过Surface将视频数据传递给AVRecorder进行数据处理。当前常用的数据采集模块为相机模块，具体请参考[相机-录像](/camera-kit/camera-dev-native/native-camera-recording)。

文件的创建与存储，请参考[应用文件访问与管理](/core-file-kit/app-file/app-file-access-management/app-file-access)，默认存储在应用的沙箱路径之下，如需存储至图库，请使用[安全控件保存媒体资源](/medialibrary-kit/photoaccesshelper-savebutton)对沙箱内文件进行存储。

开发者通过引入[avrecorder.h](/ref/media-api/media-c/media-headerfile/capi-avrecorder-h/capi-avrecorder-h)、[avrecorder\_base.h](/ref/media-api/media-c/media-headerfile/capi-avrecorder-base-h/capi-avrecorder-base-h)和[native\_averrors.h](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-averrors-h/capi-native-averrors-h)头文件，使用视频录制相关API。

AVRecorder详细的API说明请参考[AVRecorder API参考](/ref/media-api/media-c/media-module/capi-avrecorder/capi-avrecorder)。
...
3. 配置视频录制参数，调用OH\_AVRecorder\_Prepare()接口，此时进入prepared状态。
   ...
   ![](../../../../images/e4fb0cb0/note_3.0-zh-cn.png) 
   ...
   - 录制输出的url地址（即示例里avConfig中的url），形式为fd://xx (fd number)。需要调用基础文件操作接口实现应用文件访问能力，获取方式参考[应用文件访问与管理](/core-file-kit/app-file/app-file-access-management/native-fileio-guidelines)。
   ...
   ```
4. 获取视频录制需要的SurfaceID，初始化视频数据输入源。该步骤需要在输入源模块完成，以相机为例，需要创建录像输出流，包括创建Camera对象、获取相机列表、创建相机输入流等，相机详细步骤请参考[相机-录像方案](/camera-kit/camera-dev-native/native-camera-recording)。
   ...
   ```
5. 初始化视频数据输入源。该步骤需要在输入源模块完成，以相机为例，需要创建录像输出流，包括创建Camera对象、获取相机列表、创建相机输入流等，相机详细步骤请参考[相机-录像方案](/camera-kit/camera-dev-native/native-camera-recording)。
