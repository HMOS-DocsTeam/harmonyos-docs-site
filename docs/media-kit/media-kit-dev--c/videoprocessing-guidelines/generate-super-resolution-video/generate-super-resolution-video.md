...
本模块提供视频细节增强的[C API接口](/ref/media-api/media-c/media-module/capi-videoprocessing/capi-videoprocessing)，通过调用本模块的接口，可以实现视频流图像内容的清晰度增强及缩放功能，处理后的数据可以用于送显、播放和录制。
...
10. 启动细节增强处理。
    ...
    ```
11. 调用[OH\_VideoProcessing\_Stop()](/ref/media-api/media-c/media-headerfile/capi-video-processing-h/capi-video-processing-h#oh_videoprocessing_stop)停止细节增强。
