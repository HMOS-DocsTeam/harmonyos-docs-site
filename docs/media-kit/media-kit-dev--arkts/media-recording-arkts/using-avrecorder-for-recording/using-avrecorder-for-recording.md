...
使用[AVRecorder](/media-kit/media-kit-intro#avrecorder)可以实现音频录制功能，本开发指导将以“开始录制-暂停录制-恢复录制-停止录制”的一次流程为例，向开发者讲解AVRecorder音频录制相关功能。

在进行应用开发的过程中，开发者可以通过AVRecorder的state属性，主动获取当前状态或使用[on('stateChange')](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#onstatechange9)方法监听状态变化。开发过程中必须严格遵循状态机要求，例如只能在started状态下调用[pause](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#pause9-1)接口，只能在paused状态下调用[resume](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#resume9-1)接口。

****图1**** 录制状态变化示意图

![](../../../../images/4e5ab498/zh-cn_image_0000002552958596.png)
...
- 当需要读取和保存音频文件时，请优先使用[AudioViewPicker音频选择器对象](/ref/core-file-api/core-file-arkts/js-apis-file-picker/js-apis-file-picker#audioviewpicker)。

![](../../../../images/418d0ac7/note_3.0-zh-cn.png) 

仅应用需要克隆、备份或同步用户公共目录的音频类文件时，可申请ohos.permission.READ\_AUDIO、ohos.permission.WRITE\_AUDIO权限来读写音频文件，申请方式请参考[申请受控权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions-in-acl)，通过AGC审核后才能使用。为避免应用的上架申请被驳回，开发者应优先使用Picker/控件等替代方案，仅少量符合[特殊场景](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions#ohospermissionread_audio)的应用被允许申请受限权限。

## 开发音频录制应用须知

- 如果需要持续录制或后台录制，请申请长时任务避免进入挂起（Suspend）状态。具体参考[长时任务开发指导](/background-task-kit/continuous-task)。
- 录制需要在前台启动，启动后可以退后台。在后台启动录制将会失败。
- 应用录制音频时需要使用合适的录制流类型，请参考[使用合适的音频流类型](/audio-kit/using-right-streamusage-and-sourcetype)。
- 应用录制音频时需要切换输入设备路由，请参考[实现音频输入设备路由切换](/audio-kit/audio-device/audio-input-device-switcher)。

## 开发步骤及注意事项

详细的API说明请参考[AVRecorder](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder)。

1. 创建AVRecorder实例，实例创建完成进入idle状态。

   ![](../../../../images/b1c54a49/note_3.0-zh-cn.png) 
...
2. 设置业务需要的监听事件，监听状态变化及错误上报。
   ...
   ```
3. 配置音频录制参数，调用[prepare](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#prepare9-1)接口，此时进入prepared状态。

   ![](../../../../images/ac9f001f/note_3.0-zh-cn.png) 
   ...
   - prepare接口的入参avConfig中仅设置音频相关的配置参数，如示例代码所示。
     ...
     如果只需要录制音频，请不要设置视频相关配置参数；如果需要录制视频，可以参考[视频录制开发指导](/media-kit/media-kit-dev--arkts/media-recording-arkts/video-recording)进行开发。直接设置视频相关参数会导致后续步骤报错。
   - 需要使用支持的[录制规格](/media-kit/media-kit-intro#支持的格式)，具体录制参数需严格契合既定的[录制参数配置](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#avrecorderprofile9)。
   - 录制输出的url地址（即示例里avConfig中的url），形式为fd://xx (fd number)。需要基础文件操作接口（[Core File Kit的ohos.file.fs](/ref/core-file-api/core-file-arkts/js-apis-file-fs/js-apis-file-fs)）实现应用文件访问能力，获取方式参考[应用文件访问与管理](/core-file-kit/app-file/app-file-access-management/app-file-access)。
   - 示例中配置的audioCodec音频编码格式、aacProfile音频编码扩展格式、fileFormat封装格式请参考[AVRecorderProfile](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-i/arkts-apis-media-i#avrecorderprofile9)。
   ...
   ```
4. 开始录制，调用[start](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#start9-1)接口，此时进入started状态。

   ```
   // 开始录制。
   await this.avRecorder?.start();
   ```
5. 暂停录制，调用[pause](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#pause9-1)接口，此时进入paused状态。

   ```
   // 暂停录制。
   await this.avRecorder?.pause();
   ```
6. 恢复录制，调用[resume](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#resume9-1)接口，此时再次进入started状态。

   ```
   // 恢复录制。
   await this.avRecorder?.resume();
   ```
7. 停止录制，调用[stop](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#stop9-1)接口，此时进入stopped状态。

   ```
   // 停止录制。
   await this.avRecorder?.stop();
   ```
8. 重置资源，调用[reset](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#reset9-1)接口，重新进入idle状态，允许重新配置录制参数。

   ```
   // 重置资源。
   await this.avRecorder?.reset();
   ```
9. 销毁实例，调用[release](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avrecorder/arkts-apis-media-avrecorder#release9-1)接口，进入released状态，退出录制。
...
使用当前示例代码时，需要申请****ohos.permission.MICROPHONE****麦克风权限。申请方式请参考：[向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。
