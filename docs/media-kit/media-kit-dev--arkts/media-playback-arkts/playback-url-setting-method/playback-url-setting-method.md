...
当前指导仅介绍播放URL设置方法，其他场景及完整示例代码，请参考[视频播放](/media-kit/media-kit-dev--arkts/media-playback-arkts/video-playback)。
...
当服务器需要校验HTTP请求头信息时，可通过[createMediaSourceWithUrl](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-f/arkts-apis-media-f#mediacreatemediasourcewithurl12)设置HTTP请求头信息。
...
当应用需要通过解析本地Raw文件中的m3u8文件，播放在线流媒体资源时，可以通过[resourceManager.getRawFd](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#getrawfd9)获取文件描述符，将其拼接成fdUrl，并通过[setMimeType](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-mediasource/arkts-apis-media-mediasource#setmimetype12)设置MIME类型为APPLICATION\_M3U8。
...
当应用需要通过解析应用沙箱中的m3u8文件，播放在线流媒体资源时，可以通过[fileIo.openSync](/ref/core-file-api/core-file-arkts/js-apis-file-fs/js-apis-file-fs#fileioopensync)获取文件句柄，将其拼接成fdUrl，并通过[setMimeType](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-mediasource/arkts-apis-media-mediasource#setmimetype12)设置MIME类型为APPLICATION\_M3U8。
...
![](../../../../images/c77cac94/note_3.0-zh-cn.png) 
...
4. 在安装应用后，可将示例工程的/entry/src/main/resources/rawfile/test.m3u8通过以下命令加入应用沙箱，从而运行应用沙箱相关示例:（&lt;FILESDIR&gt;为物理路径，以示例工程为例，可通过console.info打印"this.context.filesDir"得到应用沙箱路径，再根据[应用沙箱指南](/core-file-kit/app-file/app-sandbox-directory)的应用沙箱路径和真实物理路径的对应关系表找到物理路径）。
