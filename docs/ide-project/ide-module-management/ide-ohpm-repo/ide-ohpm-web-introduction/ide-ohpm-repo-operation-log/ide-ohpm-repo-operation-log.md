...
![](../../../../../images/9002cf95/caution_3.0-zh-cn.png) 

操作日志的数据每隔一天会定时清除，默认保留100天内的操作日志数据，数据保留时间可通过config.yaml中配置项[operation\_log\_retention](/ide-project/ide-module-management/ide-ohpm-repo/ide-ohpm-repo-configuration#li38847353322)设定。

![](../../../../../images/e185fa43/zh-cn_image_0000002561831177.png "点击放大")

数据筛选：操作日志的数据筛选类别有五类，分别为：用户类型，事件类型，操作类型，操作时间区间和操作对象的用户名。

- 用户类型：分为系统管理员用户和普通用户，当选中普通用户时，只会显示普通用户的操作日志信息。

  ![](../../../../../images/75086ca1/zh-cn_image_0000002561751211.png)
- 事件类型：包括六种事件类型，包括用户管理，仓库管理，包权限管理，认证管理，组织管理和系统设置，通过选择事件类型进行日志的筛选。
  - 例如当事件类型选择用户管理中的新增用户时，操作日志界面仅显示事件类型为新增用户的日志信息。

    ![](../../../../../images/10d6de51/zh-cn_image_0000002530751268.png "点击放大")
  ...
  - 操作结果：选择成功/失败进行筛选。如操作结果选择失败，操作日志页面结果如下：
    ...
    ![](../../../../../images/d44567f2/zh-cn_image_0000002530911256.png "点击放大")
  - 操作时间区间：选中操作时间的区间进行筛选。如区间时间选择在2025.8.4到2025.8.11，操作日志页面结果如下：

    ![](../../../../../images/edcec87b/zh-cn_image_0000002561751213.png "点击放大")
  - 操作对象用户名：输入操作对象用户名进行筛选。如输入用户名为user，操作日志页面结果如下：

    ![](../../../../../images/11a310e5/zh-cn_image_0000002530751266.png "点击放大")
