...
- 从[下载中心](https://developer.huawei.com/consumer/cn/download/ohpm-repo)上获取最新ohpm-repo工具包。
  ...
  ![](../../../../images/9d166020/zh-cn_image_0000002530911382.png "点击放大")
...
- - ohpm-repo 1.0.1版本：不支持自定义ohpm-repo部署目录，仅支持使用默认路径。
  ...
  ![](../../../../images/57e527cb/caution_3.0-zh-cn.png) 
...
1. ****账户的注册****
   ...
   - 通过ohpm-repo管理界面的注册按钮新增用户类型：
     ...
     ![](../../../../images/50d91870/zh-cn_image_0000002561751375.png "点击放大")
   - 登录任一管理员账户，能够在ohpm-repo的用户管理界面新增用户并管理其他用户（删除用户，变更用户类型，重置密码）：

     ![](../../../../images/5ea4a8f3/zh-cn_image_0000002530911410.png "点击放大")
2. ****账户的权限说明****
   - 系统管理员：允许有多个系统管理员。
...
- 三方包数据：可以存放在本地文件（file storage）,sftp服务器（sftp storage）和自定义插件存储（custom storage，包括各种云存储）。
  ...
  ![](../../../../images/291453b7/zh-cn_image_0000002561831349.png "点击放大")
...
- ****现象****：在git-bash终端下运行ohpm publish命令出现 “The content of private key in the key\_path error”错误，报错截图为：
  ...
  ![](../../../../images/cd892ef6/zh-cn_image_0000002561831315.png "点击放大")
...
3. 有两种获得.tgz的方法：
   - 当使用DevEco Studio软件时，点击release发布，可获得.tgz包，具体步骤请参考：[开发动态共享包](/ide-project/ide-module-management/ide-creating-har-api9/ide-hsp)。
...
![](../../../../images/a3c9fe03/zh-cn_image_0000002561831337.png "点击放大")
...
- ****现象****：报错信息如下：
  ...
  ![](../../../../images/179f1aa5/zh-cn_image_0000002561751361.png "点击放大")
- ****原因分析****：部署ohpm-repo的机器没有配置server，使用的是默认的server，host为localhost，在其他机器中不能访问。
- ****解决方法****：修改部署ohpm-repo机器A的配置文件，添加store.config.server的配置。建议手动修改host为本机的ip/域名，例如listen为0.0.0.0:8088，故server需配置为`http://<本机ip/域名>:8088`。

### 执行ohpm-repo install时报错：fail to initialize encryption component: Error: invalid crypto component.

- ****现象****：报错信息如下：

  ![](../../../../images/d6b76361/zh-cn_image_0000002561831307.png "点击放大")
...
- ****解决方法****：有一些三方包包含组织名，只有发布包的用户在该组织下才具有发包的能力。报错信息表明已经有管理员账户添加了该包组织，但是当前账户没有在该组织的成员里面。在ohpm中包的命名格式为@&lt;group&gt;/&lt;package\_name&gt;，其中&lt;group&gt;为组织名，找到创建&lt;group&gt;组织的负责人账户，然后负责人账户登录ohpm-repo管理界面，进入组织的详情里，添加需要发包的账户为组织的成员，成员即可发布具有对应组织名的包。
  ...
  ![](../../../../images/873f14e9/zh-cn_image_0000002530911388.png "点击放大")
...
- ****现象****：ohpm执行publish命令，命令行报错信息为：Same ohpm package is exists!
  ...
  ![](../../../../images/338bf2b2/zh-cn_image_0000002530911422.png "点击放大")
...
- 当添加了uplink的时候，首先确认是否在管理仓库的地方，选用了新增的uplink，下图表示新增的uplink没有被选中，uplink处为空：
  ...
  ![](../../../../images/f4a7b963/zh-cn_image_0000002530751414.png "点击放大")
- 如果新增的uplink中存在所需的三方包，但是下载不下来，可能是需要配置uplink的代理信息，配置位置如下：

  ![](../../../../images/6b02f8d6/zh-cn_image_0000002530751436.png "点击放大")
...
- ****现象****：访问ohpm-repo私仓管理界面的页面功能，报错“非法请求”。
  ...
  ![](../../../../images/1158d336/zh-cn_image_0000002530751402.png "点击放大")
...
- ****现象****：打开ohpm-repo私仓管理界面，访问仓库管理页面中uplink代理配置页面，或访问认证管理页面中证书认证配置页面，报错“加密组件无效”，且已经配置的信息被清空。
  ...
  ![](../../../../images/d1bfa6dc/zh-cn_image_0000002530751394.png "点击放大")

  ![](../../../../images/0dcac1aa/zh-cn_image_0000002561751351.png "点击放大")
...
- ****现象****：访问ohpm-repo私仓管理界面，在ohpm-repo管理界面报错“非法请求”，在ohpm-repo运行日志报错：“verify reverse proxy usage: set "use\_reverse\_proxy" to false in config.yaml if not used, or refresh "x-forwarded-for" in Nginx if it is.”。
  ...
  ![](../../../../images/b3bb9c58/zh-cn_image_0000002561751337.png)

  ![](../../../../images/a4f4d39c/zh-cn_image_0000002530911398.png "点击放大")

- ****原因分析****：在ohpm-repo私仓5.0.7版本中，配置文件新增配置项[use\_reverse\_proxy](/ide-project/ide-module-management/ide-ohpm-repo/ide-ohpm-repo-configuration#section1074004784011)，用于判断是否已使用反向代理。如果配置use\_reverse\_proxy值为true，但未使用反向代理或者在配置反向代理时未刷新x-forwarded-for值，将导致从请求头获取到x-forwarded-for值为空，报此错误。
