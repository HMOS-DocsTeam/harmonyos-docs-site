...
开发者可以使用[@ohos.ai.mindSporeLite](/ref/mindspore-lite-api/mindspore-lite-arkts/js-apis-mindsporelite/js-apis-mindsporelite)，在UI代码中集成MindSpore Lite能力，快速部署AI算法，进行AI模型推理，实现图像分类的应用。

图像分类可实现对图像中物体的识别，在医学影像分析、自动驾驶、电子商务、人脸识别等领域有广泛的应用。

若需基于本Demo适配自有模型，请优先选择静态Shape模型。由于ArkTS暂不支持动态Shape，如确有相关需求，请参考[使用MindSpore Lite实现图像分类（C/C++）](/mindspore-lite-kit/mindspore-guidelines-based-native)，通过Native侧的[OH\_AI\_ModelResize](/ref/mindspore-lite-api/mindspore-lite-c/mindspore-lite-headerfile/capi-model-h/capi-model-h#oh_ai_modelresize)接口对模型inputs进行动态调整。
...
这里给出MindSpore Lite推理的通用开发流程中涉及的一些接口，具体请见下列表格。更多接口及详细内容，请见[@ohos.ai.mindSporeLite (推理能力)](/ref/mindspore-lite-api/mindspore-lite-arkts/js-apis-mindsporelite/js-apis-mindsporelite)。
...
如果开发者有其他图像分类的预训练模型，请参考[MindSpore Lite 模型转换](/mindspore-lite-kit/mindspore-lite-converter-guidelines)介绍，将原始模型转换成.ms格式。
...
1. 工程默认设备定义的能力集可能不包含MindSporeLite。需在DevEco Studio工程的entry/src/main目录下，手动创建syscap.json文件，内容如下：
   ...
   ```
2. 调用[@ohos.ai.mindSporeLite](/ref/mindspore-lite-api/mindspore-lite-arkts/js-apis-mindsporelite/js-apis-mindsporelite)实现端侧推理。具体开发过程及细节如下：
...
1. 此处以获取相册图片为例，调用[@ohos.file.picker](/ref/core-file-api/core-file-arkts/js-apis-file-picker/js-apis-file-picker) 实现相册图片文件的选择。
2. 根据模型的输入尺寸，调用[@ohos.multimedia.image](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image/arkts-apis-image) （实现图片处理）、[@ohos.file.fs](/ref/core-file-api/core-file-arkts/js-apis-file-fs/js-apis-file-fs) （实现基础文件操作） API对选择图片进行裁剪、获取图片buffer数据，并进行标准化处理。
...
![](../../images/4bf12040/zh-cn_image_0000002552959346.png) ![](../../images/e92fe8b3/zh-cn_image_0000002583479347.png)

![](../../images/59d6d806/zh-cn_image_0000002552799698.png) ![](../../images/422767db/zh-cn_image_0000002583439393.png)
