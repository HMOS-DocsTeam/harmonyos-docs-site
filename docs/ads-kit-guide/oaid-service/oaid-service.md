...
- 同一台设备上首个应用开启应用“跨应用关联访问权限”开关时，会首次生成OAID。
  ...
  ![](../../images/ef444c86/note_3.0-zh-cn.png) 
...
| 接口名 | 描述 |
| --- | --- |
| [getOAID](/ref/ads-api/ads-arkts/js-apis-oaid/js-apis-oaid#identifiergetoaid)(): Promise&lt;string&gt; | 获取OAID，通过Promise异步返回结果。 |
| [getOAID](/ref/ads-api/ads-arkts/js-apis-oaid/js-apis-oaid#identifiergetoaid-1)(callback: AsyncCallback&lt;string&gt;): void | 获取OAID，通过Callback异步回调返回值。 |

![](../../images/27ac5fe0/note_3.0-zh-cn.png) 
...
1. 在模块的module.json5文件中，申请跨应用关联权限[ohos.permission.APP\_TRACKING\_CONSENT](/system-security/access-control/app-permission-mgmt/app-permissions/permissions-for-all-user#ohospermissionapp_tracking_consent)，该权限为user\_grant权限，当申请的权限为user\_grant权限时，reason，abilities标签必填，配置方式参见[requestPermissions标签说明](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions#在配置文件中声明权限)，示例代码如下所示：
...
2. 应用在需要获取OAID信息时，应通过调用requestPermissionsFromUser接口获取对应权限。
   ...
   ![](../../images/4ffe29e4/note_3.0-zh-cn.png) 

   其中context的获取方式参见[各类Context的获取方式](/ability-kit/stage-model-development/stage-model-application-components/application-context-stage#context的获取方式)。示例代码如下所示：
