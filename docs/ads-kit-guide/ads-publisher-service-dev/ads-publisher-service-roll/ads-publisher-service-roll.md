...
![](../../../images/f07d757e/zh-cn_image_0000002552799120.png)
...
2. 获取OAID。
   ...
   ![](../../../images/f27fceaa/note_3.0-zh-cn.png) 

   使用以下示例中提供的测试广告位时，必须先获取OAID信息。
3. 请求单广告位广告。

   需要创建一个AdLoader对象，通过AdLoader的[loadAd](/ref/ads-api/ads-arkts/js-apis-advertising/js-apis-advertising#loadad)方法请求广告，最后通过[AdLoadListener](/ref/ads-api/ads-arkts/js-apis-advertising/js-apis-advertising#adloadlistener)来监听广告的加载状态。

   在请求贴片广告时，需要在[AdOptions](/ref/ads-api/ads-arkts/js-apis-advertising/js-apis-advertising#adoptions)中设置参数：totalDuration。
...
2. 展示广告。
   ...
   | onMediaError | 广告播放失败。 | - playTime：类型number，单位ms，广告播放时长，-1为异常值。  - errorCode：类型number，错误码ID。  - errorMsg：类型string，错误信息。  错误码的详细介绍请参见AVPlayer.[on('error')](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avplayer/arkts-apis-media-avplayer#onerror9)错误码。 | - |
