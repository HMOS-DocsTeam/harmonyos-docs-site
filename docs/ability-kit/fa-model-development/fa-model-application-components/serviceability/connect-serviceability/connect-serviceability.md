...
如果ServiceAbility需要与PageAbility或其他应用的ServiceAbility进行交互，则须创建用于连接的Connection。ServiceAbility支持其他Ability通过[connectAbility()](/ref/ability-api/ability-arkts/fa-model/js-apis-ability-featureability/js-apis-ability-featureability#featureabilityconnectability7)方法与其进行连接。PageAbility的connectAbility()方法定义在[featureAbility](/ref/ability-api/ability-arkts/fa-model/js-apis-ability-featureability/js-apis-ability-featureability)中，ServiceAbility的connectAbility()方法定义在[particleAbility](/ref/ability-api/ability-arkts/fa-model/js-apis-ability-particleability/js-apis-ability-particleability)中。连接ServiceAbility的规则详见[组件启动规则](/ability-kit/fa-model-development/fa-model-application-components/component-startup-rules-fa)章节。在使用connectAbility()处理回调时，需要传入目标Service的[Want](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-want/js-apis-app-ability-want)与[IAbilityConnection](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability/js-apis-inner-ability-connectoptions/js-apis-inner-ability-connectoptions)的实例。[IAbilityConnection](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability/js-apis-inner-ability-connectoptions/js-apis-inner-ability-connectoptions)提供了以下方法供开发者实现。
...
同时，Service侧也需要在[onConnect()](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability/js-apis-inner-ability-connectoptions/js-apis-inner-ability-connectoptions#onconnect)时返回[IRemoteObject](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#iremoteobject)，从而定义与Service进行通信的接口。onConnect()需要返回一个IRemoteObject对象。系统提供了IRemoteObject的默认实现，开发者可以通过继承[rpc.RemoteObject](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#remoteobject)来创建自定义的实现类。
