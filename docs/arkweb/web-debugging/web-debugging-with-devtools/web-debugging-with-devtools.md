...
Web组件支持使用DevTools工具调试前端页面。DevTools是Web前端开发调试工具，支持在电脑上调试移动设备前端页面。开发者通过[setWebDebuggingAccess()](/ref/arkweb-api/arkweb-arkts/js-apis-webview/arkts-apis-webview-webviewcontroller/arkts-apis-webview-webviewcontroller#setwebdebuggingaccess)接口开启Web组件前端页面调试能力，使用DevTools在电脑上调试移动前端网页，设备需为4.1.0及以上版本。

## 无线调试

从API version 20开始，可使用无线调试接口[setWebDebuggingAccess20+](/ref/arkweb-api/arkweb-arkts/js-apis-webview/arkts-apis-webview-webviewcontroller/arkts-apis-webview-webviewcontroller#setwebdebuggingaccess20)，来简化调试流程。
...
1. 在应用代码中开启Web调试开关，应用需要调用[setWebDebuggingAccess20+](/ref/arkweb-api/arkweb-arkts/js-apis-webview/arkts-apis-webview-webviewcontroller/arkts-apis-webview-webviewcontroller#setwebdebuggingaccess20)接口，设置TCP Socket端口号并启用Web调试功能。
   ...
   ![](../../../images/f373672f/note_3.0-zh-cn.png) 

   代码中使用的8888端口仅作为示例展示，开发者使用过程中，应保证端口号可以被应用使用。如果因为端口被占用或者应用无权限使用等因素导致端口无法被应用使用，会导致接口抛出异常或者ArkWeb无法开启调试模式。
2. 开启调试功能需要在DevEco Studio应用工程hap模块的module.json5文件中增加如下权限，添加方法请参考[在配置文件中声明权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions#在配置文件中声明权限)。
...
2. 修改Chrome调试工具的配置。
   ...
   (2) 在 "Target discovery settings" 中添加被调试设备的IP地址和[setWebDebuggingAccess20+](/ref/arkweb-api/arkweb-arkts/js-apis-webview/arkts-apis-webview-webviewcontroller/arkts-apis-webview-webviewcontroller#setwebdebuggingaccess20)接口中指定的port端口，比如：192.168.0.3:8888。

![](../../../images/96e4d59e/note_3.0-zh-cn.png) 
...
![](../../../images/a442e055/zh-cn_image_0000002583478249.jpg)

### 开始网页调试

![](../../../images/1de893d7/zh-cn_image_0000002552798600.png)

## USB连接调试

### 应用代码开启Web调试开关

调试网页前，需要应用侧代码调用[setWebDebuggingAccess()](/ref/arkweb-api/arkweb-arkts/js-apis-webview/arkts-apis-webview-webviewcontroller/arkts-apis-webview-webviewcontroller#setwebdebuggingaccess)接口开启Web调试开关。
...
2. 使用hdc命令连接上设备。
   ...
   - 如果命令返回设备的ID，表示hdc已连接上设备。
     ...
     ![](../../../images/ef85661a/zh-cn_image_0000002583438295.png)
   - 如果命令返回 [Empty]，则说明hdc还没有发现设备。

     ![](../../../images/1e5535a3/zh-cn_image_0000002552958250.jpg)
3. 进入hdc shell。
...
****推荐使用[自动映射WebView调试链接](/ide-debug-app/ide-run-debug-configurations#section48387420516)****。
...
1. 先在hdc shell里执行如下命令，查询ArkWeb在设备里创建的domain socket。
   ...
   - 如果前几步操作无误，该命令的执行结果将显示用于查询的domain socket端口。
     ...
     ![](../../../images/b0004a96/zh-cn_image_0000002583478251.jpg)
   - 如果没有查询到结果， 请再次确认。
...
2. 将查询到的domain socket转发至电脑的TCP 9222端口。
   ...
   ![](../../../images/b579c76e/note_3.0-zh-cn.png) 
   ...
   ![](../../../images/713d4602/zh-cn_image_0000002552798602.jpg)
3. 在命令行里执行如下命令，检查端口是否转发成功。
   ...
   - 如果有返回端口转发的任务，则说明端口转发成功。
     ...
     ![](../../../images/c6cafb9d/zh-cn_image_0000002583438297.png)
   - 如果返回 [Empty]， 则说明端口转发失败。

     ![](../../../images/f3636442/zh-cn_image_0000002552958252.jpg)
...
2. 修改Chrome调试工具的配置。
   ...
   ![](../../../images/8ec1b18a/zh-cn_image_0000002583478253.jpg)
3. 为了同时调试多个应用，请在Chrome浏览器的调试工具网页内，于“Devices”选项中的“configure”部分添加多个端口号。

   ![](../../../images/14d7a42c/zh-cn_image_0000002552798604.png)

### 等待发现被调试页面

如果前面的步骤执行成功，Chrome的调试页面将显示待调试的网页。

![](../../../images/25004388/zh-cn_image_0000002583478249.jpg)

### 开始网页调试

![](../../../images/8335084b/zh-cn_image_0000002552798600.png)
...
- 转发成功后，请用电脑端的Chrome浏览器打开网址 &lt;`http://localhost:9222/json&gt;` ，URL里的9222需要改为自己实际配置的TCP端口。
  ...
  - 如果网页有内容， 说明端口转发成功，请在Chrome的调试页面[等待被调试页面的出现](#等待发现被调试页面)。
    ...
    ![](../../../images/caeda554/zh-cn_image_0000002583438299.jpg)
  - 如果展示的是错误网页， 说明端口转发失败， 请参阅[端口转发不成功](#端口转发不成功)中的解决方法。

    ![](../../../images/e1d9bd15/zh-cn_image_0000002552958254.jpg)
