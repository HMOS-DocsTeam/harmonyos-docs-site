...
1. 建议配置[渲染模式](/arkweb/web-render-layout/web-render-mode)为[同步渲染模式](/arkweb/web-render-layout/web-render-mode#同步渲染模式)，避免因为组件大小超出限制导致异常场景（白屏，布局错误）。
2. 建议配置[过滚动模式(overScrollMode)](/ref/arkweb-api/arkweb-comp/ts-basic-components-web/arkts-basic-components-web-attributes/arkts-basic-components-web-attributes#overscrollmode11)为关闭状态。当过滚动模式开启时，当用户在Web界面上滑动到边缘时，Web会通过弹性动画弹回界面，会与Scroll组件的回弹相互冲突，影响体验。
3. [键盘避让(keyboardAvoidMode)](/ref/arkweb-api/arkweb-comp/ts-basic-components-web/arkts-basic-components-web-attributes/arkts-basic-components-web-attributes#keyboardavoidmode12)属性配置为RESIZE\_CONTENT时，该避让模式不生效。
4. 不支持对页面进行缩放。
...
- 内部H5页面高度超过了7680px（物理像素），但没有设置渲染模式为[同步渲染模式](/arkweb/web-render-layout/web-render-mode#同步渲染模式)。
- 未配置[metaviewport](/ref/arkweb-api/arkweb-comp/ts-basic-components-web/arkts-basic-components-web-attributes/arkts-basic-components-web-attributes#metaviewport12)属性。

****解决方案：****

- 更改渲染模式为[同步渲染模式](/arkweb/web-render-layout/web-render-mode#同步渲染模式)。
