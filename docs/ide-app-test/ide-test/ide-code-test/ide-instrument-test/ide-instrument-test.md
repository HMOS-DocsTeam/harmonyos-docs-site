...
1. 在工程目录下打开待测试模块（支持HAP、HAR、HSP模块）下的ets文件，将光标置于代码中任意位置，单击****右键 > Show Context Actions**** ****> Create Instrument Test****或快捷键****Alt+Enter**** ****（macOS为Option+Enter）> Create Instrument Test****创建测试类。
   ...
   ![](../../../../images/36ffb17a/zh-cn_image_0000002561753237.png)
2. 在弹出的Create Instrument Test窗口，输入或选择如下参数。
   ...
   ![](../../../../images/e6a9d852/zh-cn_image_0000002561833197.png)
3. DevEco Studio在ohosTest/ets/test目录下自动生成对应的测试类。在测试类中，DevEco Studio会生成对应方法的用例模板，具体测试代码需要开发者根据业务逻辑进行开发，具体请参考[自动化测试框架使用指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkxtest-guidelines)。
   ...
   ![](../../../../images/343410ea/note_3.0-zh-cn.png) 
...
![](../../../../images/c0e6df79/note_3.0-zh-cn.png) 
...
1. 创建以下目录或文件，文件内容示例可在[运行Instrument Test测试用例](#section1574003717165)后，在对应模块的.test/\{productName\}/intermediates/src/ohosTest（DevEco Studio 6.1.0 Beta1及以上版本）或build/\{productName\}/intermediates/src/ohosTest（DevEco Studio 6.1.0 Beta1以下版本）下查看，其中productName是当前生效的product，可以通过点击DevEco Studio右上方![](../../../../images/0b26d50e/zh-cn_image_0000002561753229.png)图标进行查看。
...
2. 在module.json5文件中补充ability配置字段mainElement、pages、abilities，关于字段的具体说明请参考[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)。
...
使用DevEco Studio运行测试用例前，需要将设备与电脑进行连接，将工程编译成带签名信息的HAP，再安装到真机设备或模拟器上运行，具体请参考[使用本地真机运行应用](/ide-run-device)或[使用模拟器运行应用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-run-emulator)。

可以采用运行工程目录（test）、测试文件（如Ability.test.ets）、测试套件（describe）、测试方法（it）的方式来运行测试用例：

- 在工程目录中，单击****右键 > Run'测试文件名称'****，执行测试。

  ![](../../../../images/1aa56b66/zh-cn_image_0000002561753243.png)
- 打开测试文件，单击测试套件左侧按钮。

  ![](../../../../images/9b484087/zh-cn_image_0000002561833189.png)
- 如果要根据自定义的配置执行Instrument Test，在[创建测试用例运行任务](#section65264166107)后，通过如下方式的其中之一，执行Instrument Test：
  - 在工具栏主菜单单击****Run > Run'测试名称'****。
  - 在DevEco Studio的右上角，选择测试任务，然后单击右侧的![](../../../../images/3f0ad1a6/zh-cn_image_0000002561833183.png)按钮，执行Instrument Test。

    ![](../../../../images/d5490205/zh-cn_image_0000002530913302.png)

执行完测试任务后，查看测试结果。

![](../../../../images/8d6565a5/zh-cn_image_0000002530913296.png)
...
![](../../../../images/074a63d8/zh-cn_image_0000002530753298.png)

在断点命中时，下方将出现Debug窗口。开发者可在该窗口中进行断点管理与基础调试能力的可视化操作，在断点命中时可查看当前线程的变量和堆栈信息。

![](../../../../images/aac3cfdb/zh-cn_image_0000002561833191.png)

断点命中时，在代码编辑器窗口单击右键，在弹出的菜单中将出现调试模式特有功能，如计算表达式、添加变量监视等。

![](../../../../images/f7bd7f38/zh-cn_image_0000002530753322.png)

在跳出所有断点后，测试结束，与运行模式相同，在测试窗口查看测试结果。

![](../../../../images/e757865c/zh-cn_image_0000002561833251.png)

![](../../../../images/821fff4b/note_3.0-zh-cn.png) 
...
开发者可以自定义需要参与覆盖率测试的文件，具体配置方法请参考[配置覆盖率过滤文件](/ide-app-test/ide-test/ide-ui-test#section13756446154)。
...
- 方式一：在工程目录中，选中文件，单击****右键 > Run '测试文件名称' with Coverage****，执行测试。
  ...
  ![](../../../../images/d5f520e5/zh-cn_image_0000002561833187.png)
- 方式二：在DevEco Studio的右上角，选择测试任务，然后单击右侧的![](../../../../images/c332ecb4/zh-cn_image_0000002561753249.png)按钮，执行测试。

  ![](../../../../images/e3609632/zh-cn_image_0000002530753328.png)

启动测试后，进行编译构建，底部将出现Cover窗口，构建结束后自动拉起Cover窗口，测试任务结束后，窗口中会打印测试报告的路径。

![](../../../../images/906d452a/zh-cn_image_0000002530913262.png)

点击链接可打开报告，查看ArkTS代码覆盖率详情，关于覆盖率的计算方式请参考[查看覆盖率报告](/ide-app-test/ide-test/ide-ui-test#section10394362109)。

![](../../../../images/245c60be/zh-cn_image_0000002561833207.png)

在Cover窗口中，单击rerun按钮可以按照之前的设置，重新执行覆盖率用例。

![](../../../../images/0b7334e8/zh-cn_image_0000002530913260.png)
...
2. 在****Run/Debug Configurations****界面，单击+按钮，在弹出的下拉菜单中，单击Instrument Test。
   ...
   ![](../../../../images/f090de4a/zh-cn_image_0000002561833203.png)
3. 根据实际情况，配置Instrument Test的运行参数。然后单击****OK****，完成配置。
   ...
   ![](../../../../images/7108a1f4/zh-cn_image_0000002561753275.png)

### 使用过滤条件筛选待运行的测试用例

1. 在用例编写时，通过配置it的第二个入参，为每个用例添加过滤参数。此参数用于为测试用例添加标注，不添加则参数默认为0表示未被标注。

   ![](../../../../images/840f6ae2/zh-cn_image_0000002561833199.png)
2. 打开****Run/Debug Configurations****窗口，点击Test Args![](../../../../images/6169e299/zh-cn_image_0000002530753318.png)，打开****Test Args****界面，添加命令行参数。

   ![](../../../../images/f877fe84/zh-cn_image_0000002530753288.png)

   例如将测试参数配置为level=1, size=medium

   ![](../../../../images/cfd0c262/zh-cn_image_0000002530913276.png)
...
1. 创建cpp测试目录，鼠标右键单击ohosTest目录，选择****New > C/C++ File(Napi)****，在ohosTest下生成cpp测试目录，以entry模块为例，目录结构如下。
   ...
   ![](../../../../images/6a04a560/note_3.0-zh-cn.png) 

   DevEco Studio生成的cpp测试目录中不包含C++测试框架，需要开发者自行选择开源测试框架使用。

   ![](../../../../images/619561d0/zh-cn_image_0000002561753213.png)
...
3. 运行testNative测试用例，查看测试结果。
   ...
   ![](../../../../images/9ad06dee/zh-cn_image_0000002530753304.png)
...
3. 运行覆盖率测试，选中ArkTS测试文件，单击****右键 >**** ****Run '测试文件名称' with Coverage****，执行测试。
   ...
   ![](../../../../images/666c0631/zh-cn_image_0000002530753310.png)

   启动测试后，进行编译构建，底部将出现Cover窗口，构建结束后自动拉起Cover窗口，测试任务结束后，窗口中会打印测试报告的路径。

   ![](../../../../images/5a121d36/zh-cn_image_0000002530753290.png)

   点击链接可打开报告，查看C++代码覆盖率详情。

   ![](../../../../images/f7d8658b/zh-cn_image_0000002561833213.png)
...
- coverage：是否生成覆盖率报告，缺省默认是true，在&lt;module-path&gt;/.test/default/outputs/ohosTest/reports路径下生成两份报告，一份是html格式（index.html），一份是json格式（coverageReport.json），具体参考[查看覆盖率报告](/ide-app-test/ide-test/ide-ui-test#section10394362109)。
...
![](../../../../images/fcc0e9ef/note_3.0-zh-cn.png) 
