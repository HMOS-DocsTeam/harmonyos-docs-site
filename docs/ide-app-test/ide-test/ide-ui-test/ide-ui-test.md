...
将设备与电脑进行连接，并对应用/元服务签名，具体请参考[使用本地真机运行应用](/ide-run-device)和[应用/元服务签名](/ide-signing)。
...
![](../../../images/514d1c26/zh-cn_image_0000002530752874.png)
...
![](../../../images/15509e47/note_3.0-zh-cn.png) 
...
- ****方式一：通过DevEco Studio进行编译与安装，从DevEco Studio 6.0.2 Beta1版本开始支持。****
  1. 点击菜单栏****Run**** > ****Edit Configurations**** ****>**** ****Diagnostics****，勾选****Black Coverage****，开启黑盒覆盖率测试。
     ...
     ![](../../../images/a38f37e6/note_3.0-zh-cn.png) 

     - 调试场景下，该配置不生效，运行的是未插桩的应用。
     - attach调试和等待调试场景下，该配置会导致断点不准确，建议取消该配置。

     ![](../../../images/69b3753f/zh-cn_image_0000002530752878.png)
  2. 点击工具栏![](../../../images/038a555a/zh-cn_image_0000002561752825.png)，DevEco Studio会启动编译插桩，并推包安装到设备上。
- ****方式二：通过命令行进行编译与安装****
  1. 执行hvigor插桩编译命令，编译后在\{projectPath\}/\{moduleName\}/.test/default/intermediates/ohosTest路径下会生成init\_coverage.json文件，供后续生成覆盖率报告使用。
     ...
     - moduleName：执行测试的模块。
     - targetName/productName：当前生效的target/product，可以通过点击DevEco Studio右上方![](../../../images/e51c0381/zh-cn_image_0000002561752809.png)图标进行查看。

     ![](../../../images/68b3e01f/note_3.0-zh-cn.png) 

     如果HAP依赖HSP，需要单独编译HSP，将以上命令的assembleHap替换为assembleHsp即可。
  2. 如果设备上已存在待测试的应用，先卸载应用，不存在则跳过此步骤，关于hdc工具的使用指导请参考[hdc](/system-debug-optimize/debugging-commands/hdc)。
...
- 针对UIAbility和存在onDump方法的ExtensionAbility，执行命令生成覆盖率数据。UIAbility从DevEco Studio 5.1.0 Release版本开始支持，ExtensionAbility从DevEco Studio 6.0.1 Beta1版本开始支持。
  ...
  ```
- 针对UIAbility，退出应用，触发[onDestroy()回调](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-lifecycle#ondestroy)，生成覆盖率数据。
- 针对存在onDestroy方法的ExtensionAbility，Ability销毁时，触发onDestroy()回调，生成覆盖率数据。从DevEco Studio 6.0.1 Beta1版本开始支持。
- 针对UIAbility，支持通过[EventHub接口](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-eventhub/js-apis-inner-application-eventhub)通知UIAbility生成覆盖率数据。从DevEco Studio 6.0.2 Beta1版本开始支持。
...
![](../../../images/aca35fb2/note_3.0-zh-cn.png) 
...
1. 从设备上取出覆盖率数据json文件存放到电脑本地，该命令会将cache目录下的所有文件都保存到LocalPath目录下。
   ...
   ![](../../../images/30ac3907/note_3.0-zh-cn.png) 
...
2. 生成覆盖率报告：
   ...
   ![](../../../images/3c5f5e1b/note_3.0-zh-cn.png) 
...
3. 在本地找到报告文件路径并在浏览器中打开，查看代码覆盖率详情，关于覆盖率的计算方式请参考[查看覆盖率报告](#section10394362109)。
   ...
   ![](../../../images/201ec02a/zh-cn_image_0000002530752880.png)
...
![](../../../images/4102204c/zh-cn_image_0000002530912884.png)
...
- ****行（Lines of Source Code） vs 可执行代码行（Lines of Executable Code）****
  ...
  - 对于DevEco Studio的覆盖率测试引擎来说，只会统计方法内的语句，方法外的语句都不会被统计覆盖率。
    ...
    ![](../../../images/55cadc62/note_3.0-zh-cn.png) 
...
- 测试覆盖率报告左侧的标识：
  ...
  - Nx：表示当前可执行代码行被执行了N次。
    ...
    ![](../../../images/96bb969e/zh-cn_image_0000002561832805.png)
- ****通过注释语法忽略指定代码****

  代码中的某些分支可能很难、甚至无法测试，DevEco Studio提供了instrument ignore \* 语法来进行忽略，使得某些代码不计入覆盖率。

  ![](../../../images/0faab9a1/note_3.0-zh-cn.png) 
...
- ****functions****
  ...
  - ****regions****
    ...
    - 如果方法内没有任何实现，是个空方法，则regions数组只有一个元素，即方法对应的代码区域，示例如下。
      ...
      ![](../../../images/98f416b6/zh-cn_image_0000002530912892.png)
    ...
    - 如果方法内只有一个代码区域，则regions数组有两个元素，示例如下。
      ...
      ![](../../../images/9c9726ad/zh-cn_image_0000002530752892.png)
    ...
    - 如果方法内存在多个代码区域，则每新增一个代码区域，regions数组就增加一个元素，示例如下。
      ...
      ![](../../../images/cc73ab6a/zh-cn_image_0000002561832813.png)
  ...
  - ****branches****
    ...
    ****示例一：****调用eeee(2)。
    ...
    ![](../../../images/d4253eae/zh-cn_image_0000002530752884.png)
    ...
    ****示例二：****调用bbb(2)和bbb(-1)，该方法触发两次。
    ...
    ![](../../../images/451fef53/zh-cn_image_0000002530912866.png)
...
- ****exeLine****
  ...
  ![](../../../images/9b56c8eb/zh-cn_image_0000002530752896.png)
