...
模型加载后，可以通过ArkUI的[Component3D](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-component3d/ts-basic-components-component3d)渲染组件呈现给用户，Component3D负责将ArkGraphics 3D场景渲染到界面中。在自定义场景模式下，开发者可以使用ArkTS API创建并管理相机和光源节点，从而设置合适的观察角度和光照效果；在自动场景模式下，框架会根据模型自动创建基础相机和光照。ArkTS API可通过napi调用AGP中由C++实现的相应能力。

![](../../images/c22b1c18/zh-cn_image_0000002552799054.png)
