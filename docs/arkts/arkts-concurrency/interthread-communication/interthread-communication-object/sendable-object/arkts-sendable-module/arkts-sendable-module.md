...
- 共享模块导出的变量必须是可共享对象。
  ...
  共享模块在并发实例间可共享，因此导出的所有对象必须是可共享的。可共享对象参考[Sendable支持的数据类型](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable#sendable支持的数据类型)。
- 共享模块不支持re-export写法。
