...
Sendable数据不得持有非Sendable数据，因此Sendable类或接口的成员变量必须是[Sendable支持的数据类型](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable#sendable支持的数据类型)。
...
![](../../../../../../images/7eefb571/note_3.0-zh-cn.png) 
...
通过在[工程级build-profile.json5文件](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app)的"buildOption"字段下的"strictMode"中增加"disableSendableCheckRules"字段，配置该能力。
...
![](../../../../../../images/5eb1a658/note_3.0-zh-cn.png) 
...
![](../../../../../../images/d98c1c60/note_3.0-zh-cn.png) 

改变属性类型不包括Sendable对象类型的改变，例如从Sendable class A变为Sendable class B。

### NAPI规则（目前只针对Sendable对象）

NAPI相关接口请参考[Sendable相关的NAPI接口](/coding/using-napi-interaction-with-cpp/napi-use/use-napi-about-extension#sendable相关)，具体使用请参考[Native与Sendable ArkTS对象绑定](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-sendable-napi)。
...
Sendable数据需要与[makeObserved](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-uiutils/arkts-new-makeobserved)配合使用，才可以观察Sendable对象的数据变化，具体使用请参考[makeObserved和@Sendable装饰的class配合文档](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-uiutils/arkts-new-makeobserved#makeobserved和sendable装饰的class配合使用)。

## 在HAR包中的使用规则

Sendable可在[HAR](/development-fundamentals/application-package-fundamentals/application-package-dev/har-package)包中使用。当在字节码HAR中使用Sendable时，无需进行额外配置，可直接使用。当在TS HAR中使用Sendable时，需在HAR模块下的module.json5文件中将"metadata"字段下的"name"设置为“UseTsHar”，配置如下所示。
