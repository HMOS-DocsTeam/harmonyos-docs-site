...
ArkTS支持的模块类型包括ets/ts/js文件、json文件、Native模块。ArkTS中支持[ECMAScript模块规范](#ecmascript模块)及[CommonJS模块规范](#commonjs模块)，此外，ArkTS也对加载方式进行了拓展，包含[动态加载](/arkts/arkts-runtime/arkts-runtime-module/arkts-dynamic-import)、静态加载、[延迟加载](/arkts/arkts-runtime/arkts-runtime-module/arkts-lazy-import)、[同步动态加载Native模块](/arkts/arkts-runtime/arkts-runtime-module/js-apis-load-native-module)和[Node-API接口加载文件](/arkts/arkts-runtime/arkts-runtime-module/load-module-base-nodeapi)。
...
![](../../../../images/265764ae/zh-cn_image_0000002552957540.png)

A文件称为入口文件，即执行起点。一些内置的加载接口，如[windowStage.loadContent](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-window/arkts-apis-window-window#loadcontent9)和[路由跳转](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-architecture)等页面拉起接口（即不是通过import写法拉起的文件），入参文件都会作为入口文件执行。

以A文件为入口，会加载一整套文件，包括A文件及其依赖文件，以及这些文件后续依赖的文件，直到各分支的叶节点。

普通模块在同一线程内只加载一次，而在不同线程中会加载多次，每个线程都会生成新的模块对象。如果需要在进程内只加载一次，请使用[共享模块](/arkts/arkts-concurrency/interthread-communication/interthread-communication-object/sendable-object/arkts-sendable-module)。
...
ArkTS中export和import用法详见[ArkTS语言介绍](/learning-arkts/introduction-to-arkts#模块)。
...
![](../../../../images/95d0b6a4/note_3.0-zh-cn.png) 
...
Native模块（so）的导入导出与加载ets/ts/js语法规格一致。详情请参考：[静态方式加载native模块](/arkts/arkts-runtime/arkts-runtime-module/arkts-import-native-module)。

![](../../../../images/31c6f6c5/note_3.0-zh-cn.png) 
