...
1. 在[工程级build-profile.json5文件](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app)中的useNormalizedOHMUrl配置为true。
...
2. 清除ohpm和编译缓存，下载ohpm包并重新编译，操作如下：
   ...
   - 点击菜单Build->Clean Project，或在控制台执行[ohpm clear](/ide-ohpm-cli/ide-ohpm-common-commands/ide-ohpm-clean)。
   - 点击菜单File->Invalidate Caches...，弹窗选项全选执行并重启。
   - 在控制台执行[ohpm install](/ide-ohpm-cli/ide-ohpm-common-commands/ide-ohpm-install)。
...
![](../../../../images/da198b08/note_3.0-zh-cn.png) 

由于此类异常与代码结构和运行环境相关，目前无法提供完整列表；如上述情况均无法解释，可结合崩溃日志或使用[GDB](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-usage-of-the-commissioning-tool/cannkit-commissioning-functions/cannkit-more-functions/cannkit-gdb)/[LLDB](/debugging-profiling/debug-lldb)进一步定位。
