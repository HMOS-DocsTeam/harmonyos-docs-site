...
准备一个操作方舟字节码文件的动态库文件，在工程的配置文件build-profile.json5中[配置编译选项transformLib](/arkts/arkts-compilation-tool-chain/arkoptions-guide#transformlib)，选项值为这个动态库的路径，编译器会在指定时机加载该动态库，并执行其中指定的Transform方法。
...
3. 在DevEco Studio中配置build-profile.json5的transformLib选项（以Windows环境为例）。
   ...
   ![](../../../../images/f51f3e8b/zh-cn_image_0000002583477545.png)
