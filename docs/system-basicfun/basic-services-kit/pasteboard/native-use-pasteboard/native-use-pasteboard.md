...
- [****OH\_PasteboardObserver****](/ref/system-basicfun-api/basic-services-api/basic-services-c/basic-services-struct/capi-pasteboard-oh-pasteboardobserver/capi-pasteboard-oh-pasteboardobserver)：剪贴板数据变更观察者对象，用以监听剪贴板数据变更事件。
- [****OH\_Pasteboard****](/ref/system-basicfun-api/basic-services-api/basic-services-c/basic-services-struct/capi-pasteboard-oh-pasteboard/capi-pasteboard-oh-pasteboard)：剪贴板对象，用来进行查询、写入等操作。
- [****OH\_UdmfData****](/ref/arkdata-api/arkdata-c/arkdata-struct/capi-udmf-oh-udmfdata/capi-udmf-oh-udmfdata)：统一数据对象。

## 约束限制

- 剪贴板内容包含剪贴板系统服务元数据和应用设置的数据，总大小上限默认为128MB，PC/2in1设备可通过系统配置修改上限，有效范围为128MB~2GB。
- 为保证剪贴板数据的准确性，同一时间只能支持一个复制操作。
- 当前支持的数据类型：纯文本类型(OH\_UdsPlainText)、超文本标记语言类型(OH\_UdsHtml)、文件Uri类型(OH\_UdsFileUri)、像素图片类型(OH\_UdsPixelMap)、超链接类型(OH\_UdsHyperlink)、桌面图标类型(OH\_UdsAppItem)、自定义类型。ArkTS接口与NDK接口支持数据类型不完全一致，使用时须匹配接口支持类型，详情见[ArkTS接口与NDK接口数据类型对应关系](/system-basicfun/basic-services-kit/pasteboard/use-pasteboard-to-copy-and-paste#arkts接口与ndk接口数据类型对应关系)。
...
- API version 12中新增的复制、粘贴接口[setUnifiedData](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/data-file-processing-arkts/js-apis-pasteboard/js-apis-pasteboard#setunifieddata12)/[getUnifiedData](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/data-file-processing-arkts/js-apis-pasteboard/js-apis-pasteboard#getunifieddata12)与本文档中的复制、粘贴接口[OH\_Pasteboard\_SetData](/ref/system-basicfun-api/basic-services-api/basic-services-c/basic-services-headerfile/capi-oh-pasteboard-h/capi-oh-pasteboard-h#oh_pasteboard_setdata)/[OH\_Pasteboard\_GetData](/ref/system-basicfun-api/basic-services-api/basic-services-c/basic-services-headerfile/capi-oh-pasteboard-h/capi-oh-pasteboard-h#oh_pasteboard_getdata)当前相互独立，进行写入、读取操作时请使用对应配套接口。

## 接口说明

详细接口见[Pasteboard文档](/ref/system-basicfun-api/basic-services-api/basic-services-c/basic-services-module/capi-pasteboard/capi-pasteboard)。
