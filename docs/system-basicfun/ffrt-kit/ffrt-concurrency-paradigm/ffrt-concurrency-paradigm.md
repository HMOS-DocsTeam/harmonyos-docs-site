...
![](../../../images/5c00a2ec/zh-cn_image_0000002552798806.png)

串行队列并发范式开发样例可以参考[串行队列(C)](/system-basicfun/ffrt-kit/ffrt-example-c/ffrt-concurrency-serial-queue-c)/[串行队列(C++)](/system-basicfun/ffrt-kit/ffrt-example-cpp/ffrt-concurrency-serial-queue-cpp)
...
![](../../../images/cccf15cd/zh-cn_image_0000002583438501.png)

并发队列并发范式开发样例可以参考[并发队列(C)](/system-basicfun/ffrt-kit/ffrt-example-c/ffrt-concurrency-concurrent-queue-c)/[并发队列(C++)](/system-basicfun/ffrt-kit/ffrt-example-cpp/ffrt-concurrency-concurrent-queue-cpp)
...
![](../../../images/743d1100/zh-cn_image_0000002552958456.png)

图依赖并发范式开发样例可以参考[图依赖并发(C)](/system-basicfun/ffrt-kit/ffrt-example-c/ffrt-concurrency-graph-c)/[图依赖并发(C++)](/system-basicfun/ffrt-kit/ffrt-example-cpp/ffrt-concurrency-graph-cpp)
...
1. ****多线程协作****: 在许多实际应用中，某些功能需要在特定环境进行运行，而其他功能可以在任何环境运行，这个时候需要多线程协作，部分功能在A线程运行，然后回到B线程，最后再回到A线程。
   ...
   ![](../../../images/a7006c93/zh-cn_image_0000002583478457.png)
2. ****动态并发调度****: 有些场景并发任务数量动态变化，时多时少，所以可以通过动态调整worker数量来最大提升性能，降低调度开销。

   ![](../../../images/4600f7a9/zh-cn_image_0000002552798808.png)
...
协作并发范式开发样例可以参考[任务伙伴(C++)](/system-basicfun/ffrt-kit/ffrt-example-cpp/ffrt-concurrency-job-partner-cpp)
