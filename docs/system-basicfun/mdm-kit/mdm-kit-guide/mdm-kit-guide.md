...
MDM Kit为[MDM应用](/system-basicfun/mdm-kit/mdm-kit-term#mdm应用设备管理应用)提供设备管理能力，包括企业设备管理与事件监听、应用管理、禁用管理、安全管理、设备设置、设备控制、设备信息获取、硬件外设管理、系统管理、网络通信管理等，具体API接口说明详见[API参考](/ref/system-basicfun-api/mdm-api/mdm-arkts/mdm-arkts)。

设备管理应用：具备[企业设备管理扩展能力](/system-basicfun/mdm-kit/mdm-kit-admin)的应用。
...
请参阅[EnterpriseAdminExtensionAbility开发指南](/system-basicfun/mdm-kit/mdm-kit-admin)完成EnterpriseAdminExtensionAbility的创建。

### 声明接口所需权限

在申请权限前，请保证符合[权限使用的基本原则](/system-security/access-control/app-permission-mgmt/app-permission-mgmt-overview#权限使用的基本原则)。然后在工程Module对应的[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)配置文件中"requestPermissions"标签下声明要使用的接口所需的权限。例如：
...
![](../../../images/c46239ad/note_3.0-zh-cn.png) 

所需要申请的权限请参考具体接口，这里提供了[企业设备管理](/ref/system-basicfun-api/mdm-api/mdm-arkts/js-apis-enterprise-adminmanager/js-apis-enterprise-adminmanager)的链接，可基于该文档查看MDM Kit内其他API文档。
...
![](../../../images/34590e04/note_3.0-zh-cn.png) 
