...
以下内容介绍了使用[Text](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text)和[Span](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-span/ts-basic-components-span)组件进行文本显示时可能遇到的常见问题，包括文本截断、添加标签、显示表情等场景的处理方法。
...
![](../../../../images/f6cee42e/zh-cn_image_0000002583478167.gif)
...
设置[wordBreak](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#wordbreak11)属性为WordBreak.BREAK\_ALL，任意2个字符间断行使文本内容尽量占满组件区域。
...
![](../../../../images/6170feba/zh-cn_image_0000002552798518.gif)

### Text组件如何实现行末展开样式

****解决措施****

自行测算截断字符，并在行末添加...展开或者...图标作为组件内容。实现方式请参考[属性字符串转Paragraph数组](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-measureutils/arkts-apis-uicontext-measureutils#getparagraphs20)、[文本展开折叠](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-text-expand-collapse)。
...
设置[heightAdaptivePolicy](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#heightadaptivepolicy10)为TextHeightAdaptivePolicy.LAYOUT\_CONSTRAINT\_FIRST，该模式会删除超过布局约束的行，从而实现类似设置maxLines的效果。
...
![](../../../../images/aa18cde8/zh-cn_image_0000002583438213.gif)

### 在文本前后添加自定义标签

****问题现象****

如何在文本的前后各添加一个标签，例如“专题”或“Top1”，且这些标签的[背景设置](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/basic-property/ts-universal-attributes-background/ts-universal-attributes-background)、[尺寸设置](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/layout-property/ts-universal-attributes-size/ts-universal-attributes-size)需要能够自定义。

****解决措施一****

如果标签和中间的长文本需在同一行显示，开发者可能会考虑使用[Span](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-span/ts-basic-components-span)实现，但是Span不支持设置尺寸。此时，可以在[弹性布局 (Flex)](/arkui/arkts-ui-development/arkts-layout-development/arkts-build-layout/arkts-layout-development-flex-layout)或者[Row](/ref/arkui-api/arkui-declarative-comp/rows-columns-and-stacking/ts-container-row/ts-container-row)中放置标签和长文本，并为长文本设置[textOverflow](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#textoverflow)属性，以确保文本超长时能够自适应截断，显示在一行之内。
...
实现案例请参考[实现热搜榜](/arkui/arkts-ui-development/arkts-use-text/arkts-common-components-text-display#实现热搜榜)，该示例中，文字“1”、“爆”就是“我是热搜词条”的两个标签。这种实现方式写法简便，适合单行文本添加标签的场景。

****解决措施二****

如果需在多行文本前后添加标签并且不截断文本，上述方案会导致三个Text中的文本不能对齐，因为多行文本会在Row的宽度内折行。此时，可以在[层叠布局 (Stack)](/arkui/arkts-ui-development/arkts-layout-development/arkts-build-layout/arkts-layout-development-stack-layout)中放置标签和长文本，给中间多行文本设置首行文本缩进距离[textIndent](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#textindent10)。多行文本后面的标签则需要通过[offset](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/layout-property/ts-universal-attributes-location/ts-universal-attributes-location#offset)属性调整位置。这种实现方式，可以让三个Text组件中的文字水平对齐。实现步骤如下：

实现步骤：

1.将标签和长文本放在Stack中。

2.在组件显示之前的回调[aboutToAppear](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-lifecycle/ts-custom-component-lifecycle#abouttoappear)中，使用[measureTextSize](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-measureutils/arkts-apis-uicontext-measureutils#measuretextsize12)计算前标签的宽度，作为中间多行文本的首行缩进距离。

3.在组件显示之前的回调aboutToAppear中，通过[getParagraphs](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-measureutils/arkts-apis-uicontext-measureutils#getparagraphs20)计算中间多行文本最后一行的宽度、除最后一行文本之外的高度，作为后标签的偏移量offset。
...
![](../../../../images/9ed93c5b/zh-cn_image_0000002552958168.png)
...
使用正则表达式解析表情符号，再将表情符号与图片资源建立映射，通过[Span](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-span/ts-basic-components-span)和[ImageSpan](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-imagespan/ts-basic-components-imagespan)同时展示表情和文字。
...
![](../../../../images/c389c64f/zh-cn_image_0000002583478169.png)

### 文本超长时如何展示

****问题现象****

Text组件中内容过多，超出父组件容器[Column](/ref/arkui-api/arkui-declarative-comp/rows-columns-and-stacking/ts-container-column/ts-container-column)的高度，会导致文本溢出容器边界，如何让文本显示在父组件容器的区域内？

****解决措施一****

Text文本是自动折行的，当没有限制Text高度[height](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/layout-property/ts-universal-attributes-size/ts-universal-attributes-size#height)时，Text高度在文本的行数增加时自动调整。可以通过设置[maxLines](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#maxlines)属性限制文本的最大行数，如果有多余的文本默认会被截断。也可以通过[textOverflow](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#textoverflow)属性来指定截断方式。
...
![](../../../../images/526e1ca7/zh-cn_image_0000002552798520.png)

****解决措施二****

上述方法会导致部分文本被裁剪掉，如果需要保留全部文本，可以把Text组件放在滚动容器[Scroll](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-scroll/ts-container-scroll)内，再通过手势滑动来浏览全部文本，具体示例如下：
...
![](../../../../images/b18a4aa2/zh-cn_image_0000002583438215.gif)

### selection如何触发弹出自定义菜单并设置菜单字体大小

****问题现象****

在[bindSelectionMenu](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#bindselectionmenu11)自定义选择菜单中，可通过TextResponseType设置文本选择菜单的响应类型。通过[selection](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#selection11)如何触发弹出自定义菜单并设置菜单字体大小。

****解决措施****

若希望由selection触发自定义菜单，可将TextResponseType设置为DEFAULT。同时，在[Menu](/ref/arkui-api/arkui-declarative-comp/menus/ts-basic-components-menu/ts-basic-components-menu)组件上通过配置font属性，即可自定义菜单的字体大小，灵活适配界面设计需求。
...
![](../../../../images/516d66bf/zh-cn_image_0000002552958170.gif)

### 如何屏蔽文本的长按手势

****问题现象****

配置[CopyOptions](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#copyoptions9)将文本设置为可选择，此时长按文本会选择文字内容并弹出系统菜单，如何使长按手势不生效。
...
Text组件调用[fontWeight](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-text/ts-basic-components-text#fontweight12)接口，[FontSettingOptions](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-text-common/ts-text-common#fontsettingoptions12对象说明)类型的入参options设置enableVariableFontWeight值为true之后，在系统设置的“字体大小和界面缩放”页面中调节字体粗细，Text组件的字重不跟随变化。

****解决措施****

1. 如果需要Text组件字重跟随系统设置变化，需要在[configuration标签](/development-fundamentals/application-configuration-file-stage/app-configuration-file#configuration标签)设置followSystem。
...
TextInput被遮挡时，如果通过[TextInputController](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-textinput/ts-basic-components-textinput#textinputcontroller8)设置焦点和选中区域会出现上述现象。涉及的选中区域设置接口包括[selectAll](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-textinput/ts-basic-components-textinput#selectall11)和[setTextSelection](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-textinput/ts-basic-components-textinput#settextselection10)。建议检查应用代码中以下接口的调用时机，在输入框被遮挡后避免设置选中区域。

| 组件 | 接口 |
| --- | --- |
| Search | [setTextSelection](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-search/ts-basic-components-search#settextselection12) |
| TextArea | [setTextSelection](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-textarea/ts-basic-components-textarea#settextselection10) |
| TextInput | [selectAll](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-textinput/ts-basic-components-textinput#selectall11) |
| TextInput | [setTextSelection](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-textinput/ts-basic-components-textinput#settextselection10) |
...
![](../../../../images/69724610/zh-cn_image_0000002583478171.gif)
