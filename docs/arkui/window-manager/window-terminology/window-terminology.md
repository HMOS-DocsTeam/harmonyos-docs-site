...
- 智慧多窗悬浮窗是指设备屏幕上悬浮的、非全屏的应用窗口。
  ...
  相关参考：[智慧多窗简介](/arkui/window-manager/multi-window-guide/multi-window-intro)、[智慧多窗最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-window-practice)。
...
相关参考：[设置全局悬浮窗](/arkui/window-manager/application-window-stage#设置全局悬浮窗受限开放)。
...
![](../../../images/ef900fdd/zh-cn_image_0000002552958202.png)
...
![](../../../images/7427c974/zh-cn_image_0000002583478203.png)
...
非[自由窗口](#自由窗口)可以通过调用[setWindowLayoutFullScreen](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-window/arkts-apis-window-window#setwindowlayoutfullscreen9)进入沉浸式布局。
...
应用窗口进入沉浸式布局之后，页面的所有组件布局范围从安全区域扩展为整个窗口。可以通过[isImmersiveLayout](/ref/arkui-api/arkui-arkts/window-manager-api/js-apis-window/arkts-apis-window-window/arkts-apis-window-window#isimmersivelayout20)判断当前窗口是否处于沉浸式布局。
...
全局坐标系是指在设备连接[扩展屏](/arkui/display-manager/display-terminology#扩展屏)（多物理屏幕）的场景下，以主屏幕左上角为原点(0, 0)，屏幕右侧为x轴正方向，屏幕下侧为y轴正方向，对窗口、指针等对象的位置进行统一描述的坐标体系。

在该坐标系中，所有物理屏幕被映射到同一连续的虚拟坐标空间内，各类窗口操作、坐标转换及窗口矩形变化事件均基于该坐标空间进行计算和回调。

![](../../../images/f84ce605/zh-cn_image_0000002552798556.png)
