...
ForEach接口基于数组循环渲染，需要与容器组件配合使用，且接口返回的组件应当是允许包含在ForEach父容器组件中的子组件。例如，ListItem组件要求ForEach的父容器组件必须为[List组件](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)。

API参数说明见：[ForEach API参数说明](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-foreach/ts-rendering-control-foreach)。

![](../../../../images/2a793a95/note_3.0-zh-cn.png) 
...
![](../../../../images/dd67a7c0/zh-cn_image_0000002552957644.png)

![](../../../../images/480d155c/note_3.0-zh-cn.png) 
...
![](../../../../images/86bb4535/zh-cn_image_0000002583477645.png)
...
![](../../../../images/b1e3c290/zh-cn_image_0000002552797996.png)
...
![](../../../../images/161e5fc1/zh-cn_image_0000002583437691.gif)

从本例可以看出[@State](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-state)能够监听到简单数据类型数组simpleList数组项的变化。
...
![](../../../../images/7bf68b67/zh-cn_image_0000002552957646.png)
...
![](../../../../images/55aa02a4/zh-cn_image_0000002583477647.png)

在本示例中，ArticleCardChangeSource组件作为ArticleListViewChangeSource组件的子组件，通过[@Prop](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-prop)装饰器接收一个ArticleChangeSource对象，用于渲染文章卡片。
...
当数据源的数组项为对象数据类型，并且只修改某个数组项的属性值时，由于数据源为复杂数据类型，ArkUI框架无法监听到@State装饰器修饰的数据源数组项的属性变化，从而无法触发ForEach的重新渲染。为实现ForEach子组件重新渲染，需要结合[@Observed和@ObjectLink](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-observed-and-objectlink)装饰器使用。例如，在文章列表卡片上点击“点赞”按钮，从而修改文章的点赞数量。
...
![](../../../../images/0a781e1b/zh-cn_image_0000002552797998.png)
...
在List组件下使用ForEach，并设置[onMove](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/interaction-property/ts-universal-attributes-drag-sorting/ts-universal-attributes-drag-sorting#onmove)事件，每次迭代生成一个ListItem时，可以使能拖拽排序。拖拽排序离手后，如果组件位置发生变化，将触发onMove事件，上报组件移动原始索引号和目标索引号。在onMove事件中，需要根据上报的起始索引号和目标索引号修改数据源。数据源修改前后，要保持每个数据的键值不变，只是顺序发生变化，才能保证落位动画正常执行。
...
![](../../../../images/8325edb6/zh-cn_image_0000002583437693.gif)

注释掉onMove事件调用中的两行代码，点击Add one item触发渲染后的效果如下图所示。

****图9**** ForEach拖拽排序效果在重新渲染后没有保留

![](../../../../images/21087f23/zh-cn_image_0000002552957648.png)
...
- ForEach在滚动容器组件 [List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)、[Grid](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-grid/ts-container-grid)、[Swiper](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-swiper/ts-container-swiper)以及[WaterFlow](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-waterflow/ts-container-waterflow) 内使用的时候，不建议与[LazyForEach](/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-lazyforeach) 同时使用。
- 在大量子组件的场景下，ForEach可能会导致卡顿。请考虑使用[LazyForEach](/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-lazyforeach)替代。最佳实践请参考[使用懒加载优化性能](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-lazyforeach-optimization)。
...
![](../../../../images/f0c89f19/zh-cn_image_0000002583477649.gif)
...
![](../../../../images/3cc13b2b/zh-cn_image_0000002552798000.gif)

点击“Insert Item After First Item”文本组件后，DevEco Studio的日志打印结果如下所示。

****图12**** 渲染性能降低案例日志打印图

![](../../../../images/ef3ddb4a/zh-cn_image_0000002583437695.png)

插入新项后，ForEach为new item、 two、 three三个数组项创建了对应的ReducedChildItem组件，并执行了组件的[aboutToAppear()](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-lifecycle/ts-custom-component-lifecycle#abouttoappear)生命周期函数。这是因为：
...
![](../../../../images/037ce1a5/zh-cn_image_0000002552957650.png)
...
![](../../../../images/d3f4e232/zh-cn_image_0000002583477651.png)

****图15**** 自定义键值生成函数下的内存占用

![](../../../../images/0228e27f/zh-cn_image_0000002552798002.png)
...
![](../../../../images/adacfc12/zh-cn_image_0000002583437697.png)
