...
![](../../../../images/4c4e2e3b/note_3.0-zh-cn.png) 

- Repeat从API version 12开始支持。
- 本文档仅为开发指南。组件接口规范见[Repeat API参数说明](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-repeat/ts-rendering-control-repeat)。
...
![](../../../../images/3704a3bf/note_3.0-zh-cn.png) 

Repeat与[LazyForEach](/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-lazyforeach)组件的区别：

- Repeat直接监听状态变量的变化，而LazyForEach需要开发者实现[IDataSource](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-lazyforeach/ts-rendering-control-lazyforeach#idatasource)接口，手动管理子组件内容/索引的修改。
...
- Repeat必须在滚动类容器组件内使用，仅有[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)、[ListItemGroup](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-listitemgroup/ts-container-listitemgroup)、[Grid](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-grid/ts-container-grid)、[Swiper](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-swiper/ts-container-swiper)以及[WaterFlow](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-waterflow/ts-container-waterflow)组件支持Repeat懒加载场景。

  循环渲染只允许创建一个子组件，子组件应当是允许包含在容器组件中的子组件。例如：Repeat与[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)组件配合使用时，子组件必须为[ListItem](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-listitem/ts-container-listitem)组件。
- Repeat[懒加载模式](#懒加载能力说明)不支持与[状态管理（V1）](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-overview#状态管理v1)配合使用，否则会导致渲染异常。
- Repeat当前不支持动画效果。
- 滚动容器组件内只能包含一个Repeat。以List为例，不建议同时包含ListItem、ForEach、LazyForEach，不建议同时包含多个Repeat。
- 当Repeat与自定义组件或[@Builder](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-extend-components/arkts-builder)函数混用时，必须将RepeatItem类型整体进行传参，组件才能监听到数据变化。详见[与@Builder混用时状态变量未刷新](#与builder混用时状态变量未刷新)。
- Repeat子组件复用时不会触发[aboutToRecycle](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-lifecycle/ts-custom-component-lifecycle#abouttorecycle10)、[aboutToReuse](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-lifecycle/ts-custom-component-lifecycle#abouttoreuse10)生命周期。

![](../../../../images/9cd26677/note_3.0-zh-cn.png) 

Repeat功能依赖数组属性的动态修改。如果数组对象被密封（sealed）或冻结（frozen），将导致Repeat部分功能失效，因为密封操作会禁止对象扩展属性并锁定现有属性的配置。

常见触发场景：

1）可观察数据的转换：使用[makeObserved](/ref/arkui-api/arkui-arkts/ui/js-apis-statemanagement/js-apis-statemanagement#makeobserved)将普通数组（如[collections.Array](/ref/arkts-api/arkts-arkts/js-apis-arkts-collections/arkts-apis-arkts-collections-array/arkts-apis-arkts-collections-array)）转换为可观察数据时，某些实现会自动密封数组。
...
![](../../../../images/8406bd4a/zh-cn_image_0000002552798014.png)

****多种类型子组件****

Repeat提供渲染模板（template）能力，可以在同一个数据源中渲染多种子组件。每个数据项会根据[.templateId()](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-repeat/ts-rendering-control-repeat#templateid)得到template type，从而渲染type对应的.template()中的子组件。

![](../../../../images/036bda6b/note_3.0-zh-cn.png) 
...
![](../../../../images/6cab16dd/zh-cn_image_0000002583437709.png)

### 键值生成规则

Repeat的[.key()](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-repeat/ts-rendering-control-repeat#key)属性为每个子组件生成一个键值。Repeat通过键值识别数组增加、删除哪些数据以及哪些数据改变了位置（索引）。

当.key()缺省时，Repeat会生成新的随机键值。当发现有重复key时，Repeat会在已有键值的基础上递归生成新的键值，直到没有重复键值。

![](../../../../images/91e43732/note_3.0-zh-cn.png) 
...
Repeat加载子节点具有懒加载和全量加载两种模式。开发者可通过设置[.virtualScroll()](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-repeat/ts-rendering-control-repeat#virtualscroll)属性选择合适的加载模式。对于长列表场景，懒加载模式支持按需加载子组件，建议开发者优先使用懒加载模式。

****懒加载模式****

使用Repeat的.virtualScroll()属性，即可使能懒加载能力。在懒加载模式下，Repeat根据当前的容器组件显示区域和预加载区域范围，按需加载子组件。如下图所示：

![](../../../../images/e4209d35/zh-cn_image_0000002552957664.png)

![](../../../../images/57baf47e/note_3.0-zh-cn.png) 

- 懒加载模式需要和滚动容器组件[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)、[ListItemGroup](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-listitemgroup/ts-container-listitemgroup)、[Grid](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-grid/ts-container-grid)、[Swiper](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-swiper/ts-container-swiper)或[WaterFlow](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-waterflow/ts-container-waterflow)配合使用。
- 懒加载模式需要和[状态管理（V2）](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-overview#状态管理v2)配合使用。
- 键值变化或数据变化均会触发页面刷新。
...
![](../../../../images/88947fce/note_3.0-zh-cn.png) 

- 渲染模板特性（template）不可用。
- 不受滚动容器组件的限制，可以在任意场景使用。
- 支持与[状态管理（V1）](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-overview#状态管理v1)配合使用。
...
Repeat组件默认开启节点复用功能。从API version 18开始，在懒加载模式下，可以通过配置reusable字段选择是否启用复用功能。为了提高渲染性能，建议开发者保持节点复用。代码示例见[VirtualScrollOptions](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-repeat/ts-rendering-control-repeat#virtualscrolloptions)。

从API version 18开始，Repeat支持懒加载模式下[缓存池自定义组件冻结](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-custom-components/arkts-component-freeze/arkts-custom-components-freezev2#repeat)。

![](../../../../images/4b32d6d6/note_3.0-zh-cn.png) 
...
Repeat节点复用时，不会触发子组件的[aboutToRecycle](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-lifecycle/ts-custom-component-lifecycle#abouttorecycle10)和[aboutToReuse](/ref/arkui-api/arkui-declarative-comp/custom-comp/ts-custom-component-lifecycle/ts-custom-component-lifecycle#abouttoreuse10)生命周期。
...
1. 首次渲染。
   ...
   ![](../../../../images/5b687458/zh-cn_image_0000002583477665.png)
2. 滑动场景。

   将列表向下滑动一个节点的距离，Repeat会复用缓存池中的节点。

   1）index=10的节点进入预加载区域，计算出其template type为bb。由于bb缓存池非空，Repeat会从bb缓存池中取出一个空闲节点进行复用，更新其节点属性（数据item和索引index），该子组件中涉及数据item和索引index的其他孙子组件会根据[状态管理（V2）](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-overview#状态管理v2)的规则做同步更新。
   ...
   ![](../../../../images/7ee91f34/zh-cn_image_0000002552798016.png)
3. 数据更新场景。
   ...
   ![](../../../../images/1c059b5b/zh-cn_image_0000002583437711.png)
...
![](../../../../images/36dbddb1/zh-cn_image_0000002552957666.png)

以下图中的数组变化为例，图中的item\_X表示数据项的键值key。

![](../../../../images/a7f36a87/zh-cn_image_0000002583477667.png)

根据上述判断逻辑：item\_0没有变化，item\_1和item\_2只更新了索引，item\_n1和item\_n2分别由item\_4和item\_3进行节点更新获得，item\_n3为新创建的节点。

![](../../../../images/22fbce5a/note_3.0-zh-cn.png) 

Repeat全量加载模式与[ForEach](/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-foreach)组件的区别：
...
![](../../../../images/b239a72d/zh-cn_image_0000002583437691.gif)
...
![](../../../../images/a48d969d/zh-cn_image_0000002552798018.png)
...
开发者可以设置.virtualScroll()的totalCount属性值或onTotalCount自定义方法用于计算期望加载的数据项总数，设置onLazyLoading属性实现数据精准懒加载，实现在节点首次渲染时加载对应的数据。详细说明和注意事项见[VirtualScrollOptions](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-repeat/ts-rendering-control-repeat#virtualscrolloptions)。
...
![](../../../../images/2ef8dfe0/zh-cn_image_0000002583437713.gif)
...
![](../../../../images/6d365309/zh-cn_image_0000002552957668.gif)

****示例3****

使用数据懒加载，并配合设置onTotalCount: () => \{ return this.arr.length + 1; \}，可实现数据无限懒加载。

![](../../../../images/50b3c6c4/note_3.0-zh-cn.png) 
...
![](../../../../images/d9be1421/zh-cn_image_0000002583477669.gif)

### 拖拽排序

当Repeat在[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)组件下使用，并且设置了[onMove](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/interaction-property/ts-universal-attributes-drag-sorting/ts-universal-attributes-drag-sorting#onmove)事件，Repeat每次迭代都生成一个[ListItem](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-listitem/ts-container-listitem)时，可以使能拖拽排序。Repeat拖拽排序特性从API version 19开始支持。

![](../../../../images/40791708/note_3.0-zh-cn.png) 
...
![](../../../../images/502ba978/zh-cn_image_0000002552798020.gif)

### 数据前插保持

数据前插保持，即在列表显示区域之前插入或删除数据后，保持显示区域子组件的滚动位置不变。

从API version 20开始，仅当父容器组件为[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)且[maintainVisibleContentPosition](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list#maintainvisiblecontentposition12)属性设置为true后，在List显示区域之前插入或删除数据时保持List显示区域子组件位置不变。
...
![](../../../../images/56b8be7c/zh-cn_image_0000002583437715.gif)
...
该示例代码展示了100项自定义类RepeatClazz的message字符串属性，[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)组件的[cachedCount](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list#cachedcount)属性设为2，模板'odd'和'even'的空闲节点缓存池大小分别设为3和1。运行后界面如下图所示：

![](../../../../images/0110fc51/zh-cn_image_0000002552957670.gif)
...
![](../../../../images/ec7eaa5e/zh-cn_image_0000002583477671.png)
...
在[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)容器组件中使用Repeat，示例代码如下：
...
![](../../../../images/22cf0a70/zh-cn_image_0000002552798022.gif)

****与Grid组合使用****

在[Grid](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-grid/ts-container-grid)容器组件中使用Repeat，示例如下：
...
![](../../../../images/00992064/zh-cn_image_0000002583437717.gif)

****与Swiper组合使用****

在[Swiper](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-swiper/ts-container-swiper)容器组件中使用Repeat，示例如下：
...
![](../../../../images/2c1f5ccd/zh-cn_image_0000002552957672.gif)

## 常见问题

### 显示区域外增删数据时保持滚动位置不变

下面的场景示例中，滚动列表显示区域外的增删数据操作将影响[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)列表滚动条停留的位置：
...
![](../../../../images/67657fe5/zh-cn_image_0000002583477673.gif)

以下为修正后的示例：

在部分场景中，我们不希望显示区域外的数据源增删操作或高度变化影响屏幕中[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)列表Scroller停留的位置，可以通过List组件的[onScrollIndex](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list#onscrollindex)事件对列表滚动动作进行监听，当列表发生滚动时，获取列表滚动位置。使用Scroller组件的[scrollToIndex](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-scroll/ts-container-scroll#scrolltoindex)特性，滑动到指定index位置，实现屏幕外的数据源增加/删除数据时，Scroller停留的位置不变的效果。

示例代码仅对增加数据的情况进行展示。

![](../../../../images/8b85cc17/note_3.0-zh-cn.png) 
...
![](../../../../images/4dbf68d8/zh-cn_image_0000002552798024.gif)
...
上述规范可以通过实现父组件[List](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list)/[Grid](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-grid/ts-container-grid)的[onScrollIndex](/ref/arkui-api/arkui-declarative-comp/scroll-and-swipe/ts-container-list/ts-container-list#onscrollindex)属性的回调函数完成。示例代码如下：

![](../../../../images/0ebab4d5/note_3.0-zh-cn.png) 
...
![](../../../../images/d65ae928/zh-cn_image_0000002583437719.gif)

### 与@Builder混用时状态变量未刷新

当Repeat与[@Builder](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-extend-components/arkts-builder)混用时，如果只传递RepeatItem.item或RepeatItem.index，参数值的改变不会引起@Builder函数内的UI刷新。推荐使用[按引用传递](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-extend-components/arkts-builder#按引用传递参数)，即将RepeatItem类型整体进行传参，组件才能监听到数据变化。除此之外，从API version 20开始，开发者可以通过使用[UIUtils.makeBinding()](/ref/arkui-api/arkui-arkts/ui/js-apis-statemanagement/js-apis-statemanagement#makebinding20)函数、[Binding类](/ref/arkui-api/arkui-arkts/ui/js-apis-statemanagement/js-apis-statemanagement#bindingt20)和[MutableBinding类](/ref/arkui-api/arkui-arkts/ui/js-apis-statemanagement/js-apis-statemanagement#mutablebindingt20)实现@Builder函数中状态变量的刷新。
...
![](../../../../images/92b7a410/zh-cn_image_0000002552957674.png)
