...
在阅读本文档前，建议提前阅读：[状态管理概述](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-overview)，从而对状态管理框架中AppStorage的定位有一个宏观了解。
...
AppStorage提供了API接口，允许开发者在自定义组件外手动触发AppStorage对应属性的增、删、改、查操作。建议配合[AppStorage API文档](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-state-management/ts-state-management#appstorage)阅读。最佳实践请参考[状态管理最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-status-management)。

![](../../../../../../images/c8646226/note_3.0-zh-cn.png) 
...
不涉及UI组件同步的数据处理工作，建议[通过用户首选项实现数据持久化](/arkdata/app-data-persistence/data-persistence-by-preferences)。

## 概述

AppStorage是在应用启动时创建的单例，用于提供应用状态数据的中心存储。这些状态数据在应用级别可访问。AppStorage在应用运行过程中保留其属性。

AppStorage中的属性通过唯一的字符串类型属性名（key）访问，支持与UI组件同步，并可在应用业务逻辑中被访问。其支持应用的[主线程](/ability-kit/stage-model-development/thread-model-stage)内多个[UIAbility](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability)实例间的UI状态数据共享。

AppStorage中的属性可以被双向同步，并具有不同的功能，比如数据持久化（详见[PersistentStorage](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-application-state-management/arkts-persiststorage)）。这些UI状态是通过业务逻辑实现，与UI解耦，如果希望这些UI状态在UI中使用，需要用到[@StorageProp](#storageprop)和[@StorageLink](#storagelink)。

## @StorageProp

@StorageProp与AppStorage中对应的属性建立单向数据同步。

![](../../../../../../images/afca3e61/note_3.0-zh-cn.png) 
...
| 初始化子节点 | 支持，可用于初始化[@State](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-state)、[@Link](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-link)、[@Prop](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-prop)、[@Provide](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-provide-and-consume)。 |
| 是否支持组件外访问 | 否。 |

****图1**** @StorageProp初始化规则图示

![](../../../../../../images/e3672a02/zh-cn_image_0000002583437657.png)
...
![](../../../../../../images/efa3a80d/note_3.0-zh-cn.png) 
...
![](../../../../../../images/377c4f16/zh-cn_image_0000002552957612.png)
...
3. AppStorage与[PersistentStorage](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-application-state-management/arkts-persiststorage)以及[Environment](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-application-state-management/arkts-environment)配合使用时，需要注意以下几点：

   a. 在AppStorage中创建属性后，调用PersistentStorage.[persistProp](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-state-management/ts-state-management#persistprop10)接口时，会使用AppStorage中已存在的值，并覆盖PersistentStorage中的同名属性。因此，建议使用相反的调用顺序。反例可见[在PersistentStorage之前访问AppStorage中的属性](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-application-state-management/arkts-persiststorage#在persistentstorage之前访问appstorage中的属性)。

   b. 如果在AppStorage中已创建属性，再调用Environment.[envProp](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-state-management/ts-state-management#envprop10)创建同名属性，会调用失败。因为AppStorage已有同名属性，Environment环境变量不会再写入AppStorage中，所以建议不要在AppStorage中使用Environment预置环境变量名。
   ...
   ```
4. 状态装饰器装饰的变量，改变会引起UI的渲染更新。如果改变的变量仅用于消息传递，不用于UI更新，推荐使用[emitter](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/process-thread-communi-arkts/js-apis-emitter/js-apis-emitter)方式。具体示例可见[不建议借助@StorageLink的双向同步机制实现事件通知](#不建议借助storagelink的双向同步机制实现事件通知)。
...
AppStorage是单例，其所有API均为静态方法，使用方法类似于[LocalStorage](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-application-state-management/arkts-localstorage)中对应的非静态方法。
...
![](../../../../../../images/1f3a5f09/note_3.0-zh-cn.png) 
...
![](../../../../../../images/7128b142/note_3.0-zh-cn.png) 
...
![](../../../../../../images/e3b4302b/note_3.0-zh-cn.png) 
...
使用该机制实现事件通知时，应确保AppStorage中的变量不直接被绑定到UI上，同时控制[@Watch](/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v1/arkts-v1-component-state-management/arkts-watch)函数的复杂度。如果@Watch函数执行时间过长，会影响UI刷新效率。
...
![](../../../../../../images/513da856/note_3.0-zh-cn.png) 
