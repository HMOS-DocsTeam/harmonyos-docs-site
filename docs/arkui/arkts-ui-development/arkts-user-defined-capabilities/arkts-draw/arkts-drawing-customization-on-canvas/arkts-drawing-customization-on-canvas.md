...
- 使用[CanvasRenderingContext2D](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d)对象在Canvas画布上绘制。
...
![](../../../../../images/8dad4462/zh-cn_image_0000002552798368.jpg)
...
- 离屏绘制是指将需要绘制的内容先绘制在缓存区，再将其转换成图片，一次性绘制到Canvas上，加快了绘制速度。过程为：
  ...
  具体使用参考[OffscreenCanvasRenderingContext2D](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-offscreencanvasrenderingcontext2d/ts-offscreencanvasrenderingcontext2d)对象。
...
![](../../../../../images/74025356/zh-cn_image_0000002552798368.jpg)

![](../../../../../images/af7db171/note_3.0-zh-cn.png) 
...
![](../../../../../images/9ab240d0/zh-cn_image_0000002583438063.jpg)
...
![](../../../../../images/3f65942e/zh-cn_image_0000002552958018.jpg)

- 先单独定义path2D对象构造理想的路径，再通过调用CanvasRenderingContext2D对象和OffscreenCanvasRenderingContext2D对象的stroke接口或者fill接口进行绘制，具体使用可以参考[Path2D](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-components-canvas-path2d/ts-components-canvas-path2d)对象。
...
![](../../../../../images/d245d4a5/zh-cn_image_0000002583478019.jpg)

## 画布组件常用方法

OffscreenCanvasRenderingContext2D对象和CanvasRenderingContext2D对象提供了大量的属性和方法，可以用来绘制文本、图形，处理像素等，是Canvas组件的核心。常用接口有[fill](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#fill)（对封闭路径进行填充）、[clip](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#clip)（设置当前路径为剪切路径）、[stroke](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#stroke)（进行边框绘制操作）等等，同时提供了[fillStyle](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#fillstyle)（指定绘制的填充色）、[globalAlpha](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#globalalpha)（设置透明度）与[strokeStyle](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#strokestyle)（设置描边的颜色）等属性修改绘制内容的样式。将通过以下几个方面简单介绍画布组件常见使用方法：

- 绘制基础形状。

  可以通过[arc](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#arc)（绘制弧线路径）、 [ellipse](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#ellipse)（绘制一个椭圆）、[rect](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#rect)（创建矩形路径）等接口绘制基础形状。
...
![](../../../../../images/7122921c/zh-cn_image_0000002552958020.png)

- 使用自定义字体绘制文本。

  从API version 20开始，可以通过[getGlobalInstance](/ref/arkgraphics-api/arkgraphics-arkts/js-apis-graphics-text/js-apis-graphics-text#getglobalinstance)获取应用全局字体管理器的实例，然后使用[loadFontSync](/ref/arkgraphics-api/arkgraphics-arkts/js-apis-graphics-text/js-apis-graphics-text#loadfontsync)接口从设置的路径中加载自定义字体并通过[font](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#font)（设置文本绘制中的字体样式）接口设置文本绘制中的字体样式，接着通过[fillText](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#filltext)（绘制填充类文本）、[strokeText](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#stroketext)（绘制描边类文本）等接口进行文本绘制。
...
![](../../../../../images/6943b109/zh-cn_image_0000002583478021.jpeg)

- 绘制图片和图像像素信息处理。

  可以通过[drawImage](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#drawimage)（图像绘制）、[putImageData](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#putimagedata)（使用[ImageData](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-components-canvas-imagedata/ts-components-canvas-imagedata)数据填充新的矩形区域）等接口绘制图片，通过[createImageData](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#createimagedata)（创建新的ImageData 对象）、[getPixelMap](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#getpixelmap)（以当前canvas指定区域内的像素创建[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)对象）、[getImageData](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#getimagedata)（以当前canvas指定区域内的像素创建ImageData对象）等接口进行图像像素信息处理。
...
![](../../../../../images/36495592/zh-cn_image_0000002552798372.png)

- 其他方法。

  Canvas中还提供其他类型的方法。渐变（[CanvasGradient](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-components-canvas-canvasgradient/ts-components-canvas-canvasgradient)对象）相关的方法：[createLinearGradient](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#createlineargradient)（创建一个线性渐变色）、[createRadialGradient](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#createradialgradient)（创建一个径向渐变色）等。
...
![](../../../../../images/9a3a8279/zh-cn_image_0000002583438067.jpg)
...
![](../../../../../images/5760d24b/zh-cn_image_0000002552958022.gif)

## 控制在画布组件不可见时不进行绘制

可以使用以下两种方式监听Canvas组件可见性，避免不可见时仍在无效绘制。

- 从API version 13开始，使用[setOnVisibleAreaApproximateChange](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-uicommonevent/ts-uicommonevent#setonvisibleareaapproximatechange)接口监听Canvas组件可见性。
  ...
  ![](../../../../../images/d8302c86/zh-cn_image_0000002583478023.gif)
- 从API version 17开始，使用[onVisibleAreaApproximateChange](/ref/arkui-api/arkui-declarative-comp/ts-component-general-events/component-change/ts-universal-component-visible-area-change-event/ts-universal-component-visible-area-change-event#onvisibleareaapproximatechange17)接口监听Canvas组件可见性。
  ...
  ![](../../../../../images/18cd6802/zh-cn_image_0000002552798374.gif)
...
![](../../../../../images/c55e4ddc/zh-cn_image_0000002583438069.jpg)
...
![](../../../../../images/4839304d/zh-cn_image_0000002552958024.jpg)

- 绘制可拖动的光标。

  可以通过[beginPath](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#beginpath)、[moveTo](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#moveto)、[lineTo](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#lineto)和[arc](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#arc)方法设置光标的位置，使用[stroke](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#stroke)和[fill](/ref/arkui-api/arkui-declarative-comp/canvas-drawing/ts-canvasrenderingcontext2d/ts-canvasrenderingcontext2d#fill)方法绘制光标，将是否按下和位置变化通过@Watch监听，并绑定自定义的drawCursor()方法。当拖动光标时，@Watch绑定的方法会执行绘制逻辑，计算并更新光标的颜色和位置。
...
![](../../../../../images/ba634493/zh-cn_image_0000002583478025.gif)
