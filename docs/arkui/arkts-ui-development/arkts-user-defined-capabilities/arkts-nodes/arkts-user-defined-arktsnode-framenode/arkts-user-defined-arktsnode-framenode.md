...
![](../../../../../images/0ecd5515/zh-cn_image_0000002583438051.png)

上述转换过程需要依赖额外的数据驱动，绑定至[Builder](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-extend-components/arkts-builder)中，较为复杂且性能欠佳。这类框架通常依赖于ArkUI的布局、事件处理、基础的节点操作和自定义能力。大部分组件通过自定义实现，但需结合使用部分系统组件以实现混合显示，如下图示例既使用了FrameNode的自定义方法进行绘制，又使用了系统组件Column及其子组件Text，通过BuilderNode的方式将其挂载到根节点的FrameNode上混合显示。

![](../../../../../images/be7bb90f/zh-cn_image_0000002552958006.png)

[FrameNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode)的设计初衷正是为了解决上述转换问题。FrameNode表示组件树中的实体节点，与自定义占位容器组件[NodeContainer](/ref/arkui-api/arkui-declarative-comp/custom-placeholder-comp/ts-basic-components-nodecontainer/ts-basic-components-nodecontainer)相配合，实现在占位容器内构建一棵自定义的节点树。该节点树支持动态操作，如节点的增加、修改和删除。基础的FrameNode具备设置通用属性和事件回调的功能，同时提供完整的自定义能力，涵盖自定义测量、布局和绘制等方面。

除此之外，ArkUI还提供了获取和遍历系统组件对应代理FrameNode对象的能力（下文简称代理节点）。代理节点能够用于遍历整个UI的树形结构，支持获取系统组件节点的详细信息，以及额外注册组件的事件监听回调。

## 创建和删除节点

FrameNode提供了节点创建和删除的能力。可以通过FrameNode的构造函数创建自定义FrameNode节点，通过构造函数创建的节点对应一个实体的节点。同时，可以通过FrameNode中的[dispose](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#dispose12)接口来实现与实体节点的绑定关系的解除。

![](../../../../../images/1b306c55/note_3.0-zh-cn.png) 
...
[isModifiable](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#ismodifiable12)用于查询当前节点类型是否为系统组件的代理节点。当FrameNode节点作为系统组件的代理节点的时候，该节点不可修改。即无法修改代理节点的自身属性以及其子节点的结构。

## 获取对应的RenderNode节点

FrameNode提供了[getRenderNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getrendernode)接口，用于获取FrameNode中的RenderNode。可以通过对获取到的RenderNode对象进行操作，动态修改FrameNode上绘制相关的属性，具体可修改的属性参考[RenderNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-rendernode/js-apis-arkui-rendernode)的接口。

![](../../../../../images/17ac4c57/note_3.0-zh-cn.png) 

- 无法获取系统组件代理FrameNode的RenderNode对象。
- BuilderNode中调用[getFrameNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-buildernode/js-apis-arkui-buildernode#getframenode)获取得到的FrameNode节点对象中，可以通过getRenderNode获取对应的根节点的RenderNode对象。

## 操作节点树

FrameNode提供了节点的增、删、查、改的能力，能够修改非代理节点的子树结构。可以对所有FrameNode的节点的父子节点做出查询操作，并返回查询结果。

![](../../../../../images/24131d7d/note_3.0-zh-cn.png) 
...
使用[moveTo](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#moveto18)接口可以将FrameNode节点移动到新的父节点下，从而按需改变节点树结构。

![](../../../../../images/53aa1377/note_3.0-zh-cn.png) 

当前FrameNode如果不可修改，抛出异常信息。

目标父节点为[typeNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#typenode12)时会校验子组件类型或个数，不满足抛出异常信息，限制情况请查看[typeNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#typenode12)描述。

当前不支持对无组件类型的命令式节点进行移动。

当前仅支持以下类型的[TypedFrameNode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#typedframenode12)进行移动操作：[Stack](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#stack12)、[XComponent](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#xcomponent12)。对于其他类型的节点，移动操作不会生效。
...
![](../../../../../images/e40fcc40/zh-cn_image_0000002583478007.gif)

## 设置节点通用属性和事件回调

FrameNode提供了[commonAttribute](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#commonattribute12)和[commonEvent](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#commonevent12)两个对象用于设置节点的[通用属性](/ref/arkui-api/arkui-declarative-comp/ts-component-general-attributes/ts-component-general-attributes)和[设置事件回调](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-uicommonevent/ts-uicommonevent)。

![](../../../../../images/45354d51/note_3.0-zh-cn.png) 
...
通过重写[onDraw](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#ondraw12)方法，可以自定义FrameNode的绘制内容。[invalidate](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#invalidate12)接口可以主动触发节点的重新绘制。

通过重写[onMeasure](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#onmeasure12)可以自定义FrameNode的测量方式，使用[measure](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#measure12)可以主动传递布局约束触发重新测量。

通过重写[onLayout](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#onlayout12)方法可以自定义FrameNode的布局方式，使用[layout](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#layout12)方法可以主动传递位置信息并触发重新布局。

[setNeedsLayout](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#setneedslayout12)可以将当前节点标记，在下一帧触发重新布局。

![](../../../../../images/2a5e6f6c/note_3.0-zh-cn.png) 
...
1. 使用[getFrameNodeById](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-uicontext/arkts-apis-uicontext-uicontext#getframenodebyid12)获取。
2. 使用[getFrameNodeByUniqueId](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-uicontext/arkts-apis-uicontext-uicontext#getframenodebyuniqueid12)获取。
3. 通过[无感监听](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-observer/js-apis-arkui-observer)获取。

![](../../../../../images/71a14ef2/note_3.0-zh-cn.png) 

1、当前接口提供的可查询的信息包括：

- 节点大小：[getMeasuredSize](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getmeasuredsize12)，[getUserConfigSize](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigsize12)
- 布局信息：[getPositionToWindow](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontowindow12)，[getPositionToParent](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoparent12)，[getLayoutPosition](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getlayoutposition12)，[getUserConfigBorderWidth](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigborderwidth12)，[getUserConfigPadding](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigpadding12)，[getUserConfigMargin](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigmargin12)
- 节点信息：[getId](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getid12)，[getUniqueId](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuniqueid12)，[getNodeType](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getnodetype12)，[getOpacity](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getopacity12)，[isVisible](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#isvisible12)，[isClipToFrame](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#iscliptoframe12)，[isAttached](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#isattached12)，[getInspectorInfo](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getinspectorinfo12)，[getCustomProperty](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getcustomproperty12)

2、无法获取UINode类型节点，例如：JsView节点、[Span](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-span/ts-basic-components-span)、[ContainerSpan](/ref/arkui-api/arkui-declarative-comp/text-and-input/ts-basic-components-containerspan/ts-basic-components-containerspan)、[ContentSlot](/ref/arkui-api/arkui-declarative-comp/custom-placeholder-comp/ts-components-contentslot/ts-components-contentslot)、[ForEach](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-foreach/ts-rendering-control-foreach)、[LazyForEach](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-lazyforeach/ts-rendering-control-lazyforeach)、if/else组件等。

## 获取节点位置偏移信息

FrameNode提供了查询节点相对窗口、父组件以及屏幕位置偏移的信息接口（[getPositionToWindow](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontowindow12)，[getPositionToParent](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoparent12)，[getPositionToScreen](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoscreen12)，[getGlobalPositionOnDisplay](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getglobalpositionondisplay20)，[getPositionToWindowWithTransform](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontowindowwithtransform12)，[getPositionToParentWithTransform](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoparentwithtransform12)，[getPositionToScreenWithTransform](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoscreenwithtransform12)，[getLayoutPosition](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getlayoutposition12)，[getUserConfigBorderWidth](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigborderwidth12)，[getUserConfigPadding](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigpadding12)，[getUserConfigMargin](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuserconfigmargin12)）。

[getPositionToWindow](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontowindow12)，[getPositionToParent](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoparent12)，[getPositionToScreen](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getpositiontoscreen12)三个接口获取到的位置信息关系如下图所示：

![](../../../../../images/2ba7a637/zh-cn_image_0000002552798358.png)
...
使用[dispose](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#dispose12)接口可以立即解除当前FrameNode对象对实体FrameNode节点的引用关系。

![](../../../../../images/3207a732/note_3.0-zh-cn.png) 

在调用dispose方法后，FrameNode对象不再对应任何实际的FrameNode节点。此时，若尝试调用以下查询接口：getMeasuredSize、getLayoutPosition、getUserConfigBorderWidth、getUserConfigPadding、getUserConfigMargin、getUserConfigSize，将导致应用程序触发[jscrash](/arkui/ui-debug-optimize/ui-stability/arkts-stability-guide#jscrash)。

通过[getUniqueId](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getuniqueid12)可以判断当前FrameNode是否对应一个实体FrameNode节点。当UniqueId大于0时表示该对象对应一个实体FrameNode节点。
...
从API version 20开始，使用[isDisposed](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#isdisposed20)接口查询当前FrameNode对象是否已解除与后端实体节点的引用关系，从而可以在操作节点前检查其有效性，避免潜在风险。
...
提供[NodeAdapter](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#nodeadapter12)对象替代ArkTS侧的LazyForEach功能，提供自定义节点的数据懒加载功能，实现按需迭代数据。

![](../../../../../images/7ae0ba32/note_3.0-zh-cn.png) 
...
如果FrameNode子节点中包含[LazyForEach](/ref/arkui-api/arkui-declarative-comp/state-management-and-rendering-control/ts-rendering-control-lazyforeach/ts-rendering-control-lazyforeach)节点，[getChild](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getchild15)接口支持指定子节点展开模式[ExpandMode](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#expandmode15)，以不同展开模式获取子节点。
...
可以使用[getFirstChildIndexWithoutExpand](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getfirstchildindexwithoutexpand15)和[getLastChildIndexWithoutExpand](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#getlastchildindexwithoutexpand15)获取当前节点第一个和最后一个在主节点树上的子节点的序列号，其中子节点序列号按所有子节点计算。
...
从API version 12开始，通过重写FrameNode的[onDraw](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#ondraw12)方法，可以重写默认绘制方法。

通过[concatMatrix](/ref/arkgraphics-api/arkgraphics-arkts/js-apis-graphics-drawing/arkts-apis-graphics-drawing-canvas/arkts-apis-graphics-drawing-canvas#concatmatrix12)可以调整自定义绘制画布的变换矩阵。

![](../../../../../images/a00f646a/note_3.0-zh-cn.png) 

- [getTotalMatrix](/ref/arkgraphics-api/arkgraphics-arkts/js-apis-graphics-drawing/arkts-apis-graphics-drawing-canvas/arkts-apis-graphics-drawing-canvas#gettotalmatrix12)获取的是用来记录绘制指令的临时canvas的变换矩阵。
- 如果开发者希望对画布进行预期的变换，应使用[concatMatrix](/ref/arkgraphics-api/arkgraphics-arkts/js-apis-graphics-drawing/arkts-apis-graphics-drawing-canvas/arkts-apis-graphics-drawing-canvas#concatmatrix12)而不是[setMatrix](/ref/arkgraphics-api/arkgraphics-arkts/js-apis-graphics-drawing/arkts-apis-graphics-drawing-canvas/arkts-apis-graphics-drawing-canvas#setmatrix12)，因为setMatrix会覆盖原本真实canvas上存在的变换矩阵。
...
![](../../../../../images/26b3b2cb/zh-cn_image_0000002583438053.png)

## 更新当前帧节点

从API version 21开始，通过使用frameNode的[invalidateAttributes](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#invalidateattributes21)方法，可以在当前帧触发节点更新，避免组件切换过程中出现闪烁。
...
![](../../../../../images/4051b41e/zh-cn_image_0000002552958008.png)

## 判断节点是否处于渲染状态

从API version 23开始，通过使用FrameNode的[isInRenderState](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-framenode/js-apis-arkui-framenode#isinrenderstate23)方法，判断FrameNode节点是否处于渲染状态。
...
![](../../../../../images/81e48815/zh-cn_image_0000002583478009.png)
