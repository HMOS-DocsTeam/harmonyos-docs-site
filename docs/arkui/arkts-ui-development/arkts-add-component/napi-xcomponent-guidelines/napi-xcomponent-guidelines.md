...
XComponent组件作为一种渲染组件，可用于EGL/OpenGLES和媒体数据写入，通过使用XComponent持有的“[NativeWindow](/arkgraphics-2d/native-surface/native-window-guidelines)”渲染画面，满足开发需要实现高级自定义渲染的需求，例如相机预览流的显示和游戏画面的渲染。开发者可通过指定XComponent组件的type字段来实现不同的渲染方式，分别为[XComponentType](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-appendix-enums/ts-appendix-enums#xcomponenttype10).SURFACE和XComponentType.TEXTURE。对于SURFACE类型，开发者将定制的绘制内容单独展示到屏幕上。对于TEXTURE类型，开发者将定制的绘制内容和XComponent组件的内容合成后展示到屏幕上。
...
目前ArkUI提供了三种UI组件的创建方式，分别是使用[ArkTS声明式UI描述](/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-declarative-ui-description)创建、使用[ArkTS自定义组件节点](/arkui/arkts-ui-development/arkts-user-defined-capabilities/arkts-nodes/arkts-user-defined-arktsnode-framenode)创建以及使用[NDK接口](/arkui/arkts-use-ndk/ndk-build-ui-overview)创建。
...
XComponent推荐使用两种方式获取XComponent持有Surface的生命周期，分别为在ArkTS侧使用[XComponentController](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent#xcomponentcontroller)管理Surface生命周期，以及在Native侧使用[OH\_ArkUI\_SurfaceHolder](/ref/arkui-api/arkui-c/arkui-struct/component-native-xcomponent-oh-arkui-surfaceholder/component-native-xcomponent-oh-arkui-surfaceholder)管理Surface生命周期。
...
- onSurfaceCreated回调
  ...
  ![](../../../../images/177fd2b5/zh-cn_image_0000002583437933.png)
- onSurfaceChanged回调
  ...
  ![](../../../../images/1f27f365/zh-cn_image_0000002552957888.png)
- onSurfaceDestroyed回调
  ...
  ![](../../../../images/ba77d30f/zh-cn_image_0000002583477889.png)
...
- OnSurfaceCreated回调
  ...
  ![](../../../../images/08c9d181/zh-cn_image_0000002552798240.png)
- OnSurfaceChanged回调
  ...
  ![](../../../../images/971c12bb/zh-cn_image_0000002583437935.png)
- OnSurfaceDestroyed回调
  ...
  ![](../../../../images/5068bad4/zh-cn_image_0000002552957890.png)
...
从API version 8开始，开发者可以通过基于[OH\_NativeXComponent](/ref/arkui-api/arkui-c/arkui-struct/vexcomponent-native-xcomponent-oh-nativexcomponent/vexcomponent-native-xcomponent-oh-nativexcomponent)实例相关的接口进行XComponent组件Surface的生命周期监听、获取NativeWindow实例以及监听基础事件，实现渲染绘制和响应交互功能。但使用OH\_NativeXComponent相关的接口存在以下问题：
...
- OH\_ArkUI\_SurfaceHolder
  ...
  以下只以注册touch事件为例，鼠标、按键以及更多的手势请参考[监听组件事件](/arkui/arkts-use-ndk/arkts-add-event/ndk-listen-to-component-events)。
...
- XComponent组件上的基础事件（如点击、触摸）和手势事件（如滑动、缩放）可通过ArkUI\_NodeHandle对象使用ArkUI NDK接口来监听，具体可参考[监听组件事件](/arkui/arkts-use-ndk/arkts-add-event/ndk-listen-to-component-events)。
...
以下步骤通过在ArkTS侧创建SURFACE类型的XComponent为例（Native侧如何创建XComponent组件对应的ArkUI\_NodeHandle可参考[ArkUI\_NativeNodeAPI\_1](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativenodeapi-1/capi-arkui-nativemodule-arkui-nativenodeapi-1)），描述了如何使用XComponent组件调用OH\_ArkUI\_SurfaceHolder相关接口管理Surface生命周期，并在Native侧创建EGL/GLES环境，实现在主页面绘制图形，以及可以改变图形的颜色。
...
1. 在界面中定义XComponent。
   ...
   ```
2. Node-API模块注册，具体使用请参考[Node-API开发规范](/coding/using-napi-interaction-with-cpp/napi-guidelines)。
...
5. CMakeLists，使用CMake工具链将C++源代码编译成动态链接库文件。
   ...
   ![](../../../../images/f1f7f814/zh-cn_image_0000002583477891.jpeg)
...
![](../../../../images/c7b57f78/note_3.0-zh-cn.png) 

更多AVPlayer用法请参考[AVPlayer](/ref/media-api/media-arkts/js-apis-media/arkts-apis-media-avplayer/arkts-apis-media-avplayer)。
