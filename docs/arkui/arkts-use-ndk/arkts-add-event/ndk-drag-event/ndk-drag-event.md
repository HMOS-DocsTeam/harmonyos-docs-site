...
ArkUI开发框架针对拖拽事件提供了[NODE\_ON\_PRE\_DRAG](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)，[NODE\_ON\_DRAG\_START](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)，[NODE\_ON\_DROP](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)，[NODE\_ON\_DRAG\_ENTER](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)，[NODE\_ON\_DRAG\_MOVE](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)，[NODE\_ON\_DRAG\_LEAVE](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)，[NODE\_ON\_DRAG\_END](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)等组件事件，当拖拽在不同的阶段时会触发对应的组件事件，完成对应的数据处理操作，实现期望的拖拽交互能力。
...
1. 组件拖拽设置。
   ...
   通过[OH\_ArkUI\_GetModuleInterface](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-interface-h/capi-native-interface-h#oh_arkui_getmoduleinterface)接口初始化nodeAPI，创建节点等操作均需通过nodeAPI完成。
   ...
   创建Image节点，通过[OH\_ArkUI\_SetNodeDraggable](/ref/arkui-api/arkui-c/arkui-headerfile/capi-drag-and-drop-h/capi-drag-and-drop-h#oh_arkui_setnodedraggable)设置节点可拖拽，并设置其他相关属性。
...
2. 自定义拖拽预览和背板图。
   ...
   创建[pixelMap](/ref/image-api/image-c/image-headerfile/capi-pixelmap-native-h/capi-pixelmap-native-h#oh_pixelmapnative_createpixelmap)，设置pixelMap的宽高等各项属性。设置Image节点的[ArkUI\_DragPreviewOption](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-dragpreviewoption/capi-arkui-nativemodule-arkui-dragpreviewoption)，可用于设置跟手图的圆角、角标等。
...
3. 设置相关事件。
   ...
   C API的事件通过统一的回调来接收，当收到事件时通过[eventType](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)进行区分。
...
4. 处理NODE\_ON\_DRAG\_START事件。
   ...
   在NODE\_ON\_DRAG\_START事件中，应用可以执行起拖阶段所需的操作，通常涉及处理起拖过程的数据。例如，创建[OH\_UdmfRecord](/ref/arkdata-api/arkdata-c/arkdata-struct/capi-udmf-oh-udmfrecord/capi-udmf-oh-udmfrecord)，将用于拖拽图片所需的数据imageValue以[OH\_UdsFileUri](/ref/arkdata-api/arkdata-c/arkdata-struct/capi-udmf-oh-udsfileuri/capi-udmf-oh-udsfileuri)类型添加到OH\_UdmfRecord中，接着将OH\_UdmfRecord设置到[OH\_UdmfData](/ref/arkdata-api/arkdata-c/arkdata-struct/capi-udmf-oh-udmfdata/capi-udmf-oh-udmfdata)中，最后将OH\_UdmfData设置到[DragEvent](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-dragevent/capi-arkui-nativemodule-arkui-dragevent)中。
...
5. 处理NODE\_ON\_DROP事件。
   ...
   在NODE\_ON\_DROP事件中，应用可以执行与落入阶段相关的操作，通常需要获取拖拽过程中传递的数据。例如，引用[udmf\_meta.h](/ref/arkdata-api/arkdata-c/arkdata-headerfile/capi-udmf-meta-h/capi-udmf-meta-h)头文件，获取[OH\_UdmfData](/ref/arkdata-api/arkdata-c/arkdata-struct/capi-udmf-oh-udmfdata/capi-udmf-oh-udmfdata)，判断是否存在所需的数据类型，从[OH\_UdmfRecord](/ref/arkdata-api/arkdata-c/arkdata-struct/capi-udmf-oh-udmfrecord/capi-udmf-oh-udmfrecord)中提取相应的数据，最后销毁指针。
...
1. 节点注册事件。
   ...
   创建Button节点，设置按钮相关属性，同时需要注册[NODE\_ON\_TOUCH\_INTERCEPT](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodeeventtype)事件。
...
2. 接收NODE\_ON\_TOUCH\_INTERCEPT事件。
   ...
   DragAction主动发起拖拽需通过事件触发，在NODE\_ON\_TOUCH\_INTERCEPT事件中执行发起拖拽所需的操作，通过[targetId](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeevent_gettargetid)区分不同按钮触发的事件。
...
3. 起拖阶段设置。
   ...
   在NODE\_ON\_TOUCH\_INTERCEPT事件中，需要对DragAction进行相关设置。为了主动发起拖拽，需要创建[pixelMap](/ref/image-api/image-c/image-headerfile/capi-pixelmap-native-h/capi-pixelmap-native-h#oh_pixelmapnative_createpixelmap)，设置[ArkUI\_DragPreviewOption](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-dragpreviewoption/capi-arkui-nativemodule-arkui-dragpreviewoption)和跟手点，并将拖拽过程中的文本数据设置到DragAction中。
