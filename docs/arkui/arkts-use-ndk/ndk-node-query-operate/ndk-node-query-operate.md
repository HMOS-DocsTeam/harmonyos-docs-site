...
以下场景基于[接入ArkTS页面](/arkui/arkts-use-ndk/ndk-access-the-arkts-page)章节，创建前置工程。

## 查询节点uniqueId及通过uniqueId获取节点信息

uniqueId是系统分配的唯一标识的节点Id。

从API version 20开始，使用[OH\_ArkUI\_NodeUtils\_GetNodeUniqueId](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getnodeuniqueid)接口，可以获取目标节点的uniqueId。使用[OH\_ArkUI\_NodeUtils\_GetNodeHandleByUniqueId](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getnodehandlebyuniqueid)接口，可以通过uniqueId获取目标节点的指针。
...
使用[OH\_ArkUI\_NodeUtils\_GetAttachedNodeHandleById](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getattachednodehandlebyid)接口，可以通过用户设置的id获取目标节点的指针。
...
使用[OH\_ArkUI\_NodeUtils\_MoveTo](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_moveto)接口，可以将Native节点移动到新的父节点下，从而按需改变节点树结构。

![](../../../images/8ac0bdc1/note_3.0-zh-cn.png) 

当前仅支持以下类型的[ArkUI\_NodeType](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#arkui_nodetype)进行移动操作：ARKUI\_NODE\_STACK、ARKUI\_NODE\_XCOMPONENT、ARKUI\_NODE\_EMBEDDED\_COMPONENT。对于其他类型的节点，移动操作不会生效。
...
4. 运行程序，点击按钮，Stack节点会移动到目标位置。
   ...
   ![](../../../images/a1b04e5a/zh-cn_image_0000002552958072.gif)

## 在当前即时帧触发节点属性更新

从API version 21开始，使用[OH\_ArkUI\_NativeModule\_InvalidateAttributes](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nativemodule_invalidateattributes)接口，在当前帧即时触发节点属性更新，避免组件切换过程中出现闪烁。
...
4. 运行程序，点击按钮，切换图片正常展示。
   ...
   ![](../../../images/6e42d06b/zh-cn_image_0000002583478073.png)

## 用不同的展开模式获取对应下标的子节点

NDK支持通过不同的展开方式获取目标节点下的有效节点信息。例如，在LazyForEach场景下，可以处理存在多个子节点的情况。

从API version 20开始，使用[OH\_ArkUI\_NodeUtils\_GetFirstChildIndexWithoutExpand](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getfirstchildindexwithoutexpand)接口，可以获取目标节点的第一个存在于组件树的节点。使用[OH\_ArkUI\_NodeUtils\_GetLastChildIndexWithoutExpand](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getlastchildindexwithoutexpand)接口，可以获取目标节点的最后一个存在于组件树的节点。[OH\_ArkUI\_NodeUtils\_GetChildWithExpandMode](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getchildwithexpandmode)接口，可以通过不同的节点展开模式获取对应下标的子节点。

![](../../../images/ef1b1131/note_3.0-zh-cn.png) 

节点展开方式请参考[ArkUI\_ExpandMode](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-type-h/capi-native-type-h#arkui_expandmode)，此处推荐使用ARKUI\_LAZY\_EXPAND懒展开方式，智能识别对应场景。
...
1. 通过ArkTS构造LazyForEach及ArkTS的下树节点展开场景。
   ...
   ```
2. NDK侧通过[OH\_ArkUI\_NodeUtils\_GetAttachedNodeHandleById](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nodeutils_getattachednodehandlebyid)接口获取ArkTS组件，并通过懒展开模式获取对应的子组件信息。
...
从API version 23开始，使用[OH\_ArkUI\_NativeModule\_IsInRenderState](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-h/capi-native-node-h#oh_arkui_nativemodule_isinrenderstate)接口，可以查询节点是否在渲染树上。
...
4. 运行程序，点击change text visibility后打印text是否在渲染树上。
   ...
   ![](../../../images/d2e0468c/zh-cn_image_0000002552798424.png)
