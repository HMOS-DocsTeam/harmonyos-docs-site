...
针对需要使用ArkTS侧独立能力的场景，ArkUI开发框架提供了Native侧嵌入ArkTS组件的能力，该能力依赖[ComponentContent](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-componentcontent/js-apis-arkui-componentcontent)机制，通过ComponentContent完成对ArkTS组件的封装，然后将封装对象传递到Native侧，通过Native侧的[OH\_ArkUI\_GetNodeHandleFromNapiValue](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-node-napi-h/capi-native-node-napi-h#oh_arkui_getnodehandlefromnapivalue)接口转化为ArkUI\_NodeHandle对象用于Native侧组件挂载使用。

![](../../../images/c041379b/note_3.0-zh-cn.png) 

- 通过OH\_ArkUI\_GetNodeHandleFromNapiValue接口获得的ArkUI\_NodeHandle对象只能作为子组件参数使用，如[addChild](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativenodeapi-1/capi-arkui-nativemodule-arkui-nativenodeapi-1#addchild)接口的第二个参数，将该对象使用在其他场景下，如[setAttribute](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativenodeapi-1/capi-arkui-nativemodule-arkui-nativenodeapi-1#setattribute)设置属性将不生效并返回错误码。
- 针对Native侧修改ArkTS组件的场景，需要在Native侧通过Node-API方式构建ArkTS侧的更新数据，再通过ComponentContent的[update](/ref/arkui-api/arkui-arkts/ui/ui-interface-arkui/js-apis-arkui-componentcontent/js-apis-arkui-componentcontent#update)接口更新。
- [构建自定义组件](/arkui/arkts-use-ndk/ndk-build-custom-components)时，相关函数如measureNode等无法对ArkTS模块内部的组件进行调用。

以下示例代码在[接入ArkTS页面](/arkui/arkts-use-ndk/ndk-access-the-arkts-page)章节基础上引入ArkTS的Refresh组件。

****图1**** Refresh组件挂载文本列表

![](../../../images/d1afb54d/zh-cn_image_0000002583438115.gif)
...
6. 定时器模块相关简单实现。
   ...
   ```
7. 使用[接入ArkTS页面](/arkui/arkts-use-ndk/ndk-access-the-arkts-page)章节的页面结构，将Refresh组件作为文本列表的父组件。
