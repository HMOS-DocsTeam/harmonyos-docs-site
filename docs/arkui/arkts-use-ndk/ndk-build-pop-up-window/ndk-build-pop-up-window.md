...
[通过创建自定义弹窗的内容对象显示自定义弹窗](#通过创建自定义弹窗的内容对象显示自定义弹窗)：该方式下的弹窗接口在[native\_dialog.h](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-dialog-h/capi-native-dialog-h#函数)的函数中定义。

![](../../../images/c20c7fd4/note_3.0-zh-cn.png) 

- 通过创建弹窗控制器来显示自定义弹窗，使用方式可以参考[openCustomDialogWithController](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-promptaction/arkts-apis-uicontext-promptaction#opencustomdialogwithcontroller18)接口。
- 通过创建自定义弹窗的内容对象来显示自定义弹窗，使用方式可以参考[openCustomDialog](/ref/arkui-api/arkui-arkts/ui/js-apis-arkui-uicontext/arkts-apis-uicontext-promptaction/arkts-apis-uicontext-promptaction#opencustomdialog12)接口。
- [OH\_ArkUI\_QueryModuleInterfaceByName](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-interface-h/capi-native-interface-h#oh_arkui_querymoduleinterfacebyname)用于获取指定类型的Native模块接口集合，可以通过其返回ArkUI\_NativeDialogHandle类型的数据调用Native模块中的接口。
...
- 创建弹窗控制器：
  ...
  [ArkUI\_NativeDialogHandle](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialog8h/capi-arkui-nativemodule-arkui-nativedialog8h)表示指向弹窗控制器的指针，可以通过调用[ArkUI\_NativeDialogAPI\_x](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-1/capi-arkui-nativemodule-arkui-nativedialogapi-1)的[create](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-1/capi-arkui-nativemodule-arkui-nativedialogapi-1#create)接口创建一个弹窗控制器。
  ...
  ```
- 当不再需要弹窗操作时，需要主动调用[dispose](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-1/capi-arkui-nativemodule-arkui-nativedialogapi-1#dispose)接口销毁弹窗控制器对象。
...
- 创建弹窗的内容对象：
  ...
  [ArkUI\_CustomDialogOptions](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-customdialogoptions/capi-arkui-nativemodule-arkui-customdialogoptions)自定义弹窗的内容对象，可以通过调用[OH\_ArkUI\_CustomDialog\_CreateOptions](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-dialog-h/capi-native-dialog-h#oh_arkui_customdialog_createoptions)接口创建一个自定义弹窗的内容对象。
  ...
  ![](../../../images/86683bad/note_3.0-zh-cn.png) 

  ArkUITextNode的声明方式可以查看[ArkUINode.h](/arkui/arkts-use-ndk/ndk-access-the-arkts-page#示例)文件中的实现文本组件。
- 当不再需要弹窗操作时，需要主动调用[OH\_ArkUI\_CustomDialog\_DisposeOptions](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-dialog-h/capi-native-dialog-h#oh_arkui_customdialog_disposeoptions)接口销毁弹窗控制器对象。
...
1. 创建弹窗内容节点。
   ...
   ![](../../../images/f34d17b3/note_3.0-zh-cn.png) 

   此示例的资源不在src > main > resource目录下，从DevEco Studio 6.0.0 Beta2开始，新建工程或者模块时，默认创建的模块不会对非resources目录下的资源进行打包，需使能相关开关：模块的build-profile.json5中buildOptions > resOptions > copyCodeResource > enable设置为true，详见[resOptions](/ide-hvigor-configuration-file/ide-hvigor-build-profile#table1476161719356)中相关介绍。
   ...
   ```
2. 以下介绍两种控制弹窗样式的方式，弹窗接口请参考[native\_dialog.h](/ref/arkui-api/arkui-c/arkui-headerfile/capi-native-dialog-h/capi-native-dialog-h)。
...
1. 创建可交互界面，点击Button后弹窗。其中获取与使用ArkUI\_NodeContentHandle类型节点可参考[接入ArkTS页面](/arkui/arkts-use-ndk/ndk-access-the-arkts-page)。
...
![](../../../images/e9b301f8/zh-cn_image_0000002583438113.gif)

## 弹窗的生命周期

从API version 19开始，弹窗显示和关闭前后，存在四个生命周期：[registerOnWillAppear](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-3/capi-arkui-nativemodule-arkui-nativedialogapi-3#registeronwillappear)、[registerOnDidAppear](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-3/capi-arkui-nativemodule-arkui-nativedialogapi-3#registerondidappear)、[registerOnWillDisappear](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-3/capi-arkui-nativemodule-arkui-nativedialogapi-3#registeronwilldisappear)、[registerOnDidDisappear](/ref/arkui-api/arkui-c/arkui-struct/capi-arkui-nativemodule-arkui-nativedialogapi-3/capi-arkui-nativemodule-arkui-nativedialogapi-3#registerondiddisappear)。
...
创建一个弹窗，弹窗显示和关闭时会触发生命周期的回调函数。其中 ArkUI\_NodeContentHandle 类型节点的获取与使用可参考[接入ArkTS页面](/arkui/arkts-use-ndk/ndk-access-the-arkts-page)。

![](../../../images/e4d98b52/note_3.0-zh-cn.png) 
