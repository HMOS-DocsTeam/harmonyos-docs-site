...
1. 在使用语音识别时，将实现语音识别相关的类添加至工程。
   ...
   ```
2. 调用[createEngine](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#speechrecognizercreateengine)方法，对引擎进行初始化，并创建[SpeechRecognitionEngine](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#speechrecognitionengine)实例。

   createEngine方法提供了两种调用形式，当前以其中一种作为示例，其他方式可参考[API参考](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer)。
   ...
   ```
3. 得到[SpeechRecognitionEngine](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#speechrecognitionengine)实例对象后，实例化[RecognitionListener](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#recognitionlistener)对象，调用[setListener](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#setlistener)方法设置回调，用来接收语音识别相关的回调信息。
   ...
   ```
4. 分别为音频文件转文字和麦克风转文字功能设置开始识别的相关参数，调用[startListening](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#startlistening)方法，开始识别。
   ...
   ```
5. 传入音频流，调用[writeAudio](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#writeaudio)方法，开始写入音频流。读取音频文件时，开发者需预先准备一个pcm格式音频文件。
   ...
   ![](../../images/538c6693/note_3.0-zh-cn.png) 

   1. 如需通过录音获取音频流，请打开麦克风权限，参考步骤10配置相关权限。
   2. 如需从音频文件中读取音频流，请在项目中的main\resources\resfile路径下存放pcm文件。
6. （可选）当需要查询语音识别服务支持的语种信息，可调用[listLanguages](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#listlanguages)方法。

   listLanguages方法提供了两种调用形式，当前以其中一种作为示例，其他方式可参考[API参考](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer)。
   ...
   ```
7. （可选）当需要结束识别时，可调用[finish](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#finish)方法。

   ```
   // 结束识别
   asrEngine?.finish(sessionId);
   ```
8. （可选）当需要取消识别时，可调用[cancel](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#cancel)方法。

   ```
   // 取消识别
   asrEngine?.cancel(sessionId);
   ```
9. （可选）当需要释放语音识别引擎资源时，可调用[shutdown](/ref/core-speech-api/core-speech-arkts/hms-ai-speechrecognizer/hms-ai-speechrecognizer#shutdown)方法。

   ```
   // 释放识别引擎资源
   asrEngine?.shutdown();
   ```
10. 需要在[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中添加ohos.permission.MICROPHONE权限，确保麦克风使用正常。详细步骤可查看[声明权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions)章节。
