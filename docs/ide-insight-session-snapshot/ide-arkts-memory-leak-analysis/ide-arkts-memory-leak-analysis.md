...
1. 使用[实时监控功能](/ide-profiler-introduction/realtime-monitor)对应用的内存资源进行监控。正常操作应用，观察运行过程中的应用内存变化情况。
   ...
   ![](../../images/68d6f153/zh-cn_image_0000002561833153.png "点击放大")
...
3. 以Allocation模板为例，创建模板后，将模板中的其余泳道去除勾选，仅录制Memory泳道的数据。
   ...
   ![](../../images/13bca90e/note_3.0-zh-cn.png) 

   其余泳道会抓取内存分配、内存对象等数据，为避免额外开销和影响分析，建议先排除录制。

   ![](../../images/0bea8070/zh-cn_image_0000002530913220.png)
4. 点击三角按钮![](../../images/4e329b39/zh-cn_image_0000002561833127.png "点击放大")即开始录制。
5. 录制过程中，不断操作应用在问题场景的功能，将问题放大，便于快速定界问题点。
6. 点击下图中方块按钮或者左侧停止按钮结束录制。

   ![](../../images/dcb4e9fd/zh-cn_image_0000002561753143.png "点击放大")
7. 录制完成后，展开Memory泳道，其中ArkTS Heap表示方舟虚拟机内存，这部分内存受到方舟虚拟机的管控。当ArkTS Heap有明显的上涨，说明在方舟虚拟机内的堆内存上可能存在内存泄漏，可以使用Snapshot模板进行下一步分析。

   ![](../../images/f9c6241e/zh-cn_image_0000002561753137.png "点击放大")
...
1. 在每次拍摄堆快照之前，虚拟机都会触发GC，所以理论上堆快照内存在的对象都是当前虚拟机已经无法GC掉的对象。我们可以将两个堆快照进行比较，来查看哪些对象是在触发问题场景时新增了且不能释放的。切换到窗口下方详情区域的“Comparison”页签，将两次快照进行对比。图中数据的含义是以Snapshot2作为基准，Snapshot2对比Snapshot1的数据变化量。
   ...
   ![](../../images/e7d11d3e/zh-cn_image_0000002530753208.png "点击放大")
2. 优先寻找与触发内存泄漏操作次数强相关、与业务代码强相关的Constructor，首先来分析这些对象是否正常。主要是按照Distance逐渐减小的方式找引用链，可以从references里面一层层去寻找，排查引用链上的可疑对象（一般指与业务代码关联的对象）。
   ...
   ![](../../images/98458c4e/note_3.0-zh-cn.png) 
...
![](../../images/fc1cd08b/zh-cn_image_0000002530753220.png)
...
![](../../images/4bf206f2/zh-cn_image_0000002530913202.png)
...
![](../../images/1299f6f7/zh-cn_image_0000002530753228.png)
...
![](../../images/1a25ac5a/zh-cn_image_0000002561833131.png "点击放大")

每个函数展开后为函数内的各个属性：

![](../../images/59545530/zh-cn_image_0000002530753234.png)
...
![](../../images/584c4b0e/zh-cn_image_0000002530753230.png)

****ArkInternalConstantPool****

虚拟机创建的常量池，ArkTS代码层面不可见，涉及到的字符串常量会在(array)标签中展示：

![](../../images/12afb6a5/zh-cn_image_0000002530913204.png "点击放大")

****LexicalEnv****

闭包变量上下文；闭包是一个链状结构，如下所示：

![](../../images/d2be9a4f/zh-cn_image_0000002561833139.png)
...
![](../../images/77cfad83/zh-cn_image_0000002531842516.png)
...
![](../../images/2096a6ff/zh-cn_image_0000002530913208.png)

对于实例化对象，一般没有constructor，则需要展开\_\_proto\_\_属性后查找constructor；

![](../../images/80a22795/zh-cn_image_0000002530913216.png)

若对象里有一些标志性属性，可以通过在代码里搜索属性名称来找到具体是哪个对象。

如果对象间有继承关系，则可以继续展开\_\_proto\_\_：

![](../../images/99538ae5/zh-cn_image_0000002561833143.png)
