...
| 属性名称 | 含义 | 数据类型 | 是否可缺省 |
| --- | --- | --- | --- |
| [app](/development-fundamentals/application-configuration-file-fa/app-structure) | 标识应用的全局配置信息。同一个应用的不同HAP的app配置必须保持一致。 | 对象 | 不可缺省。 |
| [deviceConfig](/development-fundamentals/application-configuration-file-fa/deviceconfig-structure) | 标识应用在具体设备上的配置信息。 | 对象 | 不可缺省。 |
| [module](/development-fundamentals/application-configuration-file-fa/module-structure) | 标识HAP的配置信息。该标签下的配置只对当前HAP生效。 | 对象 | 不可缺省。 |
