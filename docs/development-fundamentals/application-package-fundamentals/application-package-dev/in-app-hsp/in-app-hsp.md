...
![](../../../../images/d0212dab/note_3.0-zh-cn.png) 

- 应用内HSP：在编译过程中与应用包名（bundleName）强耦合，只能给某个特定的应用使用。
- [集成态HSP](/development-fundamentals/application-typical-scenarios/integrated-hsp)：构建、发布过程中，不与特定的应用包名耦合；使用时，工具链支持自动将集成态HSP的包名替换成宿主应用包名，并且会重新签名生成一个新的HSP包，作为宿主应用的安装包，这个新的HSP也属于宿主应用HAP的应用内HSP。
...
- 可以和依赖该HSP的HAP一起安装/运行。在安装或更新时，多模块之间存在校验，详情参考[一致性校验](/development-fundamentals/application-package-fundamentals/application-package-install/install-and-update-consistency-verification)。使用打包工具进行打包时，会进行合法性校验，详情请参考[打包工具](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool)。
- 从API version 14开始HSP支持在配置文件中[声明UIAbility](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-overview#声明配置)组件，但不支持具有入口能力的UIAbility（即skill标签配置了entity.system.home和ohos.want.action.home）。配置UIAbility的方法参考[模块中添加UIAbility](/ide-project/ide-module-management/ide-add-new-ability#section18658758104318)，HSP中UIAbility的启动方式与[应用内启动UIAbility](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction)方法相同。API version 13及之前版本，不支持在配置文件中声明[UIAbility](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-overview#声明配置)组件。
- 从API version 18开始HSP支持在配置文件中声明[ExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/extensionability-overview)组件，但不支持具有入口能力的ExtensionAbility（即skill标签配置了entity.system.home和ohos.want.action.home）。HSP中配置ExtensionAbility的方法参考[模块中添加ExtensionAbility](/ide-project/ide-module-management/ide-add-new-ability#section18891639459)。 API version 17及之前版本，不支持在配置文件中声明[ExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/extensionability-overview)组件。
- HSP可以依赖其他HAR或HSP，也可以被HAP或者HSP依赖集成，但不支持循环依赖，也不支持依赖传递。

![](../../../../images/1b680841/note_3.0-zh-cn.png) 
...
使用DevEco Studio创建一个用于调用C++代码的HSP模块。并在“Configure New Module”页面中启用“Enable native”选项。详见[创建HSP模块](/ide-project/ide-module-management/ide-creating-har-api9/ide-hsp#section7717162312546)，以创建一个名为library的HSP模块为例。基本的工程目录结构如下：
...
在工程中，常通过$r/$rawfile的形式引用应用资源。可以用$r/$rawfile访问本模块resources目录下的资源，如访问resources目录下定义的图片src/main/resources/base/media/example.png时，可以用$r("app.media.example")。有关$r/$rawfile的使用方式，请参阅文档[资源分类与访问](/resource-categories-and-access)中“资源访问-应用资源”小节。
...
要使用HSP中的接口，首先需要在使用方的 oh-package.json5 文件中配置对它的依赖。具体配置方法请参考[引用动态共享包](/ide-project/ide-module-management/ide-har-import)。
