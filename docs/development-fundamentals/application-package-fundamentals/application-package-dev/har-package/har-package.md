...
- 从API version 14开始，HAR支持在配置文件中声明[UIAbility](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-overview)组件，配置UIAbility的方法参考[在模块中添加Ability](/ide-project/ide-module-management/ide-add-new-ability#section18658758104318)，拉起HAR中UIAbility的方式与[启动应用内的UIAbility](/ability-kit/stage-model-development/stage-model-application-components/uiability/uiability-intra-device-interaction)方法相同。

![](../../../../images/22f26e93/note_3.0-zh-cn.png) 

如果使用[startAbility](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#startability)接口拉起HAR中的UIAbility，接口参数中的moduleName取值需要为依赖该HAR的[HAP](/development-fundamentals/application-package-fundamentals/application-package-dev/hap-package)/[HSP](/development-fundamentals/application-package-fundamentals/application-package-dev/in-app-hsp)的moduleName。
...
- HAR不支持在配置文件中声明[pages](/development-fundamentals/application-configuration-file-stage/module-configuration-file#pages标签)页面，但是可以包含pages页面，并通过[Navigation跳转](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-jump#路由操作)的方式进行跳转。
...
![](../../../../images/6ead553d/note_3.0-zh-cn.png) 
...
开发者可以通过DevEco Studio创建一个用于调用C++代码的HAR模块，创建过程中需要在Configure New Module界面中开启Enable native。详见[创建库模块](/ide-project/ide-module-management/ide-creating-har-api9/ide-har#section643521083015)。
...
![](../../../../images/d651d42e/note_3.0-zh-cn.png) 

HAR在和宿主应用一起编译时，会把HAR的代码直接编译到宿主应用中，HAR包是一个编译中间态产物，不是最终的运行实体。运行时，HAR运行的身份信息是其宿主应用，系统会以宿主应用的版本做行为区分。如果需要在HAR中区分宿主应用的版本做不同的行为区分，可以调用[getBundleInfoForSelf](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetbundleinfoforself)接口，获取宿主应用的targetVersion，然后根据不同的targetVersion，做不同的逻辑处理。
...
- 依赖的HAR模块，如果依赖的多个HAR之间有资源冲突，会按照工程oh-package.json5中dependencies下的依赖顺序进行覆盖，依赖顺序在前的优先级较高。例如下方示例中dayjs和lottie中包含同名文件时，会优先使用dayjs中的资源。
  ...
  ![](../../../../images/7966e05d/note_3.0-zh-cn.png) 
...
引用HAR前，需要先配置对HAR的依赖，详见[引用HAR文件和资源](/ide-project/ide-module-management/ide-har-import)。
...
详情请参见[构建HAR](/ide-hvigor-configuration/ide-hvigor-build-har)。

### 混淆配置

HAR可以作为二方库和三方库提供给其他应用使用，如果需要对代码资产进行保护，建议[开启混淆](/arkts/arkts-compilation-tool-chain/arkts-arkguard/source-obfuscation-guide#开启源码混淆)。

[混淆能力](/arkts/arkts-compilation-tool-chain/arkts-arkguard/source-obfuscation)开启后，DevEco Studio在构建HAR时，会对代码进行编译、混淆及压缩处理，保护代码资产。

HAR模块原先默认开启混淆能力，会对API 10及以上的HAR模块，且编译模块为release时，自动进行简单的代码混淆；****从DevEco Studio 5.0.3.600开始，新建工程默认关闭代码混淆功能****，可以在HAR模块的build-profile.json5文件中的ruleOptions字段下的enable进行开启混淆，详情请见[代码混淆](/ide-build-obfuscation)，配置如下所示：
...
详见[发布HAR](/ide-project/ide-module-management/ide-creating-har-api9/ide-har-publish)。
