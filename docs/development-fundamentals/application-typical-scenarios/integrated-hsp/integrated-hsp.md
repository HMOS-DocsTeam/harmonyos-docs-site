...
![](../../../images/8e03aad1/note_3.0-zh-cn.png) 
...
- 集成态HSP只支持[Stage模型](/development-fundamentals/application-package-fundamentals/application-package-structure/application-package-structure-stage)。
- 从API version 12开始，支持使用集成态HSP。
- 使用集成态HSP要求使用标准化的OHMUrl格式。需要在工程级的[build-profile.json5文件](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app)中，将[strictMode](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section13181758123312)下的useNormalizedOHMUrl字段设置为true。
...
1. 工程配置：配置工程级的build-profile.json5文件，将useNormalizedOHMUrl字段设置为true。
   ...
   ```
2. 模块配置：修改模块级构建配置文件[build-profile.json5](/ide-hvigor-configuration-file/ide-hvigor-build-profile)，将integratedHsp配置项设置为true，指定构建的HSP模块为集成态HSP。
...
3. 打包配置（tgz包）。
   ...
   (1) 配置项目签名信息，详情请参见[应用/元服务签名](/ide-signing)。

   (2) 配置release模式。

   ![](../../../images/dad15477/zh-cn_image_0000002583477481.png)
...
4. 配置签名。
   ...
   安装和运行应用前，必须配置项目签名信息，详见[应用/元服务签名](/ide-signing)。
5. [安装和运行](/ide-run-device)。
