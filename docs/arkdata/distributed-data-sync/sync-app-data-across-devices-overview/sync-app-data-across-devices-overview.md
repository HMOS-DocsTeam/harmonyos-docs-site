...
不同应用间订阅数据库变化通知，请参考[跨应用数据共享](/arkdata/cross-app-data-share/data-share-overview)实现。
...
数据跨设备同步时，数据管理基于设备等级和数据安全标签进行访问控制，具体可见[跨设备同步访问控制机制](/arkdata/data-reliability-security/access-control-by-device-and-data-level#跨设备同步访问控制机制)。
