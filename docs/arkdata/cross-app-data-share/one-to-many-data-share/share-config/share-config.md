...
以下接口为应用间配置共享的相关接口。详细接口和说明，请参考[应用间配置共享接口](/ref/arkdata-api/arkdata-arkts/js-apis-data-datashare/js-apis-data-datashare#datasharecreatedataproxyhandle20)。
...
| allowList | 允许访问该共享配置项的应用程序列表。数组最大长度为256，超过256的部分不生效。数组中每个元素为应用的[appIdentifier](/development-fundamentals/common-problem-of-application#什么是appidentifier)，单个appIdentifier为只包含数字的字符串，最大长度为128字节，超过128字节的appIdentifier不会生效。可使用[getBundleInfoForSelf](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetbundleinfoforself)接口来获取当前应用的appIdentifier。 | 字符串数组 | 是 |
