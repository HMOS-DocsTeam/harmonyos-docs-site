...
- API version 18之前：ArkTS API仅支持[XML存储模式](/arkdata/app-data-persistence/data-persistence-by-preferences#xml存储)；C API仅支持[GSKV存储模式](/arkdata/app-data-persistence/data-persistence-by-preferences#gskv存储)；存储模式互不兼容，不支持ArkTS和C API操作同一个Preferences实例。
...
详细的接口说明请参考[Preferences接口文档](/ref/arkdata-api/arkdata-c/arkdata-module/capi-preferences/capi-preferences)。
