...
以下是键值型数据库持久化功能的相关接口，更多接口及使用方式请见[分布式键值数据库](/ref/arkdata-api/arkdata-arkts/js-apis-distributedkvstore/js-apis-distributedkvstore)。
...
2. 使用getKVStore()方法创建并获取键值数据库。示例代码如下所示：
   ...
   ```
3. 使用on()方法订阅分布式数据变化，如需关闭订阅分布式数据变化，调用[off('dataChange')](/ref/arkdata-api/arkdata-arkts/js-apis-distributedkvstore/js-apis-distributedkvstore#offdatachange)关闭。示例代码如下所示：
...
4. 调用put()方法向键值数据库中插入数据。示例代码如下所示：
   ...
   ![](../../../images/79b6dfce/note_3.0-zh-cn.png) 
