...
![](../../../images/f5588d66/zh-cn_image_0000002552957522.jpg)

## 约束限制

- 系统默认日志方式是[WAL](/arkdata/data-terminology#wal模式)（Write Ahead Log）模式，系统默认落盘方式是[FULL模式](/arkdata/data-terminology#full模式)。
...
以下是关系型数据库持久化功能的相关接口，更多接口及使用方式请见[@ohos.data.relationalStore (关系型数据库)](/ref/arkdata-api/arkdata-arkts/js-apis-data-relationalstore/arkts-apis-data-relationalstore/arkts-apis-data-relationalstore)。
...
关系型数据库操作或者存储过程中，有可能会因为各种原因发生非预期的数据库异常情况（抛出14800011），此时需要对数据库进行重建并恢复数据，以保障正常的应用开发，具体可见[关系型数据库异常重建](/arkdata/data-reliability-security/data-backup-and-restore#关系型数据库异常重建)。
...
1. 使用关系型数据库实现数据持久化，需要获取一个RdbStore，其中包括建库、建表、升降级等操作。推荐使用事务接口保证数据库升级流程原子性。
   ...
   ![](../../../images/74b61b52/note_3.0-zh-cn.png) 
   ...
   - 错误码的详细介绍请参见[通用错误码](/ref/errorcode-universal/errorcode-universal)和[关系型数据库错误码](/ref/arkdata-api/arkdata-arkts-errcode/errorcode-data-rdb/errorcode-data-rdb)。
2. 获取到RdbStore，完成数据表创建后，调用insert()接口插入数据。示例代码如下所示：
   ...
   ![](../../../images/ff5f3ba1/note_3.0-zh-cn.png) 
...
4. 根据谓词指定的查询条件查找数据。
   ...
   ![](../../../images/df22d323/note_3.0-zh-cn.png) 
...
5. 使用事务对象执行数据的插入、删除和更新操作。
   ...
   具体信息请参见[关系型数据库](/ref/arkdata-api/arkdata-arkts/js-apis-data-relationalstore/arkts-apis-data-relationalstore-rdbstore/arkts-apis-data-relationalstore-rdbstore#createtransaction14)。
   ...
   ```
6. 在同路径下备份数据库。关系型数据库支持手动备份和自动备份（仅系统应用可用）两种方式，具体可见[关系型数据库备份](/arkdata/data-reliability-security/data-backup-and-restore#关系型数据库备份)。
   ...
   ```
7. 从备份数据库中恢复数据。关系型数据库支持两种方式：恢复手动备份数据和恢复自动备份数据（仅系统应用可用），具体可见[关系型数据库数据恢复](/arkdata/data-reliability-security/data-backup-and-restore#关系型数据库数据恢复)。

   此处以调用[restore](/ref/arkdata-api/arkdata-arkts/js-apis-data-relationalstore/arkts-apis-data-relationalstore-rdbstore/arkts-apis-data-relationalstore-rdbstore#restore)接口恢复手动备份数据为例：
