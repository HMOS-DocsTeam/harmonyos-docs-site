...
![](../../images/76f4b85a/note_3.0-zh-cn.png) 
...
1. 创建域名配置文件applinking.json，内容如下：
   ...
   ![](../../images/a0de69b7/note_3.0-zh-cn.png) 
...
3. 在左侧导航栏中选择“增长 > App Linking > 应用链接”，点击“创建”。
   ...
   ![](../../images/3ffdd5b8/zh-cn_image_0000002583478801.png)
4. 填写[建立域名与应用关联关系](#建立域名与应用关联关系)的网址域名，例如：`https://www.example.com。必须输入精确的域名，不可输入包含特殊字符的模糊网址`。

   ![](../../images/8495f4bd/note_3.0-zh-cn.png) 

   不可以在域名后面添加/，即不支持`https://www.example.com/形式`。

   ![](../../images/e7d661d7/zh-cn_image_0000002552799152.png)
5. 设置完成后点击“发布”，AGC会对该网站域名的配置文件所包含的应用与本项目内的应用列表进行交集校验。

   ![](../../images/5aca7c75/note_3.0-zh-cn.png) 

   应用链接发布完成后，如果距离上次更新超过24小时，系统会去域名服务器上重新获取配置文件进行交集校验。

   例如：开发者在4月7日17:21创建了应用链接，系统会在4月8日17:30去域名服务器上重新获取配置文件，然后进行交集校验，更新发布状态。

   ![](../../images/75598cd7/zh-cn_image_0000002583438847.png)

   - 如果域名的配置文件中存在本项目中的应用，则发布成功，点击“查看”可显示该域名关联的应用信息。

     ![](../../images/59641a16/zh-cn_image_0000002552958802.png)
   - 如果还在校验中，则状态为“发布中”。
   - 如果配置文件中没有包含任何本项目中的应用，则发布失败，点击“查看”可显示发布失败原因。

     ![](../../images/07be7115/zh-cn_image_0000002583478803.png)

### 在module.json5中配置关联的网址域名

在应用的[module.json5文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中进行如下配置，以声明应用关联的域名地址，并开启域名校验开关。当URL符合module.json5中配置的host和路径规则时，可拉起开发者的App。
...
![](../../images/ea542ebd/note_3.0-zh-cn.png) 
...
1. 对应用进行[手动签名](/ide-signing#section297715173233)。
...
拉起方应用通过[UIAbilityContext.openLink()](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#openlink12)接口，传入目标应用的链接，拉起目标应用。
...
3. 在“entry/src/main/ets/pages/Index.ets”文件中，使用[UIAbilityContext.openLink()](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-uiabilitycontext/js-apis-inner-application-uiabilitycontext#openlink12)接口打开应用。
...
![](../../images/56139aed/zh-cn_image_0000002552799154.gif)
...
开发者可以通过配置applinking.json以关联多个应用。如果每个应用的module.json5的uris字段配置的都是一样的，那么系统将弹出列表框供用户选择要拉起的目标应用。 为了更好的体验，开发者也可以通过链接的path去区分拉起的目标应用，如链接`https://www.example.com/path1拉起目标应用1，链接https://www.example.com/path2拉起目标应用2`。
