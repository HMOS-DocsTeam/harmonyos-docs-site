...
sidebar_position: 3
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-ohpm-install-errorcode
kit: devtools/cli
...
工程级build-profile.json5文件中[useNormalizedOHMUrl](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section13181758123312)或 .ohpmrc文件中[enforce\_dependency\_key](/ide-ohpm-cli/ide-ohpmrc#section920325116547)设置为true后，ohpm会校验配置的本地依赖名称与其对应的包名是否一致，若不一致会导致命令执行失败。
...
检查包依赖关系，确保没有依赖包冲突或不兼容的情况。具体请参考[模块内依赖版本冲突](/ide-ohpm-cli/ide-ohpmrc#section1623415477574)。
...
检查命令行，确保目标路径存在并且正确，更多说明请参考[target\_path](/ide-ohpm-cli/ide-ohpm-common-commands/ide-ohpm-install#section79331822125611)。
