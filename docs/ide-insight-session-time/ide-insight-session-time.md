...
1. 构建应用前请参考[模块级build-profile.json5文件](/ide-hvigor-configuration-file/ide-hvigor-build-profile)，增加strip字段并赋值为false（strip：是否移除当前模块.so文件中的符号表、调试信息，配置为false代表不移除）。采集函数栈解析符号需要附带符号表信息，无符号表信息可能采集不到函数名称，或ArkTS Callstack泳道无法关联到Native调用栈，因此请按照下图进行配置。

   ![](../images/9cc14200/zh-cn_image_0000002561833503.png)
2. 创建Time任务并录制相关数据，操作方法可参考[性能问题定位：深度录制](/ide-profiler-introduction/deep-recording)。或在会话区选择****Open File****，导入历史数据。

   Time分析任务支持在录制前单击![](../images/201f878f/zh-cn_image_0000002561833465.png "点击放大")指定要录制的泳道：
   ...
   ![](../images/6c142f36/note_3.0-zh-cn.png) 
   ...
   - 单击任意泳道名称后方的![](../images/97d0389a/zh-cn_image_0000002561833461.png "点击放大")可将其置顶。
   - Time分析支持Energy泳道，请参见[能耗诊断：Energy分析](/ide-profiler-energy)。
3. 在“ArkTS Callstack”泳道和“ArkTS Callstack”子泳道上长按鼠标左键并拖拽，框选要分析的时间段。
   ...
   ****Details****区域会显示所选时间段内的函数栈耗时分布情况，****Heaviest Stack****区域会展示出“Details”区域选择节点所处的耗时最长的完整调用栈。
   ...
   - 打开页面下方的****Flame Chart****开关，函数调用栈将以火焰图的形式展示。其中，横轴表示函数的执行时长，纵轴表示调用栈的深度。
     ...
     ![](../images/f061896d/note_3.0-zh-cn.png) 

     - 火焰图条块支持搜索，搜索结果不匹配的条块会被置灰。
     - “Ctrl+鼠标滚轮”的操作，或单击该区域右上角的![](../images/69b894ce/zh-cn_image_0000002530753564.png)、![](../images/632086d9/zh-cn_image_0000002561833495.png)可放大和缩小火焰图的时间轴比例，单击![](../images/1f30c4f8/zh-cn_image_0000002530753552.png)可恢复时间轴比例为初始状态。
     - “Shift+鼠标滚轮”的操作可左右横向调整可视区间，单独操作滚轮可上下纵向调整可视区间。
     - 选中节点，单击该区域右上角的![](../images/62c6df89/zh-cn_image_0000002561833459.png "点击放大")，点击添加面包屑。添加面包屑后，该节点成为根节点，耗时占比为100%，子节点的耗时占比相对于该节点重新计算。
     - 在火焰图中选中任一节点，使用“Alt+鼠标左键”可将该节点左置底并将其占比放大到100%，其上从属节点按同比例放大显示。

     ![](../images/56546c16/zh-cn_image_0000002530753572.png "点击放大")
   - 在“ArkTS Callstack”子泳道或“Callstack”子泳道上点击****Unfold CallStack****按钮，可以在时间轴上将函数调用栈以冰锥图的形式展示。其中调用栈的先后顺序与实际调用时序保持一致。

     ![](../images/dc46ded7/zh-cn_image_0000002530753560.png "点击放大")
4. 在****Callstack****泳道上长按鼠标左键并拖拽，框选要分析的时间段。
   - ****Summary****列表展示框选时段内，所有Native线程的CPU占用率的峰值、谷值、平均值。
   - ****Callstack****列表展示框选时段内，所有Native线程的函数热点。

   ![](../images/6009bf77/zh-cn_image_0000002530753556.png "点击放大")

   - 悬浮到节点，显示以此节点为根节点，点击添加面包屑。添加面包屑后，该节点成为根节点，耗时占比为100%，子节点的耗时占比相对于该节点重新计算。

   ![](../images/615ae7d1/zh-cn_image_0000002530753570.png "点击放大")
5. 在****Callstack****子泳道上长按鼠标左键并拖拽，框选要展示分析的时间段。
   ...
   ![](../images/e78618e8/zh-cn_image_0000002561833467.png "点击放大")
6. （可选）在****Details****中双击需要优化的节点（例如耗时超过预期），可快速跳转至对应工程源码，为开发者节省定位代码路径的时间。

   ![](../images/583dcb31/note_3.0-zh-cn.png) 
...
![](../images/5d71720f/zh-cn_image_0000002561753517.png "点击放大")
...
![](../images/e92ab6af/zh-cn_image_0000002561833499.png "点击放大")

您可以通过点击工具栏![](../images/02ff1125/zh-cn_image_0000002530753562.png "点击放大")按钮，导入包含debug信息的so库。

![](../images/07e39c30/note_3.0-zh-cn.png) 
...
![](../images/8d55ff30/zh-cn_image_0000002530753574.png "点击放大")
...
![](../images/f0e9b273/zh-cn_image_0000002530753554.png)
...
![](../images/e80b52cb/note_3.0-zh-cn.png) 

此外，用户自定义打点信息，还可以在Frame分析、Network分析任务中查看到。

![](../images/883a97ab/zh-cn_image_0000002530753566.png "点击放大")
