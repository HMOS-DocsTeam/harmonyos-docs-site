...
| 接口名 | 描述 |
| --- | --- |
| [on('pageShow')](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#onpageshow) | 注册章节内容分页展示结果回调。 |
| [off('pageShow')](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#offpageshow) | 注销章节内容分页展示结果回调，可在页面销毁时调用。 |

## 开发准备

在进行阅读进度通知监听之前，请先确保已经“[构建阅读器](/reader-kit-guide/reader-content/reader-read-page)”。
...
1. 通过ReaderComponentController组件控制器监听页面展示回调。
   ...
   开发者可在on('pageShow')接口回调中将阅读进度实时保存到数据库当中，防止用户异常退出阅读器时的进度丢失。当用户下次继续阅读时，可将保存domPos及resourceIndex属性传入到[startPlay](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#startplay)接口中，用于阅读进度的恢复。
