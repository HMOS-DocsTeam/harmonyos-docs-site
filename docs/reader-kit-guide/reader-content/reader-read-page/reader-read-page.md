...
![](../../../images/a98986e8/zh-cn_image_0000002552799494.png)
...
| 接口名 | 描述 |
| --- | --- |
| [getDefaultHandler](/ref/reader-api/reader-arkts/reader-book-parser/reader-book-parser#getdefaulthandler)(path: string): Promise&lt;BookParserHandler&gt; | 获取书籍默认解析器。 |
| [init](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#init)(context: common.UIAbilityContext): Promise&lt;void&gt; | 初始化ReadPageComponent控制器。  初始化接口需要优先于ReaderComponentController的其他接口之前执行。 |
...
| [registerBookParser](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#registerbookparser)(bookParserHandler: bookParser.BookParserHandler): void | 注册书籍解析器。  需要在startPlay接口调用之前执行。 |
| [startPlay](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#startplay)(spineIndex: number, domPos: string): Promise&lt;void&gt; | 以指定阅读进度打开书籍，使用Promise异步回调。  需要在registerBookParser接口调用之后执行。 |
...
3. 构建ReadPageComponent组件，用于显示阅读内容。
   ...
   ```
4. 通过提前导入到[应用沙箱目录](/core-file-kit/app-file/app-sandbox-directory)中的书籍文件，初始化书籍解析器。调用[startPlay](/ref/reader-api/reader-arkts/reader-read-core/reader-read-core#startplay)接口，以指定进度渲染阅读器页面。
