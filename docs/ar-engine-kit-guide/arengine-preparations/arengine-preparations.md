...
ArkTS（[arViewController.isARTypeSupported](/ref/ar-engine-api/arengine-arkts-api/arengine-api-arviewcontroller/arengine-api-arviewcontroller#arviewcontrollerisartypesupported)）：
...
C/C++（[HMS\_AREngine\_CheckSupported](/ref/ar-engine-api/arengine-capi/arengine-modular/arengine-capi-arengine/arengine-capi-arengine#hms_arengine_checksupported)）：
...
请参考[应用开发准备](/application-dev-overview)完成基本准备工作。

## 申请权限

在开发AR应用时，需要先申请相机相关权限，确保应用拥有访问相机硬件及其他功能的权限，需要的权限如下表。在申请权限前，请保证符合[权限使用的基本原则](/system-security/access-control/app-permission-mgmt/app-permission-mgmt-overview#权限使用的基本原则)。
...
推荐使用[组件导航(Navigation) (推荐)](/arkui/arkts-ui-development/arkts-set-navigation-routing/arkts-navigation-navigation)作为页面路由，使用[Navigation](/ref/arkui-api/arkui-declarative-comp/navigation-and-switching/ts-basic-components-navigation/ts-basic-components-navigation)的[页面生命周期](/arkui/arkts-ui-development/arkts-set-navigation-routing/arkts-navigation-navigation#页面生命周期)所示方法。
...
1. 导入所需模块。
   ...
   ```
2. 创建一个基础的页面，具体可参考[组件导航(Navigation) (推荐)](/arkui/arkts-ui-development/arkts-set-navigation-routing/arkts-navigation-navigation)。
