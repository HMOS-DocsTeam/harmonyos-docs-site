...
- 自动签名仅用于调试场景，方便开发者进行应用调试。部分调试场景下必须使用手动签名：
  ...
  2. 如果开发过程中使用到需要审批的权限时，例如：
     1. 使用部分不支持自动签名的[受限开放权限](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions)时，必须使用[手动签名](#section297715173233)。支持自动签名的ACL权限清单请参见[自动签名支持的ACL权限](#section5301916183411)。
...
1. 连接真机设备或模拟器，具体请参考[使用本地真机运行应用/元服务](/ide-run-device)或[使用模拟器运行应用/元服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-run-emulator)，真机连接成功后如下图所示：

   ![](../images/e973d830/zh-cn_image_0000002530912982.png)

   ![](../images/ba356be4/note_3.0-zh-cn.png) 
...
2. 进入****File > Project Structure... > Project > Signing Configs****界面，勾选“****Associate with registered application****”。如果未登录，请先点击****Sign In****进行登录。
   ...
   ![](../images/67e7323c/zh-cn_image_0000002530752994.png "点击放大")

   ![](../images/f13dccab/note_3.0-zh-cn.png) 
...
3. 点击****Enable open capabilities****，在DevEco Studio上开通[开放能力](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section1817619495251)。当前支持开通如下四种开放能力：Push Kit（[推送服务](/push-kit-guide/push-kit-introduction)）、Device status detection（[应用设备状态检测](/system-security/device-security-kit-guide/devicesecurity-deviceverify-develop)）、Map Kit（[地图服务](/map-kit-guide/map-introduction)）、Safety Detect（[安全检测服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/devicesecurity-safetydetect-develop)），应用根据需要进行勾选。

   ![](../images/206f0a0e/zh-cn_image_0000002530913000.png)
4. （可选）在配置文件中添加ACL权限信息，ACL权限清单请参考[自动签名支持的ACL权限](#section5301916183411)。
   ...
   ![](../images/acbd5123/zh-cn_image_0000002530752988.png)
   ...
   ![](../images/6f8644ee/zh-cn_image_0000002561752937.png "点击放大")

   ![](../images/d482eb75/note_3.0-zh-cn.png) 

   - 申请ACL前注意事项：
     - 在申请ACL权限前，请审视是否符合[受限权限的使用场景](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions)。当前仅少量符合特殊场景的应用可在通过审批后，使用受限权限。申请方式请见[申请使用受限权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions-in-acl)。
...
5. 签名完成后，在本地生成密钥（.p12）、证书请求文件（.csr）、数字证书（.cer）及Profile文件（.p7b）。将鼠标悬停在Provisioning Profile: DevEco Managed Profile后![](../images/bd778703/zh-cn_image_0000002561832929.png)，可查看证书有效期、包名（bundle name）、ACL权限（acl）、开放能力（capability）等信息；或进入工程级build-profile.json5文件，在“signingConfigs”下可查看到配置成功的签名信息。
...
1. 连接真机设备或模拟器，具体请参考[使用本地真机运行应用/元服务](/ide-run-device)或[使用模拟器运行应用/元服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-run-emulator)，真机连接成功后如下图所示：
   ...
   ![](../images/2d5fea61/zh-cn_image_0000002561832921.png)

   ![](../images/6b226547/note_3.0-zh-cn.png) 
...
2. 进入****File > Project Structure... > Project > Signing Configs****界面，勾选“****Automatically generate signature****”即可完成签名。如果未登录，请先单击****Sign In****进行登录，然后自动完成签名。
   ...
   ![](../images/40dd241b/zh-cn_image_0000002530913004.png "点击放大")
3. （可选）在配置文件中添加ACL权限信息，ACL权限清单请参考[自动签名支持的ACL权限](#section5301916183411)。修改配置文件后，点击****Ok。****
   ...
   ![](../images/4bceac86/zh-cn_image_0000002561752925.png)

   ![](../images/b15f803d/note_3.0-zh-cn.png) 

   - 在调试签名时，不会强校验配置文件中添加的ACL权限。
   - 涉及受限权限的应用，上架时，应用市场（AGC）将根据应用的使用场景审核是否可以使用对应的受限权限，如不符合，应用的上架申请将被驳回。在配置ACL权限前，请审视是否符合[受限权限的使用场景](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions)。当前仅少量符合特殊场景的应用可在通过审批后，使用受限权限，申请方式请见[申请使用受限权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions-in-acl)。
4. 签名完成后，在本地生成密钥（.p12）、证书请求文件（.csr）、数字证书（.cer）及Profile文件（.p7b）。将鼠标悬停在Provisioning Profile: DevEco Managed Profile后![](../images/fe340330/zh-cn_image_0000002530912988.png)，可查看证书有效期、包名（bundle name）、ACL权限（acl）、开放能力（capability）等信息；或进入工程级build-profile.json5文件，在“signingConfigs”下可查看到配置成功的签名信息。

****OpenHarmony工程********按以下步骤操作：****

1. 连接真机设备或模拟器，具体请参考[使用本地真机运行应用/元服务](/ide-run-device)或[使用模拟器运行应用/元服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-run-emulator)，真机连接成功后如下图所示：

   ![](../images/2b460700/zh-cn_image_0000002561752941.png)
2. 进入****File > Project Structure... > Project > Signing Configs****界面。仅勾选“****Automatically generate signature****”时，生成OpenHarmony签名；勾选“****Support HarmonyOS****”和“****Automatically generate signature****”时，生成HarmonyOS签名。如果未登录，请先单击****Sign In****进行登录。
   ...
   ![](../images/e3aaa9d2/zh-cn_image_0000002530752992.png)

   签名完成后，如下图所示。在本地生成密钥（.p12）、证书请求文件（.csr）、数字证书（.cer）及Profile文件（.p7b），数字证书在AGCt网站的“证书、APP ID和Profile”页签中可以查看。

   ![](../images/1d8b50dc/zh-cn_image_0000002561752949.png)

   ![](../images/88bb7823/note_3.0-zh-cn.png) 
...
2. 在****Generate Key**** ****and CSR****界面，可以单击****Select an existing key****选择已有的密钥库文件（存储有密钥的.p12文件），若没有密钥库文件则进行填写。下面以新创建密钥库文件为例进行说明。
   ...
   ![](../images/931e73b1/zh-cn_image_0000002561752935.png)
...
4. 在****Generate**** ****Certificate Request File (CSR)****窗口，设置CSR文件名和CSR文件存储路径后，点击****Finish****。
   ...
   ![](../images/a960e141/zh-cn_image_0000002530912990.png)
5. 创建CSR文件成功，可以在存储路径下获取生成的密钥库文件（.p12）、证书请求文件（.csr）。

   ![](../images/3797e210/zh-cn_image_0000002530752998.png)

****DevEco Studio 6.1.0 Beta2之前版本****

1. 在主菜单栏单击****Build > Generate Key**** ****and CSR****。

   ![](../images/3982725f/note_3.0-zh-cn.png) 
...
2. 在****Key store file****中，可以单击****Choose Existing****选择已有的密钥库文件（存储有密钥的.p12文件）；如果没有密钥库文件，单击****New****进行创建。下面以新创建密钥库文件为例进行说明。
   ...
   ![](../images/db77fd30/zh-cn_image_0000002561832925.png)
3. 在****Create Key Store****窗口，填写密钥库信息后，单击****OK****。
   - ****Key store file****：设置密钥库文件存储路径，并填写p12文件名。
   - ****Password****：设置密钥库密码，必须由大写字母、小写字母、数字和特殊符号中的两种以上字符的组合，长度至少为8位。请记住该密码，后续签名配置需要使用。
   - ****Confirm password****：再次输入密钥库密码。

   ![](../images/cffaa974/zh-cn_image_0000002530753006.png)
...
1. （可选）如需使用ACL权限，在AGC中[申请ACL权限](https://developer.huawei.com/consumer/cn/doc/app/agc-help-apply-acl-0000002394212138)。同时，在DevEco Studio配置文件中添加权限信息。
   ...
   ![](../images/a89612c2/note_3.0-zh-cn.png) 

   - ACL权限申请仅支持中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）。
   - 若应用因特殊场景要求使用受限开放权限，请务必在此步骤进行申请，否则应用将在审核时被驳回。受限开放权限可申请的特殊场景请参考[受限开放权限](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions)。
   ...
   ![](../images/cc2b52ca/zh-cn_image_0000002561832915.png)
...
1. 连接真机设备，确保[DevEco Studio与真机设备已连接](/ide-run-device)。

   ![](../images/5d305c67/zh-cn_image_0000002561832911.png)
2. 在****File >**** ****Project Structure >**** ****Project > Signing Configs****窗口中，取消勾选“Automatically generate signature”和“Associate with registered application”，分别配置密钥（.p12文件）、Profile（.p7b文件）和数字证书（.cer文件）的路径等信息，配置完毕后点击****Apply****。
   ...
   ![](../images/e2713d26/note_3.0-zh-cn.png) 
   ...
   ![](../images/960a5d93/zh-cn_image_0000002530753014.png "点击放大")

   配置完成后，将鼠标悬停在****Provisioning Profile: DevEco Manage Profile****后![](../images/16fa0384/zh-cn_image_0000002561752945.png)，可查看证书有效期、包名（bundle name）、企业名称（common name）、ACL权限（acl）、开放能力（capability）相关信息；或者进入工程级build-profile.json5文件，在“signingConfigs”下可查看到配置成功的签名信息。

   ![](../images/79696b43/zh-cn_image_0000002530912984.png "点击放大")
3. 进入工程级build-profile.json5文件，在“signingConfigs”下可查看到配置成功的签名信息，点击右上角的“Run”按钮运行应用/元服务。

   ![](../images/cdd23f92/zh-cn_image_0000002530912992.png)
...
从DevEco Studio 6.1.0 Beta2版本开始，自动签名支持配置的ACL权限具体参考[受限开放权限](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions)。
