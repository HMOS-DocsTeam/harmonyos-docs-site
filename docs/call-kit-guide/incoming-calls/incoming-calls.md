...
![](../../images/bcdf26b9/zh-cn_image_0000002583478831.jpg)

### 来电场景：拒接流程图

![](../../images/c609389b/zh-cn_image_0000002552799182.jpg)

## 接口说明

来电场景的接口，由[voipCall](/ref/call-api/call-arkts/call-voipcall/call-voipcall)提供。更多接口信息详见[接口文档](/ref/call-api/call-arkts/call-voipcall/call-voipcall)。
...
2. 为了感知到用户在横幅通知上做的接听、挂断、静音与解除静音等操作，应用需要注册voipCallUiEvent事件。建议在上报来电之前注册。
   ...
   ```
3. 应用内部建立通话连接之后，需要向Call Service Kit上报来电，并携带通话信息，详见[VoipCallAttribute](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallattribute)。
   ...
   对于视频通话，可以通过参数[isVoiceAnswerSupported](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallattribute)指定是否允许语音接听，示例代码如下：
...
4. 接收到来电之后，用户可以选择接听或拒接。
   ...
   - 上报两次状态（推荐）。
     ...
     以语音通话为例，应用在接收到[VOIP\_CALL\_EVENT\_VOICE\_ANSWER](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcalluievent)事件回调之后，立即向Call Service Kit上报[VOIP\_CALL\_STATE\_ANSWERED](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallstate)状态，并同时执行应用内接听。

     在完成应用内接听之后，再向Call Service Kit上报[VOIP\_CALL\_STATE\_ACTIVE](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallstate)状态，系统会更新通话横幅。
     ...
     ![](../../images/88426060/note_3.0-zh-cn.png) 
   ...
   以语音通话为例，应用在接收到[VOIP\_CALL\_EVENT\_VOICE\_ANSWER](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcalluievent)事件回调之后，执行应用内接听。

   一直到完成应用内接听后，再向Call Service Kit上报[VOIP\_CALL\_STATE\_ACTIVE](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallstate)状态。
   ...
   如果用户在横幅通知上点击拒接，则应用在接收到[VOIP\_CALL\_EVENT\_REJECT](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcalluievent)事件回调之后，在应用内完成拒接，然后向Call Service Kit 上报[VOIP\_CALL\_STATE\_DISCONNECTED](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallstate)状态，系统会取消横幅通知。
...
5. 在通话过程中，用户可以根据需要，可以静音或解除静音。
   ...
   以静音为例，用户在横幅上点击静音，Call Service Kit会给应用回调[VOIP\_CALL\_EVENT\_MUTED](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcalluievent)事件。

   应用在完成静音后，应向Call Service Kit上报[AUDIO\_EVENT\_MUTED](/ref/call-api/call-arkts/call-voipcall/call-voipcall#callaudioevent)音频状态。
   ...
   | --- | --- |
   |  |  |
6. 用户在横幅点击挂断，Call Service Kit会给应用回调[VOIP\_CALL\_EVENT\_HANGUP](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcalluievent)事件。

   应用收到该事件后，应在应用内完成挂断，然后，向Call Service Kit上报[VOIP\_CALL\_STATE\_DISCONNECTED](/ref/call-api/call-arkts/call-voipcall/call-voipcall#voipcallstate)状态，系统会取消横幅通知。
