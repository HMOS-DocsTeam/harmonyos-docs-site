...
- 已[初始化存储实例](/cloud-foundation-kit-guide/cloudfoundation-storage-service/cloudfoundation-storage-initialize-bucket)。
- 已[上传指定文件至云侧](/cloud-foundation-kit-guide/cloudfoundation-storage-service/cloudfoundation-storage-upload-file)。

## 操作步骤

1. 调用[StorageBucket.downloadFile](/ref/cloud-foundation-api/cloudfoundation-arkts-api/cloudfoundation-cloudstorage/cloudfoundation-cloudstorage#downloadfile)接口创建下载任务，监听下载任务的progress、completed、failed等事件。
2. 启动下载任务。

   ![](../../../images/34673fb3/note_3.0-zh-cn.png) 

   下载成功后，文件将保存在[context.cacheDir](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context#属性)目录下。
...
![](../../../images/d81372e0/note_3.0-zh-cn.png) 

如果本地已存在同名文件，下载文件将出现异常，可以通过设置[DownloadParams.overwrite](/ref/cloud-foundation-api/cloudfoundation-arkts-api/cloudfoundation-cloudstorage/cloudfoundation-cloudstorage#downloadparams)来决定是否覆盖本地文件。
