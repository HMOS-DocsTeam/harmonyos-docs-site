...
- 如何清除最终用户的数据
  ...
  - 最终用户或开发者主动删除上传的数据
    - 清除云存储中的用户数据：可通过[StorageBucket.deleteFile](/ref/cloud-foundation-api/cloudfoundation-arkts-api/cloudfoundation-cloudstorage/cloudfoundation-cloudstorage#deletefile)接口删除通过云存储上传的用户数据。
    - 清除云数据库中的用户数据：可通过[DatabaseZone.delete](/ref/cloud-foundation-api/cloudfoundation-arkts-api/cloudfoundation-clouddatabase/cloudfoundation-clouddatabase#delete)接口删除写入云数据库的用户数据。
  - 开发者主动删除整个项目
- 如何导出最终用户的数据
