...
调用本地云函数之前，请按照以下示例设置“设备端口”到“主机端口”的映射。详情请参见[创建反向端口转发任务](/system-debug-optimize/debugging-commands/hdc#创建反向端口转发任务)。
...
1. 在项目中导入cloudFunction组件。
   ...
   ```
2. 调用[call()](/ref/cloud-foundation-api/cloudfoundation-arkts-api/cloudfoundation-cloudfunction/cloudfoundation-cloudfunction#call)方法设置函数，在方法中传入函数名称和本地启动的云函数地址，返回调用结果。
