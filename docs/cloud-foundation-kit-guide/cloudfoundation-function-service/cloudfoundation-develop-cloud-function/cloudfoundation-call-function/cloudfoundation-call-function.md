...
![](../../../../images/74675761/zh-cn_image_0000002552958860.png)
...
1. 在项目中导入cloudFunction组件。
   ...
   ```
2. 调用[call()](/ref/cloud-foundation-api/cloudfoundation-arkts-api/cloudfoundation-cloudfunction/cloudfoundation-cloudfunction#call)方法设置函数，在方法中传入函数名称，返回调用结果。
...
3. 如果需要关注函数的返回值，可调用result属性获取。
   ...
   value为步骤2中调用call()方法返回的cloudFunction.FunctionResult对象，返回值为云函数body返回的值，以[测试函数](/cloud-foundation-kit-guide/cloudfoundation-function-service/cloudfoundation-develop-cloud-function/cloudfoundation-test-function)时返回的结果为例，value.result = \{"simple":"example"\}。
