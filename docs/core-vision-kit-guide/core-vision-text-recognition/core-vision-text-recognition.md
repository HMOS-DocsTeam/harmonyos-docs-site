...
![](../../images/a0c47c4e/zh-cn_image_0000002583439351.png)
...
2. 简单配置页面的布局，并在Button组件添加点击事件，拉起图库，选择图片。
   ...
   ```
3. 通过图库获取图片资源，将图片转换为[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)，并添加初始化和释放方法。
   ...
   ```
4. 实例化[VisionInfo](/ref/core-vision-api/core-vision-arkts/core-vision-text-recognition-api/core-vision-text-recognition-api#visioninfo)对象，并传入待检测图片的[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)。

   VisionInfo为待OCR检测识别的入参项，目前仅支持[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)类型的视觉信息。
   ...
   ```
5. 配置通用文本识别的配置项[TextRecognitionConfiguration](/ref/core-vision-api/core-vision-arkts/core-vision-text-recognition-api/core-vision-text-recognition-api#textrecognitionconfiguration)，用于配置是否支持朝向检测。
   ...
   ```
6. 调用textRecognition的[recognizeText](/ref/core-vision-api/core-vision-arkts/core-vision-text-recognition-api/core-vision-text-recognition-api#textrecognitionrecognizetext-1)接口，对识别到的结果进行处理。

   当调用成功时，获取文字识别的结果；调用失败时，将返回对应错误码。recognizeText接口提供了三种调用形式，当前以其中一种作为示例，其他方式可参考[API参考](/ref/core-vision-api/core-vision-arkts/core-vision-text-recognition-api/core-vision-text-recognition-api)。
