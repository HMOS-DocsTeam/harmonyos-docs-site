...
详细的接口说明请参考[ImageEffect](/ref/image-api/image-c/image-module/capi-imageeffect/capi-imageeffect)。
...
3. 设置处理数据。
   ...
   ****场景一：设置OH\_PixelmapNative输入类型。****
   ...
   OH\_PixelmapNative的具体使用方法请参考[Pixelmap开发指导](/image-kit/image-native-js-objects/image-pixelmap-operation-native)。
   ...
   ****场景二：设置OH\_NativeBuffer输入类型。****
   ...
   OH\_NativeBuffer的具体使用方法请参考[NativeBuffer开发指导](/arkgraphics-2d/native-surface/native-buffer-guidelines)。
   ...
   ****场景五：设置OHNativeWindow输入类型。****
   ...
   XComponent模块的具体使用方法请参考[XComponent组件参考](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)。

   NativeWindow模块的具体使用方法请参考[OHNativeWindow](/ref/arkgraphics-api/arkgraphics-c/arkgraphics-module/capi-nativewindow/capi-nativewindow)。

   Camera的具体使用方法请参考[Camera预览参考](/camera-kit/camera-dev-native/native-camera-preview)。
