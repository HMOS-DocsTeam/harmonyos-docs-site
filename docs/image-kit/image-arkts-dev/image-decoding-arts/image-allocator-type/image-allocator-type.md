...
应用侧通过解码API接口获取PixelMap，并将其传递给[Image组件](/ref/arkui-api/arkui-js-comp/arkui-js-full-comp/js-full-basic-comp/js-components-basic-image/js-components-basic-image)以进行显示。
...
系统提供了[createPixelMapUsingAllocator](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmapusingallocator15)接口，以便用户能够自定义内存分配类型进行解码。接口定义及使用示例详见[图片解码接口说明](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource)。
...
在使用[createPixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmap7)接口进行解码时，不同场景下会采取不同的内存分配类型。

以下场景将使用DMA\_ALLOC。

- 解码HDR图片。
- 解码HEIF格式图片。
- 解码JPEG格式图片，当原图的宽和高均在1024像素至8192像素之间，[desiredPixelFormat](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#decodingoptions7)为RGBA\_8888或NV21，同时硬件不繁忙（并发数为3）。
- 解码其他格式图片。要求[desiredSize](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#decodingoptions7)大于等于512像素 \* 512像素（未设置desiredSize时按原图尺寸考虑），并且宽度为64的倍数。
...
开发者使用接口[createPixelMapUsingAllocator](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmapusingallocator15)进行解码时，系统会根据传入的[解码参数](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#decodingoptions7)和[内存申请类型](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-e/arkts-apis-image-e#allocatortype15)，自动选择硬件解码和软件解码。
...
使用接口[createPixelMapUsingAllocator](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmapusingallocator15)进行解码时，若设置的内存分配模式，与图片格式或解码方式不匹配，则会抛出内存分配失败的异常。
...
stride的值可以通过[getImageInfo()](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#getimageinfo-1) 接口获取。

1. 调用[getImageInfo()](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#getimageinfo-1)方法，获取ImageInfo对象。
...
图片框架对解码单张图片设置了2GB的内存限制。进程需要主动管理自身内存，建议在不使用[PixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-pixelmap/arkts-apis-image-pixelmap)时及时释放，以避免进程被系统终止。
...
对于原始像素内存超过2GB且支持下采样的图片，建议开发者使用[createPixelMap](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmap7)或[createPixelMapUsingAllocator](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagesource/arkts-apis-image-imagesource#createpixelmapusingallocator15)接口，并在[DecodingOptions（解码参数）](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#decodingoptions7)中设置desiredSize（期望输出大小）进行下采样解码。
...
| 不支持 | .gif .bmp .webp .dng [.svg10+](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-f/arkts-apis-image-f#svg标签说明) .ico11+ |
