...
本模块提供图片细节增强的[ArkTS接口](/ref/image-api/image-arkts/js-apis-videoprocessingengine/js-apis-videoprocessingengine)，通过调用本模块，可以实现图片内容的清晰度增强及缩放功能，处理后的数据可以用于送显和输出。
...
1. 当前仅支持处理同时满足以下条件的图片：
   ...
   - 处理的PixelMap对象需为[DMA内存](/image-kit/image-arkts-dev/image-decoding-arts/image-allocator-type#内存类型介绍)。
