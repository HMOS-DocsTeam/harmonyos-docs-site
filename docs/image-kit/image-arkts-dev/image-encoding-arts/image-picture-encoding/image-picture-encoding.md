...
图片编码指将[Picture](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-picture/arkts-apis-image-picture)对象编码成不同格式的图片文件（当前仅支持编码为JPEG 和 HEIF 格式），用于后续处理，如保存、传输等。
...
2. 设置编码选项[PackingOption](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#packingoption)。
   ...
   ![](../../../../images/7eb1fca0/note_3.0-zh-cn.png) 
   ...
   ```
3. 封装函数，传入picture，使用[packing](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagepacker/arkts-apis-image-imagepacker#packing13)接口编码到ArrayBuffer，或使用[packToFile](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-imagepacker/arkts-apis-image-imagepacker#packtofile11-2)接口编码到文件。

   ![](../../../../images/94d6c716/note_3.0-zh-cn.png) 

   在进行编码前，需要先通过解码获取picture，可参考[使用ImageSource完成多图对象解码](/image-kit/image-arkts-dev/image-decoding-arts/image-picture-decoding)。
