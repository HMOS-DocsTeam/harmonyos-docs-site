...
ArkTS场景下读取和编辑图片的旋转角度信息，可参考：[编辑图片Exif信息](/image-kit/image-arkts-dev/image-editing-arkts/image-tool)，对应的propertyKey需要设置为：ORIENTATION。

C/C++场景下读取和编辑图片的旋转角度信息，可参考：[使用Image\_NativeModule编辑图片Exif信息](/image-kit/image-native/image-editing-c/image-tool-c)，对应的key需要设置为：OHOS\_IMAGE\_PROPERTY\_ORIENTATION。
...
在调用图片编码接口时，需要设置[packingOption](/ref/image-api/image-arkts/js-apis-image/arkts-apis-image-i/arkts-apis-image-i#packingoption)的needsPackProperties属性为true（该属性的默认值为false）。
