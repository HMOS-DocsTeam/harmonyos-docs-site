...
- 通过命令行构建：
  - 执行命令，其中&lt;task&gt;替换为具体任务名：
    ...
    ```
  - 在[hvigor-config.json5](/ide-hvigor-configuration-file/ide-hvigor-set-options)中配置execution.daemon选项。
...
- 修改[hvigor-config.json5文件](/ide-hvigor-configuration-file/ide-hvigor-set-options)中的nodeOptions.maxOldSpaceSize配置。
- 执行命令修改守护进程内存，示例如下：
...
- 修改[hvigor-config.json5文件](/ide-hvigor-configuration-file/ide-hvigor-set-options)中的nodeOptions.maxSemiSpaceSize配置。
- 执行命令修改守护进程内存，示例如下：
