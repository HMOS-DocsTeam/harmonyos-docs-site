---
title: "配置签名和指纹"
sidebar_position: 2
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-sign-fingerprints
kit: app-services
last_updated: "2026-04-22"
---

# 配置签名和指纹

请参考“[应用开发准备](/application-dev-overview)”章节，完成以下操作步骤：

1. 创建项目和工程（如已完成，请跳过此步骤）。
2. 配置签名信息 ****（未成年人模式接口支持自动签名，其他接口仅支持手动签名方式）****。
3. 添加公钥指纹。

   ![](../../../images/6e996072/caution_3.0-zh-cn.png) 

   ****发布阶段****，请参考[发布流程](/ide-publish-app#section6406135115814)章节，重新配置用于应用发布的签名信息、添加公钥指纹（必选）。

   - 检查是否需要配置公钥指纹：应用仅接入未成年人模式或compatibleSdkVersion>=20不需要配置公钥指纹，其他场景均需配置。

     ![](../../../images/c5f76e0f/zh-cn_image_0000002552958744.png)
   - 检查公钥指纹是否配置成功：请在[开发与服务](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html#/myProject)中选择对应的项目和应用，检查是否已成功配置该应用的公钥指纹。

     ![](../../../images/cd32114f/zh-cn_image_0000002583478745.png)
   - 公钥指纹最迟会在25小时后生效。

     ****（可选）**** 配置公钥指纹10分钟后，您可通过修改应用工程中app.json5配置文件的versionCode触发公钥指纹生效。

     ****图1**** 修改前

     ![](../../../images/ee1f0a0e/zh-cn_image_0000002552799094.png)

     ****图2**** 修改后

     ![](../../../images/48be4772/zh-cn_image_0000002583438789.png)
