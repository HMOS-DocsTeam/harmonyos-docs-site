...
执行命令前，需要先配置JDK，配置Node.js、hvigor等环境变量，具体请参考[搭建流水线](/ide-command-line-building-app)。
...
![](../images/33380242/note_3.0-zh-cn.png) 
...
常见的任务和参数如下，更多任务请参考[任务详细说明](/ide-hvigor/ide-hvigor-task-process#section196919100414)。
...
| -p debuggable=true/false | 该配置会覆盖构建模式中对应的buildOption中的debuggable配置。  关于debuggable的合并优先级，请参考[合并编译选项规则](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section1727865610255)。 |
...
| -p parameterFile=param.json/json5 | 设置oh-package.json5文件的参数配置文件，其中"param"可自行修改为对应配置文件名称。详细使用请参考[parameterFile](/ide-ohpm-cli/ide-oh-package-json5#section122411462820)。 |
| -p buildVersion=1 | 设置构建版本号为1，详细使用请参考[app.json5的buildVersion](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)。  该参数从hvigorw 6.23.3版本开始支持。 |
...
| 参数 | 说明 |
| --- | --- |
| --daemon | 启用[守护进程](/ide-build-optimized/ide-hvigor-daemon)。 |
...
| 参数 | 说明 |
| --- | --- |
| --parallel, --no-parallel | Hvigor默认开启[并行构建](/ide-build-optimized/ide-hvigor-improve-performance)能力，如需关闭在命令行后添加--no-parallel。 |
| --incremental, --no-incremental | Hvigor默认开启[增量构建](/ide-build-optimized/ide-hvigor-incremental-build)能力，如需关闭在命令行后添加--no-incremental。 |
...
| -p buildOption | 命令包含此参数时会打印buildOption配置，不含此参数时将不会展示buildOption配置，输出的buildOption优先级请参考[合并编译选项规则](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section1727865610255)。 |
...
| --config, -c | 指定hvigor-config.json5配置文件中的参数。  当前仅支持设置properties里的参数，具体支持的参数请查看[hvigor-config.json5文件](/ide-hvigor-configuration-file/ide-hvigor-set-options)中properties支持的参数。  --config properties.key=value 同 -c properties.key=value |
