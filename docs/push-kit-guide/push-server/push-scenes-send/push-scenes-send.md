...
有关场景化消息的更详细说明，请参见REST API-[场景化消息API接口](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-intro/push-scenariozed-api-intro#场景介绍)。

## 开发步骤

1. 您的服务端获取鉴权令牌，详情请参见[基于服务账号生成鉴权令牌](/push-kit-guide/push-server/push-jwt-token)。
2. 您的服务端调用API发送Push场景化消息，更多消息内容请参见REST API-[场景化消息API接口](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-intro/push-scenariozed-api-intro#场景介绍)。
   ...
   ****请求消息头示例：****
   ...
   - 请求消息头中的Authorization参数为"Bearer "拼接上您在上一步[在线生成服务账号鉴权令牌](/push-kit-guide/push-server/push-jwt-token)中获取的鉴权令牌。
   ...
   ****通知消息体示例：****
   ...
   - 更多场景化消息示例可参见[请求示例](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-request-example/push-scenariozed-api-request-example)。
   - 建议您在开发代码前先使用Postman等调试工具发送消息，测试功能。
3. （可选）您的应用服务器接收Push Kit的消息回执，详情请参见[消息回执](/push-kit-guide/push-server/push-msg-receipt)。

![](../../../images/972cb091/note_3.0-zh-cn.png) 
...
![](../../../images/f712b52d/note_3.0-zh-cn.png) 
...
1. 登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，点击“开发与服务”，在项目列表中选择对应的项目，左侧导航栏选择“项目设置”。
   ...
   ![](../../../images/04ebee38/zh-cn_image_0000002552959130.png)
2. 在项目列表中找到您的项目，通过“增长 > 推送服务 > 推送通知（V3 Beta）”导航到“推送通知（V3 Beta）”页签。在该页签下点击“添加推送通知”新建推送任务。

   ![](../../../images/27cbfbf0/zh-cn_image_0000002583479131.png)
3. 这里以Alert消息举例，配置参数如下。

   - ****配置推送任务****

     ![](../../../images/95342f2a/zh-cn_image_0000002552799482.png)
   ...
   - ****配置推送内容-通用参数****
     ...
     ![](../../../images/46cf4f83/zh-cn_image_0000002583439177.png)
     ...
     | 批量任务消息标识 (biTag) | 对应场景化接口中的biTag参数，此字段为可选字段。  批量任务消息标识，[消息回执](/push-kit-guide/push-server/push-msg-receipt)时会返回给应用服务器，长度最大64字节。 |
     | 回执ID (receiptId) | 对应场景化接口中的receiptId参数，此字段为可选字段。  回执ID指定本次下行消息的回执地址及配置。该回执ID可以在[配置回执参数](/push-kit-guide/push-server/push-msg-receipt#配置回执参数)中查看。 |
   - ****配置推送内容-发送目标设备****

     ![](../../../images/8b494e78/zh-cn_image_0000002552959132.png)
...
4. 当您完成上述步骤后，点击右上方“提交”按钮即可推送消息。
   ...
   ![](../../../images/1c6e6704/zh-cn_image_0000002552799484.png)

   ![](../../../images/f9036fff/note_3.0-zh-cn.png) 
