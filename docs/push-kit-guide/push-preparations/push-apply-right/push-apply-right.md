...
为了改善终端用户推送体验、营造良好可持续的通知生态，Push Kit对[通知消息](/push-kit-guide/push-scenes/push-alert/push-send-alert)进行分类管理。根据消息内容，Push Kit将通知消息分类为****服务与通讯****、****资讯营销****两大类别。如您希望通知消息能更精准地符合业务需要，可以根据[通知消息分类标准](#通知消息分类标准与提醒方式)，自行对消息进行分类。

未开通通知消息自分类权益的应用，通知消息类型将会默认归为****资讯营销类消息****。

分类方式示意图：

![](../../../images/0a747844/zh-cn_image_0000002552799462.png)

![](../../../images/cbdb6818/note_3.0-zh-cn.png) 

- 若应用有通知消息自分类权益，且推送通知消息时携带已开通的[category](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-request-param/push-scenariozed-api-request-param#notification)字段，将信任开发者提供的分类信息（若应用仅发送资讯营销消息，则无需申请自分类权益）。
- 若应用推送通知消息时携带未开通权益的[category](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-request-param/push-scenariozed-api-request-param#notification)字段值（例如，未开通“IM”却在推送通知消息时在category中传入“IM”），应用的通知消息将自动归类为资讯营销消息。

通知消息分类标准适用于[推送通知消息](/push-kit-guide/push-scenes/push-alert/push-send-alert)，若您的应用使用推送通知消息，需按照[通知消息分类标准](#通知消息分类标准与提醒方式)进行分类。其他场景化消息可按照相应的指导完成开发适配。
...
![](../../../images/690760c5/note_3.0-zh-cn.png) 
...
![](../../../images/fa87b5a0/zh-cn_image_0000002583439157.png)
...
![](../../../images/956590dd/note_3.0-zh-cn.png) 
...
![](../../../images/898726a9/note_3.0-zh-cn.png) 
...
- 服务与通讯类消息推送数量受设备消息频控限制，系统会根据现网使用场景和流量进行管控，不合理的使用场景系统会进行频控。更多频控说明请参见[消息频控](/ref/push-api/push-rest-api/push-rest-api-scenes/push-msg-freq-control/push-msg-freq-control)。
- 资讯营销类消息会根据应用类型对每日推送数量进行上限管理。详情请见下表。

![](../../../images/786cbb58/note_3.0-zh-cn.png) 
...
- 为避免在调测阶段消息被频控，建议调测阶段发送测试消息，详情请参见[消息频控](/ref/push-api/push-rest-api/push-rest-api-scenes/push-msg-freq-control/push-msg-freq-control)。
...
1. 登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，点击“开发与服务”，在项目列表中找到您的项目，通过“增长 > 推送服务 > 配置”，在“配置”页签下选择需要申请自分类权益的应用，点击****自分类权益****后的“申请”。
   ...
   ![](../../../images/f778cb98/zh-cn_image_0000002552959112.png)
2. 选择消息发送类型，下一步补充消息示例（订阅消息类型需要额外补充场景说明和图片），提交完成后可以通过“查看进展”按钮查看审批进展。

   ![](../../../images/a05b3ced/zh-cn_image_0000002583479113.png)

   ![](../../../images/459f64ff/note_3.0-zh-cn.png) 

   若某消息类型已经申请通过，后续满足此类型的场景范围的消息无需重复申请。

   ![](../../../images/16f66abd/zh-cn_image_0000002552799464.png)

   ![](../../../images/b6bcc243/note_3.0-zh-cn.png) 
   ...
   ![](../../../images/df826b0c/zh-cn_image_0000002583439159.png)
3. 非首次申请时可以点击“新增类型”按钮跳转进行消息类型的申请。

   ![](../../../images/737664bf/zh-cn_image_0000002552959114.png)
4. 消息类型申请审核周期为5个工作日，您可以点击自分类权益后的“****详情****”查看已申请通过的消息类型。也可以通过“申请记录”查看申请过的消息类型。

   ![](../../../images/f40c17fb/zh-cn_image_0000002583479115.png)

   ![](../../../images/2d6e502c/zh-cn_image_0000002552799466.png)

   点击“申请记录”可以查看申请过的消息类型，对申请记录进行查看、编辑、删除等操作。

   ![](../../../images/eaeb3ad3/zh-cn_image_0000002583439161.png)

   ![](../../../images/8320c663/zh-cn_image_0000002552959116.png)
5. 若您的申请已经审核通过（审核通过5分钟后，您申请的自分类权益生效），请根据申请自分类类型适配云端category字段。

   自分类权益生效后，应用推送的通知消息类型将根据您发送消息时的云端[category](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-request-param/push-scenariozed-api-request-param#notification)字段进行归类。

   category字段取值为大写的英文单词，且仅可填写在分类权益页面中已审批通过的category值，若出现分类错误或违反通知消息分类标准的场景，将被判为违规。

   违规行为及相应的处理措施请参见[通知违规处罚标准](/push-kit-guide/push-appendix/push-punishment-standards)。
...
2. 在项目列表中找到您的项目，通过“项目设置 > 选择应用”，在“开放能力管理”页签下找到推送服务的“推送应用内通话消息”权益，点击“申请”。
   ...
   ![](../../../images/793e2a0c/zh-cn_image_0000002583479117.png)

   进入申请页面，按照申请原因现有模板，补充应用信息，上传附件，点击“提交”。

   ![](../../../images/d1f677ad/zh-cn_image_0000002552799468.png)

   ![](../../../images/5cb87d65/note_3.0-zh-cn.png) 
...
3. 申请提交之后，进入“互动中心”页面，审批进展会由智能助手通知，审核期限为8个工作日。如果退出互动中心页面，后续可以点击AGC平台右上角气泡图标再次进入。
   ...
   ![](../../../images/55161d51/zh-cn_image_0000002583439163.png)
4. 审核通过，权益立即生效，推送服务的“推送应用内通话消息”权益被勾选。

   ![](../../../images/95c8b936/zh-cn_image_0000002552959118.png)
...
![](../../../images/5befb16b/note_3.0-zh-cn.png) 
...
2. 在项目列表中找到您的项目，通过“项目设置 > 选择应用”，在“开放能力管理”页签下找到推送服务的“推送语音播报消息”权益，点击“申请”。
   ...
   ![](../../../images/b6838eb0/zh-cn_image_0000002583479119.jpg)

   进入申请页面，按照申请原因现有模板，补充应用信息，上传附件，点击“提交”。

   ![](../../../images/79ed0752/zh-cn_image_0000002552799470.png)

   ![](../../../images/cbd87bed/note_3.0-zh-cn.png) 

   申请权益时，请在附件中上传语音消息通知界面截图或示意图或语音播报录像。
3. 申请提交之后，进入“互动中心”页面，审批进展会由智能助手通知，审核期限为8个工作日。如果退出互动中心页面，后续可以点击AGC平台右上角气泡图标再次进入。

   ![](../../../images/a52af3c9/zh-cn_image_0000002583439165.png)
4. 审核通过，权益立即生效，推送服务的“推送语音播报消息”权益被勾选。

   ![](../../../images/ac848cab/zh-cn_image_0000002552959120.png)

## 申请自定义铃声权益

当用户终端收到您发送的[通知消息](/push-kit-guide/push-scenes/push-alert/push-send-alert)时，如果消息携带[sound](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-request-param/push-scenariozed-api-request-param#notification)字段，通知提示会播放该自定义铃声，否则播放系统默认铃声；如果消息同时携带[soundDuration](/ref/push-api/push-rest-api/push-rest-api-scenes/push-scenariozed-api-request-param/push-scenariozed-api-request-param#notification)字段，则自定义铃声的时长受此字段控制。

![](../../../images/567c4512/note_3.0-zh-cn.png) 
