...
当前支持的编码能力请参考[AVCodec支持的格式](/avcodec-kit/avcodec-support-formats#视频编码)。
...
| 分层编码，LTR设置 | 具体可参考：[时域可分层视频编码](/avcodec-kit/audio-video-codec/video-encoding-temporal-scalability) |
...
![](../../../images/f093d767/zh-cn_image_0000002552958544.png)
...
6. Executing 状态具有三个子状态：Flushed、Running和End-of-Stream：
   ...
   - 当待处理数据全部传递给编码器后，可以在input buffers队列中为最后一个入队的input buffer中添加[AVCODEC\_BUFFER\_FLAGS\_EOS](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avbuffer-info-h/capi-native-avbuffer-info-h#oh_avcodecbufferflags)标记，遇到这个标记时，编码器会转换为End-of-Stream子状态。在此状态下，编码器不再接受新的输入，但是仍然会继续生成输出，直到输出到达尾帧。
7. 使用完编码器后，必须调用OH\_VideoEncoder\_Destroy接口销毁编码器实例，使编码器进入Released状态。

## 开发指导

详细的API说明请参考[native\_avcodec\_videoencoder.h](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-videoencoder-h/capi-native-avcodec-videoencoder-h)。
...
![](../../../images/d29cfe34/zh-cn_image_0000002583478545.png)
...
![](../../../images/4eaeac84/note_3.0-zh-cn.png) 
...
3. 调用OH\_VideoEncoder\_RegisterCallback()设置回调函数。
   ...
   - OH\_AVCodecOnError 编码器运行错误，返回的错误码详情请参见[OH\_AVCodecOnError](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-base-h/capi-native-avcodec-base-h#oh_avcodeconerror)；
   ...
   ![](../../../images/af98f104/note_3.0-zh-cn.png) 

   在回调函数中，对数据队列进行操作时，需要注意多线程同步的问题。
4. （可选）调用OH\_VideoEncoder\_RegisterParameterCallback()在Configure接口之前注册随帧通路回调。

   详情请参考[时域可分层视频编码](/avcodec-kit/audio-video-codec/video-encoding-temporal-scalability)。
...
5. 调用OH\_VideoEncoder\_Configure()配置编码器。
   ...
   参数校验规则请参考[OH\_VideoEncoder\_Configure()参考文档](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-videoencoder-h/capi-native-avcodec-videoencoder-h#oh_videoencoder_configure)。

   参数取值范围可以通过能力查询接口获取，具体示例请参考[获取支持的编解码能力文档](/avcodec-kit/audio-video-codec/obtain-supported-codecs)。
   ...
   ![](../../../images/e512b955/caution_3.0-zh-cn.png) 
...
6. 获取surface。
   ...
   OHNativeWindow\*变量类型的使用方法请参考图形子系统 [OHNativeWindow](/ref/arkgraphics-api/arkgraphics-c/arkgraphics-module/capi-nativewindow/capi-nativewindow)。
...
13. 调用OH\_VideoEncoder\_FreeOutputBuffer()释放编码帧。
    ...
    - buffer：回调函数OnNewOutputBuffer传入的参数，可以通过[OH\_AVBuffer\_GetAddr](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avbuffer-h/capi-native-avbuffer-h#oh_avbuffer_getaddr)接口得到共享内存地址的指针；
...
17. 调用OH\_VideoEncoder\_Destroy()销毁编码器实例，释放资源。
    ...
    ![](../../../images/c83e21cc/note_3.0-zh-cn.png) 
...
3. 调用OH\_VideoEncoder\_RegisterCallback()设置回调函数。
   ...
   ![](../../../images/1c6d3402/note_3.0-zh-cn.png) 
...
8. 调用OH\_VideoEncoder\_PushInputBuffer()写入编码图像。
   ...
   - buffer：回调函数OnNeedInputBuffer传入的参数，可以通过[OH\_AVBuffer\_GetAddr](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avbuffer-h/capi-native-avbuffer-h#oh_avbuffer_getaddr)接口得到共享内存地址的指针；
   ...
   ![](../../../images/4d700771/zh-cn_image_0000002552798896.png)
