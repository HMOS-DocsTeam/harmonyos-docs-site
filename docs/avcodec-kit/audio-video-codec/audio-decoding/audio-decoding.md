...
当前支持的解码能力请参考[AVCodec支持的格式](/avcodec-kit/avcodec-support-formats#音频解码)。
...
![](../../../images/12b615db/note_3.0-zh-cn.png) 

通过MP3音频编码流程生成的码流无法直接通过MP3音频解码流程进行解码。建议通过（PCM码流->MP3音频编码->封装->解封装->MP3音频解码）流程进行。

## 开发指导

详细的API说明请参考[native\_avcodec\_audiocodec.h](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-audiocodec-h/capi-native-avcodec-audiocodec-h)。
...
![](../../../images/0e5a15e8/zh-cn_image_0000002552798892.png)
...
![](../../../images/4a6536a4/note_3.0-zh-cn.png) 
...
3. 调用OH\_AudioCodec\_RegisterCallback()注册回调函数。
   ...
   ![](../../../images/a92e0bb7/caution_3.0-zh-cn.png) 
...
4. （可选）OH\_AudioCodec\_SetDecryptionConfig设置解密配置。
   ...
   DRM相关接口详见[DRM API文档](/ref/drm-api/drm-c/drm-module/capi-drm/capi-drm)。
...
5. 调用OH\_AudioCodec\_Configure()配置解码器。
   ...
   ![](../../../images/2fc474e5/zh-cn_image_0000002552798894.png)

   各音频解码类型参数范围说明：

   ![](../../../images/79719d7c/zh-cn_image_0000002583438589.png)
...
8. （可选）调用OH\_AVCencInfo\_SetAVBuffer()，设置cencInfo。
   ...
   若当前播放的节目是DRM加密节目，且由上层应用做[媒体解封装](/avcodec-kit/file-muxing-demuxing/audio-video-demuxer)，则须调用OH\_AVCencInfo\_SetAVBuffer()将cencInfo设置给AVBuffer，以实现AVBuffer中媒体数据的解密。
...
4. 调用OH\_AudioCodec\_Destroy()销毁解码器实例，释放资源。
   ...
   ![](../../../images/2d540fee/note_3.0-zh-cn.png) 
