...
![](../../../../images/a1390d76/zh-cn_image_0000002583478553.png)

## 限制约束

1. 目前仅硬件解码器支持该能力。
2. 目前仅Surface模式支持该能力。Surface模式和Buffer模式输出差异可参考[视频解码](/avcodec-kit/audio-video-codec/video-decoding)。
3. 目前使能该能力时，不支持码流分辨率变化，会通过回调函数OH\_AVCodecOnError()报告错误码[AV\_ERR\_UNSUPPORT](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-averrors-h/capi-native-averrors-h#oh_averrcode)。
...
![](../../../../images/2971ec27/note_3.0-zh-cn.png) 
...
1. 添加头文件。
   ...
   #include <fstream>
   ```
2. 参考[HDR Vivid视频播放](/avcodec-kit/audio-video-codec/hdr-vivid-capability/hdr-vivid-video-player)，添加头文件和解析文件，查询文件是否为HDR Vivid视频。

   如果非HDR Vivid视频，则参考[视频解码](/avcodec-kit/audio-video-codec/video-decoding)进行解码处理，此处不再赘述。

   如果判断为HDR Vivid视频，则继续执行以下步骤。

   ![](../../../../images/fadd41db/note_3.0-zh-cn.png) 

   如果输入源非HDR Vivid视频，会通过回调函数[OH\_AVCodecOnError()](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-base-h/capi-native-avcodec-base-h#oh_avcodeconerror)报告错误码[AV\_ERR\_VIDEO\_UNSUPPORTED\_COLOR\_SPACE\_CONVERSION](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-averrors-h/capi-native-averrors-h#oh_averrcode)。
3. 创建解码器实例。
   ...
   ![](../../../../images/1599510d/note_3.0-zh-cn.png) 

   由于目前仅硬件解码器支持该能力，因此必须根据解码器name进行创建。
4. 调用OH\_VideoDecoder\_RegisterCallback()设置回调函数。

   具体可参考：[HDR Vivid视频播放-HDR Vivid视频解码](/avcodec-kit/audio-video-codec/hdr-vivid-capability/hdr-vivid-video-player#hdr-vivid视频解码) 中的“步骤3：配置异步回调函数”
5. 调用OH\_VideoDecoder\_Configure()配置解码器。
   ...
   ![](../../../../images/7afe00b9/note_3.0-zh-cn.png) 
...
6. 后续步骤具体可参考：[视频解码Surface模式](/avcodec-kit/audio-video-codec/video-decoding#surface模式)。
