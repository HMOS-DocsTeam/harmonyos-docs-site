...
![](../../../images/2eb0e531/note_3.0-zh-cn.png) 
...
4. 注册NativeImage的回调函数。
   ...
   ![](../../../images/06499ec2/note_3.0-zh-cn.png) 

   在此示例中，回调函数的实现仅仅是将buffer取出来并释放，开发者可以根据业务需求自行拓展。
5. 配置解码器。

   具体开发指导请参考[视频解码Surface模式](/avcodec-kit/audio-video-codec/video-decoding#surface模式)“步骤-5：调用OH\_VideoDecoder\_Configure()配置解码器”。
6. 设置surface。
   ...
   - videoDec：视频解码器实例的指针。创建方式可参考[视频解码Surface模式](/avcodec-kit/audio-video-codec/video-decoding#surface模式)“步骤-2：创建解码器实例对象”。
...
7. 启动解码器。
   ...
   具体开发指导请参考[视频解码Surface模式](/avcodec-kit/audio-video-codec/video-decoding#surface模式)“步骤-8：调用OH\_VideoDecoder\_Start()启动解码器”。
8. 设置surface。
   ...
   1. 如果解码后直接显示，则从XComponent组件获取，获取方式请参考 [XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)；
   2. 如果解码后接OpenGL后处理，则从NativeImage获取，获取方式请参考 [NativeImage](/arkgraphics-2d/native-surface/native-image-guidelines)。
