...
支持的解码能力请参考[AVCodec支持的格式](/avcodec-kit/avcodec-support-formats#音频解码)。

****适用场景****

通常推荐使用异步模式，详细内容请参考[音频解码](/avcodec-kit/audio-video-codec/audio-decoding)。若需要主动请求buffer去送帧，则可以使用同步模式。

将音视频文件解码为PCM码流，通常需要以下步骤：[媒体数据解析](/avcodec-kit/file-muxing-demuxing/audio-video-demuxer) -> 音频解码。

本指南描述音频解码过程：输入音频帧和解码出PCM码流。

## 开发指导

详细的API说明请参考[AudioCodec](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-audiocodec-h/capi-native-avcodec-audiocodec-h)。
...
![](../../../images/b1144613/zh-cn_image_0000002583478543.png)
...
![](../../../images/9db285e6/note_3.0-zh-cn.png) 
...
3. （可选）OH\_AudioCodec\_SetDecryptionConfig设置解密配置。
   ...
   当获取到DRM信息（参考[音视频解封装](/avcodec-kit/file-muxing-demuxing/audio-video-demuxer)开发步骤第4步）后，通过此接口进行解密配置。

   DRM相关接口详见[DRM](/ref/drm-api/drm-c/drm-module/capi-drm/capi-drm)。
...
4. 调用OH\_AudioCodec\_Configure()配置解码器。
   ...
   ![](../../../images/b3038207/zh-cn_image_0000002552798894.png)

   各音频解码类型参数范围说明：

   ![](../../../images/654ba64e/zh-cn_image_0000002583438589.png)

   从API version 20开始，支持[采样率范围](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcapability-h/capi-native-avcapability-h#oh_avcapability_getaudiosupportedsamplerateranges)能力查询，以下几种音频解码类型支持对范围内的任意采样率进行解码：
...
7. （可选）调用OH\_AVCencInfo\_SetAVBuffer()，设置cencInfo。
   ...
   如果当前播放的节目是DRM加密节目，并且由上层应用进行[媒体数据解析](/avcodec-kit/file-muxing-demuxing/audio-video-demuxer)，则需要调用OH\_AVCencInfo\_SetAVBuffer()将cencInfo设置给AVBuffer，以实现媒体数据的解密。
...
12. 调用OH\_AudioCodec\_Destroy()销毁解码器实例，释放资源。
    ...
    ![](../../../images/43ca2f84/note_3.0-zh-cn.png) 
