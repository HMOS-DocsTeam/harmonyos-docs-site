...
当前支持的封装能力请参考[AVCodec支持的格式](/avcodec-kit/avcodec-support-formats#媒体数据封装)。
...
详细的API说明请参考[native\_avmuxer.h](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avmuxer-h/capi-native-avmuxer-h)。

![](../../../images/42492636/note_3.0-zh-cn.png) 

如果调用封装模块写本地文件，需要[向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)：ohos.permission.READ\_MEDIA, ohos.permission.WRITE\_MEDIA。
...
不同的封装格式需要配置的key请参考[AVCodec支持的格式](/avcodec-kit/avcodec-support-formats#媒体数据封装)。
...
4. 添加文件级数据。
   ...
   文件级数据已定义的key详见[AVCodec支持的格式](/avcodec-kit/avcodec-support-formats#媒体数据封装)。

   用户自定义的key必须以"com.openharmony."为开头。值类型可以为int32\_t、float、string，从API20开始增加支持uint8\_t\*。

   ![](../../../images/83d280ef/note_3.0-zh-cn.png) 
...
6. 添加视频轨。
   ...
   ![](../../../images/98baf3e8/note_3.0-zh-cn.png) 

   - 当开发者需要设置色彩信息时，必须保证OH\_MD\_KEY\_COLOR\_PRIMARIES、OH\_MD\_KEY\_TRANSFER\_CHARACTERISTICS、OH\_MD\_KEY\_MATRIX\_COEFFICIENTS三个key设置的值都在其各自的参数范围内，系统才会识别这是有效色彩信息数据。参数设置范围可参考[OH\_ColorPrimary](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-base-h/capi-native-avcodec-base-h#oh_colorprimary)、[OH\_TransferCharacteristic](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-base-h/capi-native-avcodec-base-h#oh_transfercharacteristic)和[OH\_MatrixCoefficient](/ref/avcodec-api/avcodec-c/avcodec-headerfile/capi-native-avcodec-base-h/capi-native-avcodec-base-h#oh_matrixcoefficient)。
...
7. 添加辅助轨。
   ...
   ![](../../../images/ac470843/note_3.0-zh-cn.png) 
