...
![](../../../images/5234d0c4/zh-cn_image_0000002552799496.png)

## 前提条件

参见[开发准备](/map-kit-guide/map-config-agc)。
...
2. 在容器中声明FunctionalInput，指定FunctionalInput的inputType，并设置对应的回调函数，代码如下：
   ...
   ![](../../../images/db6eaf29/note_3.0-zh-cn.png) 
   ...
   - 可从返回结果中自行处理结果回填至组件中。
   - 组件支持显示两种类型的图标：symbol和image，"icon"设置为symbol资源时，请使用"[iconSymbolModifier](/ref/scenario-fusion-api/scenario-fusion-kit-arkts-components/scenario-fusion-functionalinputcomponentmanager/scenario-fusion-functionalinputcomponentmanager#functionalinputparams)"进行图标事件、样式的设置；设置为image资源时，请使用"[iconImgModifier](/ref/scenario-fusion-api/scenario-fusion-kit-arkts-components/scenario-fusion-functionalinputcomponentmanager/scenario-fusion-functionalinputcomponentmanager#functionalinputparams)"进行图标事件、样式的设置。
   - functionalInput支持[智能填充](/scenario-fusion-guide/scenario-fusion-intelligent-filling/scenario-fusion-introduction-to-smart-fill)，对应支持的[ContentType](/scenario-fusion-guide/scenario-fusion-intelligent-filling/scenario-fusion-intelligentfilling-appendix)为"ADDRESS\_CITY\_AND\_STATE"。

   其他参数请参考：[FunctionalInput（Input组件）](/ref/scenario-fusion-api/scenario-fusion-kit-arkts-components/scenario-fusion-functionalinput/scenario-fusion-functionalinput)。
