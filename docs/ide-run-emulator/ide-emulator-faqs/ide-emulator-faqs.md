...
title: "模拟器错误码"
sidebar_position: 7
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-emulator-faqs
...
![](../../images/97637672/zh-cn_image_0000002561831011.png)
...
2. 如果CPU支持虚拟化，打开控制面板 > 程序 > 程序和功能 > 启动或关闭Windows功能（Windows11系统中打开系统 > 可选功能 > 相关设置 > 更多Windows功能），检查功能“Hyper-V”、“Windows虚拟机监控程序平台”、“虚拟机平台”是否存在。
   ...
   ![](../../images/b6e966e6/zh-cn_image_0000002530751086.png)
...
4. 如果按照上述步骤无法解决，打开****任务管理器->性能****，若显示虚拟化已禁用或未开启，则是BIOS中虚拟化没有开启，请根据计算机的主板型号，进入BIOS设置界面，并开启虚拟化功能。
   ...
   ![](../../images/18d189d4/zh-cn_image_0000002530911074.png)
...
当前计算机不支持使用模拟器，建议更换支持模拟器的计算机，详细请参考[使用环境](/ide-run-emulator/ide-emulator-overview/ide-emulator-requirements)。
...
1. 首先打开****任务管理器>详细信息****，在列表表头右键****>选择列****，找到并勾选“****提交大小****”，点击“****提交大小****”列进行排序，关闭部分提交大小占用高的进程。
   ...
   ![](../../images/2c00329f/zh-cn_image_0000002561831013.png)
2. 打开****任务管理器>性能>内存****页面，确保已提交内存的剩余量大于模拟器设置的RAM大小。

   ![](../../images/407ab478/zh-cn_image_0000002561831015.png)
...
- 重新下载镜像，具体请参考[创建模拟器](/ide-run-emulator/ide-emulator-management/ide-emulator-create)。
- 删除已创建的模拟器，然后重新创建模拟器。
...
升级显卡驱动，如果原显卡不支持OpenGL则需要更换显卡，具体要求请参考[使用环境](/ide-run-emulator/ide-emulator-overview/ide-emulator-requirements)。
...
1. 执行如下命令，结束hdc的进程，然后尝试重新连接，更多关于hdc工具的使用指导请参考[hdc](/system-debug-optimize/debugging-commands/hdc)。

   ```
   hdc kill
   ```
2. 若按照步骤1操作后还是不能连接，请重启DevEco Studio和模拟器，然后尝试重新连接。
3. 重新下载模拟器镜像，具体请参考[创建模拟器](/ide-run-emulator/ide-emulator-management/ide-emulator-create)。
...
- 请参考[9568347 解析本地so文件失败](/system-debug-optimize/debugging-commands/bm-tool#section9568347-解析本地so文件失败)。
...
建议在创建模拟器时增加模拟器的运行内存（RAM）大小，请参考[创建模拟器](/ide-run-emulator/ide-emulator-management/ide-emulator-create#section1764055173710)。
...
更换系统类型再使用模拟器，具体运行环境要求请参考[使用环境](/ide-run-emulator/ide-emulator-overview/ide-emulator-requirements)。
