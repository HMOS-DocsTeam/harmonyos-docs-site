...
![](../../images/5bfd2250/note_3.0-zh-cn.png) 
...
![](../../images/efd54281/zh-cn_image_0000002583478791.png)
...
- 同一个Ability内不允许重复调用[loadContent()](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiextensioncontentsession/js-apis-app-ability-uiextensioncontentsession#loadcontent)方法加载页面。
- 应用/元服务需要接入隐私声明托管服务。
...
隐私管理服务提供以下接口，具体API说明详见[接口文档](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager)。

| 接口名 | 描述 |
| --- | --- |
| [getAppPrivacyMgmtInfo](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagergetappprivacymgmtinfo)(): [AppPrivacyMgmtInfo](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#appprivacymgmtinfo) | 查询隐私链接信息接口，用于查询隐私链接信息。 |
| [getAppPrivacyResult](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagergetappprivacyresult)(): [AppPrivacyResult](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#appprivacyresult)[] | 查询隐私签署状态接口，用于查询隐私签署状态信息。 |
| [disableService](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagerdisableservice)():void | 撤销同意记录接口，用于撤销隐私签署同意记录。 |
| [requestAppPrivacyConsent](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagerrequestappprivacyconsent)(context:common.UIAbilityContext):Promise&lt;[ConsentResult](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#consentresult)&gt; | 请求用户同意接口，用于开发者需要主动拉起标准化隐私弹框。 |
...
1. 导入privacyManager模块及相关公共模块。
   ...
   ```
2. 调用[getAppPrivacyMgmtInfo](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagergetappprivacymgmtinfo)方法查询隐私链接信息。
...
1. 导入privacyManager模块及相关公共模块。
   ...
   import { hilog } from '@kit.PerformanceAnalysisKit';
   ```
2. 调用[getAppPrivacyResult](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagergetappprivacyresult)方法查询隐私签署状态。
...
1. 导入privacyManager模块及相关公共模块。
   ...
   import { hilog } from '@kit.PerformanceAnalysisKit';
   ```
2. 调用[disableService](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagerdisableservice)方法撤销同意记录。
...
1. 导入privacyManager模块及相关公共模块。
   ...
   ```
2. 调用[requestAppPrivacyConsent](/ref/store-api/store-arkts/store-privacymanager/store-privacymanager#privacymanagerrequestappprivacyconsent)方法拉起标准化隐私弹框请求用户同意。
...
![](../../images/a802562a/zh-cn_image_0000002552799142.png)
...
| 事件名称 | 值 | 描述 |
| --- | --- | --- |
| [COMMON\_EVENT\_PRIVACY\_STATE\_CHANGED](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/process-thread-communi-arkts/commoneventmanager-definitions/commoneventmanager-definitions#common_event_privacy_state_changed11) | usual.event.PRIVACY\_STATE\_CHANGED | 隐私弹框签署结果公共事件，事件携带数据如下：  {  'resultType': privacyResultType,  'appIndex': appIndex  }  其中：  - privacyResultType：  1：同意完整模式  0：未同意  - appIndex：分身索引 |
...
1. 将应用工程构建模式修改为[debug模式](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-sample#section390311716277)。
