...
![](../../../images/f68d8352/note_3.0-zh-cn.png) 
...
- 设置测试账号
  ...
  2. 左侧导航栏选择“沙盒测试 > 测试账号”，点击“新增”。
     ...
     ![](../../../images/5055508d/zh-cn_image_0000002583438817.png)
  3. 填写测试账号信息后，点击“确认”。

     ![](../../../images/b1769f9b/zh-cn_image_0000002552958772.png)

  ![](../../../images/3d5b83a2/note_3.0-zh-cn.png) 
...
2. 参见[配置签名信息](/ide-publish-app#section280162182818)，在DevEco Studio侧配置签名信息。
...
在满足沙盒测试条件下，开发者调用[createPurchase](/ref/iap-api/iap-arkts/iap-iap/iap-iap#iapcreatepurchase)接口拉起收银台，跳过实际支付环节，直接购买成功。

- 沙盒环境下的购买流程与正式环境的购买流程一致，仍需要完成绑定付款方式，但该过程不会真实扣费。
- 购买成功后的收据信息[PurchaseOrderPayload](/ref/iap-api/iap-arkts/iap-data-model/iap-data-model#purchaseorderpayload)中，会携带值为"SANDBOX"的environment字段，标识此次购买为沙盒测试的记录。
- 沙盒环境下购买非消耗型商品，购买之后可以确认发货以完成购买，之后可以再次购买，以方便测试。

  ![](../../../images/ee0a6955/note_3.0-zh-cn.png) 

  正式环境下，非消耗型商品仅允许用户购买一次，不能重复购买。
- 沙盒测试拉起收银台时，会在收银台展示沙盒测试提示，结果页也有沙盒环境的标志，如下图所示。

![](../../../images/64021f52/note_3.0-zh-cn.png) 

如果未显示截图的提示页面，表示本次交易未进入沙盒测试环境，继续测试会实际扣费，请检查当前是否满足沙盒测试的两个条件。开发者也可在应用中使用[isSandboxActivated](/ref/iap-api/iap-arkts/iap-iap/iap-iap#iapissandboxactivated)接口来检查当前沙盒环境不可用的原因。

![](../../../images/9bb07b7f/zh-cn_image_0000002583478773.png)
...
- 在沙盒测试环境下，订阅首期由用户发起后会自动续期五次（累计共六期），后续需用户手动操作以恢复订阅；若同时涉及[促销场景](/iap-kit-guide/iap-purchases/iap-subscription/iap-subscription-functions#提供优惠)，系统将优先完成优惠周期内的自动续期，再继续进行六次续期，此场景下总续期次数为优惠周期数与六次续期之和。
- 沙盒测试拉起收银台时，会在收银台展示沙盒测试提示，结果页也有沙盒环境的标志，如下图所示。

![](../../../images/c2fae348/zh-cn_image_0000002552799124.png)
...
![](../../../images/5e29a14c/zh-cn_image_0000002583438819.png)
...
1. 登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，选择“用户与访问”。
   ...
   ![](../../../images/19f82b3a/zh-cn_image_0000002552958774.png)
2. 左侧导航栏选择“沙盒测试 > 测试账号”，勾选对应的测试账号，点击右上角的“清除购买历史记录”按钮。

   ![](../../../images/b069d23c/zh-cn_image_0000002583478775.png)
3. 在出现的提示弹窗中，点击“确认”按钮，随后该账号在沙盒环境中产生的购买历史记录将被清除，此操作无法被撤销。如果该沙盒账号的购买次数较多，则清除其购买历史记录可能需要更长时间。

![](../../../images/f5c830fd/zh-cn_image_0000002552799126.png)
