...
应用归因服务接入调试功能提供以下接口，具体API说明详见[接口文档](/ref/store-api/store-arkts/store-attributiontestmanager/store-attributiontestmanager)。
...
1. 导入相关模块。
   ...
   ```
2. 构造参数，入参为[AdSourceInfo](/ref/store-api/store-arkts/store-attributiontestmanager/store-attributiontestmanager#adsourceinfo)、publickey。
   ...
   ```
3. 调用[attributionTestManager.validateSource](/ref/store-api/store-arkts/store-attributiontestmanager/store-attributiontestmanager#attributiontestmanagervalidatesource)方法验证归因来源。
...
1. 导入相关模块。
   ...
   import { BusinessError,deviceInfo } from '@kit.BasicServicesKit';
   ```
2. 构造参数，入参为[PostbackInfo](/ref/store-api/store-arkts/store-attributiontestmanager/store-attributiontestmanager#postbackinfo)。
   ...
   ```
3. 调用[attributionTestManager.setPostback](/ref/store-api/store-arkts/store-attributiontestmanager/store-attributiontestmanager#attributiontestmanagersetpostback)方法设置归因结果回传数据。
...
2. 构造参数adTechId。
   ...
   ```
3. 调用[attributionTestManager.flushPostbacks](/ref/store-api/store-arkts/store-attributiontestmanager/store-attributiontestmanager#attributiontestmanagerflushpostbacks)方法触发归因结果回传。
