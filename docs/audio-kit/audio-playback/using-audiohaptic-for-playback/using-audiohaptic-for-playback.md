...
使用AudioHaptic播放音频并同步开启振动，涉及到音频及振动资源的管理、音频时延模式及音频流使用类型的配置、音振播放器的创建及管理等。本开发指导将以一次音振协同播放的过程为例，向开发者讲解如何使用AudioHaptic进行音振协同播放，建议配合[audioHaptic](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic)的API说明阅读。

### 权限申请

如果应用创建的AudioHapticPlayer需要触发振动，则需要校验应用是否拥有该权限：ohos.permission.VIBRATE。

1. [声明权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions)。
2. [向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。
...
1. 获取音振管理器实例，并注册音频及振动资源，资源支持情况可以查看[AudioHapticManager](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#audiohapticmanager)。

   ![](../../../images/d5fec06d/note_3.0-zh-cn.png) 

   开发者可通过如下两种方式注册资源：

   - 方式1：使用[registerSource](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#registersource)接口，通过文件URI来注册资源。
   - 方式2（推荐）：从API version 20开始，支持使用[registerSourceFromFd](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#registersourcefromfd20)接口，通过文件描述符来注册资源，更便于开发者使用。
   ...
   ```
2. 设置音振播放器参数，各参数作用可以查看[AudioHapticManager](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#audiohapticmanager)。
   ...
   ```
3. 调用[createPlayer](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#createplayer)方法，创建AudioHapticPlayer实例。
   ...
   ```
4. 调用[start](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#start)方法，开启音频播放并同步开启振动。
   ...
   ```
5. 调用[stop](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#stop)方法，停止音频播放并同步停止振动。
   ...
   ```
6. 调用[release](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#release)方法，释放AudioHapticPlayer实例。
   ...
   ```
7. 调用[unregisterSource](/ref/audio-api/audio-arkts/js-apis-audiohaptic/js-apis-audiohaptic#unregistersource)方法，将已注册的音频及振动资源移除注册。
