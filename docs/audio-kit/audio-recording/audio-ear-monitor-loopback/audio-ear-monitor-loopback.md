...
当启用音频返听时，系统会创建低时延渲染器与低时延采集器，实现低时延耳返功能。采集的音频直接通过内部路由返回到渲染器。对于渲染器，其音频焦点策略与[STREAM\_USAGE\_MUSIC](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#streamusage)相匹配。对于采集器，其音频焦点策略与[SOURCE\_TYPE\_MIC](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#sourcetype8)相匹配。
...
- 低功耗渲染器和低时延渲染器在API version 20不能实现并发。若要启用渲染器，建议采用[STREAM\_USAGE\_UNKNOWN](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#streamusage)；系统内决策采用[STREAM\_USAGE\_MUSIC](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#streamusage)创建普通渲染器。

## 开发指导

使用AudioLoopback音频返听涉及到[isAudioLoopbackSupported](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audiostreammanager/arkts-apis-audio-audiostreammanager#isaudioloopbacksupported20)返听能力查询、AudioLoopback实例创建、返听音量设置、返听状态监听与返听启用禁用等。本开发指导将以一次启用返听的过程为例，向开发者讲解如何使用AudioLoopback进行音频返听，建议搭配[AudioLoopback](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback)的API说明阅读。
...
![](../../../images/b80f02a8/zh-cn_image_0000002583438577.png)

使用[on('statusChange')](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#onstatuschange20)方法可以监听AudioLoopback的状态变化，每个状态对应值与说明见[AudioLoopbackStatus](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#audioloopbackstatus20)。
...
1. 查询返听能力并创建AudioLoopback实例，音频返听模式可以查看[AudioLoopbackMode](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#audioloopbackmode20)。

   ![](../../../images/18066a96/note_3.0-zh-cn.png) 

   返听需要申请麦克风权限ohos.permission.MICROPHONE，申请方式参考：[向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。
   ...
   ```
2. 调用[getStatus](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#getstatus20)方法，查询当前返听状态。

   ![](../../../images/90555e4b/caution_3.0-zh-cn.png) 
   ...
   ```
3. 调用[setVolume](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#setvolume20)方法，设置音频返听音量。

   ![](../../../images/87768caf/caution_3.0-zh-cn.png) 
   ...
   ```
4. 从API21开始，支持调用[setReverbPreset](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#setreverbpreset21)方法，设置音频返听的混响模式。

   ![](../../../images/9367278c/caution_3.0-zh-cn.png) 

   - 在启用返听前设置混响模式，混响模式将在启用返听成功后生效。
   - 在启用返听后设置混响模式，混响模式将立即生效。
   - 启用返听前未设置混响模式，启用返听时将采用默认混响模式[THEATER](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#audioloopbackreverbpreset21)。
   ...
   ```
5. 从API21开始，支持调用[getReverbPreset](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#getreverbpreset21)方法，查询当前的音频返听的混响模式。

   ![](../../../images/a54e63ce/caution_3.0-zh-cn.png) 

   若未设置混响模式，查询得到将是默认混响模式[THEATER](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#audioloopbackreverbpreset21)。
   ...
   ```
6. 从API21开始，支持调用[setEqualizerPreset](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#setequalizerpreset21)方法，设置音频返听的均衡器类型。

   ![](../../../images/08d8d238/caution_3.0-zh-cn.png) 

   - 在启用返听前设置均衡器类型，均衡器类型将在启用返听成功后生效。
   - 在启用返听后设置均衡器类型，均衡器类型将立即生效。
   - 启用返听前未设置均衡器类型，启用返听时将采用默认均衡器类型[FULL](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#audioloopbackequalizerpreset21)。
   ...
   ```
7. 从API21开始，支持调用[getEqualizerPreset](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#getequalizerpreset21)方法，查询当前的音频返听的均衡器类型。

   ![](../../../images/bc4aed0c/caution_3.0-zh-cn.png) 

   若未设置均衡器类型，查询得到将是默认均衡器类型[FULL](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-e/arkts-apis-audio-e#audioloopbackequalizerpreset21)。
   ...
   ```
8. 调用[enable](/ref/audio-api/audio-arkts/js-apis-audio/arkts-apis-audio-audioloopback/arkts-apis-audio-audioloopback#enable20)方法，启用或禁用音频返听功能。
