...
需要注意的是，napi\_add\_env\_cleanup\_hook接口并不支持对同一arg绑定多个回调。若出现env已销毁，但cleanup回调未被执行的情况，可以在启用ArkTS运行时[多线程检测](/ide-log-and-fault-analysis/ide-fault-analysis/ide-multi-thread-check)功能的前提下，查看hilog流水日志AddCleanupHook Failed, data cannot register multiple times.来查找发生注册失败的调用。
...
[使用Worker进行线程间通信](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction)
