...
1. 可根据以下文档进行排查：
   ...
   [ArkTS侧import xxx from libxxx.so后，使用xxx报错显示undefined/not callable或明确的Error message](/coding/using-napi-interaction-with-cpp/napi-questions/napi-faq-about-common-basic#arkts侧报错显示undefined)

   [模块注册与模块命名](/coding/using-napi-interaction-with-cpp/napi-guidelines#模块注册与模块命名)
2. 同时也可参考动态加载能力是否可以满足该场景：

   [napi\_load\_module\_with\_info支持的场景](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-napi-load-module-with-info#napi_load_module_with_info支持的场景)

   [napi\_load\_module支持的场景](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-napi-load-module#napi_load_module支持的场景)
...
[使用Node-API接口进行线程安全开发](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-napi-thread-safety)

此外，Node-API中常见的抛任务方法的差异如下：

1. napi\_async\_work系列接口只能保证execute\_cb在complete\_cb之前执行，但无法保证不同napi\_async\_work的时序。

   [napi\_queue\_async\_work\_with\_qos](/ref/napi/napi#napi_queue_async_work_with_qos)是在普通napi\_queue\_async\_work的基础上，支持自定义qos优先级，而这里只是指定libuv调度任务时使用线程的优先级，不是指任务的优先级，所以也无法保证任务的时序。
2. napi\_threadsafe\_function系列接口内部维护了一个队列，可以保证任务执行的顺序。
   ...
   [使用Node-API接口从异步线程向ArkTS线程投递指定优先级和入队方式的任务](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-call-threadsafe-function-with-priority)
...
[使用Node-API接口进行异步任务开发](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-napi-asynchronous-task)
...
[使用Node-API接口进行object相关开发](/coding/using-napi-interaction-with-cpp/napi-use/use-napi-about-object)

[使用Node-API接口进行array相关开发](/coding/using-napi-interaction-with-cpp/napi-use/use-napi-about-array)
...
2. 在C++线程创建出ArkTS运行环境
   ...
   [使用Node-API接口创建ArkTS运行时环境](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-napi-ark-runtime)
...
1. napi\_env和ArkTS线程是绑定的，napi\_env不能跨线程使用，否则会导致稳定性问题。可参考文档
   ...
   [多线程限制](/coding/using-napi-interaction-with-cpp/use-napi-process#多线程限制)。
...
Node-API接口正常执行后，会返回一个napi\_ok的状态枚举值，若Node-API接口返回值不为napi\_ok，可先参考[Node-API接口返回状态码介绍](/coding/using-napi-interaction-with-cpp/napi_status_introduction)。
...
原因一：napi\_call\_threadsafe\_function函数调用返回值不为napi\_ok。请确认调用napi\_call\_threadsafe\_function相关函数的返回值是否都是napi\_ok，若不是，请根据[Node-API接口返回状态码介绍](/coding/using-napi-interaction-with-cpp/napi_status_introduction)排查返回值非napi\_ok的原因。
