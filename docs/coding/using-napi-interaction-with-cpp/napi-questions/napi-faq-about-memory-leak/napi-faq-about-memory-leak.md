...
[使用Node-API接口进行生命周期相关开发](/coding/using-napi-interaction-with-cpp/napi-use/use-napi-life-cycle)

使用Node-API时导致内存泄漏的常见原因：

1. napi\_value不在napi\_handle\_scope管理中，导致napi\_value持有的ArkTS对象无法释放，该问题常见于[直接使用uv\_queue\_work的场景](/coding/using-napi-interaction-with-cpp/napi-guidelines#异步任务)中。解决方法是添加napi\_open\_handle\_scope和napi\_close\_handle\_scope接口。
