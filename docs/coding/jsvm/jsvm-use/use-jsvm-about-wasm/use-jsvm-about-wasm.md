...
权限要求：WebAssembly相关接口需要应用拥有JIT权限才能执行，可参考[JSVM 申请JIT权限指导](/coding/jsvm/jsvm-scenarios/jsvm-apply-jit-profile)申请对应权限。

运行限制：当前 JSVM 版本在坚盾守护模式下将禁用 WebAssembly 全部功能模块。开发者需针对此限制进行应用兼容性评估，具体技术规范详见[JSVM 坚盾守护模式](/coding/jsvm/jsvm-scenarios/jsvm-secure-shield-mode)。
...
参考 [使用JSVM-API实现JS与C/C++语言交互开发流程](/coding/jsvm/use-jsvm-process) 了解 JSVM-API 接口开发流程。本文仅展示接口对应的 C++ 代码。
