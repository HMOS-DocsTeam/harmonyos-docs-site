...
![](../../images/41403e32/note_3.0-zh-cn.png) 

1. 以下接口的使用仅允许在当前输入法应用中调用。
2. 本示例假设已经在输入法应用中执行，如何实现一个输入法应用，请参考[实现一个输入法应用](/ime-kit/inputmethod-application-guide)开发指导。

## 切换当前输入法子类型

1. 在已完成一个输入法应用的基础上，当输入法应用是当前输入法时，在输入法应用中使用[switchCurrentInputMethodSubtype](/ref/ime-api/ime-arkts/js-apis-inputmethod/js-apis-inputmethod#inputmethodswitchcurrentinputmethodsubtype9)接口，传入当前输入法的子类型[InputMethodSubtype](/ref/ime-api/ime-arkts/js-apis-inputmethod-subtype/js-apis-inputmethod-subtype#inputmethodsubtype)作为参数即可切换当前输入法子类型。
...
在已完成一个输入法应用的基础上，当输入法应用是当前输入法时，在输入法应用中使用[switchInputMethod](/ref/ime-api/ime-arkts/js-apis-inputmethod/js-apis-inputmethod#inputmethodswitchinputmethod9)接口，传入目标输入法的[InputMethodProperty](/ref/ime-api/ime-arkts/js-apis-inputmethod/js-apis-inputmethod#inputmethodproperty8)信息，即可切换输入法到目标输入法。
...
在已完成一个输入法应用的基础上，当输入法应用是当前输入法时，在输入法应用中使用[switchCurrentInputMethodAndSubtype](/ref/ime-api/ime-arkts/js-apis-inputmethod/js-apis-inputmethod#inputmethodswitchcurrentinputmethodandsubtype9)接口，传入目标输入法的[InputMethodProperty](/ref/ime-api/ime-arkts/js-apis-inputmethod/js-apis-inputmethod#inputmethodproperty8)，目标输入法的子类型[InputMethodSubtype](/ref/ime-api/ime-arkts/js-apis-inputmethod-subtype/js-apis-inputmethod-subtype#inputmethodsubtype)信息，即可切换输入法到目标输入法的指定子类型。
