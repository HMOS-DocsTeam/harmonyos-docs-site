...
1.确保引入的SO动态库是通过[HarmonyOS NDK 编译工具链](/build-with-ndk/build-with-ndk-overview)编译生成，如何通过[HarmonyOS NDK 编译工具链](/build-with-ndk/build-with-ndk-overview)编译预构建库，请参考[CMake构建三方库适配流程](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-cmake-adapts-to-harmonyos#section1826019653918)。

2.确保引入的SO动态库的依赖库也导入到工程中且通过[HarmonyOS NDK 编译工具链](/build-with-ndk/build-with-ndk-overview)编译生成。

## 直接引入预构建库

可以通过直接将预构建的库文件复制到项目文件中, 来使用预构建库。例如在项目中需要使用预构建库libavcodec\_ffmpeg.so，其开发态存放路径如下图所示：

![](../../images/0e9633af/zh-cn_image_0000002552959360.png)
...
![](../../images/fc92112b/zh-cn_image_0000002583479361.png)
