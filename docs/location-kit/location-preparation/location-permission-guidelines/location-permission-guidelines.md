...
Location Kit接口对权限的要求参见API参考：[@ohos.geoLocationManager (位置服务)](/ref/location-api/location-arkts/js-apis-geolocationmanager/js-apis-geolocationmanager)。

## 开发步骤

1. 开发者可以在应用配置文件中声明所需要的权限并向用户申请授权，具体可参考[向用户申请授权](/system-security/access-control/app-permission-mgmt/request-app-permissions/request-user-authorization)。
...
长时任务申请可参考：[长时任务介绍](/background-task-kit/continuous-task)。
