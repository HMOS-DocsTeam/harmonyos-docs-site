...
需配合[使用PhotoPicker组件访问图片/视频](/medialibrary-kit/component-guidelines-photoviewpicker)一起使用，用户通过AlbumPickerComponent组件选择对应相册并通知PhotoPickerComponent组件刷新成对应相册的图片和视频。

界面效果如图所示。

![](../../images/30398b8e/zh-cn_image_0000002583438651.png)
...
2. 创建相册组件配置选项实例（AlbumPickerOptions）。
   ...
   通过AlbumPickerOptions，开发者可配置相册页主题颜色，详见[AlbumPickerOptions API参考](/ref/media-library-api/media-library-comp/ohos-file-albumpickercomponent/ohos-file-albumpickercomponent#albumpickeroptions)。
...
3. 初始化组件配置选项实例（AlbumPickerOptions）。
   ...
   ```
4. 创建[AlbumPickerComponent](/ref/media-library-api/media-library-comp/ohos-file-albumpickercomponent/ohos-file-albumpickercomponent#albumpickercomponent)组件。
