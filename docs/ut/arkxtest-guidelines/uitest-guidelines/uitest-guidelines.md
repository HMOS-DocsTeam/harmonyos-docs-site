...
![](../../../images/e2315621/zh-cn_image_0000002550617948.png)
...
UI测试是在[单元测试](/ut/arkxtest-guidelines/unittest-guidelines)基础上进行UITest接口调用，接口的详细定义与参数说明可参考[API文档](/ref/system-debug-optimize-api/test-api/test-arkts/js-apis-uitest/js-apis-uitest)。

### UI测试示例

下面提供一个UI测试的简单示例，介绍如何在单元测试脚本基础上进行UI测试的增量开发，具体实现功能如下：

1. 调用[程序框架服务](/ref/system-debug-optimize-api/test-api/test-arkts/test-api-interface-depend/js-apis-inner-application-abilitydelegator/js-apis-inner-application-abilitydelegator)能力，启动目标被测应用，并确认应用运行状态。
2. 调用UI测试框架能力，页面中执行点击操作。
3. 通过[添加断言](/ut/arkxtest-guidelines/unittest-guidelines#断言能力)，验证操作后当前页面的实际变化是否与预期结果一致。
...
UITest支持[依据多种属性构造匹配器](/ref/system-debug-optimize-api/test-api/test-arkts/js-apis-uitest/js-apis-uitest#on9)进行控件查找；支持查找当前页面符合匹配条件的单个或多个目标控件，并返回控件对象；支持在滚动组件内部进行滚动查找目标控件；支持[对控件对象进行操作或获取控件的属性信息](/ref/system-debug-optimize-api/test-api/test-arkts/js-apis-uitest/js-apis-uitest#component9)。
...
UITest支持向指定坐标点或指定控件输入文本内容，同时支持[指定输入方式](/ref/system-debug-optimize-api/test-api/test-arkts/js-apis-uitest/js-apis-uitest#inputtextmode20)：输入文本时是否以复制粘贴方式输入、是否以追加的方式进行输入。
...
![](../../../images/9e093e13/note_3.0-zh-cn.png) 

1. 指定截图文件保存路径，路径需为当前应用的[沙箱路径](/core-file-kit/app-file/app-sandbox-directory)。
2. 测试HAP的[APL等级级别](/system-security/access-control/app-permission-mgmt/app-permission-mgmt-overview#权限机制中的基本概念)为normal，对应要求使用用户级加密区的应用沙箱路径。且需指定将文件保存在应用在本设备上存放持久化数据的子目录。

如下给出屏幕截图的示例，指定屏幕id和截取屏幕区域，并将截图保存到指定路径下。下面代码执行前请参考UI测试示例，实现对应的Index.ets页面代码。多屏场景下，期望对指定屏幕做截图操作时，可以调用display模块的接口[获取Display对象](/arkui/display-manager/screenproperty-guideline#获取display对象)，实现[屏幕相关属性获取](/arkui/display-manager/screenproperty-guideline#获取屏幕相关属性)。
...
根据hdc命令行工具指导，完成[环境准备](/system-debug-optimize/debugging-commands/hdc#环境准备)。确保设备已成功连接，并执行hdc shell。
...
| -w | <windowId> | 获取指定ID目标窗口的控件树信息。  ****说明:****  可通过hidumper工具[获取应用窗口信息](/system-debug-optimize/debugging-commands/hidumper-tool/hidumper#获取应用窗口信息), 包含应用对应窗口的id。 |
| -m | <true|false> | 指定在获取控件树信息时是否合并窗口信息。true表示合并窗口信息，false表示不合并窗口信息，不设置时默认为true。 |
| -d | <displayId> | 多屏场景下，获取指定ID屏幕下的控件树。  ****说明：****  1. 从API version 20开始支持该命令。  2. 可通过hidumper工具[获取应用窗口信息](/system-debug-optimize/debugging-commands/hidumper-tool/hidumper#获取应用窗口信息)，包含应用对应窗口的DisplayId。 |
...
![](../../../images/8c9ae85b/note_3.0-zh-cn.png) 
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| keyID1 | 是 | 实体按键对应ID，取值范围：Back、Home、Power、或[KeyCode键码值](/ref/system-basicfun-api/input-api/input-arkts/js-apis-keycode/js-apis-keycode#keycode)。  当取值为Back、Home或Power时，不支持输入组合键。  当前注入大写锁定键（KeyCode=2074）无效，请使用组合键实现大写字母输入。如“按键shift+按键V”输入大写字母V。 |
| keyID2 | 否 | 实体按键对应ID，取值范围：[KeyCode键码值](/ref/system-basicfun-api/input-api/input-arkts/js-apis-keycode/js-apis-keycode#keycode)，默认值为空。 |
| keyID3 | 否 | 实体按键对应ID，取值范围：[KeyCode键码值](/ref/system-basicfun-api/input-api/input-arkts/js-apis-keycode/js-apis-keycode#keycode)，默认值为空。 |
...
![](../../../images/507e0a80/note_3.0-zh-cn.png) 

仅元能力aa test拉起的测试HAP才能调用Uitest的能力，且测试HAP的[APL等级级别](/system-security/access-control/app-permission-mgmt/app-permission-mgmt-overview#权限机制中的基本概念)需为normal。
