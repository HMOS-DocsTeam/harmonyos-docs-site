...
![](../images/249df50a/note_3.0-zh-cn.png) 
...
- 非resources目录下资源打包策略请参考[copyCodeResource](/ide-hvigor-configuration-file/ide-hvigor-build-profile#table1476161719356)描述。
...
| resfile目录 | 支持创建多层子目录，子目录名称可以自定义，文件夹内可以自由放置各类资源文件。  目录中的资源文件会被直接打包进应用，不经过编译，也不会分配资源ID。应用安装后，resfile资源会被解压到应用沙箱路径，通过Context属性[resourceDir](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-context/js-apis-inner-application-context#属性)获取到resfile资源目录后，可通过文件路径访问，且该路径仅能以只读方式访问。 |
...
| 目录类型 | 说明 | 资源文件 |
| --- | --- | --- |
| element | 表示元素资源，以下每一类数据都采用相应的JSON文件来表征（目录下仅支持文件类型）。  - boolean，布尔型  - color，颜色  - float，浮点型，范围是-2^128到2^128  - intarray，整型数组  - integer，整型，范围是-2^31到2^31-1  - plural，复数形式  - strarray，字符串数组  - string，字符串，[格式化字符串请参考API文档](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#getstringsync10) | element目录中的文件名称建议与下面的文件名保持一致。每个文件中只能包含同一类型的数据。  - boolean.json  - color.json  - float.json  - intarray.json  - integer.json  - plural.json  - strarray.json  - string.json |
| media | 表示媒体资源，包括图片、音频、视频等非文本格式的文件（目录下只支持文件类型）。  图片和音视频的类型说明见表3和表4。 | 文件名可自定义，例如：icon.png。 |
| profile | 表示自定义配置文件，其文件内容可[通过包管理接口](/ref/ability-api/ability-arkts/both-models/js-apis-bundlemanager/js-apis-bundlemanager#bundlemanagergetprofilebyability)获取（目录下只支持json文件类型）。 | 文件名可自定义，例如：test\_profile.json。 |
...
![](../images/3ad10624/zh-cn_image_0000002583477483.png)
...
- 通过$r或$rawfile访问资源。
  ...
  ![](../images/18ec77fb/note_3.0-zh-cn.png) 

  - 针对同一个资源，$r获取的资源信息Resource对象中的资源ID在应用重新编译时会发生变化，并非固定值，不建议缓存资源ID。如果确实需要缓存资源ID，需要对资源ID进行固定，具体请参考[固定资源ID](/system-debug-optimize/debugging-commands/restool#固定资源id)。
  - rawfile的native的访问方式请参考[Rawfile开发指导](/coding/resource-management/rawfile-guidelines)。
...
- 通过本应用上下文获取ResourceManager后，可调用不同[资源管理接口](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager)通过资源ID值或资源名称访问各类资源。例如通过getContext().resourceManager.getStringByNameSync('test')可获取字符串资源，通过getContext().resourceManager.getRawFd('rawfilepath')可获取rawfile文件所在HAP包的descriptor信息，再使用其中的\{fd, offset, length\}可访问rawfile文件。

  在API version 22及之前版本，中间码HAR、字节码HAR通过资源ID相关接口访问资源时，因ID无效会抛出异常；从API version 23开始，若将[compatibleSdkVersion](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section45865492619)配置为23及以上，则在当前Module的[AbilityStage](/ability-kit/stage-model-development/stage-model-application-components/abilitystage)的onCreate()回调执行后，中间码HAR、字节码HAR通过资源ID相关接口可以正常访问资源。

### 跨HAP/HSP包应用资源

****bundle相同，跨module访问****

- 通过[createModuleContext(context, moduleName)](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-application/js-apis-app-ability-application#applicationcreatemodulecontext)接口创建同应用中不同module的上下文，获取resourceManager对象后，调用不同[资源管理接口](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager)通过资源ID值或资源名称访问各类资源。
- 通过$r或$rawfile访问资源。具体操作如下：

  1.在entry的oh-package.json5文件中添加依赖。如"dependencies": \{"library": "file:../library"\}。

  ![](../images/146b4a67/zh-cn_image_0000002552797836.png)
  ...
  ![](../images/142f72d0/note_3.0-zh-cn.png) 
...
![](../images/0c26c298/note_3.0-zh-cn.png) 
...
![](../images/dc9dcbb3/note_3.0-zh-cn.png) 
...
关于应用界面加载资源的更多规则，请参考[国际化和本地化](/localization-kit/i18n-l10n)文档。
...
| 接口名 | 描述 |
| --- | --- |
| [getOverrideResourceManager](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#getoverrideresourcemanager12)(configuration?: [Configuration](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#configuration)) : [ResourceManager](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#resourcemanager) | 获取可以加载指定配置的资源的资源管理对象，使用同步方式返回。 |
| [getOverrideConfiguration](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#getoverrideconfiguration12)() : [Configuration](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#configuration) | 获取指定的配置，使用同步方式返回。 |
| [updateOverrideConfiguration](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#updateoverrideconfiguration12)(configuration: [Configuration](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#configuration)) : void | 更新指定的配置。 |
...
2. 应用通过[addResource(path)](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#addresource10)，实现资源覆盖；通过[removeResource(path)](/ref/localization-api/localization-arkts/js-apis-resource-manager/js-apis-resource-manager#removeresource10)，实现overlay删除。overlay资源路径需经过元能力的getContext().bundleCodeDir获取此应用对应的沙箱根目录，由应用的沙箱根目录与overlay资源包名称拼接而成。如：let path = getContext().bundleCodeDir + "overlay资源包名称"，其对应沙箱路径为/data/storage/el1/bundle/overlay资源包名称。

### 静态overlay配置方式

该功能默认使能，使能及去使能请参考[@ohos.bundle.overlay (overlay模块)](/ref/ability-api/ability-arkts/both-models/js-apis-overlay/js-apis-overlay)。
...
![](../images/914fa886/note_3.0-zh-cn.png) 
