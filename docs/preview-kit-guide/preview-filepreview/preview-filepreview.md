...
- 当前Preview Kit需要调用方存在对应uri的转授权能力，从而让预览获得该文件的访问权限来正常读取文件，具体问题可以参考[Preview Kit常见问题2](/preview-kit-guide/preview-faq/preview-faq-2)。

## 接口说明

接口返回值有两种返回形式：callback和promise，promise和callback只是返回值方式不一样，功能相同。具体API说明详见[接口文档](/ref/preview-api/preview-api-reference/preview-arkts/preview-arkts)。
