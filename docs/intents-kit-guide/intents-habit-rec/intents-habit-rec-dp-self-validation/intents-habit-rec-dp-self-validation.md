...
| 7 | 华为账号（UID） | 参考[附录A获取UID](/intents-kit-guide/intents-appendix-a-get-uid)。 |
...
1. 意图共享：在测试应用当中成功触发意图共享。即通过测试应用内的操作触发[shareIntent()](/ref/intents-api/intents-arkts-api/intents-arkts-api-insightintent/intents-arkts-api-insightintent#insightintentshareintent)接口的调用，并且意图共享成功。

   【举例】某音乐APP接入意图框架音乐续播的特性。通过播放某一首歌曲的用户操作，触发某音乐APP调用系统接口[shareIntent()](/ref/intents-api/intents-arkts-api/intents-arkts-api-insightintent/intents-arkts-api-insightintent#insightintentshareintent)。某音乐APP的开发者通过日志确认[shareIntent()](/ref/intents-api/intents-arkts-api/intents-arkts-api-insightintent/intents-arkts-api-insightintent#insightintentshareintent)接口调用成功，则可以认为某音乐APP本次意图共享是成功的。
