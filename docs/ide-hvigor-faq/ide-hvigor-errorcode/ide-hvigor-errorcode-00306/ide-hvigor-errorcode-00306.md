...
title: "规格错误码"
sidebar_position: 6
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-errorcode-00306
kit: devtools/build
...
1. 在[hvigor-config.json5](/ide-hvigor-configuration-file/ide-hvigor-set-options)中调整线程相关配置：hvigor.pool.maxSize、ohos.arkCompile.maxSize、hvigor.enableMemoryCache。
2. 执行任务时关闭[parallel模式](/ide-build-optimized/ide-hvigor-improve-performance)。
...
- 确保每个.so文件名都是唯一的。
- 通过[pickFirsts、pickLasts或select字段](/ide-hvigor-configuration/ide-hvigor-cpp#section17675528161517)设置.so文件的优先级。
...
网络不通，或[ohpm镜像仓库地址](/ide-ohpm-cli/ide-ohpmrc#zh-cn_topic_0000001792216397_默认配置项)、oh-package.json5中依赖的包名、版本号不正确。
...
2. [关闭守护进程](/ide-build-optimized/ide-hvigor-daemon)后重新编译，验证该问题是否与守护进程有关。
