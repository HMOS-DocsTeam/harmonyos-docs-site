...
title: "配置错误码"
sidebar_position: 3
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-errorcode-00303-1
kit: devtools/build
...
1、确保在工程级oh-package.json5中正确配置[parameterFile](/ide-ohpm-cli/ide-oh-package-json5#section122411462820)字段。
...
确保[abiFilters](/ide-hvigor-configuration/ide-hvigor-cpp#section0721057575)配置的是有效值。
...
在工程级oh-package.json5文件中添加[parameterFile](/ide-ohpm-cli/ide-oh-package-json5#section122411462820)字段，并确保配置正确。
...
在module.json5中type为form的ExtensionAbility中配置[metadata字段](/form-kit/arkts-ui/arkts-ui-widget-configuration)。
...
在module.json5中type为form的ExtensionAbility中增加metadata字段，补充一个name为“ohos.extension.form”的对象值，并配置对应的resource值，具体配置方式参考[配置卡片的配置文件](/form-kit/arkts-ui/arkts-ui-widget-configuration)。
...
2. 检查项目根目录下的[hvigorconfig.ts文件](/ide-hvigor/ide-hvigor-life-cycle#section810245135914)，确保includeNode方法的参数name字段存在且非空。
...
2. 检查项目根目录下的[hvigorconfig.ts文件](/ide-hvigor/ide-hvigor-life-cycle#section810245135914)，确保includeNode方法的参数srcPath字段存在且非空。
...
检查项目根目录下的[hvigorconfig.ts文件](/ide-hvigor/ide-hvigor-life-cycle#section810245135914)，确保includeNode方法的参数srcPath字段为相对路径。
...
将当前har模块配置为[字节码har](/ide-hvigor-configuration/ide-hvigor-build-har#section16598338112415)。
...
确保buildVersion是一个有效值，具体要求请参考[app.json5的buildVersion](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)。
