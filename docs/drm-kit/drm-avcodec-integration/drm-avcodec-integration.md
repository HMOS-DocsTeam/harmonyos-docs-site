...
在创建DRM之前，需获取到DRM信息(参考[媒体数据解析](/avcodec-kit/file-muxing-demuxing/audio-video-demuxer#开发步骤)开发步骤第4步)。

## 开发指导

详细的API说明请参考[Drm](/ref/drm-api/drm-c/drm-module/capi-drm/capi-drm)。
...
![](../../images/08acfd69/note_3.0-zh-cn.png) 
...
2. 获取设备支持的DRM解决方案名称和唯一标识的列表。
   ...
   获取到设备支持的DRM解决方案的名称和唯一标识的列表后，与DRM信息做匹配，创建对应的DRM解决方案。DRM信息可通过[媒体数据解析](/avcodec-kit/file-muxing-demuxing/audio-video-demuxer#开发步骤)开发步骤第4步获取，
...
6. 生成媒体密钥请求与处理媒体密钥响应。
   ...
   根据需要设置音频解密配置(详见[音频解码开发指南开发步骤](/avcodec-kit/audio-video-codec/audio-decoding#开发步骤)第4步)、设置视频解密配置(详见[视频解码开发指南开发步骤Surface模式](/avcodec-kit/audio-video-codec/video-decoding#surface模式)第4步或[Buffer模式](/avcodec-kit/audio-video-codec/video-decoding#buffer模式)第4步)，实现DRM内容解密。
