...
- 提供音频后台约束能力，音频接入AVSession后，可以进行后台音频播放。此功能需要同时申请[长时任务](/background-task-kit/continuous-task)。
...
![](../../images/1523604d/zh-cn_image_0000002552798904.png)
...
应用实现后台播放时，需要使用[Background Tasks Kit](/background-task-kit/background-task-overview)（后台任务管理）的能力，申请对应的长时任务，避免进入挂起（Suspend）状态。
