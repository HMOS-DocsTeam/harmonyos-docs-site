...
本文主要介绍AVCastPicker组件接入，实现通话设备切换功能。相关参数可参考[@ohos.multimedia.avCastPicker(投播组件)](/ref/avsession-api/avsession-comp/ohos-multimedia-avcastpicker/ohos-multimedia-avcastpicker)和[@ohos.multimedia.avCastPickerParam（投播组件参数）](/ref/avsession-api/avsession-arkts/js-apis-avcastpickerparam/js-apis-avcastpickerparam)。如果希望实现音频输出设备路由切换的效果，请参考[实现音频输出设备路由切换](/audio-kit/audio-device/audio-output-device-switcher)。
...
2. 在需要切换设备的通话界面创建AVCastPicker组件。
   ...
   ```
3. 创建VOICE\_COMMUNICATION类型的AudioRenderer，并开始播放。具体通话音频播放等实现，请参考[开发音频通话功能](/audio-kit/audio-call/audio-call-development)。
...
自定义样式通过设置[CustomBuilder](/ref/arkui-api/arkui-declarative-comp/common-definitions/ts-types/ts-types#custombuilder8)类型的参数[customPicker](/ref/avsession-api/avsession-comp/ohos-multimedia-avcastpicker/ohos-multimedia-avcastpicker#avcastpicker)实现。
...
系统不再提供音频输入设备切换的API，如果需要在应用内切换音频输入设备，并实现AVInputCastPicker组件，相关参数可参考[@ohos.multimedia.avInputCastPicker](/ref/avsession-api/avsession-comp/ohos-multimedia-avinputcastpicker/ohos-multimedia-avinputcastpicker) 和 [@ohos.multimedia.avCastPickerParam](/ref/avsession-api/avsession-arkts/js-apis-avcastpickerparam/js-apis-avcastpickerparam)。
...
1. 在需要切换设备的通话界面创建AVInputCastPicker组件。
   ...
   ```
2. 实现通话功能，请参考[开发音频通话功能](/audio-kit/audio-call/audio-call-development)。

### 自定义实现方式

自定义样式通过设置[AVInputCastPicker](/ref/avsession-api/avsession-comp/ohos-multimedia-avinputcastpicker/ohos-multimedia-avinputcastpicker#avinputcastpicker)中的参数customPicker实现。
