...
- 可通过以下方式查询相关扩展特性是否支持：
  ...
  如查询结果包含[XEG\_SPATIAL\_UPSCALE\_EXTENSION\_NAME](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#xeg_spatial_upscale_extension_name)，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为OpenGL ES和Vulkan空域GPU超分设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine)。
...
- 下面是基于GLES图形API平台集成空域GPU超分的主要业务流程
  ...
  ![](../../images/fad21b4e/zh-cn_image_0000002552799086.jpg)
...
- 下面是基于Vulkan图形API平台集成空域GPU超分的主要业务流程
  ...
  ![](../../images/c66c5a76/zh-cn_image_0000002583438781.jpg)
...
使用EGL和OpenGL ES图形API搭建图像渲染管线并集成空域GPU超分在Native层实现，渲染结果通过[XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_GetString](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_getstring)接口，获取XEngine支持的扩展信息，只有在支持XEG\_SPATIAL\_UPSCALE\_EXTENSION\_NAME扩展时才可以使用空域GPU超分的相关接口。
   ...
   ```
2. 调用[HMS\_XEG\_SpatialUpscaleParameter](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_spatialupscaleparameter)接口，对空域GPU超分的参数赋值。
   ...
   ```
3. 调用[HMS\_XEG\_RenderSpatialUpscale](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_renderspatialupscale)接口进行超分。
...
使用Vulkan图形API搭建图像渲染管线并集成空域GPU超分在Native层实现，渲染结果通过[XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_enumeratedeviceextensionproperties)接口，获取XEngine支持的扩展信息，只有在支持XEG\_SPATIAL\_UPSCALE\_EXTENSION\_NAME扩展时才可以使用空域GPU超分的相关接口。
...
2. 声明实例句柄。
   ...
   ```
3. 调用[HMS\_XEG\_CreateSpatialUpscale](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_createspatialupscale)接口，创建超分实例。
   ...
   ```
4. 调用[HMS\_XEG\_CmdRenderSpatialUpscale](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdrenderspatialupscale)接口下发超分，每帧都需要调用。
   ...
   ```
5. 调用[HMS\_XEG\_DestroySpatialUpscale](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_destroyspatialupscale)接口销毁实例。
