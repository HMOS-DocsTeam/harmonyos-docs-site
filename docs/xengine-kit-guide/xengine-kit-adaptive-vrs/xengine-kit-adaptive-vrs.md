...
- 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于OpenGL ES，使用[HMS\_XEG\_GetString](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_getstring)扩展特性查询接口进行查询，如查询结果包含[XEG\_ADAPTIVE\_VRS\_EXTENSION\_NAME](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#xeg_adaptive_vrs_extension_name)，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为自适应VRS设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine)。
...
- 下面是基于GLES图形API平台集成自适应VRS的主要业务流程
  ...
  ![](../../images/b5a6322a/zh-cn_image_0000002583438783.jpg)
...
- 下面是基于Vulkan图形API平台集成自适应VRS的主要业务流程
  ...
  ![](../../images/86db7e9e/zh-cn_image_0000002552958738.jpg)
...
1. 调用[HMS\_XEG\_GetString](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_getstring)接口，获取XEngine支持的扩展信息，只有在支持[XEG\_ADAPTIVE\_VRS\_EXTENSION\_NAME](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#xeg_adaptive_vrs_extension_name)扩展时才可以使用自适应VRS的相关接口。
   ...
   ```
2. 调用[HMS\_XEG\_AdaptiveVRSParameter](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_adaptivevrsparameter)接口，对自适应VRS的参数赋值。
   ...
   ```
3. 调用[HMS\_XEG\_DispatchAdaptiveVRS](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_dispatchadaptivevrs)接口计算着色率图像。
   ...
   ```
4. 调用[HMS\_XEG\_ApplyAdaptiveVRS](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_applyadaptivevrs)接口，将着色率图像应用到渲染目标中。
...
2. 声明实例句柄。
   ...
   ```
3. 调用[HMS\_XEG\_CreateAdaptiveVRS](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_createadaptivevrs)接口，定义并创建实例。
   ...
   ```
4. 调用[HMS\_XEG\_CmdDispatchAdaptiveVRS](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmddispatchadaptivevrs)接口，下发自适应VRS命令，生成perImage着色率纹理。
   ...
   ```
5. 调用[HMS\_XEG\_DestroyAdaptiveVRS](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_destroyadaptivevrs)接口，卸载VRS实例，清理VRS相关资源。
