...
- 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于Vulkan，使用[HMS\_XEG\_EnumerateDeviceExtensionProperties](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_enumeratedeviceextensionproperties)扩展特性查询接口进行查询，如查询结果包含XEG\_RTGI\_EXTENSION\_NAME，则表示支持该特性，若查询结果未包含，则表示不支持该特性。
...
以下接口为RTGI设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine)。
...
- 下面是基于Vulkan图形API平台集成动态漫反射全局光照的主要业务流程
  ...
  ![](../../images/f11764cb/zh-cn_image_0000002583478741.jpg)
...
使用Vulkan图形API搭建图像渲染管线，并集成RT DDGI在Native层实现，渲染结果通过[XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_enumeratedeviceextensionproperties)接口，获取XEngine支持的扩展信息，只有在支持XEG\_RTGI\_EXTENSION\_NAME扩展时才可以使用RT DDGI的相关接口。
...
2. 声明实例句柄。
   ...
   XEG_RTGI xegRTGI;
   ```
3. 调用[HMS\_XEG\_CreateRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_creatertgi)接口，创建RT DDGI实例。
   ...
   ```
4. 调用[HMS\_XEG\_CmdRenderRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdrenderrtgi)接口执行渲染命令，每帧都需要调用。
   ...
   ```
5. 若使用延迟渲染管线，则可以在调用[HMS\_XEG\_CmdRenderRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdrenderrtgi)接口之后，调用[HMS\_XEG\_CmdSetSynchronization](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdsetsynchronization)接口，设置同步信号，等待GI渲染结果写入指定图像，[HMS\_XEG\_CmdSetSynchronization](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdsetsynchronization)接口需要每帧调用。

   ```
   // GI渲染结果会写入到XEG_DDGIDescription中的outputGIImage图像中
   HMS_XEG_CmdSetSynchronization(commandBuffer, &xegRTGI);
   ```
6. 调用[HMS\_XEG\_DestroyRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_destroyrtgi)接口销毁实例。
...
![](../../images/0496e70a/zh-cn_image_0000002552799092.jpg)
...
使用Vulkan图形API搭建图像渲染管线，并集成RT NNGI在Native层实现，渲染结果通过[XComponent](/ref/arkui-api/arkui-declarative-comp/rendering-drawing/ts-basic-components-xcomponent/ts-basic-components-xcomponent)组件显示到屏幕。

本节阐述Vulkan图形API的RT NNGI使用。

在调用XEngine Kit能力前，需要先通过[Syscap](/ref/syscap/syscap#判断-api-是否可以使用)查询您的目标设备是否支持SystemCapability.Graphic.XEngine系统能力。

1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_enumeratedeviceextensionproperties)接口，获取XEngine支持的扩展信息，只有在支持XEG\_RTGI\_EXTENSION\_NAME扩展时才可以使用RT NNGI的相关接口。
...
2. 声明实例句柄。
   ...
   XEG_RTGI xegRTGI;
   ```
3. 调用[HMS\_XEG\_CreateRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_creatertgi)接口，创建RT NNGI实例。
...
4. 调用[HMS\_XEG\_CmdRenderRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdrenderrtgi)接口执行渲染命令，每帧都需要调用。
   ...
   ```
5. 在调用[HMS\_XEG\_CmdRenderRTGI](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdrenderrtgi)接口之后，调用[HMS\_XEG\_CmdSetSynchronization](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdsetsynchronization)接口，执行训练步骤，[HMS\_XEG\_CmdSetSynchronization](/ref/xengine-api/xengine-c/xengine-kit-index/xengine-kit-xengine/xengine-kit-xengine#hms_xeg_cmdsetsynchronization)接口需要每帧调用。
