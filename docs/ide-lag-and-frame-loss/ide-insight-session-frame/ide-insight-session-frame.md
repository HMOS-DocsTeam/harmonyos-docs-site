...
![](../../images/f18809a9/note_3.0-zh-cn.png) 

- 在任务分析窗口中，可通过[快捷键](/ide-profiler-appendix/ide-shortcut-key)缩放时间轴、移动时间轴、添加时间标签等。
- Frame分析支持离线符号解析能力，请参见[离线符号解析](/ide-insight-session-time#section186881175012)。
- Frame分析支持Energy泳道，请参见[定位能耗问题](/ide-profiler-energy#section889733410010)。

## 查看GPU使用情况

1. 创建Frame分析任务并录制相关数据，操作方法可参考[性能问题定位：深度录制](/ide-profiler-introduction/deep-recording)，或在会话区选择****Open File****，导入历史数据。
2. “Frame”泳道显示当前设备的GPU的使用率，将其展开，子泳道显示Render Service侧帧数据和App侧帧数据。

   ![](../../images/92652762/note_3.0-zh-cn.png) 
   ...
   - 除“RS Frame”和“App Frame”泳道外的“ArkTS Callstack”、“Callstack”、“CPU Core”等泳道信息，请参考[基础耗时：Time分析](/ide-insight-session-time)、[CPU活动分析](/ide-insight-session-cpu)。

   ![](../../images/ab6bddfb/zh-cn_image_0000002530752902.png "点击放大")
...
2. 框选Frame主泳道。
   ...
   ![](../../images/5841e9e2/zh-cn_image_0000002561832811.png "点击放大")
3. 点击“Statistics”列表中任一进程的跳转按钮，在“Frame List”区域将展现该进程对应的Frame列表。体现各帧的起始时间、总耗时、GPU耗时以及卡顿丢帧类型。

   ![](../../images/a0c17f9e/zh-cn_image_0000002561752841.png "点击放大")
4. 单击“Frame List”列表中任意一帧，右侧的“More”区域会中显示该帧更多关键信息。在获取该帧的预期起始时间、预期持续时间之外，您可以单击![](../../images/9466745c/zh-cn_image_0000002530912912.png "点击放大")跳转至关联的切片。

## 查看指定Frame页面布局信息

从DevEco Studio 5.1.0 Release版本开始，支持查看最新录制的Session中指定的Frame页面布局信息。

从DevEco Studio 6.1.0 Beta1版本开始，![](../../images/199eb625/zh-cn_image_0000002530912910.png "点击放大")按钮中新增Frame Layout开关，开发者可自行设置开关状态。开关关闭时，不支持查看最新录制的Session中指定的Frame页面布局信息，默认关闭。

暂不支持在Wearable设备上查看指定Frame页面布局信息。

1. 单击RS Frame泳道或APP Frame泳道中任意一帧，“Details”区域中会展示该帧具体信息。点击****Open Layout****按钮，将在ArkUI Inspector中直接打开相应arkli文件；点击****Download Layout****将arkli文件下载到指定目录，之后可手动导入[ArkUI Inspector](/ide-debug-app/ide-arkui-inspector)查看页面布局信息。

   ![](../../images/40f0af8c/note_3.0-zh-cn.png) 

   单击“Download Layout”或 “Open Layout”前，需应用进程置于前台，才能正确回放全量渲染数据，获取arkli文件。

   ![](../../images/8aaf79f0/zh-cn_image_0000002530912882.png "点击放大")
2. 在ArkUI Inspector中可查看组件树和组件属性信息，当前支持BackgroundFilter、nodeGroup、nodeGroupReuseCache组件。
   ...
   ![](../../images/824b8981/zh-cn_image_0000002530752900.png "点击放大")
...
2. 选择要观察的子泳道（例如带“RS Frame”标签的泳道）。
   ...
   ![](../../images/d1eabfed/zh-cn_image_0000002561832831.png "点击放大")
3. 单击列表中任意一帧，右侧的“More”区域会中显示该帧更多关键信息。在获取该帧的预期起始时间、预期持续时间之外，您可以单击![](../../images/6f28309b/zh-cn_image_0000002561752835.png "点击放大")跳转至关联的切片。

## 查看指定Frame信息

在子泳道（例如带“APP Frame”标签的泳道）中选中要查看的Frame，该泳道上方是耗时最长的非UI函数，下方是UI主线程泳道。

![](../../images/c2e63122/zh-cn_image_0000002561752857.png "点击放大")
...
![](../../images/a35c73c1/note_3.0-zh-cn.png) 
...
![](../../images/18afc658/zh-cn_image_0000002561752829.png "点击放大")
...
1. 创建Frame模板并录制会话，如存在卡顿和丢帧现象，会在Lost Frames和Hitch Time泳道对应时间显示矩形图。
   ...
   ![](../../images/e6dda08e/zh-cn_image_0000002561752821.png "点击放大")
2. 鼠标点选某一时间点，提示信息会显示该点所属时间段内的丢帧数以及卡顿时间。

   ![](../../images/a22c2e45/zh-cn_image_0000002561832845.png "点击放大")
...
![](../../images/00814694/note_3.0-zh-cn.png) 
...
![](../../images/8737e42c/zh-cn_image_0000002530912924.png "点击放大")
...
单击列表中任意一动画，右侧的“More”区域会中显示该动画所影响的组件属性的具体变化过程。![](../../images/43b8b354/zh-cn_image_0000002561832843.png "点击放大")
...
- DisplaySync\_cb(tid)：该泳道显示对应组件的帧率，如DisplaySync、XComponent两类接口组件动画对应的帧率。调测时，不同场景下由于帧率可变，系统实际表现是否符合预期，需要有实际的检测手段。尤其是由于DisplaySync的渲染均在UI主线程执行，当存在多个需要渲染的组件需要同时执行时，只能在UI主线程排队，此时任何一个组件的延迟都会对其他组件的渲染产生影响，导致UI卡顿。
  ...
  ![](../../images/67756eba/zh-cn_image_0000002561832795.png "点击放大")

1. 选择Display Vsync泳道，在时间轴上拖拽鼠标选定要查看的时间段。
2. 详情区显示当前时间段的屏幕刷新率，当前帧最大持续时间、最小持续时间、平均持续时间以及该时间段内平均帧数。

   ![](../../images/deb3c96c/zh-cn_image_0000002561752849.png "点击放大")
3. 点选Display Vsync泳道，可以查看当前帧的耗时和帧率。

   ![](../../images/f425e95c/zh-cn_image_0000002561832803.png "点击放大")
4. 框选DisplaySync\_cb泳道，可以查看应用侧对应组件的帧率，渲染时间等信息。

   ![](../../images/ba0df4f2/zh-cn_image_0000002530912902.png "点击放大")
5. 同时如果组件有可能的掉帧情况，DisplaySync\_cb泳道显示对应的掉帧情况并标红展示。

   ![](../../images/4c0e1ffa/zh-cn_image_0000002561752871.png "点击放大")
...
2. 泳道出现fps标记，展示当前框选范围内的帧率统计信息。
   ...
   ![](../../images/b20bdc4f/zh-cn_image_0000002530752908.png "点击放大")
3. 打开Only ArkWeb data开关，筛选过滤出包含ArkWeb帧的数据。

   ![](../../images/d8c81ce5/zh-cn_image_0000002530752888.png "点击放大")
...
2. 详情区给出录制时段内解码过度耗时的统计情况，包括类型，图片名，计数，总耗时，最小耗时、平均耗时、最大耗时，耗时标准差、 图源尺寸大小，目标尺寸大小等。
   ...
   ![](../../images/e285cf09/zh-cn_image_0000002561832799.png "点击放大")
...
4. 详情区给出录制时段内序列化、反序列化耗时情况统计信息，包括类型、计数、总耗时、最小耗时、平均耗时、最大耗时、耗时标准差等。
   ...
   ![](../../images/412ddb38/zh-cn_image_0000002530912896.png "点击放大")

   ![](../../images/36ce2b63/note_3.0-zh-cn.png) 
...
2. 详情区列表给出录制时间段内用户事件详情，包括用户事件ID、事件开始时间Input Time、应用开始处理时间Processing Start、应用处理耗时Duration和事件类型User Event Type。
   ...
   ![](../../images/4c334186/zh-cn_image_0000002561752833.png "点击放大")
3. 点选User Event泳道中的条块，Slice详情区展示该事件的详情信息。

   ![](../../images/f5adec83/zh-cn_image_0000002561832853.png "点击放大")
