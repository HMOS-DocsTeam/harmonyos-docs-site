...
sidebar_position: 3
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-frame-case
...
应用卡顿分析基础功能请参考[Frame分析](/ide-lag-and-frame-loss/ide-insight-session-frame)。
...
4. 点选该卡顿帧，窗口下方的“Details”区域中显示卡顿帧的关键信息。右侧应用进程前方跳转按钮可以跳转到应用进程Trace。
   ...
   ![](../../images/5b3aad16/zh-cn_image_0000002561753341.png "点击放大")
...
2. 查看下图调用栈，除(program)外，其他调用栈耗时小于一帧绘制的期望耗时8.3ms（被调优的设备fps为120），因此该卡顿帧主要分析调用栈(program)的耗时。
   ...
   ![](../../images/a0f22d0f/zh-cn_image_0000002530753408.png "点击放大")
3. 切换到Callstack泳道，查看Callstack泳道的主线程（线程号与进程号一致）子泳道，滑动观察下方Heaviest Stack区域“%”列中占比最大的函数调用栈，Category中亮色代表开发者调用栈，灰色代表系统调用栈，可以看出下图中耗时主要在系统侧的so，无法识别具体异常原因，接下来进一步分析应用进程Trace。
   ...
   ![](../../images/15267545/note_3.0-zh-cn.png) 

   也可通过底部的“Call Trees”选择框来隐藏系统调用栈，减少干扰信息。

   ![](../../images/dee2d085/zh-cn_image_0000002561833327.png "点击放大")
4. 切换到应用进程Process泳道，查看主线程（线程号与进程号一致）。

   ![](../../images/04d26b8c/zh-cn_image_0000002530913408.png "点击放大")
5. 窗口下方详情区可查看到Trace统计信息列表，逐层展开耗时最长的Trace，定位到主要耗时是在3次H:CreateImagePixelMap。接下来进一步分析这3次H:CreateImagePixelMap耗时的原因。

   ![](../../images/2a47fb2d/zh-cn_image_0000002561753345.png "点击放大")
6. H:CreateImagePixelMap和图片加载相关，再结合业务代码查看，可以看到是因为同步加载网络图片，建议修改为异步加载。

   ![](../../images/f2c031fa/zh-cn_image_0000002561833331.png "点击放大")

   ![](../../images/eafc691e/note_3.0-zh-cn.png) 
