...
详细的接口说明请参考[NativeDisplaySoloist](/ref/arkgraphics-api/arkgraphics-c/arkgraphics-module/capi-nativedisplaysoloist/capi-nativedisplaysoloist)。

## 开发示例

本范例是通过Drawing在Native侧实现图形的绘制，通过异步线程设置期望的帧率，再根据帧率进行图形的绘制并将其呈现在NativeWindow上，图形绘制部分可参考[使用Drawing实现图形绘制与显示](/arkgraphics-2d/graphic-drawing-and-display/graphic-drawing-overview)。
...
4. Native层配置帧率和注册回调函数。
   ...
   ![](../../../images/5fa607d2/note_3.0-zh-cn.png) 
