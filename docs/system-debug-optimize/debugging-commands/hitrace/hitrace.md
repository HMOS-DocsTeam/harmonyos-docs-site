...
hitrace命令行工具提供trace信息采集能力，支持采集系统提供的打点和开发者使用[HitraceMeter接口](/system-debug-optimize/performance-analysis-kit/hitracemeter/hitracemeter-intro)在应用中设置的打点。该工具支持多种方式采集文本格式或二进制格式的trace信息，帮助开发者观测程序运行情况和定位故障问题。

## 环境要求

- 根据hdc命令行工具指导，完成[hdc环境准备](/system-debug-optimize/debugging-commands/hdc#环境准备)。
...
![](../../../images/3f962e82/note_3.0-zh-cn.png) 
...
| app | 用户态trace标签，应用模块。通过[HiTraceMeter](/system-debug-optimize/performance-analysis-kit/hitracemeter/hitracemeter-intro)接口做的trace打点属于app tag。 |
...
开发者可使用带trace级别的打点接口（参考[@ohos.hiTraceMeter](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-hitracemeter/js-apis-hitracemeter)和[trace.h](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-c/performance-analysis-headerfile/capi-trace-h/capi-trace-h)中的API version 19的trace打点接口），测试不同阈值下的trace输出是否符合预期。
