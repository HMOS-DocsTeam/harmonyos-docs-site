...
在使用本工具前，开发者需要先获取[hdc工具](/system-debug-optimize/debugging-commands/hdc#环境准备)，执行hdc shell。
...
表示当前系统账号的编号，系统账号的相关接口请参考[系统账号管理模块](/ref/system-basicfun-api/basic-services-api/basic-services-arkts/account-management-arkts/js-apis-osaccount/js-apis-osaccount)，下面给出几种常见的系统账号。
...
注：hqf文件制作方式可参考[HQF打包指令](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool#hqf打包指令)。
...
![](../../../images/e54a1dd3/note_3.0-zh-cn.png) 
...
方法一. 使用[自动签名](/ide-signing#section18815157237)。在连接设备后，重新为应用进行签名。

方法二. 使用手动签名，请参考[手动签名](/ide-signing#section297715173233)。
...
请开发者根据实际场景选择自动签名或者手动签名，例如无法连接互联网的情况下推荐使用手动签名方式，详情参考[使用场景说明](/ide-signing#section54361623194519)。

方法一. 使用[自动签名](/ide-signing#section18815157237)。在连接设备后，重新为应用进行签名。

方法二. 使用手动签名，请参考[手动签名](/ide-signing#section297715173233)。

方法三. 如果安装APP时报这个错误码，需要在[工程级build-profile.json5文件](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app)里配置[packOptions](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section03812484215)的appWithSignedPkg属性为true，保证APP里的HAP/HSP有签名。
...
![](../../../images/c431f75c/zh-cn_image_0000002552958514.png)
...
1. [app.json5配置文件](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的bundleName、[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)中name不符合命名规则。
...
![](../../../images/178d4e41/zh-cn_image_0000002552958516.png)
...
![](../../../images/bb859a80/zh-cn_image_0000002583478517.png)
...
- 方法1：请参考[app.json5配置文件](/development-fundamentals/application-configuration-file-stage/app-configuration-file)和[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)查看并补充必填字段。
- 方法2：通过hilog日志判断缺失字段。
...
![](../../../images/bc8f27f2/zh-cn_image_0000002552798868.png)
...
![](../../../images/64b3c587/zh-cn_image_0000002583438563.png)
...
- 场景二：在[build-profile.json5](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section153288223224)文件中，配置的profile文件与.cer文件不匹配。
...
1. 使用[自动签名](/ide-signing#section18815157237)。在连接设备后，重新为应用进行签名。
...
![](../../../images/ea435420/zh-cn_image_0000002552958518.png)
...
应用APL等级为normal，只能使用APL等级为normal的权限，如果使用了system\_basic或system\_core等级的权限，将导致报错，具体请参考[权限机制中的基本概念](/system-security/access-control/app-permission-mgmt/app-permission-mgmt-overview#权限机制中的基本概念)。

****处理步骤****

根据报错描述中的权限名称在[应用权限列表](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-permissions)中排查权限是否存在。

- 如果不存在，需要查看需要该权限的API接口的文档说明，以确保权限支持申请。例如：[setDevicePairingConfirmation](/ref/system-network-api/connectivity-api/connectivity-arkts/js-apis-bluetooth-connection/js-apis-bluetooth-connection#connectionsetdevicepairingconfirmation)接口所需权限中，ohos.permission.MANAGE\_BLUETOOTH仅系统应用支持申请，此时请更换为ohos.permission.ACCESS\_BLUETOOTH权限。
- 如果存在，请查看对应的权限文档的要求，申请权限，同时请关注权限是否支持申请。例如：[受限开放权限](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions)请参考文档[申请受限权限](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions-in-acl)。[企业类应用可用权限](/system-security/access-control/app-permission-mgmt/app-permissions/permissions-for-enterprise-apps)仅企业类应用支持申请，三方应用不支持。
...
多个HAP/HSP中配置的[SysCap](/ref/syscap/syscap)不一致。
...
![](../../../images/a2192d46/zh-cn_image_0000002583478519.png)
...
![](../../../images/6bcdc640/zh-cn_image_0000002552798870.png)
...
1. 设备上已安装的应用与新安装的应用中签名不一致或者多个包（HAP和HSP）之间的签名存在差异。若两个应用的签名[密钥](/ide-signing#section462703710326)或[APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section16423184171915)中至少有一项相同，则认为它们的签名一致。如果在DevEco Studio的“Edit Configurations”中勾选了“Keep Application Data”（即不卸载应用，直接覆盖安装），并且重新进行了签名，将导致该报错。
2. 如果某个应用被卸载但是保留了数据，那么后面安装相同包名的应用时，需要校验其签名信息的一致性。如果两者签名信息中的[密钥](/ide-signing#section462703710326)和[APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section16423184171915)都不一致，则会导致该报错。

****处理步骤****

1. 请卸载设备上已安装的应用（PC/2in1设备需要确保所有用户下都卸载完成，手机/平板侧需要关注隐私空间和主用户下是否卸载完成），或取消勾选“Keep Application Data”后，重新安装新的应用。
2. 如果是因不同团队提供的HSP导致签名不一致问题，可以采用[集成态HSP](/development-fundamentals/application-typical-scenarios/integrated-hsp)的方式统一提供HSP；在多HAP包的情况下，必须确保所有HAP包的签名一致。
...
![](../../../images/f31bf1e2/zh-cn_image_0000002583438565.png)
...
- 场景一：用户导入了三方提供的HSP模块，且该HSP既非[集成态HSP](/development-fundamentals/application-typical-scenarios/integrated-hsp)，又非同包名的HSP，造成包名不一致。
- 场景二：用户使用了错误的签名文件（后缀为.p7b）进行签名，造成包名不一致。

****处理步骤****

- 场景一：HSP只能给同包名的应用使用，只有集成态HSP可以给不同包名的应用使用。需要用户与三方开发者确认，三方开发者应提供集成态HSP、或同包名的HSP给用户使用。
- 场景二：检查签名流程和签名证书，参考[应用/元服务签名](/ide-signing)。
...
![](../../../images/e84c0965/zh-cn_image_0000002552958520.png)
...
1. 检查应用定义的proxyData内容是否符合要求，参考[proxyData标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#proxydata标签)。
...
使用[自动签名](/ide-signing#section18815157237)或者[手动签名](/ide-signing#section297715173233)重新签名后安装调试。
...
使用[自动签名](/ide-signing#section18815157237)或者重新申请签名证书，在[手动签名](/ide-signing#section297715173233)重新签名后安装调试。
...
校验唯一标识[appIdentifier](/development-fundamentals/common-problem-of-application#什么是appidentifier)不一样，导致安装失败。
...
1. 确认签名文件p7b中apl字段是否有误。
   ...
   ![](../../../images/818fe166/zh-cn_image_0000002583478521.png)
2. 若apl字段有误，修改UnsgnedReleasedProfileTemplate.json文件中apl字段，并重新签名。

   ![](../../../images/376f6a11/zh-cn_image_0000002552798872.png)
...
检查overlay应用中的[app.json5配置文件](/development-fundamentals/application-configuration-file-stage/app-configuration-file)的targetBundleName字段是否配置。
...
检查overlay应用中的[module.json5配置文件](/development-fundamentals/application-configuration-file-stage/module-configuration-file)的targetModuleName字段是否配置。
...
当前设备不允许安装[Profile签名文件](https://developer.huawei.com/consumer/cn/doc/app/agc-help-profile-overview-0000002283260125)中如下两种类型的应用：enterprise\_mdm（企业MDM应用）、enterprise\_normal（普通企业应用）。Profile签名文件类型的取值及含义请参考[ApplicationInfo.appDistributionType](/ref/ability-api/ability-arkts/ability-api-interface-depend/bundlemanager/js-apis-bundlemanager-applicationinfo/js-apis-bundlemanager-applicationinfo#applicationinfo-1)。
...
应用配置的[设备类型](/development-fundamentals/application-configuration-file-stage/module-configuration-file#devicetypes标签)或者[requiredDeviceFeatures标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#requireddevicefeatures标签)配置的设备特性不支持安装。
...
1. 使用[自动签名](/ide-signing#section18815157237)或者[手动签名](/ide-signing#section297715173233)重新签名后安装调试。
...
通过重新签名，确保应用签名信息中的[密钥](/ide-signing#section462703710326)和[APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section16423184171915)至少有一项与预置应用保持一致。
...
修改安装应用的[bundleName](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)，确保与预置应用的bundleName不一致。
...
方式二：重新[自动签名](/ide-signing#section18815157237)。
...
1. 参考[权限申请指导](/system-security/access-control/app-permission-mgmt/request-app-permissions/declare-permissions)申请[ohos.permission.kernel.SUPPORT\_PLUGIN权限](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions#ohospermissionkernelsupport_plugin)。
...
检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)的name字段是否为空。
...
重新签名，使多个HAP包签名信息一致。参考[应用/元服务签名](/ide-signing)。
...
[module.json](/development-fundamentals/application-configuration-file-stage/module-configuration-file)、[pack.info](/ide-hvigor/ide-compile-build#section43931054115513)等配置文件缺失。
...
[module.json](/development-fundamentals/application-configuration-file-stage/module-configuration-file)、[pack.info](/ide-hvigor/ide-compile-build#section43931054115513)等配置文件格式异常。
...
[module.json](/development-fundamentals/application-configuration-file-stage/module-configuration-file)、[pack.info](/ide-hvigor/ide-compile-build#section43931054115513)等配置文件存在数据类型错误的字段。
...
[module.json](/development-fundamentals/application-configuration-file-stage/module-configuration-file)、[pack.info](/ide-hvigor/ide-compile-build#section43931054115513)等配置文件存在字符串长度或者数组大小过大的字段。
...
安装过程中，解析安装包获取[SysCap](/ref/syscap/syscap)信息失败。
...
![](../../../images/0378066c/note_3.0-zh-cn.png) 
...
1. 将设备或模拟器与DevEco Studio进行连接，具体指导及要求可查看[运行应用/元服务](/ide-run-device)。
2. 在命令行执行如下[hdc命令](#环境要求hdc工具)，查询设备支持的Abi列表。
   ...
   ```
3. 根据查询返回结果，检查[模块级build-profile.json5](/ide-hvigor-configuration-file/ide-hvigor-build-profile)文件中的[“abiFilters”参数](/hardware-compatibility/ohos-abi#在编译架构中指定abi)中的配置，规则如下：
...
使用[param工具](/system-debug-optimize/debugging-commands/param-tool)设置const.bms.support\_plugin的值为true，即执行hdc shell param set const.bms.support\_plugin true。
...
检查多HSP之间的包信息是否一致，包括[app.json5配置文件](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中bundleName、bundleType、versionCode、apiReleaseType字段。
...
方案一：重新签名，签名过程中，请参考[自动签名](/ide-signing#section18815157237)的支持ACL权限、或者[手动签名](/ide-signing#section297715173233)的使用ACL的签名配置指导进行配置，确保待安装应用与已安装应用配置一致。
...
重新签名，签名过程中，请参考[自动签名](/ide-signing#section18815157237)的支持ACL权限、或者[手动签名](/ide-signing#section297715173233)的使用ACL的签名配置指导进行配置，使多个HAP包签名信息中allowed-acls的U1Enabled信息一致。
...
重新[打包APP](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool#app打包指令)。
...
如需要适配当前设备，请在应用设备类型配置中增加当前[设备类型](/development-fundamentals/application-configuration-file-stage/module-configuration-file#devicetypes标签)，然后重新[打包APP](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool#app打包指令)。
