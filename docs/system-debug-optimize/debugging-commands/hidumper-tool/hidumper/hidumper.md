...
- 根据hdc命令行工具指导，完成[环境准备](/system-debug-optimize/debugging-commands/hdc#环境准备)。
...
| [-e](#获取系统故障日志) | 获取记录的故障日志。输出日志的规格可参考：[CppCrash](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#日志规格)、[JSCrash](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/jscrash-guidelines#日志规格)、[AppFreeze](/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。 |
| [-e --list [process\_name] [-n num] [--since timestamp] [--until timestamp]](#获取异常退出记录列表) | 获取异常退出记录列表。  ****说明****：从API version 22开始，支持该参数。 |
| [-e --print [process\_name] [-n num] [--since timestamp] [--until timestamp]](#获取异常退出故障日志) | 获取异常退出故障日志。输出日志的规格可参考：[CppCrash](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#日志规格)、[JSCrash](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/jscrash-guidelines#日志规格)、[AppFreeze](/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。  ****说明****：从API version 22开始，支持该参数。 |
| [-e --print record\_id](#获取异常退出故障日志) | 获取指定异常退出记录id的故障日志。输出日志的规格可参考：[CppCrash](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#日志规格)、[JSCrash](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/jscrash-guidelines#日志规格)、[AppFreeze](/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。  ****说明****：从API version 22开始，支持该参数。 |
...
| [--mem-smaps pid [-v]](#查询进程内存) | 获取pid内存统计信息，数据来源于/proc/pid/smaps，使用-v指定更多详细信息。（仅支持导出[debug版本应用](/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)）  ****说明****：从API version 20开始，支持该参数。 |
| [--mem-jsheap pid [-T tid] [--gc] [--leakobj] [--raw]](#查询虚拟机堆内存) | 必选参数pid。触发ArkTS应用JS线程的gc和堆内存快照导出。指定线程tid时，仅触发该线程的gc和堆内存快照导出；指定--gc时，仅触发gc，不导出快照；指定--leakobj时，应用开启泄露检测可获取泄露对象列表。  文件命名格式为：hidumper-jsheap-进程号-JS线程号-时间戳，文件内容为JSON结构的JS堆快照。  指定--raw时，堆快照以rawheap格式导出。  ****说明****：从API version 19开始，支持--raw参数。 |
...
hidumper --mem-smaps [pid] [-v]命令调试的进程应为“使用调试证书签名的应用”，同[debug版本应用](/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)。
...
![](../../../../images/7a1344c0/caution_3.0-zh-cn.png) 

hidumper --mem-jsheap pid [-T tid] [--gc] [--leakobj] [--raw]命令调试的进程应为“使用调试证书签名的应用”，同[debug版本应用](/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)。
...
- 可使用hidumper --mem-jsheap pid [-T tid] --raw获取指定进程或指定JS线程的虚拟机堆内存，生成的堆内存文件为rawheap格式，文件命名为hidumper-jsheap-进程号-JS线程号-时间戳.rawheap。rawheap的解析转换可参考使用：[rawheap-translator工具](/system-debug-optimize/debugging-commands/command-line-utilities/rawheap-translator)。
...
- 可使用hidumper --mem-jsheap pid --leakobj获取指定进程的虚拟机堆内存和泄漏对象信息，文件命名为：hidumper-leaklist-进程号-时间戳。
  ...
  获取指定进程的虚拟机堆内存和泄露对象信息的前提是应用已通过[@ohos.hiviewdfx.jsLeakWatcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-jsleakwatcher/js-apis-jsleakwatcher)接口开启了泄漏检测功能。

  具体使用步骤为：

  1. 应用调用[jsLeakWatcher.enable](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-jsleakwatcher/js-apis-jsleakwatcher#jsleakwatcherenable)接口。
  2. 应用调用[jsLeakWatcher.watch](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-jsleakwatcher/js-apis-jsleakwatcher#jsleakwatcherwatch)接口。
...
上述生成的文件，可以通过hdc[文件传输](/system-debug-optimize/debugging-commands/hdc#文件传输)命令从设备中获取。
...
![](../../../../images/4fb247fd/caution_3.0-zh-cn.png) 
...
![](../../../../images/da9501eb/note_3.0-zh-cn.png) 
...
以下异常退出原因，可参考[应用终止分析思路和分析步骤](/system-debug-optimize/performance-analysis-kit/fault-analysis/appkilled-guidelines#分析思路和分析步骤)进行问题排查处理。
...
![](../../../../images/7b74c5b4/note_3.0-zh-cn.png) 
...
![](../../../../images/533a3267/note_3.0-zh-cn.png) 
...
![](../../../../images/bd871d84/note_3.0-zh-cn.png) 
...
hidumper --mem 系列命令与[HiDebug](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-hidebug/js-apis-hidebug)接口获取到的内存信息存在差异。
...
| [hidebug.getAppNativeMemInfo](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-hidebug/js-apis-hidebug#hidebuggetappnativememinfo12) | 获取****应用自身****的内存使用情况 | 系统节点：/proc/pid/smaps\_rollup | 否 |

若想通过Hidebug获取图形内存，请参考[HiDebug能力概述](/system-debug-optimize/performance-analysis-kit/hidebug/hidebug-guidelines)中的相关说明。
...
hidumper --mem-jsheap [pid] --leakobj 命令依赖[JsLeakWatcher](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-jsleakwatcher/js-apis-jsleakwatcher)。应用需通过JsLeakWatcher接口开启泄漏检测功能。具体步骤参考：[查询虚拟机堆内存](#查询虚拟机堆内存)。
