...
PrivacyManagerService是访问控制基于[hidumper](/system-debug-optimize/debugging-commands/hidumper-tool/hidumper)增强开发的命令行能力，可显示访问控制基础信息，获取敏感权限使用记录。

## 环境准备

根据hidumper工具指导，完成[环境准备](/system-debug-optimize/debugging-commands/hidumper-tool/hidumper#环境要求)。
...
命令所需的tokenId可以通过[atm-tool](/system-debug-optimize/debugging-commands/atm-tool#查询命令)进行查询。
