...
打包指令中的文件来源于[DevEco Studio编译构建产物](/ide-hvigor/ide-compile-build)，文件路径查看操作如下。
...
| module.json属性 | 含义 | [module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)配置项 | [app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)配置项 | [工程级build-profile.json5](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app)配置项 |
| --- | --- | --- | --- | --- |
...
- [Stage模型](/ability-kit/application-models#应用模型概况)示例：
  ...
  ```
- [FA模型](/ability-kit/application-models#应用模型概况)示例：
...
![](../../../../images/0446356c/note_3.0-zh-cn.png) 

- module.json文件为DevEco Studio编译构建产物，其中的字段与配置文件的对应关系，请参考[表1 module.json与配置文件属性的对照表](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool)。

****打包APP时的压缩规则：**** 打包APP时，对release模式的HAP、HSP包会进行压缩，对debug模式的HAP、HSP包不会压缩。

![](../../../../images/39ad147c/note_3.0-zh-cn.png) 
...
| --pac-json-path | 否 | NA | [pac.json](/ide-appendix/agc-pac)文件路径，文件名必须为pac.json。  从API version 20开始支持该参数。 |
...
![](../../../../images/34c3a84b/note_3.0-zh-cn.png) 
...
| --pac-json-path | 否 | NA | [pac.json](/ide-appendix/agc-pac)文件路径，文件名必须为pac.json。  最终app产物中pac.json文件只来源于该参数，不配置的话，最终app产物不包含该文件。  --app-list参数指定的app包中的pac.json不会打包进最终app。  从API version 20开始支持该参数。 |
| --atomic-service-entry-size-limit | 否 | NA | 设置元服务entry包大小（包含其依赖包的大小）限制，仅Stage模型应用且bundleType为atomicService时生效。取值范围为[0,4194304]的整数，取值为0表示不限制大小，单位KB。不设置该参数时默认值为2048KB。如果entry包是release模式（module.json5文件中type字段值为entry，且app.json5中debug字段的值为false），该限制作用于打包app时压缩后的entry包大小（包含其依赖包的大小）。 |
| --atomic-service-non-entry-size-limit | 否 | NA | 设置元服务非entry包大小（包含其依赖包的大小）限制，仅Stage模型应用且bundleType为atomicService时生效。取值范围为[0,4194304]的整数，取值为0表示不限制大小，单位KB。不设置该参数时默认值为2048KB。如果非entry包是release模式（module.json5文件中type字段值不是entry，且app.json5中debug字段的值为false），该限制作用于打包app时压缩后的非entry包大小（包含其依赖包的大小）。 |
| --stat-duplicate | 否 | boolean | 打包完成后，是否扫描重复so文件，该参数可用于识别重复so，以减小包大小。取值为true时，执行扫描，扫描完成会在--out-path参数指定的输出文件所在目录下生成scan\_report目录，其中包含文件名为scan\_result的[重复so文件扫描报告](#扫描重复so文件)，并在告警中打印scan\_report目录路径。取值为false时，不执行扫描。默认值为false。  从API version 23开始支持该参数。 |
...
HQF包适用于[增量调试](/ide-debug-app/ide-code-debugging/ide-incremental-debugging)场景。开发者可以使用打包工具的jar包对应用进行打包，通过传入打包选项、文件路径，生成所需的HQF包。

![](../../../../images/f747fb26/note_3.0-zh-cn.png) 
...
| --- | --- | --- | --- |
| --mode | 是 | hqf | 打包类型。 |
| --json-path | 是 | NA | .json文件路径，文件名必须为[patch.json](/ide-debug-app/ide-code-debugging/ide-incremental-debugging#section28031446182019)。 |
...
- 在打包生成APPQF包时，确保每个HQF的[patch.json文件](/ide-debug-app/ide-code-debugging/ide-incremental-debugging#section28031446182019)中的versionName、versionCode、patchVersionName、patchVersionCode保持一致。
...
| --- | --- | --- | --- |
| --mode | 是 | appqf | 打包类型。 |
| --hqf-list | 是 | NA | [HQF文件](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool#hqf打包指令)路径，多个HQF以英文逗号隔开。 |
...
![](../../../../images/cd579a57/note_3.0-zh-cn.png) 
...
| --bundle-name | 否 | 包名 | 指定的Bundle名称，传入的包的Bundle名称会被修改为该Bundle名称。指定的值不能为空，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的bundleName字段。 |
| --version-code | 否 | 版本号 | 指定的版本号，传入的包的版本号会被修改为该版本号。取值范围为0~2147483647的整数，指定的值不能为空值，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的versionCode字段。 |
| --version-name | 否 | 版本名称 | 指定的版本名称，传入的包的版本名称会被修改为该版本名称。指定的值不能为空，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的versionName字段。 |
| --min-compatible-version-code | 否 | 能够兼容的最低历史版本号 | 指定的最低历史版本号，传入的包的最低历史版本号会被修改为该版本号。取值范围为0~2147483647的整数，指定的值不能为空值，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的minCompatibleVersionCode字段。 |
| --min-api-version | 否 | SDK的API最小版本 | 指定的SDK的API最小版本，传入的包的SDK的API最小版本会被修改为该版本。取值范围为0~2147483647的整数，指定的值不能为空值，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的minAPIVersion字段。 |
| --target-api-version | 否 | API目标版本 | 指定的API目标版本，传入的包的API目标版本会被修改为该版本。取值范围为0~2147483647的整数，指定的值不能为空值，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的targetAPIVersion字段。 |
| --api-release-type | 否 | API目标版本的类型 | 指定的API目标版本的类型，传入的包的API目标版本的类型会被修改为该类型。指定的值不能为空，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的apiReleaseType字段。 |
| --bundle-type | 否 | Bundle类型 | 指定的Bundle类型，传入的包的Bundle类型会被修改为该类型。指定的值不能为空，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的bundleType字段。 |
| --installation-free | 否 | 是否支持免安装特性 | 指定的免安装特性，传入的包的免安装特性会被修改为该类型。指定的值不能为空，该字段的详细定义和规格请参考Stage模型[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)/FA模型[config.json](/development-fundamentals/application-configuration-file-fa/application-configuration-file-overview-fa)中的installationFree字段。 |
| --delivery-with-install | 否 | 当前HAP是否在用户主动安装HAP所在应用的时候一起安装 | 指定的HAP是否需要一起安装，传入的包的deliveryWithInstall会被修改为该类型。指定的值不能为空，该字段的详细定义和规格参考Stage模型[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)/FA模型[config.json](/development-fundamentals/application-configuration-file-fa/application-configuration-file-overview-fa)中的deliveryWithInstall字段。 |
| --device-types | 否 | 允许Ability运行的设备类型 | 指定的设备类型，传入的包的设备类型会被修改为该类型。指定的值不能为空，该字段的详细定义和规格请参考Stage模型[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)/FA模型[config.json](/development-fundamentals/application-configuration-file-fa/application-configuration-file-overview-fa)中的deviceTypes字段，传入值的形式为字符串格式，多个设备类型之间使用“,”分隔。 |
| --out-path | 是 | NA | 目标文件路径，需要为一个有读写权限的目录。 |
| --build-version | 否 | 构建版本号 | 指定的构建版本号，传入的包的构建版本号会被修改为该版本号。指定的值不能为空值，该字段的详细定义和规格请参考[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)中的buildVersion字段。 |
...
![](../../../../images/9e326580/note_3.0-zh-cn.png) 

- module.json文件为DevEco Studio编译构建产物，其中的字段与配置文件的对应关系，请参考[表1 module.json与配置文件属性的对照表](/system-debug-optimize/debugging-commands/packing-unpacking/packing-tool)。

****打包App时的压缩规则：**** 打包App时，对release模式的HAP、HSP包会进行压缩，对debug模式的HAP、HSP包不会压缩。

![](../../../../images/f3601cd9/note_3.0-zh-cn.png) 
...
1. 根据报错信息检查[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)和[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中的配置项是否准确。当有多条报错信息时，优先根据第一条报错信息进行排查。
...
检查[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)，修改asanEnabled和tsanEnabled，确保二者不会同时为true。
...
检查[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)，确保asanEnabled、tsanEnabled、GWPAsanEnabled三项的中任一项与hwasanEnabled，不会同时配置为true。
...
1. 检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)，确保module.json5文件中正确配置了abilities标签，并且至少包含一个ability，详细请参见[abilities标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#abilities标签)。
2. 检查module.json5和[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)，当bundleType为atomicService时，确保installationFree配置为true，反之，需要配置为false。
...
修改continueBundleName，确保其与[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)中的bundleName不同。
...
1. 检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)，确保targetModuleName和requestPermissions不会同时出现。
2. 检查module.json5，确保targetModuleName和name不同。
3. 检查module.json5，确保配置targetPriority时，同时配置了targetModuleName。
4. 检查[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)和module.json5，确保配置targetBundleName时，同时配置了targetModuleName。
...
构建[bundleType](/development-fundamentals/application-configuration-file-stage/app-configuration-file#配置文件标签)为shared的App包时，检查HSP包无效。

****可能原因****

1. 存在两个以上的[HSP包](/development-fundamentals/application-package-fundamentals/application-package-dev/in-app-hsp)。例如下图使用DevEco Studio构建App时，工程中包含了两个HSP包library和library1，此时打包APP包失败。

   ![](../../../../images/599fc349/zh-cn_image_0000002583438567.png)
2. HSP包在module.json5中配置了dependencies。

****处理步骤****

1. 检查打包文件，确保bundleType为shared的App包中，HSP包不超过一个。
2. 检查打包文件，删除HSP包中[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)配置的dependencies。
...
打包App时，应用的[pack.info](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section03812484215)文件覆盖HAP或HSP包内pack.info文件时抛I/O异常。
...
res打包模式下，--pack-info-path指定的[pack.info](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section03812484215)文件缺失moduleName标签或moduleName标签值为空。
...
5. --json-path指定的module.json文件中bundleType是appPlugin时，module.json文件不包含[ohos.permission.kernel.SUPPORT\_PLUGIN](/system-security/access-control/app-permission-mgmt/app-permissions/restricted-permissions#ohospermissionkernelsupport_plugin)权限。
...
打包HSP/HAP时，校验[deduplicateHar](/ide-hvigor-configuration-file/ide-hvigor-build-profile-app#section03812484215)属性失败。
...
1. 模块的[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)配置文件中bundleType为app，但[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中的installationFree属性值为true。
...
检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)，确保Entry类型包abilities正确配置了ability。
...
1. 检查[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)，确保bundleType设置为app，atomicService，shared之一。
2. 如果bundleType为shared，确保[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中的installationFree设置为false。
...
4. 当前参与打包的HAP或HSP中，存在模块的apiReleaseType（[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)配置文件中的属性）与其他模块不一致。
...
[HAP唯一性校验](/ide-hvigor-configuration/ide-hvigor-verification-rule)需要判断分发策略是否相交，由于分发策略配置错误，导致无法判断分发策略是否相交。
...
检查分发策略相关配置，确保policy的值为include或exclude，value取值参见[distributionFilter标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#distributionfilter标签)。
...
根据报错信息检查[app.json5](/development-fundamentals/application-configuration-file-stage/app-configuration-file)和[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中的配置项是否准确。当有多条报错信息时，优先根据第一条报错信息进行排查。
...
参考[HAP唯一性校验](/ide-hvigor-configuration/ide-hvigor-verification-rule)，调整工程中的Entry类型HAP配置。
...
1. 检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中的dependencies，删除循环依赖，确保App中不存在循环依赖。
...
检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中的[preloads属性](/development-fundamentals/application-configuration-file-stage/module-configuration-file#atomicservice标签)下的moduleName，不能配置为自身模块module.json5中的name，且moduleName对应的模块必须存在。
...
检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)中元服务[preloads属性](/development-fundamentals/application-configuration-file-stage/module-configuration-file#atomicservice标签)，确保没有配置重复的模块，配置所有的模块都存在且不能配置自身的模块名。
...
检查targetModuleName配置项，确保其正确配置（详细请参见[module.json5配置文件标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)及targetModuleName属性），必要时创建目标模块。
...
检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)，删除proxyData中重复的uri，确保每个uri都是唯一的（详细请参见[proxyData标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#proxydata标签)）。
...
检查[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file)，删除continueType或deviceType重复的配置项。
...
检查Entry模块分发策略是否正确配置，例如policy的值应为exclude或include，详细请参考[distributionFilter标签](/development-fundamentals/application-configuration-file-stage/module-configuration-file#distributionfilter标签)。
...
1. 传入的参数类型错误。
2. 传入参数范围错误。
3. 传入HAP或HSP包不完整，缺少json文件（json文件配置请参考Stage模型[module.json5](/development-fundamentals/application-configuration-file-stage/module-configuration-file#配置文件标签)/FA模型[config.json](/development-fundamentals/application-configuration-file-fa/application-configuration-file-overview-fa)）。
