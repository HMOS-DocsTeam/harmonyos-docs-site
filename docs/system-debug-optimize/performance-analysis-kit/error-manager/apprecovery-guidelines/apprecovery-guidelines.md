...
如果应用在[AbilityStage](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilitystage/js-apis-app-ability-abilitystage#abilitystage)中使能[应用恢复功能](#应用恢复接口功能介绍)，并对临时数据进行保存，应用非预期退出后的下一次启动会恢复先前的状态和数据，给用户更连贯的使用体验。这里状态包括应用的页面栈以及onSaveState接口中保存的数据。
...
- 故障监听指的是通过[errorManager](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-errormanager/js-apis-app-ability-errormanager)注册[ErrorObserver](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-errorobserver/js-apis-inner-application-errorobserver)，监听故障的发生，并通知到监听方。
- 故障恢复指的是[appRecovery](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-apprecovery/js-apis-app-ability-apprecovery)，及故障发生后，将应用重启恢复到故障之前的状态。
- 故障查询指的是[faultLogger](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/performance-analysis-arkts-dep/js-apis-faultlogger/js-apis-faultlogger)通过其查询接口获取当前的故障信息。

下图中并没有标记faultLogger的调用时机，开发者可以根据应用启动时传入的[LastExitReason](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-abilityconstant/js-apis-app-ability-abilityconstant#lastexitreason)来决定是否调用faultLogger查询上次的故障信息。

![](../../../../images/aecf7ada/zh-cn_image_0000002583478501.png)
...
如果开发者使能应用恢复，框架会首先检查当前故障是否支持状态保存以及开发者是否配置了状态保存，如果支持则会回调UIAbility的[onSaveState](/ref/ability-api/ability-arkts/stage-model/js-apis-app-ability-uiability/js-apis-app-ability-uiability#onsavestate)的接口。最后重启应用。
...
| --- | --- | --- | --- | --- |
| [JS\_CRASH](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/performance-analysis-arkts-dep/js-apis-faultlogger/js-apis-faultlogger#faulttype) | 支持 | 支持 | 支持 | 支持 |
| [APP\_FREEZE](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/performance-analysis-arkts-dep/js-apis-faultlogger/js-apis-faultlogger#faulttype) | API18及以上支持 | 支持 | 支持 | 支持 |
| [CPP\_CRASH](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/performance-analysis-arkts-dep/js-apis-faultlogger/js-apis-faultlogger#faulttype) | 不支持 | 不支持 | 不支持 | 支持 |
...
- 定义和注册[ErrorObserver](/ref/ability-api/ability-arkts/ability-api-interface-depend/ability-arkts-application/js-apis-inner-application-errorobserver/js-apis-inner-application-errorobserver) callback，具体可参考[errorManager](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-errormanager/js-apis-app-ability-errormanager)里的使用方法。
...
发生故障的UIAbility再次重新启动时，在调用onCreate生命周期里，参数want的parameters成员会有[ABILITY\_RECOVERY\_RESTART](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-wantconstant/js-apis-app-ability-wantconstant#params)标记数据，并且值为true。
