...
- [订阅地址越界事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/address-sanitizer-events/hiappevent-watcher-address-sanitizer-events-arkts)
- [订阅地址越界事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/address-sanitizer-events/hiappevent-watcher-address-sanitizer-events-ndk)

![](../../../../../../../images/a94dac23/note_3.0-zh-cn.png) 

地址越界事件支持在[应用分身](/development-fundamentals/application-typical-scenarios/app-clone)场景下使用 HiAppEvent 进行订阅，从 API version 22 开始支持在[输入法应用](/ime-kit/inputmethod-application-guide)场景下使用 HiAppEvent 进行订阅。不支持在元服务场景下使用 HiAppEvent 进行订阅。

## 检测原理

详见[地址越界类问题检测](/system-debug-optimize/performance-analysis-kit/fault-analysis/address-sanitizer-guidelines)。
...
| FDSAN | 从API version 20开始，可以支持订阅由[fdsan](/coding/c-cpp/fdsan)触发的错误类型。 |
