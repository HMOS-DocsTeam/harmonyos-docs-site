...
4. ****信息记录****：对于启用HiTraceChain的业务流程，其输出的各类信息中（包括HiTraceMeter打点、应用事件、hilog日志等）都会记录该跟踪标识，开发者可以通过HiTraceId将这些信息关联起来，从而实现端到端的调用链跟踪。
   ...
   ![](../../../../images/f7f2fbe0/zh-cn_image_0000002583478499.png)

## 约束与限制

通过设置[HiTraceFlag](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-hitracechain/js-apis-hitracechain#hitraceflag)以启用异步调用跟踪，HiTraceId可在HiTraceChain的自动传递机制中自动传递。
...
| 场景 | 异步任务 | 跨线程 | 跨进程 | 跨设备 |
| --- | --- | --- | --- | --- |
| 支持HiTraceChain自动传递的机制 | [async/await](/arkts/arkts-concurrency/async-concurrency-overview#asyncawait)  [Promise](/arkts/arkts-concurrency/async-concurrency-overview#promise) | [HiAppEvent](/system-debug-optimize/performance-analysis-kit/hiappevent/hiappevent-intro)  [napi\_async\_work](/coding/using-napi-interaction-with-cpp/napi-scenarios/use-napi-asynchronous-task)  [FFRT](/system-basicfun/ffrt-kit/ffrt-overview) | [IPC](/ipc-kit/ipc-rpc-overview) | [RPC](/ipc-kit/ipc-rpc-overview) |
| 不支持HiTraceChain自动传递的机制 | 宏任务及其异步任务（例如[setTimeout](/ref/common-basic-arkts/js-apis-timer/js-apis-timer#settimeout)、[setInterval](/ref/common-basic-arkts/js-apis-timer/js-apis-timer#setinterval)等） | [TaskPool](/arkts/arkts-concurrency/multithread-concurrency/taskpool-introduction)  [Worker](/arkts/arkts-concurrency/multithread-concurrency/worker-introduction)  C++标准库std::thread、pthread\_create、std::async等创建的线程 | [Socket](/system-network/network-kit/network-kit-data-transmission/socket-connection)  [Ashmem](/ref/ipc-api/ipc-arkts/js-apis-rpc/js-apis-rpc#ashmem8) | - |
