...
DevEco Studio会收集设备/data/log/faultlog/faultlogger/路径下的进程崩溃故障日志到FaultLog下，根据进程名和故障和时间分类显示。获取日志的方法参见：[DevEco Studio使用指南-FaultLog](/ide-log-and-fault-analysis/ide-fault-analysis/ide-fault-log)。

****方式二：通过HiAppEvent接口订阅****

HiAppEvent给开发者提供了故障订阅接口，详见[HiAppEvent介绍](/system-debug-optimize/performance-analysis-kit/hiappevent/hiappevent-intro)。参考[订阅地址越界事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/address-sanitizer-events/hiappevent-watcher-address-sanitizer-events-arkts)或[订阅地址越界事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/address-sanitizer-events/hiappevent-watcher-address-sanitizer-events-ndk)完成地址越界事件订阅，并通过事件的[external\_log](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/address-sanitizer-events/hiappevent-watcher-address-sanitizer-events#params字段说明)字段读取故障日志文件内容。
...
1. ****提取故障类型****
   ...
   - ASan/HWASan/MemDebug
     ...
     在[ASan](#asan日志规格)、[HWASan](#hwasan日志规格)和[MemDebug](#memdebug日志规格)日志中，故障原因通过日志中的"Reason"字段提取，提取结果将作为后续聚类依据。详细字段说明可参考[AddrSanitizer日志type字段说明](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/address-sanitizer-events/hiappevent-watcher-address-sanitizer-events)。
2. ****标准化栈信息****
