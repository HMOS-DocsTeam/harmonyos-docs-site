...
任务超时检测主要包括主线程超时检测和任务执行超时检测，二者均可使用HiCollie实现自定义时长来对开发者选择的业务逻辑进行主动检测，主线程超时也可采用默认时长进行检测。详见：[HiCollie使用指导](/system-debug-optimize/performance-analysis-kit/hicollie/hicollie-guidelines-ndk)。

![](../../../../images/14e05c59/note_3.0-zh-cn.png) 

[AppFreeze（应用冻屏）检测](/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines)相比任务超时检测，主要是由系统侧提供检测周期性检测逻辑，无需开发者感知和适配。
...
| --- | --- | --- | --- |
| 堆栈采集 | 150ms < 主线程处理时长 < 450ms | 文件名格式：MAIN\_THREAD\_JANK\_秒级时间\_进程PID.txt。  例如：MAIN\_THREAD\_JANK\_20240613211739\_40986.txt。 | - ****应用启动10s内不进行检测。****  - ****关闭[开发者选项](/ide-debug-app/ide-developer-mode#section0736139111917)：应用在一个生命周期内，一天最多触发一次主线程超时事件堆栈采集流程。****  - ****启用[开发者选项](/ide-debug-app/ide-developer-mode#section530763213432)：应用在一个生命周期内，一小时最多触发一次主线程超时事件采集堆栈采集流程。**** |
| trace采集 | 主线程处理时长 > 450ms | 文件名格式：MAIN\_THREAD\_JANK\_unix时间戳\_进程PID.trace。  例如：MAIN\_THREAD\_JANK\_1762064185461\_40986.trace。 | - 触发trace采集的前提：****开发者使用[nolog](/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#nolog版本)版本，并且关闭[开发者选项](/ide-debug-app/ide-developer-mode#section530763213432)****。  - ****应用一天内，最多触发一次主线程超时事件trace采集流程。**** |

![](../../../../images/9fbb7738/caution_3.0-zh-cn.png) 
...
2. trace采集流程
   ...
   ![](../../../../images/848ad891/zh-cn_image_0000002552958494.png)

   （2）20轮检测至少有一轮检测发生主线程处理超时（主线程处理时长 > 150ms），生成trace文件并上报事件，结束检测。

   ![](../../../../images/e4d4ab1f/zh-cn_image_0000002583478495.png)
...
HiAppEvent给开发者提供了故障订阅接口，详见[HiAppEvent介绍](/system-debug-optimize/performance-analysis-kit/hiappevent/hiappevent-intro)。参考[订阅主线程超时事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events-arkts)或[订阅主线程超时事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events-ndk)完成主线程超时事件订阅，并通过事件的[external\_log](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events#事件字段说明)字段读取故障日志文件名。
...
![](../../../../images/9d119a67/zh-cn_image_0000002552798846.png)
...
HiAppEvent给开发者提供了故障订阅接口，详见[HiAppEvent介绍](/system-debug-optimize/performance-analysis-kit/hiappevent/hiappevent-intro)。参考[订阅任务执行超时事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/app-hicollie-events/hiappevent-watcher-apphicollie-events-arkts)或[订阅任务执行超时事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/app-hicollie-events/hiappevent-watcher-apphicollie-events-ndk)完成任务执行超时事件订阅，并通过事件的[external\_log](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/app-hicollie-events/hiappevent-watcher-apphicollie-events#事件字段说明)字段读取故障日志文件名。

### 日志规格

任务执行超时事件日志规格与应用冻屏日志相同，可详见[应用冻屏日志规格](/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。
