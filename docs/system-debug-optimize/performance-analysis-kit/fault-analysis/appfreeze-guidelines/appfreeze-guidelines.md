...
![](../../../../images/51713aad/note_3.0-zh-cn.png) 
...
![](../../../../images/2860e7a0/caution_3.0-zh-cn.png) 

AppFreeze检测仅对[release版本应用](/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#release版本应用)生效，对[debug版本应用](/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)不生效。
...
当应用发生上述故障时，为了保证可恢复，会杀死应用。并上报应用冻屏事件，可通过HiAppEvent订阅[应用冻屏事件](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events)。
...
![](../../../../images/b6351149/zh-cn_image_0000002583438535.png)
...
![](../../../../images/18316ff9/zh-cn_image_0000002552958490.png)
...
DevEco Studio会收集设备/data/log/faultlog/faultlogger/路径下的进程崩溃故障日志到FaultLog下，根据进程名和故障时间分类显示。获取日志的方法参见：[DevEco Studio使用指南-FaultLog](/ide-log-and-fault-analysis/ide-fault-analysis/ide-fault-log)。

****方式二：通过HiAppEvent接口订阅****

HiAppEvent给开发者提供了故障订阅接口，详见[HiAppEvent介绍](/system-debug-optimize/performance-analysis-kit/hiappevent/hiappevent-intro)。参考[订阅应用冻屏事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events-arkts)或[订阅应用冻屏事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events-ndk)完成应用冻屏事件订阅，并通过事件的[external\_log](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events#事件字段说明)字段读取故障日志文件内容。
...
从API version 20开始，发生THREAD\_BLOCK\_6S故障时，日志中新增[HiTraceId](/ref/system-debug-optimize-api/performance-analysis-api/performance-analysis-arkts/js-apis-hitracechain/js-apis-hitracechain#hitraceid)信息打印。HitraceId是HiTraceChain提供的唯一跟踪标识，用于跟踪业务流程调用链。可以协助开发者查看故障时间段内，故障流程的hilog日志，分析日志查看应用的执行状态。
...
| [Page switch history](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#有页面切换轨迹的故障场景日志规格) | 从API version 20开始，维测进程会记录应用切换历史。应用发生故障后，生成的故障文件将包含页面切换历史轨迹。如果维测服务进程崩溃或被终止运行，或者未能成功缓存页面切换轨迹，则不包含此字段。 |
...
![](../../../../images/7ceee276/note_3.0-zh-cn.png) 
...
![](../../../../images/2d74612c/note_3.0-zh-cn.png) 
...
2. 应用进程在运行时发生THREAD\_BLOCK\_6S或APP\_INPUT\_BLOCK时，会停止上述流程的采集主线程调用栈流程，并计算周期内的CPU信息。一般情况下，会抓取1~10次堆栈日志。
   ...
   ![](../../../../images/b771e321/note_3.0-zh-cn.png) 

   由于应用冻屏事件的采样栈会与[MAIN\_THREAD\_JANK](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/main-thread-jank-events/hiappevent-watcher-mainthreadjank-events)冲突，如果应用接入MAIN\_THREAD\_JANK的setEventConfig接口自定义配置采集堆栈的个数，应用冻屏事件的采集堆栈的会与应用当前配置的采集堆栈的个数一致。
...
HiAppEvent给开发者提供了故障订阅接口，开发者可以使用HiAppEvent监听应用冻屏事件，获取文件内容，详见[应用冻屏事件介绍](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events)。参考[订阅应用冻屏事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events-arkts)或[订阅应用冻屏事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events-ndk)完成应用冻屏事件订阅。

开发者可以通过事件的[external\_log](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/freeze-events/hiappevent-watcher-freeze-events#事件字段说明)字段读取应用冻屏事件生成的故障文件和应用冻屏事件的增强日志文件。增强日志文件的命名与应用冻屏事件生成的故障文件名称格式相同。
...
AppFreeze故障信息聚类方法同Cpp Crash一致，参考[CppCrash聚类](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#cppcrash聚类)。

![](../../../../images/b9eed779/caution_3.0-zh-cn.png) 
