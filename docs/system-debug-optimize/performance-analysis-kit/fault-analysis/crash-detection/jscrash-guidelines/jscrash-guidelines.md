...
4. 上报崩溃事件，开发者可通过HiAppEvent订阅[崩溃事件](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/crash-events/hiappevent-watcher-crash-events)。如需了解JS Crash问题分析方法，请参见[JS Crash类问题分析方法](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-app-crash-js-way)。

## 约束与限制

在async修饰的异步函数中主动抛出异常，不会产生JS Crash导致应用崩溃，开发者可以通过[ErrorManager](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-errormanager/js-apis-app-ability-errormanager#errormanageronerror)观测该异常，样例代码参考[Async函数内部异常的处理机制](/arkts/arkts-runtime/arkts-runtime-faq#async函数内部异常的处理机制)。
...
DevEco Studio会收集设备/data/log/faultlog/faultlogger/路径下的进程崩溃故障日志到FaultLog中，根据进程名、故障和时间分类显示。获取日志的方法参见：[DevEco Studio使用指南-FaultLog](/ide-log-and-fault-analysis/ide-fault-analysis/ide-fault-log)。

****方式二：通过HiAppEvent接口订阅****

HiAppEvent给开发者提供了故障订阅接口，详见[HiAppEvent介绍](/system-debug-optimize/performance-analysis-kit/hiappevent/hiappevent-intro)。参考[订阅崩溃事件（ArkTS）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/crash-events/hiappevent-watcher-crash-events-arkts)或[订阅崩溃事件（C/C++）](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/crash-events/hiappevent-watcher-crash-events-ndk)完成崩溃事件订阅，再通过事件的[external\_log](/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/crash-events/hiappevent-watcher-crash-events#事件字段说明)字段读取故障日志文件内容。
...
| ReleaseType | 应用的版本类型。release表示应用为[release版本应用](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section192461528194916)，debug表示应用为[debug版本应用](/ide-build-customization/ide-hvigor-compilation-options-customizing/ide-hvigor-compilation-options-customizing-guide#section192461528194916)。 | 23 | 是 | - |
| CpuAbi | 二进制接口类型 | 23 | 是 | - |
| Version | hap版本 | 8 | 是 | - |
...
| SubmitterStacktrace | 提交者线程栈 | 20 | 否 | 异步线程栈跟踪维测功能默认仅在ARM 64位系统中开启。  对于****API version 22****之前版本，****三方和系统应用****[libuv](/ref/libuv/libuv)和[ffrt](/ref/system-basicfun-api/function-flow-runtime-api/function-flow-runtime-c/function-flow-runtime-module/capi-ffrt/capi-ffrt)提交异步任务仅debug版本默认开启。  对于****API version 22****及之后版本，****三方应用****通过libuv提交异步任务debug和release版本均默认开启；****三方和系统应用****通过ffrt提交异步任务仅debug版本默认开启。 |
...
![](../../../../../images/88679707/caution_3.0-zh-cn.png) 
...
![](../../../../../images/e93e0dae/caution_3.0-zh-cn.png) 
...
3. 编译产物结构如下：
   ...
   - 编译产物路径：详见[异常堆栈解析原理 sourcemap结构：key字段介绍](/ide-log-and-fault-analysis/ide-fault-analysis/ide-exception-stack-parsing-principle#section1145914292713)。
...
CPP代码调用栈详细说明[CPP异常代码调用栈格式规范](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#一般故障场景日志规格)。
...
Promise异步任务中抛出的异常默认不会导致JS Crash，但可以通过[on('unhandledRejection')](/ref/ability-api/ability-arkts/both-models/js-apis-app-ability-errormanager/js-apis-app-ability-errormanager#errormanageronunhandledrejection12)捕获Rejected Promise后，主动将异常抛出，从而触发JS Crash。
...
聚类方法同Cpp Crash一致，参考[CppCrash聚类](/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#cppcrash聚类)。
