...
- 若类型不匹配，则跳过已注册的沙箱接口能力，采用华为分享默认逻辑接收文件数据。参考：[目标设备接收分享数据一步直达体验](/share-kit-guide/share-access-precautions/share-access-one-step)。
...
当本次沙箱接收回调触发时，如果应用因为业务实现需要拒绝本次接收时，可使用[ReceivableTarget.reject()](/ref/share-api/share-arkts/share-harmony-share/share-harmony-share#reject-1)方法拒绝本次接收。
