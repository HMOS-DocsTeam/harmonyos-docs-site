...
详细接口说明可参考[接口文档](/ref/enterprise-space-api/enterprisespace-arkts/enterprisespace-spacemanager/enterprisespace-spacemanager#setlockdownexemptionapps)。

| 接口名 | 描述 |
| --- | --- |
| [setLockdownExemptionApps](/ref/enterprise-space-api/enterprisespace-arkts/enterprisespace-spacemanager/enterprisespace-spacemanager#setlockdownexemptionapps)(appIds: string[], workspaceId?: number): Promise&lt;void&gt; | 设置深度冻结豁免名单。使用Promise异步回调。 |
...
1. 导入Enterprise Space Kit模块和相关依赖模块。
   ...
   ```
2. 调用接口[setLockdownExemptionApps](/ref/enterprise-space-api/enterprisespace-arkts/enterprisespace-spacemanager/enterprisespace-spacemanager#setlockdownexemptionapps)，设置深度冻结豁免名单，并且查看打印信息。
