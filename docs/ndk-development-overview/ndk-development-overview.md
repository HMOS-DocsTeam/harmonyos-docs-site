...
- ****[Node-API](/coding/using-napi-interaction-with-cpp/napi-introduction)****

  曾用名NAPI，是HarmonyOS中提供ArkTS/JS与C/C++跨语言调用的接口，该接口基于Node.js的Node-API扩展而来，但不完全兼容。

### NDK目录简介

获取NDK开发包，请参考[NDK开发工具目录](/build-with-ndk/build-with-ndk-cmake#获取ndk开发包)。

- build目录：放置预定义的toolchain脚本文件[hmos.toolchain.cmake](/build-with-ndk/build-with-ndk-overview#hmostoolchaincmake简介)

  ![](../images/08acbf34/zh-cn_image_0000002552959354.png)
...
- llvm文件夹：放置NDK提供的编译器
  ...
  ![](../images/2dcd40ea/zh-cn_image_0000002583479355.png)
