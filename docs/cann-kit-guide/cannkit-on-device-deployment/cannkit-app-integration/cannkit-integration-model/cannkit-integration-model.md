...
本节阐述同步模式下单模型的使用，从流程上分别阐述每个步骤在应用层和native层的实现和调用。接口请参见[API参考](/ref/cann-api/hiai-foundation-c/cannkit-index/cannkit/cannkit)，示例请参见[SampleCode](https://gitcode.com/HarmonyOS_Samples/cannkit-samplecode-clientdemo-cpp)，本示例支持加载离线模型对图片中的物体进行分类，App运行效果图如下所示。

![](../../../../images/28417338/zh-cn_image_0000002583479223.png)

## 预置模型

为了让App运行时能够读取到模型文件和处理推理结果，需要先把离线模型和模型对应的结果标签文件预置到工程的“entry/src/main/resources/rawfile”目录中。

本示例所使用的离线模型的转换和生成请参考[Caffe模型转换](/cann-kit-guide/cannkit-model-conversion/cannkit-offline-model-conversion/cannkit-model-conversion-example#caffe模型转换)。
...
2. （可选）根据需要调用[HMS\_HiAIOptions\_SetOmOptions](/ref/cann-api/hiai-foundation-c/cannkit-index/cannkit/cannkit#hms_hiaioptions_setomoptions)接口，打开维测功能（如Profiling）。
3. 把模型buffer传递给HIAIModelManager类的HIAIModelManager::LoadModelFromBuffer接口，该接口调用[OH\_NNCompilation\_ConstructWithOfflineModelBuffer](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nncompilation_constructwithofflinemodelbuffer)创建模型的编译实例。
4. 设置模型的deviceID。
   ...
   ```
5. 调用[OH\_NNCompilation\_Build](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nncompilation_build)，执行模型编译。
6. 调用[OH\_NNExecutor\_Construct](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nnexecutor_construct)，创建模型执行器。
7. 调用[OH\_NNCompilation\_Destroy](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nncompilation_destroy)，释放模型编译实例。
...
![](../../../../images/89fdbee0/note_3.0-zh-cn.png) 

如果不更换模型，则首次编译加载完成后可多次推理，即一次编译加载，多次推理。

调用[OH\_NNExecutor\_RunSync](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nnexecutor_runsync)，完成模型的同步推理。
...
1. 调用[OH\_NNTensor\_GetDataBuffer](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nntensor_getdatabuffer)，获取输出的Tensor，在输出Tensor中会得到模型的输出数据。
...
![](../../../../images/4d05d8fe/note_3.0-zh-cn.png) 

开发者可根据需要自行设置模型推理优先级。使用[OH\_NNCompilation\_SetPriority](/ref/neural-network-runtime-api/neural-network-runtime-c/neural-network-runtime-headerfile/capi-neural-network-core-h/capi-neural-network-core-h#oh_nncompilation_setpriority)接口，默认值为OH\_NN\_PRIORITY\_NONE，本接口应在模型推理前调用。
