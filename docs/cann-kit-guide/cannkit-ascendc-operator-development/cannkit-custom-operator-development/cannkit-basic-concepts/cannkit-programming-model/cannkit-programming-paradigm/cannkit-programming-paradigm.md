...
![](../../../../../../images/cfcdca35/zh-cn_image_0000002552799582.png)
...
任务间数据传递使用到的内存、事件等资源统一由管理模块Pipe进行管理。如下所示的内存管理示意图，TPipe通过[InitBuffer](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tpipe/cannkit-tpipe-initbuffer)接口对外提供Queue内存初始化功能，开发者可以通过该接口为指定的Queue分配内存。

Queue队列内存初始化完成后，需要使用内存时，通过调用[AllocTensor](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tque/cannkit-tque-alloctensor)来为LocalTensor分配内存，当创建的LocalTensor完成相关计算无需再使用时，再调用[FreeTensor](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tque/cannkit-tque-freetensor)来回收LocalTensor的内存。

![](../../../../../../images/6731417d/zh-cn_image_0000002583439277.png)

编程过程中使用到的临时变量内存同样通过Pipe进行管理。临时变量可以使用TBuf数据结构来申请指定TPosition上的存储空间。使用TBuf申请的内存空间只能参与计算，无法执行Queue队列的入队出队操作。具体的接口使用说明请参考[TBuf](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tbuf/cannkit-tbuf-overview)。
...
![](../../../../../../images/b6deade3/zh-cn_image_0000002552959232.png)
...
![](../../../../../../images/dd2e0d42/zh-cn_image_0000002583479233.png)

## Cube编程范式

Cube计算的典型数据流图如下所示：

![](../../../../../../images/dcf3c1ab/zh-cn_image_0000002552799584.png)
...
![](../../../../../../images/cd60222a/zh-cn_image_0000002583439279.png)
...
![](../../../../../../images/1600276f/zh-cn_image_0000002552959234.png)

基于Matmul高阶API的融合算子编程范式，对上述数据流简化表达如下。

![](../../../../../../images/1f6adc7b/zh-cn_image_0000002583479235.png)
...
1. 获取Local Memory的内存，调用[AllocTensor](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tque/cannkit-tque-alloctensor)申请内存，或者从上游队列[DeQue](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tque/cannkit-tque-deque)一块内存数据。
2. 完成计算或者数据搬运。
3. 把上一步处理好的数据调用[EnQue](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tque/cannkit-tque-enque)入队。
4. 调用[FreeTensor](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-ascend-c-api/cannkit-basic-apis/cannkit-memory-management-and-synchronization/cannkit-tque/cannkit-tque-freetensor)释放不再需要的内存。

以最简单的矢量编程范式为例，在调用上述接口时，实际上会给各执行单元下发一些指令，如下图所示：

![](../../../../../../images/8885155a/zh-cn_image_0000002552799586.png)
...
![](../../../../../../images/12efc316/zh-cn_image_0000002583439281.png)

由此可以看出，EnQue/DeQue主要解决了存在数据依赖时，并行执行单元的写后读同步控制问题。

![](../../../../../../images/528a9896/zh-cn_image_0000002552959236.png)
...
![](../../../../../../images/18242350/zh-cn_image_0000002583479237.png)

由此可以看出，AllocTensor/FreeTensor主要解决了存在数据依赖时，并行执行单元的读后写同步控制问题。

![](../../../../../../images/4ad57532/zh-cn_image_0000002552799588.png)
