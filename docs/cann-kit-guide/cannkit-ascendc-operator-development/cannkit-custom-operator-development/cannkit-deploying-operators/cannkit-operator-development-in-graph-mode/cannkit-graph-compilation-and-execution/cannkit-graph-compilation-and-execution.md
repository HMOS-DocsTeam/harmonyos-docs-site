...
单算子模型执行是指基于图IR执行算子，先编译算子（例如，使用OMG工具将Ascend IR定义的单算子描述文件编译成算子omc模型文件），再调用模型加载推理接口执行单算子网络。下文仅提供单算子模型执行的样例和基础内容讲解，详细内容请参考[端侧部署](/cann-kit-guide/cannkit-on-device-deployment/cannkit-whole-deployment-process)章节。

![](../../../../../../images/169630d6/note_3.0-zh-cn.png) 

HarmonyOS Next暂不支持图编译与图执行，仅支持通过[AI框架算子适配](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-deploying-operators/cannkit-ai-framework-operator-adaptation/cannkit-overview-of-ai-framework-operator)方式集成自定义算子，以下步骤仅供参考。

## 环境要求

- 已参考[环境准备](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-environment-preparation)，完成DDK驱动和软件的安装，配置DDK软件所需基本环境变量。
...
命令行参数详见[NPU调测参数](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-featured-topics/cannkit-commissioning-tool-parameter/cannkit-cli-parameters#npu调测参数)，会在$\{debug\_workspace\} /$\{op\_type\} /npu/src下生成验证样例工程，代码工程目录结构如下。
