...
1. 进行CPU域的功能验证，观察是否有日志报错。
   ...
   参考[工程化算子开发](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-operator-implementation/cannkit-project-based-operator-development/cannkit-overview-of-engineering-operator)章节，编写CPU侧的运行验证代码，并进行运行验证。得到CPU域的精度比对结果如下。
...
2. gdb调试。下面的样例展示了拉起leakyrelu算子CPU侧运行程序的样例，该样例程序会直接抛出异常，直接gdb运行，查看调用栈信息分析定位即可。其他场景下开发者可以使用gdb打断点等基本操作进行调试。使用gdb调试AscendC程序的详细内容请参考[gdb调试](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-usage-of-the-commissioning-tool/cannkit-commissioning-functions/cannkit-more-functions/cannkit-gdb)。
...
1. 在kernel侧的CMakeLists中通过add\_ops\_compile\_options针对报错算子添加编译选项“-mllvm -cce-aicore-jump-expand=true”，示例如下。
   ...
   add\_ops\_compile\_options的具体使用方法请参考[支持自定义编译选项](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-operator-implementation/cannkit-project-based-operator-development/cannkit-compilation-and-deployment/cannkit-operator-project-compilation#支持自定义编译选项)。
2. 重新编译该算子。正常编译无报错。
...
1. 执行如下命令，获取工具安装路径。
   ...
   ![](../../../../images/ba7bcd6a/note_3.0-zh-cn.png) 
...
2. 如果有重复，重新打开一个终端窗口，按照[环境准备](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-environment-preparation)章节设置环境变量。
...
![](../../../../images/246b59d7/zh-cn_image_0000002552799606.png)
...
2. 获取日志文件。
   ...
   通过命令行方式，日志落盘地址由[Simulator仿真参数](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-featured-topics/cannkit-commissioning-tool-parameter/cannkit-cli-parameters#simulator仿真参数)接口指定，缺省为当前操作路径的debug\_op.log。请根据实际路径打开日志文件。
3. 截取调测命令，重新执行后再分析。
   ...
   2. 手动拷贝opc npu compile start后的命令，如图2所示，并在终端窗口执行，通过打屏或者落盘的日志文件进一步分析问题。
      ...
      ****图2**** NPU编译命令
      ...
      ![](../../../../images/0c6f05c1/zh-cn_image_0000002583439301.png)
...
![](../../../../images/f81821bd/zh-cn_image_0000002552959256.png)
...
2. 获取日志文件。
   ...
   通过命令行方式，日志落盘地址由[NPU调测参数](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-featured-topics/cannkit-commissioning-tool-parameter/cannkit-cli-parameters#npu调测参数)接口指定，缺省为当前操作路径的debug\_op.log。请根据实际路径打开日志文件。
...
![](../../../../images/2aa1ad09/zh-cn_image_0000002583479257.png)
...
![](../../../../images/2683ab25/zh-cn_image_0000002552799608.png)
...
2. 截取Tiling调测命令，重新执行，根据提示进一步定位Tiling代码问题。
   ...
   ![](../../../../images/c8e63a17/note_3.0-zh-cn.png) 

   执行命令之前，请确保当前终端[环境准备](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-environment-preparation)设置并生效。
...
- 建议4：请参考[Simulator仿真参数](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-custom-operator-development/cannkit-featured-topics/cannkit-commissioning-tool-parameter/cannkit-cli-parameters#simulator仿真参数)设置CAModel仿真参数，如block num建议设置为1。
