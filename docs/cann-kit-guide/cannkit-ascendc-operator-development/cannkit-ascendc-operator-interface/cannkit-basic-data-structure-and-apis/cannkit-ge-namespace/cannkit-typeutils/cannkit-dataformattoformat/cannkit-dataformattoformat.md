...
输入合法时，返回转换后的Format enum值，枚举定义请参考[Format](/cann-kit-guide/cannkit-ascendc-operator-development/cannkit-ascendc-operator-interface/cannkit-basic-data-structure-and-apis/cannkit-ge-namespace/cannkit-ge-format)，仅支持部分格式，详见[约束说明](#约束说明)；输入不合法时，返回FORMAT\_RESERVED，并打印报错信息。
